.. _tutorials:


#########
Tutorials
#########

These tutorials will teach you everything you need to know to get up and running with Great Expectations.

If you're the inpatient type, head to :ref:`Quick start` to get going with no fuss or explanation.

If you're a new user of Great Expectations, check out :ref:`_tutorials__getting_started` to learn important concepts by setting your first deployment.

If you're interested in contributing to the open source project, thank you! Please check out :ref:`Contribute to Great Expectations` to learn how.


.. toctree::
   :maxdepth: 2

   /guides/tutorials/quick_start
   /guides/tutorials/getting_started
   /guides/tutorials/contributing


