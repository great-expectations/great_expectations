...
### Running tests
You can run all unit tests by running `pytest` in the `great_expectations` directory root. By default the tests will be run against `pandas` and `sqlite`, with additional backends like `postgresql`, `spark`, and `mssql` requested using pytest flags. 
...
For example, you can run `pytest --no-sqlalchemy` to skip all local backend tests (with the exception of the pandas backend). Please note that these tests will still be run by the CI as soon as you open a PR, so some tests might fail if your code changes affected them. To run a test against a specific backend like PostgreSQL, you can run `pytest --postgresql`.
