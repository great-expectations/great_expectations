...
export default function FeedbackModal(){
    ...
    const handleChange = (e) => {
        ...
        // If the input is a file input, use files[0] to get the selected file
        const file = name === 'attachment' ? files[0] : null;
        setFormData((prevData) => ({
            ...prevData,
            [name]: name === 'attachment' ? file : value,
        }));
    };

    let thumbsUpImg = useBaseUrl(`img/thumbs_up_icon.svg`);
    let thumbsDownImg = useBaseUrl(`img/thumbs_down_icon.svg`);
    let closeImg = useBaseUrl(`img/close_icon.svg`);
    ...
    return <>
        ...
        {isOpen && <>
            <div className={styles.darkBackground} onClick={() => setIsOpen(false)}/>
