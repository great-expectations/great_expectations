---
title: Configure a Checkpoint Store
---
...
A `Checkpoint Store` is a <TechnicalTag tag="store" text="Store" /> that stores Checkpoint data. A `Checkpoint Store` tracks the `run_id` of the Validation and the <TechnicalTag tag="expectation_suite" text="Expectation Suite" /> name in addition to the Metric name and Metric kwargs.
...
## Add a Checkpoint Store

To define a `Checkpoint Store`, add a <TechnicalTag tag="metric_store" text="Metric Store" /> configuration to the `stores` section of your `great_expectations.yml`. The configuration must include the following keys:

- `class_name` - Enter `Checkpoint Store`. This key determines which class is instantiated to create the `StoreBackend`. Other fields are passed through to the `StoreBackend` class on instantiation. The only backend Store under test for use with a `Checkpoint Store` is the `DatabaseStoreBackend` with Postgres.
...
The following is an example of how the `Checkpoint Store` configuration appears in `great_expectations.yml`:

```yaml
stores:
    #  ...
    metric_store:  # You can choose any name as the key for your metric store
        class_name: Checkpoint Store
        store_backend:
...
When a `Checkpoint Store` is available, add a `StoreMetricsAction` validation <TechnicalTag tag="action" text="Action" /> to your <TechnicalTag tag="checkpoint" text="Checkpoint" /> to save Metrics during Validation. The validation Action must include the following fields:

- `class_name` - Enter `StoreMetricsAction`. Determines which class is instantiated to execute the Action.

- `target_store_name` - Enter the key for the Checkpoint Store you added in your `great_expectations.yml`. In the previous example, the `metrics_store`field defines which Store backend to use when persisting the metrics.
...
:::note Note:
If you use an Expectation Suite name as a key, Metrics are only added to the `Checkpoint Store` when the Expectation Suite runs. When you use the wildcard `"*"`, Metrics are added to the `Checkpoint Store` for each Expectation Suite that runs in the Checkpoint.
:::
...
## Test your Checkpoint Store and StoreMetricsAction
