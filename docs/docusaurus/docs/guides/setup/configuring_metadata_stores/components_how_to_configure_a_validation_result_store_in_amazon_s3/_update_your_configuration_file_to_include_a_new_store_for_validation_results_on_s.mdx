...
As shown in the previous example, you need to change the default ``store_backend`` settings to make the Store work with S3.  The ``class_name`` is set to ``TupleS3StoreBackend``, ``bucket`` is set to the address of your S3 bucket, and ``prefix`` is set to the folder in your S3 bucket where Validation results are located.
