...
In our example we will use a a Checkpoint (`my_checkpoint`) that runs an ExpectationSuite (`visitors_exp`) with 1 Expectation type : `ExpectColumnValuesToBeInSet` on the `event_type` column. The `set`
