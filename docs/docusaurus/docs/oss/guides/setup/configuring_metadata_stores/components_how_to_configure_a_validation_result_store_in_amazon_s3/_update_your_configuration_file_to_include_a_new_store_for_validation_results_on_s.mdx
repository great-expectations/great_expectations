...
```python title="Python" name="docs/docusaurus/docs/snippets/aws_cloud_storage_pandas.py new_validation_results_store"
```
...
```
In the previous example, the Store name is ``validations_S3_store``. If you use a personalized Store name, you must also update the value of the `validation_results_store_name` key to match the Store name. For example:

```python title="Python" name="docs/docusaurus/docs/snippets/aws_cloud_storage_pandas.py set_new_validation_results_store"
```
When you update the `validation_results_store_name` key value, Great Expectations uses the new Store for Validation Results.
