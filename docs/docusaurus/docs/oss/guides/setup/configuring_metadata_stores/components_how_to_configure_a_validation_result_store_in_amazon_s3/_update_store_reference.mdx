To make Great Expectations look for Validation Results in an S3 bucket, set the ``validation_results_store_name`` variable in `great_expectations.yml` to the name of your S3 Validations Store as shown in the following example:
