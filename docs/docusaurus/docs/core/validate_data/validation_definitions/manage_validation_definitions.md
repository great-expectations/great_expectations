...
3. Get an Expectation Suite with Expectations.  This can be [an existing Expectation Suite retrieved from your Data Context](/core/create_expectations/expectation_suites/manage_expectation_suites#get-an-existing-expectation-suite) or a new Expectation Suite in your current code.
...
batch_definition = context.sources.get(existing_data_source_name).get(existing_data_asset_name).get(existing_batch_definition_name)
