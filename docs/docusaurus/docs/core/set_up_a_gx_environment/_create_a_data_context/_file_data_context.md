...
1. Run the following code to request a File Data Context:
   ...
   When you specify `mode='file'`, the `get_context()` method will instantiate and return the first File Data Context it finds in the folder hierarchy of your current working directory.  

   If a File Data Context configuration is not found, `get_context(mode='file')` will create a new File Data Context in your current working directory and then instantiate and return the newly created File Data Context.

2. Optional. Run the following code to review the configuration of the File Data Context you recieved:
