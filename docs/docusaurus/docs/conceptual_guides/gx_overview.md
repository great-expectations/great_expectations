...
## Workflow for using GX the first time

Working with GX for the first time can be broken down into four steps.

  1. Install GX along with any dependencies specific to your Source Data.  
  2. Connect your Source Data System to GX.
  3. Define your Expectations for the connected data. 
  4. Validate those Expectations and review the results.

Installation and Setup (Install GX, create a Data Context, configure your Data Context's settings: Stores, Data Docs, and Credentials)-> Connect to data (Connect to Source Data with Datasources, Add Data Assets Datasources to describe sets of records) -> Create Expectations -> Validate data (Retrieve data with a Batch Request, retrieve Expectations from an Expectations Store, define a checkpoint, run the checkpoint)

### Installation of GX

GX is a library for <PythonVersion />, and you can use pip to install it. If you're using a Source Data format such as SQL or hosting your Source Data in an environment such as AWS, special pip commands are provided.  These commands bundle the installation of GX with the installation of additional Python dependencies used to access specific data formats and hosting environments.

<!-- To install GX and additional dependencies, see Installation guides. -->

### Project setup and the Data Context
...
Your Data Context will contain the configurations for Stores, Credentials, and Data Docs.  Store configurations describe locations where metadata information used by GX resides.  Credentials for Source Data Systems that require them can be provided in a special Store and accessed through string substitution so that you do not have to include them in plain text configurations elsewhere.  Data Docs configurations tell GX where to produce the human-readable output that is generated when GX validates data.

<!-- To initialize and instantiate a Data Context, see Setting up a GX environment. -->

#### Metadata Stores and Data Docs locations

Part of setting up a GX project includes defining where you want any persisting information used by GX to reside if the default locations are unsuitable.  This information primarily consists of configurations for Stores and Data Docs. 
...
### Expectations describe your Source Data
...
## The ongoing, iterative GX workflow
