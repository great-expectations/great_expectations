...
```python title="Python" name="spark Data Source configuration up to adding an empty inferred data connector"
```
...
```python title="Python" name="version-0.17.23 inferred data connector define class_name as InferredAssetFilesystemDataConnector"
```
...
```python title="Python" name="version-0.17.23 inferred spark datasource_config up to adding empty default_regex and batch_spec_passthrough dictionaries"
```
