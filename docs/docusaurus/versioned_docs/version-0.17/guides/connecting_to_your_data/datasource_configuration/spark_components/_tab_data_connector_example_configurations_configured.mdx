...
```python name="version-0.17.23 configured data connector define class_name as ConfiguredAssetFilesystemDataConnector"
```
...
```python name="version-0.17.23 configured spark datasource_config up to adding empty assets and batch_spec_passthrough dictionaries"
```