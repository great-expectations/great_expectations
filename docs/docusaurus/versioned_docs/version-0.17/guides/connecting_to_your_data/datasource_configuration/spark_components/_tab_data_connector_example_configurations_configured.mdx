...
```python title="Python" name="spark Data Source configuration up to adding an empty configured data connector"
```
...
```python title="Python" name="version-0.17.23 configured data connector define class_name as ConfiguredAssetFilesystemDataConnector"
```
...
```python title="Python" name="version-0.17.23 configured spark datasource_config up to adding empty assets and batch_spec_passthrough dictionaries"
```