...
```python name="version-0.17.23 datasource_configuration_inferred_multi_batch_regex_pattern"
```
...
```python name="version-0.17.23 group_names for inferred multi batch default_regex key"
```
...
```python name="version-0.17.23 data connector config for inferred filesystem data connector multi batch"
```
