...
```python name="version-0.18.8 empty_data_asset_configuration_configured_single_batch"
```
...
```python name="version-0.18.8 pattern_for_single_batch_configured_data_asset_configuration"
```
...
```python name="version-0.18.8 group_names for single batch configured assets configuration"
```

Put entirely together, your `assets` entry will look like:

```python name="version-0.18.8 full_data_asset_for_single_batch_configured_data_connector"
```
...
```python name="version-0.18.8 full_single_batch_configured_data_connector_configuration"
```