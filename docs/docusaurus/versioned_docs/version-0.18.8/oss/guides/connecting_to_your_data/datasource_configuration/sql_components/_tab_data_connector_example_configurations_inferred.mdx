...
```python name="version-0.18.8 sql datasource configuration with empty inferred sql data_connector"
```

#### Required Data Connector configuration keys

<PartDataConnectorRequiredKeysOverview data_connector_type="InferredAssetSqlDataConnector" data_connector_name="version-0.18.8 name_of_my_inferred_data_connector" inferred={true} configured={false} runtime={false} />
...
```python name="version-0.18.8 define data_connector class_name for inferred sql datasource"
```
...
```python name="version-0.18.8 inferred sql datasource configuration with data_connector class_name defined"
```
