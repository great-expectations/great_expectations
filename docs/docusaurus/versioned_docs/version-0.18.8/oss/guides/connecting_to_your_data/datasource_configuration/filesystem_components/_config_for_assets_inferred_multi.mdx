...
```python name="version-0.18.8 datasource_configuration_inferred_multi_batch_regex_pattern"
```
...
```python name="version-0.18.8 group_names for inferred multi batch default_regex key"
```
...
```python name="version-0.18.8 data connector config for inferred filesystem data connector multi batch"
```
