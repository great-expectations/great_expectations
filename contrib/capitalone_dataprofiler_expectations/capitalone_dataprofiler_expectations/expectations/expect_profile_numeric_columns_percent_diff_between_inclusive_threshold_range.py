...
class DataProfilerProfileNumericColumnsPercentDiffBetweenInclusiveThresholdRange(DataProfilerProfileMetricProvider):
        ...
        for col, stats in limit_check_report_keys_copy.items():
            ...
            for stat, bounds in stats.items():
                ...
                if diff_val == "unchanged":  # In the case there is no delta
                    diff_val = 0
                between_bounds = is_value_between_bounds(diff_val, bounds["lower"], bounds["upper"], inclusive=True)
    ...
    def _get_evaluation_dependencies(
        ...
        if metric.metric_name == "data_profiler.profile_numeric_columns_percent_diff_between_inclusive_threshold_range":
...
class ExpectProfileNumericColumnsPercentDiffBetweenInclusiveThresholdRange(ProfileNumericColumnsDiffExpectation):
