...
def is_valid_new_jersey_zip(zip: str):
    list_of_dicts_of_new_jersey_zips = zipcodes.filter_by(state="NJ")
    list_of_new_jersey_zips = [d["zip_code"] for d in list_of_dicts_of_new_jersey_zips]
    if len(zip) > 10 or type(zip) != str:  # noqa: E721
        return False
    elif zip in list_of_new_jersey_zips:  # noqa: SIM103
        return True
    else:
        return False
...
class ColumnValuesToBeValidNewJerseyZip(ColumnMapMetricProvider):
...
class ExpectColumnValuesToBeValidNewJerseyZip(ColumnMapExpectation):
