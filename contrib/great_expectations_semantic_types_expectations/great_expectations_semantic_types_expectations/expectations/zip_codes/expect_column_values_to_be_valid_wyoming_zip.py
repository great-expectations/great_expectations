...
def is_valid_wyoming_zip(zip: str):
    list_of_dicts_of_wyoming_zips = zipcodes.filter_by(state="WY")
    list_of_wyoming_zips = [d["zip_code"] for d in list_of_dicts_of_wyoming_zips]
    if len(zip) > 10:
        return False
    elif type(zip) != str:
        return False
    elif zip in list_of_wyoming_zips:
        return True
    else:
        return False
...
class ColumnValuesToBeValidWyomingZip(ColumnMapMetricProvider):

    # This is the id string that will be used to reference your metric.
    condition_metric_name = "column_values.valid_wyoming_zip"

    # This method implements the core logic for the PandasExecutionEngine
    @column_condition_partial(engine=PandasExecutionEngine)
    def _pandas(cls, column, **kwargs):
        return column.apply(lambda x: is_valid_wyoming_zip(x))
