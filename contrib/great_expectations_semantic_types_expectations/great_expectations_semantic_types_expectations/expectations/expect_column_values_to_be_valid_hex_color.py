...
class ColumnValuesToBeValidHexColor(ColumnMapMetricProvider):
    ...
    def _pandas(cls, column, **kwargs):
        def matches_hexcolor_regex(x):
            return bool(re.match(HEX_COLOR_REGEX, str(x)))

        return column.apply(lambda x: matches_hexcolor_regex(x) if x else False)
...
class ExpectColumnValuesToBeValidHexColor(ColumnMapExpectation):
            ...
            "tests": [
                {
                    ...
                    "in": {"column": "well_formed_hexcolor"},
                    "out": {"success": True},
                },
                {
    ...
    map_metric = "column_values.valid_hexcolor"
...
if __name__ == "__main__":
    ExpectColumnValuesToBeValidHexColor().print_diagnostic_checklist()
