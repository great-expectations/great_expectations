...
@pytest.fixture
def simple_multi_year_polars_df():
    ...
    assert isinstance(df.timestamp.dtype, ptypes.Datetime)
...
def test_get_batch_with_split_on_year(
    ...
    num_values_in_df,
    simple_multi_year_polars_df: pl.DataFrame,
):
    engine: PolarsExecutionEngine = PolarsExecutionEngine()

    split_df: pl.DataFrame = engine.get_batch_data(
        RuntimeDataBatchSpec(
            batch_data=simple_multi_year_polars_df,
...
def test_split_on_date_parts_single_date_parts(
    batch_identifiers_for_column, date_parts, simple_multi_year_polars_df
):
    ...
    """
    data_splitter: PolarsDataSplitter = PolarsDataSplitter()
    column_name: str = "timestamp"
    result: pd.DataFrame = data_splitter.split_on_date_parts(
        df=simple_multi_year_polars_df,
...
@mock.patch(
    "great_expectations.execution_engine.split_and_sample.polars_data_splitter.PolarsDataSplitter.split_on_date_parts"
)
@pytest.mark.parametrize(
...
def test_get_batch_with_split_on_whole_table_runtime(test_df):
    ...
    split_df = PolarsExecutionEngine().get_batch_data(
        RuntimeDataBatchSpec(
...
def test_get_batch_with_split_on_whole_table_filesystem(
    ...
    test_df = PolarsExecutionEngine().get_batch_data(
        PathBatchSpec(
...
def test_get_batch_with_split_on_whole_table_s3(
    ...
    test_df_small = pl.from_pandas(test_df_small)
    df = PolarsExecutionEngine().get_batch_data(
...
def test_get_batch_with_split_on_whole_table_s3_with_configured_asset_s3_data_connector(
    ...
    execution_engine: PolarsExecutionEngine = PolarsExecutionEngine()
