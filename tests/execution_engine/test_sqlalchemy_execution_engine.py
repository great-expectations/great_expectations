...
def test_instantiation_via_url_with_kwargs(sa):
    ...
    my_execution_engine = SqlAlchemyExecutionEngine(url="sqlite:///" + db_file, connect_args={"timeout": 10})
    assert my_execution_engine.connection_string is None
    assert my_execution_engine.credentials is None
    assert my_execution_engine.url[-36:] == "test_cases_for_sql_data_connector.db"
...
def test_instantiation_via_fluent_data_source__trino_add_sql(sa, empty_data_context):
    ...
    validator = context.get_validator(datasource_name="test_datasource", data_asset_name="taxi_data")
...
def test_sa_batch_aggregate_metrics(caplog, sa):
    ...
    execution_engine = build_sa_execution_engine(pd.DataFrame({"a": [1, 2, 1, 2, 3, 3], "b": [4, 4, 4, 4, 4, 4]}), sa)
...
def test_get_domain_records_with_column_domain(sa):
    ...
    assert domain_data == expected_data, "Data does not match after getting full access compute domain"
...
def test_get_domain_records_with_column_pair_domain(sa):
    ...
    domain_data = execution_engine.execute_query(sa.select(sa.text("*")).select_from(data)).fetchall()

    expected_column_pair_df = pd.DataFrame({"a": [2, 3, 4, 6], "b": [3.0, 4.0, 5.0, 6.0], "c": [2.0, 3.0, 4.0, None]})
...
def test_get_compute_domain_with_no_domain_kwargs(sa):
    execution_engine = build_sa_execution_engine(pd.DataFrame({"a": [1, 2, 3, 4], "b": [2, 3, 4, None]}), sa)

    data, compute_kwargs, accessor_kwargs = execution_engine.get_compute_domain(domain_kwargs={}, domain_type="table")
...
def test_get_compute_domain_with_unmeetable_row_condition(sa):
    ...
    raw_data = execution_engine.execute_query(
        sa.select(sa.text("*"))
        .select_from(cast(SqlAlchemyBatchData, execution_engine.batch_manager.active_batch_data).selectable)
...
def test_resolve_metric_bundle_with_compute_domain_kwargs_json_serialization(sa):
    ...
    try:
        results = execution_engine.resolve_metrics(metrics_to_resolve=(desired_metric,), metrics=results)
...
def test_get_batch_data_and_markers_using_query(sqlite_view_engine, test_df):
    ...
    _batch_data, batch_markers = my_execution_engine.get_batch_data_and_markers(batch_spec=batch_spec)
...
def test_sa_batch_unexpected_condition_temp_table(caplog, sa):
    def validate_tmp_tables(execution_engine):
        temp_tables = [name for name in get_sqlite_temp_table_names(execution_engine) if name.startswith("ge_temp_")]
        tables = [name for name in get_sqlite_table_names(execution_engine) if name.startswith("ge_temp_")]
    ...
    results = execution_engine.resolve_metrics(metrics_to_resolve=(condition_metric,), metrics=metrics)
    metrics.update(results)
    ...
    # noinspection PyUnusedLocal
    results = execution_engine.resolve_metrics(metrics_to_resolve=(desired_metric,), metrics=metrics)
...
class TestConnectionPersistence:
    ...
    def test_same_connection_used_from_static_pool_sqlite(self, sa, pd_dataframe: pd.DataFrame):
        ...
        assert execution_engine.dialect_name == GXSqlDialect.SQLITE, "Error here means test setup failed."
    ...
    def test_same_connection_accessible_from_execution_engine_sqlite(self, sa, pd_dataframe: pd.DataFrame):
        """What does this test and why?
