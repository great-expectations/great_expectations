...
def test_get_partition_query_for_data_for_batch_identifiers_for_partition_on_date_parts_single_date_parts(
    ...
    result: sa.sql.elements.BooleanClauseList = (
        data_partitioner.get_partition_query_for_data_for_batch_identifiers_for_partition_on_date_parts(
            selectable=selectable,
            column_name=column_name,
            date_parts=date_parts,
        )
    ...
    query_str: str = (
        str(result.compile(compile_kwargs={"literal_binds": True})).replace("\n", "").replace(" ", "").lower()
    )
    assert (
        query_str
        == (
...
def in_memory_sqlite_taxi_ten_trips_per_month_execution_engine(sa):
    ...
    convert_string_columns_to_datetime(df=df, column_names_to_convert=["pickup_datetime", "dropoff_datetime"])
        ...
        if test_case.table_domain_test_case:
            ...
            if taxi_test_cases.test_column_name:
                ...
                batch_spec = SqlAlchemyDatasourceBatchSpec(
                    ...
                    partitioner_method=test_case.partitioner_method_name,
                    partitioner_kwargs=test_case.partitioner_kwargs,
                    batch_identifiers={taxi_test_cases.test_column_name: test_case.expected_column_values[0]},
                )
...
@pytest.mark.sqlite
def test_sqlite_partition_and_sample_using_limit(sa, in_memory_sqlite_taxi_ten_trips_per_month_execution_engine):
    """What does this test and why?
