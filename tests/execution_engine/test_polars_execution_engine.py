...
def test_constructor_with_boto3_options():
    ...
    engine = PolarsExecutionEngine(discard_subset_failing_expectations=True)
    ...
    engine = PolarsExecutionEngine(boto3_options=custom_boto3_options)
...
def test_reader_fn():
    ...
    # Testing that can recognize basic feather/ipc file
    fn = engine._get_reader_fn(path="myfile.ipc")
    assert "<function read_ipc" in str(fn)

    # Testing that can recognize basic avro file
    fn = engine._get_reader_fn(path="myfile.avro")
    assert "<function read_avro" in str(fn)

    # Testing that can recognize basic json file
    fn = engine._get_reader_fn(path="myfile.json")
    assert "<function read_json" in str(fn)

    # Testing that can recognize basic parquet file
    fn = engine._get_reader_fn(path="myfile.parquet")
    assert "<function read_parquet" in str(fn)
...
def test_get_domain_records_with_column_domain():
    ...
    data = engine.get_domain_records(
        domain_kwargs={
            "column": "a",
            "row_condition": "pl.col('b')<5",
            "condition_parser": "python",
        }
    )

    expected_column_df = df[:3]

    assert data.frame_equal(
        expected_column_df
...
def test_get_domain_records_with_column_pair_domain():
    ...
    data = engine.get_domain_records(
        domain_kwargs={
            "column_A": "a",
            "column_B": "b",
            "row_condition": "pl.col('b')>2",
            "condition_parser": "polars",
            "ignore_row_if": "both_values_are_missing",
        }
    )

    expected_column_pair_df = pl.DataFrame(
        {
            "a": [2, 3, 4, 6],
            "b": [3.0, 4.0, 5.0, 6.0],
            "c": [2.0, 3.0, 4.0, None],
...
def test_get_domain_records_with_multicolumn_domain():
    ...
    expected_multicolumn_df = pl.DataFrame(
        {"a": [2, 3, 4, None, 5], "b": [3, 4, 5, 6, 7], "c": [2, 3, 4, None, 6]},
    )
...
def test_get_compute_domain_with_no_domain_kwargs():
    engine = PolarsExecutionEngine()
    df = pl.DataFrame({"a": [1, 2, 3, 4], "b": [2, 3, 4, None]})
    ...
    assert data.frame_equal(df), "Data does not match after getting compute domain"
...
def test_get_compute_domain_with_column_pair_domain():
    engine = PolarsExecutionEngine()
    df = pl.DataFrame({"a": [1, 2, 3, 4], "b": [2, 3, 4, 5], "c": [1, 2, 3, 4]})
...
def test_resolve_metric_bundle():
    ...
    engine = PolarsExecutionEngine(batch_data_dict={"made-up-id": df})
...
def test_resolve_metric_bundle_with_nonexistent_metric():
    ...
    # Building engine and configurations in attempt to resolve metrics
    engine = PolarsExecutionEngine(batch_data_dict={"made_up_id": df})
    mean = MetricConfiguration(
...
def test_dataframe_property_given_loaded_batch():
    ...
    assert engine.dataframe.frame_equal(df)
...
def test_get_batch_data(test_df):
    ...
    with pytest.raises(ge_exceptions.InvalidBatchSpecError):
        PolarsExecutionEngine().get_batch_data(RuntimeDataBatchSpec())
...
def test_get_batch_s3_compressed_files(test_s3_files_compressed, test_df_small):
    ...
    df = PolarsExecutionEngine().get_batch_data(batch_spec=batch_spec)
...
def test_get_batch_with_no_s3_configured():
    ...
    execution_engine_no_s3 = PolarsExecutionEngine()
...
def test_constructor_with_azure_options(mock_azure_conn):
    ...
    engine = PolarsExecutionEngine(azure_options=custom_azure_options)
...
def test_get_batch_data_with_azure_batch_spec(
    ...
    df = PolarsExecutionEngine().get_batch_data(batch_spec=azure_batch_spec)
...
def test_get_batch_with_no_azure_configured(azure_batch_spec):
    ...
    execution_engine_no_azure = PolarsExecutionEngine()
...
def test_constructor_with_gcs_options(mock_gcs_conn, mock_auth_method):
    ...
    engine = PolarsExecutionEngine(gcs_options=custom_gcs_options)
...
def test_get_batch_data_with_gcs_batch_spec(
    ...
    df = PolarsExecutionEngine(**kwargs).get_batch_data(batch_spec=gcs_batch_spec)
...
def test_get_batch_data_with_gcs_batch_spec_no_credentials(gcs_batch_spec, monkeypatch):
    ...
    with pytest.raises(Exception):
        PolarsExecutionEngine().get_batch_data(batch_spec=gcs_batch_spec)
...
def test_get_batch_with_gcs_misconfigured(gcs_batch_spec):
    ...
    execution_engine_no_gcs = PolarsExecutionEngine()
