...
@pytest.fixture()
def context_with_single_sqlite_table(empty_data_context, tmp_path_factory):
    ...
    visitor_csv_source_file_path: str = file_relative_path(
        __file__,
        "../test_sets/visits.db",
    )
    visitor_csv_destination_file_path: str = str(
        os.path.join(base_directory, "data/visits.db")
    )
    shutil.copy(
        visitor_csv_source_file_path,
        visitor_csv_destination_file_path,
    )
    ...
    config = yaml.load(
        f"""
            ...
            execution_engine:
                ...
                connection_string: sqlite:///{base_directory}/data/visits.db
            data_connectors:
...
@pytest.fixture
def context_with_single_sqlite_table_and_expectation(
...
@pytest.fixture()
def context_with_single_csv_pandas(empty_data_context, tmp_path_factory):
    context = empty_data_context
    yaml = YAMLHandler()
    ...
    visitor_csv_source_file_path: str = file_relative_path(
        __file__,
        "../test_sets/visits.csv",
    )
    visitor_csv_destination_file_path: str = str(
        os.path.join(base_directory, "data/visits.csv")
    )
    shutil.copy(
        visitor_csv_source_file_path,
        visitor_csv_destination_file_path,
    ...
    config = yaml.load(
        f"""
            ...
            data_connectors:
                my_pandas_data_connector:
                    class_name: InferredAssetFilesystemDataConnector
                    base_directory: {visitor_asset_base_directory_path}
                    default_regex:
                        group_names:
                            - data_asset_name
                        pattern: (.*)
...
@pytest.fixture
def context_with_single_csv_pandas_and_expectation(
