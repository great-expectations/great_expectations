...
pop_stores = ["checkpoint_store", "evaluation_parameter_store", "validations_GCS_store"]
...
configured_expectations_store = yaml.safe_load(
    expected_existing_expectations_store_yaml
)
...
# replace example code with integration test configuration
configured_validations_store = yaml.safe_load(expected_existing_validations_store_yaml)
...
with open(great_expectations_yaml_file_path) as f:
    great_expectations_yaml = yaml.safe_load(f)
# Goes great_expectation.yml contain our expected configuration?
assert (
    great_expectations_yaml["data_docs_sites"]
    == yaml.safe_load(data_docs_site_yaml)["data_docs_sites"]
)
