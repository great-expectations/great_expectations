...
ec = ExpectationConfiguration(expectation_type= 'expect_column_values_to_not_be_null', 
                              kwargs={ 'column': 'passenger_count'}
                             )
suite = context.add_expectation_suite(expectation_suite_name = 'example_suite', expectations= [ec])
# </snippet>

# <snippet name="tests/integration/docusaurus/validation/checkpoints/how_to_validate_multiple_batches_within_single_checkpoint.py add_batch_request_list">
...
validations = [{"batch_request": batch.batch_request, "expectation_suite_name": "example_suite"} for batch in batch_list]

checkpoint = gx.checkpoint.SimpleCheckpoint(name = 'example_checkpoint', data_context=context, 
