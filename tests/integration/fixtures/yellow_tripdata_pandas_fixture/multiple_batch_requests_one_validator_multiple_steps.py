...
validator: Validator = context.get_validator(batch_request=jan_batch_request, expectation_suite=suite)
...
march_batch_list: List[Batch] = context.get_batch_list(batch_request=march_batch_request)
...
total_batch_definition_list: list = [v.batch_definition for k, v in validator.batches.items()]

# Filter to all batch_definitions prior to March
jan_feb_batch_filter: BatchFilter = build_batch_filter(
    data_connector_query_dict={"custom_filter_function": lambda batch_identifiers: int(batch_identifiers["month"]) < 3}
)
...
result = validator.expect_column_values_to_be_between("fare_amount", min_value=cumulative_min, max_value=cumulative_max)
