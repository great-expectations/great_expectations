...
def assets_to_validate():
    ...
    my_ge_df_1 = gx.dataset.PandasDataset(my_df_1, batch_kwargs={"ge_batch_id": "82a8de83-e063-11e9-8226-acde48001122"})

    # failed "failure-level" suite
    # failed "warning-level" suite
    my_df_2 = pd.DataFrame({"x": [1, 2, 3, 4, 99], "y": [1, 2, 3, 4, None]})
    my_ge_df_2 = gx.dataset.PandasDataset(my_df_2, batch_kwargs={"ge_batch_id": "82a8de83-e063-11e9-8133-acde48001122"})

    # succeeded "failure-level" suite
    # succeeded "warning-level" suite
    my_df_3 = pd.DataFrame({"x": [1, 2, 3, 4, 5], "y": [1, 2, 3, 4, 5]})
    my_ge_df_3 = gx.dataset.PandasDataset(my_df_3, batch_kwargs={"ge_batch_id": "82a8de83-e063-11e9-a53d-acde48001122"})

    # failed "failure-level" suite
    # succeeded "warning-level" suite
    my_df_4 = pd.DataFrame({"x": [1, 2, 3, 4, 99], "y": [1, 2, 3, 4, 5]})
    my_ge_df_4 = gx.dataset.PandasDataset(my_df_4, batch_kwargs={"ge_batch_id": "82a8de83-e063-11e9-8133-acde48001122"})
...
def warning_failure_validation_operator_data_context(
    ...
    df.expect_column_values_to_be_between(column="x", min_value=1, max_value=9, mostly=0.8)
    ...
    data_context.suites.add(
        ExpectationSuite(
            name="f2.failure",
            expectations=[copy(expectation) for expectation in failure_expectations.expectations],
        )
    )
    ...
    data_context.suites.add(
        ExpectationSuite(
            name="f2.warning",
            expectations=[copy(expectation) for expectation in warning_expectations.expectations],
        )
    )
    data_context.suites.add(
        ExpectationSuite(
...
@pytest.mark.filterwarnings("ignore:System time is way off:urllib3.exceptions.SystemTimeWarning:urllib3")
...
def test_errors_warnings_validation_operator_run_slack_query(
    ...
    slack_query["blocks"][7]["text"]["text"] = slack_query["blocks"][7]["text"]["text"].replace(
        "09/26/2019 13:42:41", "LOCALEDATE"
    )
    slack_query["blocks"][7]["text"]["text"] = slack_query["blocks"][7]["text"]["text"].replace(
        "09/26/2019 01:42:41 PM", "LOCALEDATE"
    )
    expected_slack_query["blocks"][7]["text"]["text"] = expected_slack_query["blocks"][7]["text"]["text"].replace(
        "09/26/2019 13:42:41", "LOCALEDATE"
    )
    expected_slack_query["blocks"][7]["text"]["text"] = expected_slack_query["blocks"][7]["text"]["text"].replace(
        "09/26/2019 01:42:41 PM", "LOCALEDATE"
...
def test_errors_warnings_validation_operator_failed_vo_result(
    ...
    assert any(
        ...
        if run_result["expectation_suite_severity_level"] == "failure" and not run_result["validation_result"].success
    )
    # no failed warning suites
    assert not any(
        run_result
        for run_result in run_results
        if run_result["expectation_suite_severity_level"] == "warning" and not run_result["validation_result"].success
    )
