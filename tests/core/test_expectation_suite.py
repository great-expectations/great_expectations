...
class TestInit:
    ...
    def test_expectation_suite_init_overrides_expectations_dict_and_obj(
        ...
        assert len(suite.expectation_configs) == 2
        assert suite.expectation_configs == test_expected_expectations
...
class TestIsEquivalentTo:
    ...
    def test_is_equivalent_to_expectation_suite_and_dict_false(
        ...
        modified_suite.expectation_configs[0]["kwargs"]["value_set"][0] = -1
    ...
    def test_is_equivalent_to_expectation_suite_classes_false(
        ...
        different_and_not_equivalent_suite.expectation_configs[0].kwargs = {
            "column": "b",
            "value_set": [1, 2, 3],
            "result_format": "BASIC",
        }
    ...
    def test_is_equivalent_to_expectation_suite_classes_false_multiple_equivalent_expectations(
        ...
        different_and_not_equivalent_suite.expectation_configs.append(
            different_and_not_equivalent_suite.expectation_configs[0]
        )
        assert len(suite_with_single_expectation.expectation_configs) == 1
        assert len(different_and_not_equivalent_suite.expectation_configs) == 2
...
def test_expectation_suite_copy(baseline_suite):
    ...
    suite_copy.expectation_configs[0].meta["notes"] = "a different note"
    assert (
        baseline_suite.expectation_configs[0].meta["notes"] == "a different note"
...
def test_expectation_suite_deepcopy(baseline_suite):
    ...
    suite_deepcopy.expectation_configs[0].meta["notes"] = "a different note"
    # deepcopy on deep attributes does not propagate
    assert (
        baseline_suite.expectation_configs[0].meta["notes"] == "This is an expectation."
    )
