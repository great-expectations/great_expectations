...
def test_alice_fixture_generation(
    ...
    assert (
        alice_columnar_table_single_batch["sample_data_relative_path"] == "alice_columnar_table_single_batch_data.csv"
    )
    ...
    assert len(alice_columnar_table_single_batch["expected_expectation_suite"].expectation_configurations) == 22
...
def test_bobby_fixture_generation(
    ...
    assert isinstance(profiler_config, str)
    assert isinstance(
        bobby_columnar_table_multi_batch["test_configuration_quantiles_estimator"]["expected_expectation_suite"],
        ExpectationSuite,
    )
    assert (
        len(
