...
def test_spark_null_filters(spark_session):
    ...
    df = spark_session.createDataFrame(
        ...
        schema=pyspark.sql.types.StructType([pyspark.sql.types.StructField("a", pyspark.sql.types.FloatType())]),
    )
    assert df.agg(F.max(F.col("a"))).collect()[0][0] == 4

    df = spark_session.createDataFrame(
        pd.DataFrame({"a": [1, 2, 3, 4]}),
        schema=pyspark.sql.types.StructType([pyspark.sql.types.StructField("a", pyspark.sql.types.IntegerType())]),
    )
    assert df.agg(F.max(F.col("a"))).collect()[0][0] == 4
    ...
    assert df.agg(F.max(F.when(~F.isnull(F.col("a")) & ~F.isnan(F.col("a")), F.col("a")))).collect()[0][0] == 4
    ...
    assert (
        df.agg(F.max(F.when(~F.isnull(F.col("a")) & ~F.isnan(F.col("a")), F.col("a")).otherwise(0))).collect()[0][0]
        == 4
    )
