...
def test__column_map_metric_provider_with_pandas_column_condition_partial_decorator(
    ...
    register_metric = mocker.patch(
        "great_expectations.expectations.metrics.map_metric_provider.map_metric_provider.register_metric"
    )
    ...
    # assertion about count of calls combined with assertion about included calls ensures that only the intended
    # metrics were registered
    assert register_metric.call_count == len(metrics_to_be_created)
    calls = [
        call(
            metric_name=name,
            metric_domain_keys=ANY,
            metric_value_keys=ANY,
            execution_engine=ANY,
            metric_class=ANY,
            metric_provider=ANY,
            metric_fn_type=ANY,
        )
        for name in metrics_to_be_created
    ]
    register_metric.assert_has_calls(calls, any_order=True)
...
def test__column_map_metric_provider_with_incorrect_column_pair_decorators():
    ...
    assert [
        metric
        for metric in _registered_metrics.keys()
        if metric.startswith("column_values.equal_seven")
    ] == []
...
def test__column_map_metric_provider_with_all_three_column_condition_partial_decorators():
    ...
    class_, method_ = _registered_metrics[
        "column_values.equal_seven.unexpected_values"
    ...
    class_, method_ = _registered_metrics[
        "column_values.equal_seven.unexpected_values"
    ]["providers"]["SqlAlchemyExecutionEngine"]
    # print(class_)
    print(method_)
    print(signature(method_))
    ...
    class_, method_ = _registered_metrics[
        "column_values.equal_seven.unexpected_values"
    ]["providers"]["SparkDFExecutionEngine"]
    # print(class_)
    print(method_)
    print(signature(method_))
...
def test__column_aggregate_metric_provider_with_all_three_decorators():
    ...
    assert [
        metric
        for metric in _registered_metrics.keys()
        if metric.startswith("column.sum_equals_seven")
    ] == []
    ...
    assert set(
        _registered_metrics["column.sum_equals_seven.aggregate_fn"]["providers"]
    ) == {
        "SqlAlchemyExecutionEngine",
        "SparkDFExecutionEngine",
    }
    ...
    class_, method_ = _registered_metrics["column.sum_equals_seven"]["providers"][
        "PandasExecutionEngine"
    ]
    print("=" * 80)
    print(class_)
