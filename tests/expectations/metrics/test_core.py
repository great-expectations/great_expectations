...
def test_column_quoted_name_type_sa(sa):
    ...
    quoted_batch_column_list = [
        sqlalchemy.quoted_name(value=str(column_name), quote=True) for column_name in batch_column_list
    ]
...
def test_map_of_type_sa(sa):
    ...
    batch_data = SqlAlchemyBatchData(execution_engine=eng, table_name="test", source_table_name="test")
...
@pytest.mark.filterwarnings("ignore:pandas.Int64Index is deprecated*:FutureWarning:tests.expectations.metrics")
@pytest.mark.big
def test_map_column_values_increasing_pd():
    ...
    unexpected_rows_metric = MetricConfiguration(
        ...
        metric_domain_kwargs={"column": "a"},
        metric_value_kwargs={"result_format": {"result_format": "SUMMARY", "partial_unexpected_count": 1}},
    )
...
def test_map_unique_column_exists_sa(sa):
    ...
    desired_metric = MetricConfiguration(
        ...
        metric_domain_kwargs={"column": "a"},
        metric_value_kwargs={"result_format": {"result_format": "BASIC", "partial_unexpected_count": 20}},
    )
    desired_metric.metric_dependencies = {
        "unexpected_condition": condition_metric,
        "table.columns": table_columns_metric,
...
def test_z_score_under_threshold_pd():
    ...
    results = engine.resolve_metrics(metrics_to_resolve=(column_values_z_score_map_metric,), metrics=metrics)
    metrics.update(results)
    ...
    assert list(results[column_values_z_score_under_threshold_condition_metric.id][0]) == [False, False, False]
    metrics.update(results)
    desired_metric = MetricConfiguration(
...
def test_table_metric_pd(caplog):
    ...
    assert 'Unexpected key(s) "column" found in domain_kwargs for Domain type "table"' in caplog.text
...
def test_map_column_pairs_equal_metric_pd():  # noqa: PLR0915
    ...
    condition_metric_name: str = f"{metric_name}.{MetricPartialFunctionTypeSuffixes.CONDITION.value}"
    unexpected_count_metric_name: str = f"{metric_name}.{SummarizationMetricNameSuffixes.UNEXPECTED_COUNT.value}"
    unexpected_rows_metric_name: str = f"{metric_name}.{SummarizationMetricNameSuffixes.UNEXPECTED_ROWS.value}"
    unexpected_values_metric_name: str = f"{metric_name}.{SummarizationMetricNameSuffixes.UNEXPECTED_VALUES.value}"
    ...
    unexpected_rows_metric = MetricConfiguration(
        ...
        metric_value_kwargs={"result_format": {"result_format": "SUMMARY", "partial_unexpected_count": 3}},
    )
    unexpected_rows_metric.metric_dependencies = {
        "unexpected_condition": condition_metric,
        "table.columns": table_columns_metric,
    ...
    results = engine.resolve_metrics(metrics_to_resolve=(unexpected_values_metric,), metrics=metrics)
    metrics.update(results)
...
def test_map_column_pairs_greater_metric_pd():
    ...
    assert results[condition_metric.id][0].reset_index(drop=True).equals(pd.Series([False, False, False, False]))
...
def test_map_column_pairs_in_set_metric_pd():
    ...
    assert results[condition_metric.id][0].reset_index(drop=True).equals(pd.Series([True, False, False, False]))
...
def test_column_median_metric_sa(sa, dataframe: pd.DataFrame, median: int):
    ...
    results = engine.resolve_metrics(metrics_to_resolve=(column_values_null_condition_metric,), metrics=metrics)
    metrics.update(results)
    ...
    results = engine.resolve_metrics(metrics_to_resolve=(column_values_nonnull_count_metric,), metrics=metrics)
    metrics.update(results)
...
def test_value_counts_metric_sa(sa):
    engine = build_sa_execution_engine(pd.DataFrame({"a": [1, 2, 1, 2, 3, 3], "b": [4, 4, 4, 4, 4, 4]}), sa)
...
def test_value_counts_metric_spark(spark_session):
    ...
    assert pd.Series(index=[1.0, 2.0, 3.0, np.nan], data=[2, 2, 2, 1]).equals(metrics[desired_metric.id])
...
def test_distinct_metric_spark(
    ...
    results = engine.resolve_metrics(metrics_to_resolve=(column_distinct_values_count_metric,), metrics=metrics)
    metrics.update(results)
...
def test_distinct_metric_pd():
    ...
    results = engine.resolve_metrics(metrics_to_resolve=(column_distinct_values_metric,), metrics=metrics)
    metrics.update(results)
    assert metrics[column_distinct_values_metric.id] == {1, 2, 3}
...
def test_map_multicolumn_sum_equal_pd():  # noqa: PLR0915
    engine = build_pandas_engine(pd.DataFrame(data={"a": [0, 1, 2], "b": [5, 4, 3], "c": [0, 0, 1], "d": [7, 8, 9]}))
...
def test_map_compound_columns_unique_pd():  # noqa: PLR0915
    engine = build_pandas_engine(pd.DataFrame(data={"a": [0, 1, 1], "b": [1, 2, 3], "c": [0, 2, 2]}))
...
def test_map_compound_columns_unique_sa(sa):  # noqa: PLR0915
    ...
    prerequisite_function_metric_name: str = f"compound_columns.count.{MetricPartialFunctionTypeSuffixes.MAP.value}"
...
def test_map_select_column_values_unique_within_record_pd():  # noqa: PLR0915
    ...
    unexpected_rows_metric = MetricConfiguration(
        ...
        metric_value_kwargs={"result_format": {"result_format": "SUMMARY", "partial_unexpected_count": 8}},
    )
    unexpected_rows_metric.metric_dependencies = {
        "unexpected_condition": condition_metric,
        "table.columns": table_columns_metric,
    ...
    for unexpected_value_dict in metrics[unexpected_values_metric.id]:
        updated_unexpected_value_dict = {
            key: "NULL" if np.isnan(value) else value for key, value in unexpected_value_dict.items()
        }
