...
def test_unexpected_map_series_and_domain_values_column_value_lengths_between_pd():
    engine = build_pandas_engine(
        pd.DataFrame({"a": ["a", "aaa", "bcbc", "defgh", None]})
    )
    ...
    map_metric = MetricConfiguration(
        metric_name="column_values.value_length.map",
        metric_domain_kwargs={"column": "a"},
        metric_value_kwargs=None,
    )
    map_metric.metric_dependencies = {
        "table.columns": table_columns_metric,
    }
    results = engine.resolve_metrics(metrics_to_resolve=(map_metric,), metrics=metrics)
    metrics.update(results)

    condition_metric = MetricConfiguration(
        metric_name="column_values.value_length.between.condition",
        metric_domain_kwargs={"column": "a"},
        metric_value_kwargs={
            "min_value": 3,
            "max_value": 4,
    ...
    condition_metric.metric_dependencies = {
        "column_values.value_length.map": map_metric,
        "table.columns": table_columns_metric,
    }
    results = engine.resolve_metrics(
        metrics_to_resolve=(condition_metric,), metrics=metrics
    ...
    unexpected_map_series_and_domain_values_metric = MetricConfiguration(
        metric_name="column_values.value_length.unexpected_map_series_and_domain_values",
        metric_domain_kwargs={"column": "a"},
        metric_value_kwargs={
            "result_format": {"result_format": "SUMMARY", "partial_unexpected_count": 5}
        },
    )
    unexpected_map_series_and_domain_values_metric.metric_dependencies = {
        "unexpected_condition": condition_metric,
        "metric_partial_fn": map_metric,
        "table.columns": table_columns_metric,
    }
    results = engine.resolve_metrics(
        metrics_to_resolve=(unexpected_map_series_and_domain_values_metric,),
        metrics=metrics,
    )
    metrics.update(results)

    assert results[unexpected_map_series_and_domain_values_metric.id] == (
        ["a", "defgh"],
        [1, 5],
    )
