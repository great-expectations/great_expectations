...
def basic_execution_environment(tmp_path_factory):
    base_directory = str(
        tmp_path_factory.mktemp(
            "basic_execution_environment__filesystem_data_connector"
        )
    )
...
def test_get_batch_list_from_batch_request(basic_execution_environment):
    ...
    batch_request: dict = {
        ...
        "partition_request": {"key": "Titanic.csv"},
        # "limit": None,
...
def test_some_very_basic_stuff(basic_execution_environment):
    my_data_connector = basic_execution_environment.get_data_connector(
        "my_filesystem_data_connector"
    )
    create_files_in_directory(
    ...
    batch_list = basic_execution_environment.get_batch_list_from_batch_request(
        BatchRequest(
            execution_environment_name="my_execution_environment",
            data_connector_name="my_filesystem_data_connector",
            data_asset_name="B1",
            partition_request={"letter": "B", "number": "1",},
        )
    )
