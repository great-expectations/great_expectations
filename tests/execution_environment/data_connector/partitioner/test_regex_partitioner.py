...
def test_regex_partitioner_regex_no_groups_named():
    ...
    regex_partitioner = RegexPartitioner(
        name="test_regex_partitioner", pattern=pattern,
    )
    # noinspection PyProtectedMember
...
def test_regex_partitioner_regex_groups_named():
    ...
    regex_partitioner = RegexPartitioner(
        name="test_regex_partitioner", pattern=pattern, group_names=group_names
    )
    # noinspection PyProtectedMember
    assert regex_partitioner._pattern == r".+\/(.+)_(.+)_(.+)\.csv"
    # noinspection PyProtectedMember
...
def test_regex_partitioner_regex_does_not_match_paths():
    ...
    # Nothing configured
    partitions = regex_partitioner.find_or_create_partitions(
        paths=paths, data_asset_name="test_asset_0"
    )
    assert partitions == []
...
def test_regex_partitioner_compute_partitions_paths_with_default_regex_config_autodiscover_assets():
    ...
    # no sorters configured
    partitions = regex_partitioner.find_or_create_partitions(
        paths=paths, auto_discover_assets=True
    )
    assert partitions == [
        Partition(
            name="alex_20200809_1000.csv",
        ...
        Partition(
            ...
            definition=PartitionDefinition({"group_0": "eugene_20200810_1500.csv"}),
            data_reference="eugene_20200810_1500.csv",
            data_asset_name="eugene_20200810_1500",
        ),
        Partition(
            name="abe_20200831_1040.csv",
            definition=PartitionDefinition({"group_0": "abe_20200831_1040.csv"}),
            data_reference="abe_20200831_1040.csv",
            data_asset_name="abe_20200831_1040",
        ),
    ]
...
def test_regex_partitioner_compute_partitions_auto_discover_assets_true():
    ...
    assert partitions == [
        Partition(
            ...
            data_reference="my_dir/alex_20200809_1000.csv",
            data_asset_name="alex_20200809_1000",
        ),
        Partition(
            name="eugene-20200810-1500",
            definition=PartitionDefinition(
                {"name": "eugene", "timestamp": "20200810", "price": "1500"}
            ),
            data_reference="my_dir/eugene_20200810_1500.csv",
            data_asset_name="eugene_20200810_1500",
        ),
        Partition(
            name="abe-20200831-1040",
            definition=PartitionDefinition(
                {"name": "abe", "timestamp": "20200831", "price": "1040"}
            ),
            data_reference="my_dir/abe_20200831_1040.csv",
            data_asset_name="abe_20200831_1040",
        ),
    ]
...
def test_regex_partitioner_compute_partitions_auto_discover_assets_false_no_data_asset_name():
    ...
    partitions = regex_partitioner.find_or_create_partitions(
        paths=paths, auto_discover_assets=False
    )
    assert partitions == [
        Partition(
            name="alex-20200809-1000",
...
def test_regex_partitioner_compute_partitions_sorters_and_groups_names_do_not_match():
    ...
    with pytest.raises(ge_exceptions.PartitionerError):
        regex_partitioner.find_or_create_partitions(
            paths=paths, data_asset_name="test_asset_0"
        )
...
def test_convert_batch_request_to_data_reference():
    ...
    assert (
        regex_partitioner.convert_batch_request_to_data_reference(
            BatchRequest(
...
def test__invert_regex_to_data_reference_template():
    ...
    assert (
        regex_partitioner._invert_regex_to_data_reference_template()
    ...
    assert (
        regex_partitioner._invert_regex_to_data_reference_template() == "{name}-*.csv"
    )
...
def test_convert_data_reference_to_batch_request():
    ...
    print(
        regex_partitioner.convert_data_reference_to_batch_request(
            "alex_20200809_1000.csv"
        )
    )
    ...
    assert (
        regex_partitioner.convert_data_reference_to_batch_request(
            "DOESNT_MATCH_CAPTURING_GROUPS.csv"
        )
        is None
    )
