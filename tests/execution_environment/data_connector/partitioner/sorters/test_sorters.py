...
def test_sorter_instantiation_datetime():
    sorter_params: dict = {"config_params": {"datetime_format": "%Y%m%d",}}
    # DateTimeSorter
...
def test_sorter_instantiation_custom_list():
    # CustomListSorter
    sorter_params: dict = {"config_params": {"reference_list": ["a", "b", "c"],}}
    ...
    with pytest.raises(ge_exceptions.SorterError):
        my_custom = CustomListSorter(name="custom", orderby="asc", **sorter_params)
    sorter_params: dict = {"config_params": {"reference_list": None}}
    with pytest.raises(ge_exceptions.SorterError):
        my_custom = CustomListSorter(name="custom", orderby="asc", **sorter_params)
    sorter_params: dict = {"config_params": {"reference_list": 1}}  # not a list
    with pytest.raises(ge_exceptions.SorterError):
        my_custom = CustomListSorter(name="custom", orderby="asc", **sorter_params)
...
def test_sorter_instantiation_custom_list_with_periodic_table(
    ...
    # CustomListSorter
    sorter_params: dict = {
        "config_params": {"reference_list": periodic_table_of_elements,}
    }
