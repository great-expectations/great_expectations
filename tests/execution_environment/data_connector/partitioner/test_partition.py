...
def test_partition():
        test_partition = Partition(name="test", data_asset_name="fake", definition={"name": "hello"}, source="nowhere")
    # properties
    assert test_partition.name == 'test'
    assert test_partition.data_asset_name == 'fake'
    assert test_partition.definition == {"name": "hello"}
    assert test_partition.source == 'nowhere'
    assert test_partition.__repr__() == str({'name': 'test', 'data_asset_name': 'fake', 'definition': {'name': 'hello'},
                                            'source': 'nowhere'})
    # test __eq__()
    test_partition1 = Partition(name="test", data_asset_name="fake", definition={"name": "hello"}, source="nowhere")
    test_partition2 = Partition(name="test", data_asset_name="fake", definition={"name": "hello"}, source="nowhere")
    test_partition3 = Partition(name="i_am_different", data_asset_name="fake", definition={"name": "hello"}, source="nowhere")
