...
def test_add_batch_definition_fluent_sql__add_batch_definition_whole_table():
    # arrange
    mock_datasource = create_autospec(SQLDatasource)
    asset = _SQLAsset(
        name="test_asset",
    )
    asset._datasource = mock_datasource  # same pattern Datasource uses to init Asset
    name = "batch_def_name"
    expected_batch_definition = BatchDefinition(name=name, partitioner=None, batching_regex=None)
    mock_datasource.add_batch_definition.return_value = expected_batch_definition
    ...
    mock_datasource.add_batch_definition.assert_called_once_with(expected_batch_definition)
