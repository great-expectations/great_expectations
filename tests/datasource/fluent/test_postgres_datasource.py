...
def _source(
    ...
    partitioner_response = partitioner_query_response or (
        [{"year": year, "month": month} for year in _DEFAULT_TEST_YEARS for month in _DEFAULT_TEST_MONTHS]
    )
...
def mock_test_connection(monkeypatch: pytest.MonkeyPatch):
    ...
    def _mock_test_connection(self: PostgresDatasource) -> bool:
        LOGGER.warning(f"Mocked {self.__class__.__name__}.test_connection() called and returning True")
        return True
...
def assert_batch_request(batch_request, source_name: str, asset_name: str, options: BatchRequestOptions):
...
def test_add_table_asset_with_partitioner(mocker, create_source: CreateSourceFixture):
    with create_source(validate_batch_spec=lambda _: None, dialect="postgresql") as source:
        ...
        assert_batch_request(
            batch_request=asset.build_batch_request(options={"year": 2021, "month": 10}, partitioner=partitioner),
...
def test_datasource_gets_batch_list_no_partitioner(empty_data_context, create_source):
    ...
    with create_source(
        ...
        (
            source,  # noqa: PLW2901
            asset,
        ) = create_and_add_table_asset_without_testing_connection(source=source, name="my_asset", table_name="my_table")
...
def test_datasource_gets_batch_list_with_fully_specified_batch_request_options(
    ...
    with create_source(
        ...
        partitioner = PartitionerYearAndMonth(column_name="my_col")
        batches = source.get_batch_list_from_batch_request(
            asset.build_batch_request(options={"month": month, "year": year}, partitioner=partitioner)
        )
...
def test_validate_valid_postgres_connection_string(create_source: CreateSourceFixture, connection_string):
...
def test_validate_invalid_postgres_connection_string(create_source: CreateSourceFixture, connection_string):
...
def test_adding_partitioner_persists_results(
    ...
    gx_yaml = pathlib.Path(empty_data_context.root_directory, FileDataContext.GX_YML).resolve(strict=True)

    empty_data_context.sources.add_postgres(
        name="my_datasource",
        connection_string="postgresql://postgres:@localhost/not_a_real_db",
    ).add_query_asset(name="my_asset", query="select * from table", order_by=["year"]).add_batch_config(
    ...
    assert final_yaml["my_datasource"]["assets"]["my_asset"]["batch_configs"]["my_batch_config"]["partitioner"]
...
def test_partitioner_year(
    ...
    with create_source(
        ...
        # in this unit test.
        asset = source.add_query_asset(name="my_asset", query="select * from table", order_by=["year"])
        partitioner = PartitionerYear(column_name="my_col")
        batches = source.get_batch_list_from_batch_request(asset.build_batch_request(partitioner=partitioner))
...
def test_partitioner_year_and_month(
    ...
    with create_source(
        ...
        dialect="postgresql",
        data_context=empty_data_context,
        partitioner_query_response=[{"year": year, "month": month} for year in years for month in months],
    ) as source:
        # We use a query asset because then we don't have to mock out db connection tests
        # in this unit test.
        asset = source.add_query_asset(name="my_asset", query="select * from table", order_by=["year", "month"])
        partitioner = PartitionerYearAndMonth(column_name="my_col")
        batches = source.get_batch_list_from_batch_request(asset.build_batch_request(partitioner=partitioner))
...
def test_partitioner_year_and_month_and_day(
    ...
    with create_source(
        ...
        partitioner_query_response=[
            {"year": year, "month": month, "day": day} for year in years for month in months for day in days
        ],
    ) as source:
        # We use a query asset because then we don't have to mock out db connection tests
        # in this unit test.
...
def test_partitioner(
    ...
    with create_source(
        ...
        all_batches = asset.get_batch_list_from_batch_request(asset.build_batch_request(partitioner=partitioner))
