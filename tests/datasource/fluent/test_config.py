...
CSV_PATH = FLUENT_DATASOURCE_TEST_DIR.joinpath(pathlib.Path("..", "..", "test_sets", "taxi_yellow_tripdata_samples"))
...
@pytest.mark.parametrize("asset_dict", [{"type": "json", "orient": "records"}, {"type": "csv", "sep": "|"}])
...
def test_id_only_serialized_if_present(ds_dict_config: dict):
    ...
    for ds in ds_dict_config[_FLUENT_DATASOURCES_KEY]:
        ...
        with_ids[ds_name]["assets"] = {
            asset_config[_DATA_ASSET_NAME_KEY]: asset_config for asset_config in with_ids[ds_name]["assets"]
        }
        no_ids[ds_name]["assets"] = {
            asset_config[_DATA_ASSET_NAME_KEY]: asset_config for asset_config in no_ids[ds_name]["assets"]
        }
        ...
        for asset_config in ds["assets"]:
            ...
            if _BATCH_CONFIGS_KEY in asset_config:
                ...
                for batch_config in with_ids[ds_name]["assets"][asset_name][_BATCH_CONFIGS_KEY].values():
                ...
                for batch_config in no_ids[ds_name]["assets"][asset_name][_BATCH_CONFIGS_KEY].values():
...
def test_batch_configs_are_assigned_data_assets(inject_engine_lookup_double, load_method: Callable, input_):
    loaded: GxConfig = load_method(input_)
    assert loaded
...
@pytest.mark.filesystem
def test_yaml_config_round_trip_ordering(inject_engine_lookup_double, from_yaml_gx_config: GxConfig):
    dumped: str = from_yaml_gx_config.yaml()
...
def test_custom_sorter_serialization(inject_engine_lookup_double, from_json_gx_config: GxConfig):
    ...
    expected_sorter_strings: List[str] = COMPLEX_CONFIG_DICT[_FLUENT_DATASOURCES_KEY]["my_pg_ds"]["assets"][
        "with_dslish_sorters"
    ]["order_by"]
...
def test_dict_default_pandas_config_round_trip(inject_engine_lookup_double):
    ...
    from_dict_default_pandas_config = GxConfig.parse_obj(DEFAULT_PANDAS_DATASOURCE_AND_DATA_ASSET_CONFIG_DICT)
    assert (
        DEFAULT_PANDAS_DATA_ASSET_NAME
    ...
    from_dict_only_default_pandas_config = GxConfig.parse_obj(only_default_pandas_datasource_and_data_asset_config_dict)
...
def test_config_substitution_retains_original_value_on_save(
    ...
    original: dict = cast(dict, yaml.load(file_dc_config_file_with_substitutions.read_text()))[_FLUENT_DATASOURCES_KEY][
        "my_sqlite_ds_w_subs"
    ]
    ...
    round_tripped = cast(dict, yaml.load(file_dc_config_file_with_substitutions.read_text()))[_FLUENT_DATASOURCES_KEY][
        "my_sqlite_ds_w_subs"
    ]
...
def test_config_substitution_retains_original_value_on_save_w_run_time_mods(
    ...
    round_tripped_datasources = cast(dict, yaml.load(file_dc_config_file_with_substitutions.read_text()))[
        _FLUENT_DATASOURCES_KEY
    ]
