...
def test_get_batch_definition_list_from_batch_request(glue_titanic_catalog):
    ...
    my_data_connector: ConfiguredAssetAWSGlueDataCatalogDataConnector = instantiate_class_from_config(
...
def test_instantiation_with_partitioner_sampling_and_prefix(partitioner_method_name_prefix, glue_titanic_catalog):
    # noinspection PyTypeChecker
    my_data_connector = ConfiguredAssetAWSGlueDataCatalogDataConnector(
        name="my_glue_catalog_data_connector",
        datasource_name="FAKE_Datasource_NAME",
...
def test_build_batch_spec_with_all_options(partitioner_method_name_prefix, glue_titanic_catalog):
    ...
    assert batch_spec.get("partitioner_method", None) == f"{partitioner_method_name_prefix}partition_on_column_value"
    assert batch_spec.get("partitioner_kwargs", {}) == {
        "column_name": "PClass",
        "batch_identifiers": {"PClass": "1st"},
    }
    assert batch_spec.get("sampling_method", None) == f"{partitioner_method_name_prefix}sample_using_random"
