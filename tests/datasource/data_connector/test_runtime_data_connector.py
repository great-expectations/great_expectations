...
def test_add_batch_identifiers_correct(basic_datasource_with_assets):
    test_runtime_data_connector: RuntimeDataConnector = basic_datasource_with_assets.data_connectors["runtime"]
...
def test_error_checking_unknown_datasource(basic_datasource):
    ...
    test_runtime_data_connector: RuntimeDataConnector = basic_datasource.data_connectors["test_runtime_data_connector"]
...
@pytest.mark.unit
def test_asset_is_name_batch_identifier_correctly_used(basic_datasource_with_assets, test_df_pandas):
    """
    Using asset_a, which is named in the RuntimeDataConnector configuration, and using batch_identifier that is named.
    """
    runtime_data_connector: RuntimeDataConnector = basic_datasource_with_assets.data_connectors["runtime"]
    res: List[BatchDefinition] = runtime_data_connector.get_batch_definition_list_from_batch_request(
        batch_request=RuntimeBatchRequest(
...
@pytest.mark.unit
def test_asset_is_named_but_batch_identifier_in_other_asset(basic_datasource_with_assets, test_df_pandas):
    runtime_data_connector: RuntimeDataConnector = basic_datasource_with_assets.data_connectors["runtime"]
    with pytest.raises(gx_exceptions.DataConnectorError) as data_connector_error:
...
@pytest.mark.unit
def test_asset_is_named_but_batch_identifier_not_defined_anywhere(basic_datasource_with_assets, test_df_pandas):
    runtime_data_connector: RuntimeDataConnector = basic_datasource_with_assets.data_connectors["runtime"]
    with pytest.raises(gx_exceptions.DataConnectorError) as data_connector_error:
        runtime_data_connector.get_batch_definition_list_from_batch_request(
            batch_request=RuntimeBatchRequest(
...
def test_batch_identifiers_and_batch_identifiers_success_all_keys_present(
    ...
    batch_definition_list: List[BatchDefinition] = (
        test_runtime_data_connector.get_batch_definition_list_from_batch_request(batch_request=batch_request)
    )
...
def test_data_references_cache_updating_after_batch_request_named_assets(
    ...
    # run with my_data_asset_1
    batch_definitions: List[BatchDefinition] = runtime_data_connector.get_batch_definition_list_from_batch_request(
...
def test__get_data_reference_name(basic_datasource):
    ...
    assert test_runtime_data_connector._get_data_reference_name(batch_identifiers) == "1234567890-1111111111"
