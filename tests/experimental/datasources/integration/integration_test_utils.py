...
def run_data_assistant_and_checkpoint(
    ...
    print(
        f"[ALEX_TEST] [run_data_assistant_and_checkpoint()] expectation_num: {expectation_num} ; TYPE: {str(type(expectation_num))}"
    )
...
def run_multibatch_data_assistant_and_checkpoint(
    ...
    expectation_num = len(data_assistant_result.expectation_configurations)
    print(
        f"[ALEX_TEST] [run_multibatch_data_assistant_and_checkpoint()] expectation_num: {expectation_num} ; TYPE: {str(type(expectation_num))}"
    )
    if isinstance(execution_engine, (PandasExecutionEngine, SparkDFExecutionEngine)):
        assert expectation_num == 109

    if isinstance(execution_engine, SqlAlchemyExecutionEngine):
        assert expectation_num == 111
