...
def test_compute_metric_run_with_no_metric_retrievers():
    ...
    metric_run = batch_inspector.compute_metric_run(data_asset_id=data_asset_id, batch_request=mock_batch_request)
...
def test_compute_metric_run_calls_metric_retrievers():
    ...
    batch_inspector = BatchInspector(context=mock_context, metric_retrievers=[mock_metric_retriever])
    mock_batch_request = Mock(spec=BatchRequest)

    data_asset_id = uuid.uuid4()

    batch_inspector.compute_metric_run(data_asset_id=data_asset_id, batch_request=mock_batch_request)
