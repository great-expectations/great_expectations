...
def _assert_serialized_datasource_configs_are_equal(
    ...
    if serializers is None:
        serializers = [DictConfigSerializer(schema=datasourceConfigSchema)] * (len(datasource_configs) + 1)
    else:
        if len(serializers) <= 1:
            raise AssertionError("Must provide at least 2 datasource serializers")
        if not len(datasource_configs) == len(serializers):
            raise AssertionError("Must provide the same number of serializers as datasource configs")

    for idx, config in enumerate(datasource_configs[:-1]):
        assert serializers[idx].serialize(config) == serializers[idx + 1].serialize(datasource_configs[idx + 1])
...
def test__assert_serialized_datasource_configs_are_equal(
    ...
    # Happy path
    _assert_serialized_datasource_configs_are_equal([block_config_datasource_config, block_config_datasource_config])
    _assert_serialized_datasource_configs_are_equal(
        [
    ...
    # Unequal configs
    with pytest.raises(AssertionError):
        _assert_serialized_datasource_configs_are_equal([block_config_datasource_config, datasource_config_with_names])
...
def test_datasource_store_retrieval(
    ...
    retrieved_config_serializer = YAMLReadyDictDatasourceConfigSerializer(schema=datasourceConfigSchema)
    _assert_serialized_datasource_configs_are_equal(
...
def test_datasource_store_set(
    ...
    assert serializer.serialize(retrieved_datasource_config) == serializer.serialize(block_config_datasource_config)
...
def test_datasource_store_update_by_name(
    ...
    updated_datasource_config.data_connectors["tripdata_monthly_configured"]["base_directory"] = updated_base_directory
