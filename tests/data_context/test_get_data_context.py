...
@pytest.mark.unit
def test_ephemeral_context(clear_env_vars):
...
@pytest.mark.unit
def test_file_context__with_overridden_yml(tmp_path: pathlib.Path, clear_env_vars):
...
@pytest.mark.unit
def test_file_context(tmp_path: pathlib.Path, clear_env_vars):
    project_path = tmp_path / "empty_data_context"
    project_path.mkdir()
    project_path_str = str(project_path)
    gx.data_context.DataContext.create(project_path_str)
...
@pytest.mark.unit
def test_data_context_root_dir_returns_file_context(
...
@pytest.mark.unit
def test_file_context_invalid_root_dir(clear_env_vars):
    config: DataContextConfig = DataContextConfig(
        config_version=3.0,
        plugins_directory=None,
        evaluation_parameter_store_name="evaluation_parameter_store",
