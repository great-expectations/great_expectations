...
@pytest.mark.filterwarnings("ignore:SQLAlchemy is not installed*:UserWarning:great_expectations.data_context.util")
def test_password_masker_mask_db_url(  # noqa: PLR0912,PLR0915,C901- 11
    ...
    assert (
        PasswordMasker.mask_db_url(f"postgresql://scott:tiger@{db_hostname}/mydatabase", use_urlparse=True)
    ...
    try:
        assert (
            PasswordMasker.mask_db_url(f"postgresql+psycopg2://scott:tiger@{db_hostname}:65432/mydatabase")
    ...
    try:
        assert (
            PasswordMasker.mask_db_url(f"postgresql+pg8000://scott:tiger@{db_hostname}:65432/mydatabase")
    ...
    assert (
        PasswordMasker.mask_db_url(f"mysql://scott:tiger@{db_hostname}:65432/foo", use_urlparse=True)
        == f"mysql://scott:{PasswordMasker.MASKED_PASSWORD_STRING}@{db_hostname}:65432/foo"
    )
    ...
    try:
        ...
        pass
    assert (
        PasswordMasker.mask_db_url(f"mysql+mysqldb://scott:tiger@{db_hostname}:65432/foo", use_urlparse=True)
        == f"mysql+mysqldb://scott:{PasswordMasker.MASKED_PASSWORD_STRING}@{db_hostname}:65432/foo"
    )
    ...
    try:
        ...
        pass
    assert (
        PasswordMasker.mask_db_url(f"mysql+pymysql://scott:tiger@{db_hostname}:65432/foo", use_urlparse=True)
        == f"mysql+pymysql://scott:{PasswordMasker.MASKED_PASSWORD_STRING}@{db_hostname}:65432/foo"
    )
    ...
    assert (
        PasswordMasker.mask_db_url(f"oracle://scott:tiger@{url_host}:1521/sidname", use_urlparse=True)
    ...
    try:
        assert PasswordMasker.mask_db_url("mssql+pyodbc://scott:tiger@mydsn") == "mssql+pyodbc://scott:***@mydsn"
    except ModuleNotFoundError:
        pass
    assert (
    ...
    try:
        ...
        pass
    assert (
        PasswordMasker.mask_db_url(f"mssql+pymssql://scott:tiger@{db_hostname}:12345/dbname", use_urlparse=True)
        == f"mssql+pymssql://scott:{PasswordMasker.MASKED_PASSWORD_STRING}@{db_hostname}:12345/dbname"
    )
    ...
    assert PasswordMasker.mask_db_url("sqlite:///something/foo.db", use_urlparse=True) == "sqlite:///something/foo.db"

    # absolute path
    # Unix/Mac - 4 initial slashes in total
    assert PasswordMasker.mask_db_url("sqlite:////absolute/path/to/foo.db") == "sqlite:////absolute/path/to/foo.db"
    assert (
    ...
    # Windows
    assert PasswordMasker.mask_db_url("sqlite:///C:\\path\\to\\foo.db") == "sqlite:///C:\\path\\to\\foo.db"
    assert (
    ...
    assert PasswordMasker.mask_db_url(r"sqlite:///C:\path\to\foo.db") == r"sqlite:///C:\path\to\foo.db"
    assert (
        PasswordMasker.mask_db_url(r"sqlite:///C:\path\to\foo.db", use_urlparse=True) == r"sqlite:///C:\path\to\foo.db"
    )
...
def test_sanitize_config_masks_cloud_store_backend_access_tokens(
    ...
    config_with_creds_in_stores = PasswordMasker.sanitize_config(data_context_config_dict_with_cloud_backed_stores)
    for name, store_config in config_with_creds_in_stores["stores"].items():
        if (
            ...
            assert store_config["store_backend"]["ge_cloud_credentials"]["access_token"] != ge_cloud_access_token
...
def test_sanitize_config_masks_execution_engine_connection_strings(
    ...
    config_with_creds_masked = PasswordMasker.sanitize_config(data_context_config_dict_with_datasources)
    ...
    for name, processed_config in masked_datasources.items():
        ...
        if processed_config.get("execution_engine") and processed_config["execution_engine"].get("connection_string"):
            # check if the connection string contains a password
            if conn_string_password in unaltered_datasources[name]["execution_engine"]["connection_string"]:
                # it does contain a password, so make sure its masked
                assert conn_string_password not in processed_config["execution_engine"]["connection_string"]
            else:
...
@pytest.mark.unit
def test_sanitize_config_with_nested_url_field(conn_string_password, conn_string_with_embedded_password):
...
def test_sanitize_config_masks_cloud_access_token(ge_cloud_access_token):
    ...
    res = PasswordMasker.sanitize_config(config_copy)
    assert res != config
    assert res["store_backend"]["ge_cloud_credentials"]["access_token"] == PasswordMasker.MASKED_PASSWORD_STRING
