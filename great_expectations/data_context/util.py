...
def instantiate_class_from_config(config, runtime_environment, config_defaults=None):
    ...
    if class_name is None:
        logger.warning(
            "Instantiating class from config without an explicit class_name is dangerous. Consider adding an explicit class_name for %s"
            % config.get("name")
        )
        try:
...
def substitute_config_variable(
    template_str, config_variables_dict, dollar_sign_escape_string: str = "\\$"
):
    import inspect
    ...
    try:
        match = re.finditer(
            "(?<!\\\\)\\$\\{(.*?)\\}|(?<!\\\\)\\$([_a-zA-Z][_a-zA-Z0-9]*)", template_str
        )
    except TypeError:
        return template_str
    for m in match:
...
def substitute_value_from_secret_store(value):
    ...
    if isinstance(value, str) and value.startswith("secret|"):
        ...
        elif re.compile("^secret\\|projects\\/[a-z0-9\\_\\-]{6,30}\\/secrets").match(
            value
        ):
...
def substitute_value_from_aws_secrets_manager(value):
    ...
    regex = re.compile(
        "^secret\\|arn:aws:secretsmanager:([a-z\\-0-9]+):([0-9]{12}):secret:([a-zA-Z0-9\\/_\\+=\\.@\\-]+)(?:\\:([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}))?(?:\\|([^\\|]+))?$"
    )
    if not boto3:
        logger.error(
            "boto3 is not installed, please install great_expectations with aws_secrets extra > pip install great_expectations[aws_secrets]"
        )
...
def substitute_value_from_gcp_secret_manager(value):
    ...
    regex = re.compile(
        "^secret\\|projects\\/([a-z0-9\\_\\-]{6,30})\\/secrets/([a-zA-Z\\_\\-]{1,255})(?:\\/versions\\/([a-z0-9]+))?(?:\\|([^\\|]+))?$"
    )
    if not secretmanager:
        logger.error(
            "secretmanager is not installed, please install great_expectations with gcp extra > pip install great_expectations[gcp]"
        )
...
def substitute_value_from_azure_keyvault(value):
    ...
    regex = re.compile(
        "^secret\\|(https:\\/\\/[a-zA-Z0-9\\-]{3,24}\\.vault\\.azure\\.net)\\/secrets\\/([0-9a-zA-Z-]+)(?:\\/([a-f0-9]{32}))?(?:\\|([^\\|]+))?$"
    )
    if not SecretClient:
        logger.error(
            "SecretClient is not installed, please install great_expectations with azure_secrets extra > pip install great_expectations[azure_secrets]"
        )
...
def substitute_all_config_variables(
    data, replace_variables_dict, dollar_sign_escape_string: str = "\\$"
):
    import inspect
