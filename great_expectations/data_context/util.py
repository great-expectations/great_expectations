...
def instantiate_class_from_config(config, runtime_environment, config_defaults=None):
    ...
    if (module_name is None):
        try:
            ...
            raise KeyError('Neither config : {} nor config_defaults : {} contains a module_name key.'.format(config, config_defaults))
    else:
        config_defaults.pop('module_name', None)
    verify_dynamic_loading_support(module_name=module_name)
    class_name = config.pop('class_name', None)
    if (class_name is None):
        logger.warning(('Instantiating class from config without an explicit class_name is dangerous. Consider adding an explicit class_name for %s' % config.get('name')))
        try:
            class_name = config_defaults.pop('class_name')
        except KeyError:
            raise KeyError('Neither config : {} nor config_defaults : {} contains a class_name key.'.format(config, config_defaults))
    else:
    ...
    if (runtime_environment is not None):
        ...
        config_with_defaults.update({missing_arg: runtime_environment[missing_arg] for missing_arg in missing_args if (missing_arg in runtime_environment)})
    ...
    try:
        ...
        raise TypeError(("Couldn't instantiate class: {} with config: \n\t{}\n \n".format(class_name, format_dict_for_error_message(config_with_defaults)) + str(e)))
    return class_instance

def build_store_from_config(store_name: str=None, store_config: dict=None, module_name: str='great_expectations.data_context.store', runtime_environment: dict=None):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if ((store_config is None) or (module_name is None)):
        return None
    try:
        config_defaults: dict = {'store_name': store_name, 'module_name': module_name}
        new_store = instantiate_class_from_config(config=store_config, runtime_environment=runtime_environment, config_defaults=config_defaults)
    ...
    if (not new_store):
        ...
        raise ge_exceptions.ClassInstantiationError(module_name=module_name, package_name=None, class_name=class_name)
    return new_store
...
def substitute_config_variable(template_str, config_variables_dict, dollar_sign_escape_string: str='\\$'):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (template_str is None):
        return template_str
    try:
        match = re.finditer('(?<!\\\\)\\$\\{(.*?)\\}|(?<!\\\\)\\$([_a-zA-Z][_a-zA-Z0-9]*)', template_str)
    except TypeError:
        return template_str
    for m in match:
        ...
        if (config_variable_value is not None):
            ...
            raise ge_exceptions.MissingConfigVariableError(f'''
...
See https://great-expectations.readthedocs.io/en/latest/reference/data_context_reference.html#managing-environment-and-secrets''', missing_config_variable=config_variable_name)
...
def substitute_value_from_secret_store(value):
    ...
    if (isinstance(value, str) and value.startswith('secret|')):
        ...
        elif re.compile('^secret\\|projects\\/[a-z0-9\\_\\-]{6,30}\\/secrets').match(value):
            return substitute_value_from_gcp_secret_manager(value)
        elif re.compile('^secret\\|https:\\/\\/[a-zA-Z0-9\\-]{3,24}\\.vault\\.azure\\.net').match(value):
...
def substitute_value_from_aws_secrets_manager(value):
    ...
    regex = re.compile('^secret\\|arn:aws:secretsmanager:([a-z\\-0-9]+):([0-9]{12}):secret:([a-zA-Z0-9\\/_\\+=\\.@\\-]+)(?:\\:([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}))?(?:\\|([^\\|]+))?$')
    if (not boto3):
        logger.error('boto3 is not installed, please install great_expectations with aws_secrets extra > pip install great_expectations[aws_secrets]')
...
def substitute_value_from_gcp_secret_manager(value):
    ...
    regex = re.compile('^secret\\|projects\\/([a-z0-9\\_\\-]{6,30})\\/secrets/([a-zA-Z\\_\\-]{1,255})(?:\\/versions\\/([a-z0-9]+))?(?:\\|([^\\|]+))?$')
    if (not secretmanager):
        logger.error('secretmanager is not installed, please install great_expectations with gcp extra > pip install great_expectations[gcp]')
...
def substitute_value_from_azure_keyvault(value):
    ...
    regex = re.compile('^secret\\|(https:\\/\\/[a-zA-Z0-9\\-]{3,24}\\.vault\\.azure\\.net)\\/secrets\\/([0-9a-zA-Z-]+)(?:\\/([a-f0-9]{32}))?(?:\\|([^\\|]+))?$')
    if (not SecretClient):
        logger.error('SecretClient is not installed, please install great_expectations with azure_secrets extra > pip install great_expectations[azure_secrets]')
...
def substitute_all_config_variables(data, replace_variables_dict, dollar_sign_escape_string: str='\\$'):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    return substitute_config_variable(data, replace_variables_dict, dollar_sign_escape_string)
...
class PasswordMasker():
    ...
    def mask_db_url(cls, url: str, use_urlparse: bool=False, **kwargs) -> str:
        ...
        if ((sa is not None) and (use_urlparse is False)):
            ...
            warnings.warn('SQLAlchemy is not installed, using urlparse to mask database url password which ignores **kwargs.')
    ...
    def _mask_db_url_no_sa(cls, url: str) -> str:
        ...
        masked_url = f"{parsed_url.scheme}://{parsed_url.username}:{cls.MASKED_PASSWORD_STRING}@{parsed_url.hostname}{colon}{(parsed_url.port or '')}{(parsed_url.path or '')}"
    ...
    def sanitize_config(cls, config: dict) -> dict:
        ...
        if (not isinstance(config, dict)):
            raise TypeError((f'PasswordMasker.sanitize_config expects param `config` ' + f'to be of type Dict, not of type {type(config)}'))
        ...
        def recursive_cleaner_method(config: Any) -> None:
            ...
            if isinstance(config, dict):
                ...
                for val in config:
                    recursive_cleaner_method(val)
        recursive_cleaner_method(config_copy)
        return config_copy
