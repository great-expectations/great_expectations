...
class TupleStoreBackend(StoreBackend, metaclass=ABCMeta):
        ...
        if filepath_prefix and (len(filepath_prefix) > 0):
            if filepath_prefix[(-1)] in self.forbidden_substrings:
                raise StoreBackendError(
                    "Unable to initialize TupleStoreBackend: filepath_prefix may not end with a forbidden substring. Current forbidden substrings are "
    ...
    def _convert_filepath_to_key(self, filepath):
        ...
        if self.filepath_prefix:
            if (not filepath.startswith(self.filepath_prefix)) and (
                len(filepath) >= (len(self.filepath_prefix) + 1)
            ):
                raise ValueError(
        ...
        if self.filepath_template:
            ...
            for i in range(len(tuple_index_list)):
                tuple_index = int(
                    re.search("\\d+", indexed_string_substitutions[i]).group(0)
                )
    ...
    def verify_that_key_to_filepath_operation_is_reversible(self):
        ...
        if key != new_key:
            raise ValueError(
                "filepath template {} for class {} is not reversible for a tuple of length {}. Have you included all elements in the key tuple?".format(
    ...
    def list_keys(self, prefix: Tuple = ()) -> List[Tuple]:
        ...
        for (root, dirs, files) in os.walk(
            ...
            for file_ in files:
                ...
                if self.filepath_prefix and (
                    not filepath.startswith(self.filepath_prefix)
                ):
                    continue
                elif self.filepath_suffix and (
                    not filepath.endswith(self.filepath_suffix)
                ):
                    continue
...
class TupleS3StoreBackend(TupleStoreBackend):
    ...
    def list_keys(self, prefix: Tuple = ()) -> List[Tuple]:
        ...
        for s3_object_info in objects:
            ...
            if self.filepath_prefix and (
                not s3_object_key.startswith(self.filepath_prefix)
            ):
                continue
            elif self.filepath_suffix and (
                not s3_object_key.endswith(self.filepath_suffix)
            ):
                continue
...
class TupleGCSStoreBackend(TupleStoreBackend):
    ...
    def list_keys(self, prefix: Tuple = ()) -> List[Tuple]:
        ...
        for blob in gcs.list_blobs(self.bucket, prefix=self.prefix):
            ...
            if self.filepath_prefix and (
                not gcs_object_key.startswith(self.filepath_prefix)
            ):
                continue
            elif self.filepath_suffix and (
                not gcs_object_key.endswith(self.filepath_suffix)
            ):
                continue
...
class TupleAzureBlobStoreBackend(TupleStoreBackend):
    ...
    def list_keys(self, prefix: Tuple = ()) -> List[Tuple]:
        ...
        for obj in self._get_container_client().list_blobs(
            ...
            if self.filepath_prefix and (
                not az_blob_key.startswith(self.filepath_prefix)
            ):
                continue
            elif self.filepath_suffix and (
                not az_blob_key.endswith(self.filepath_suffix)
            ):
                continue
