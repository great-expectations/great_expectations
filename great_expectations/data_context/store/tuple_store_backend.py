    ...
    def _assume_role_and_get_secret_credentials(self):
        ...
        if "tags" in self._boto3_options:
            ...
            if not isinstance(tags, list):
                raise StoreBackendError("Error: tags in boto3_options must be a list of dictionaries")
            if not all(isinstance(tag, dict) and set(tag.keys()) == {'Key', 'Value'}
                       for tag in tags):
                raise StoreBackendError(
                    "Error: tags in boto3_options must be a list of dictionaries, each with keys 'Key' and 'Value'"
                )
