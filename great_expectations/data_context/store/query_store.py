...
class SqlAlchemyQueryStore(Store):
    ...
    def __init__(self, credentials, queries=None, store_backend=None, runtime_environment=None, store_name=None) -> None:
        import inspect
        ...
        if (not sqlalchemy):
            raise ge_exceptions.DataContextError('sqlalchemy module not found, but is required for SqlAlchemyQueryStore')
        ...
        if queries:
            try:
                ...
                assert ((store_backend is None) or (store_backend['class_name'] == 'InMemoryStoreBackend')), 'If queries are provided in configuration, then store_backend must be empty or an InMemoryStoreBackend'
        ...
        self._config = {'credentials': credentials, 'queries': queries, 'store_backend': store_backend, 'runtime_environment': runtime_environment, 'store_name': store_name, 'module_name': self.__class__.__module__, 'class_name': self.__class__.__name__}
    ...
    def get_query_result(self, key, query_parameters=None):
        ...
        if isinstance(result, dict):
            ...
            if (return_type not in ['list', 'scalar']):
                raise ValueError("The return_type of a SqlAlchemyQueryStore query must be one of either 'list' or 'scalar'")
        else:
            query = result
            return_type = None
