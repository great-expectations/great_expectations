...
from great_expectations.data_context.types.resource_identifiers import ExpectationSuiteIdentifier, SiteSectionIdentifier, ValidationResultIdentifier
from great_expectations.data_context.util import file_relative_path, instantiate_class_from_config, load_class
...
class HtmlSiteStore():
    ...
    def __init__(self, store_backend=None, runtime_environment=None) -> None:
        ...
        if (not issubclass(store_class, (TupleStoreBackend, GeCloudStoreBackend))):
            raise DataContextError('Invalid configuration: HtmlSiteStore needs a TupleStoreBackend or GeCloudStoreBackend')
        if (('filepath_template' in store_backend) or (('fixed_length_key' in store_backend) and (store_backend['fixed_length_key'] is True))):
            logger.warning('Configuring a filepath_template or using fixed_length_key is not supported in SiteBuilder: filepaths will be selected based on the type of asset rendered.')
        module_name = 'great_expectations.data_context.store'
        filepath_suffix = '.html'
        is_ge_cloud_store = (store_backend['class_name'] == 'GeCloudStoreBackend')
        expectation_config_defaults = {'module_name': module_name, 'filepath_prefix': 'expectations', 'filepath_suffix': filepath_suffix, 'suppress_store_backend_id': True}
        if is_ge_cloud_store:
            expectation_config_defaults = {'module_name': module_name, 'suppress_store_backend_id': True}
        expectation_suite_identifier_obj = instantiate_class_from_config(config=store_backend, runtime_environment=runtime_environment, config_defaults=expectation_config_defaults)
        if (not expectation_suite_identifier_obj):
            raise ClassInstantiationError(module_name=module_name, package_name=None, class_name=store_backend['class_name'])
        validation_result_config_defaults = {'module_name': module_name, 'filepath_prefix': 'validations', 'filepath_suffix': filepath_suffix, 'suppress_store_backend_id': True}
        if is_ge_cloud_store:
            validation_result_config_defaults = {'module_name': module_name, 'suppress_store_backend_id': True}
        validation_result_idendifier_obj = instantiate_class_from_config(config=store_backend, runtime_environment=runtime_environment, config_defaults=validation_result_config_defaults)
        if (not validation_result_idendifier_obj):
            raise ClassInstantiationError(module_name=module_name, package_name=None, class_name=store_backend['class_name'])
        filepath_template = 'index.html'
        index_page_config_defaults = {'module_name': module_name, 'filepath_template': filepath_template, 'suppress_store_backend_id': True}
        if is_ge_cloud_store:
            index_page_config_defaults = {'module_name': module_name, 'suppress_store_backend_id': True}
        index_page_obj = instantiate_class_from_config(config=store_backend, runtime_environment=runtime_environment, config_defaults=index_page_config_defaults)
        if (not index_page_obj):
            raise ClassInstantiationError(module_name=module_name, package_name=None, class_name=store_backend['class_name'])
        static_assets_config_defaults = {'module_name': module_name, 'filepath_template': None, 'suppress_store_backend_id': True}
        if is_ge_cloud_store:
            static_assets_config_defaults = {'module_name': module_name, 'suppress_store_backend_id': True}
        static_assets_obj = instantiate_class_from_config(config=store_backend, runtime_environment=runtime_environment, config_defaults=static_assets_config_defaults)
        if (not static_assets_obj):
            raise ClassInstantiationError(module_name=module_name, package_name=None, class_name=store_backend['class_name'])
        self.store_backends = {ExpectationSuiteIdentifier: expectation_suite_identifier_obj, ValidationResultIdentifier: validation_result_idendifier_obj, 'index_page': index_page_obj, 'static_assets': static_assets_obj}
        self.keys = set()
        self._config = {'store_backend': store_backend, 'runtime_environment': runtime_environment, 'module_name': self.__class__.__module__, 'class_name': self.__class__.__name__}
    ...
    def set(self, key, serialized_value):
        ...
        self._validate_key(key)
        self.keys.add(key)
        return self.store_backends[type(key.resource_identifier)].set(key.resource_identifier.to_tuple(), serialized_value, content_encoding='utf-8', content_type='text/html; charset=utf-8')
    ...
    def get_url_for_resource(self, resource_identifier=None, only_if_exists=True):
        ...
        if (resource_identifier is None):
            ...
            raise ValueError(f'Cannot get URL for resource {str(resource_identifier):s}')
    ...
    def _validate_key(self, key):
        ...
        for key_class in self.store_backends.keys():
            try:
                ...
                continue
        raise TypeError('resource_identifier in key: {!r} must one of {}, not {!r}'.format(key, set(self.store_backends.keys()), type(key)))
    ...
    def write_index_page(self, page):
        ...
        return self.store_backends['index_page'].set((), page, content_encoding='utf-8', content_type='text/html; charset=utf-8')
            ...
            if os.path.isdir(os.path.join(static_assets_source_dir, item)):
                ...
                with open(source_name, 'rb') as f:
                    ...
                    if (not isinstance(self.store_backends['static_assets'], GeCloudStoreBackend)):
                        self.store_backends['static_assets'].set(store_key, f.read(), content_encoding=content_encoding, content_type=content_type)
    ...
    def _unzip_assets(self, assets_full_path: str, unzip_directory: str) -> bool:
        ...
        zip_re = re.match(f'(.+[.]zip){re.escape(os.sep)}(.+)', static_assets_source_absdir, flags=re.IGNORECASE)
        if zip_re:
