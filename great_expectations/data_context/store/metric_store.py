...
from great_expectations.util import filter_properties_dict, load_class, verify_dynamic_loading_support
        ...
        if (store_backend is not None):
            ...
            if issubclass(store_backend_class, DatabaseStoreBackend):
                ...
                if ('key_columns' not in store_backend):
                    store_backend['key_columns'] = store_backend.get('key_columns', ['run_name', 'run_time', 'data_asset_name', 'expectation_suite_identifier', 'metric_name', 'metric_kwargs_id'])
...
class EvaluationParameterStore(MetricStore):
    ...
    def __init__(self, store_backend=None, store_name=None) -> None:
        ...
        super().__init__(store_backend=store_backend, store_name=store_name)
        self._config = {'store_backend': store_backend, 'store_name': store_name, 'module_name': self.__class__.__module__, 'class_name': self.__class__.__name__}
