...
class Store():
    ...
    def __init__(self, store_backend=None, runtime_environment=None, store_name='no_store_name') -> None:
        import inspect
        ...
        self._store_backend = instantiate_class_from_config(config=store_backend, runtime_environment=(runtime_environment or {}), config_defaults={'module_name': module_name, 'store_name': self._store_name})
    ...
    def set(self, key, value, **kwargs):
        ...
        if (key == StoreBackend.STORE_BACKEND_ID_KEY):
            ...
            self._validate_key(key)
            return self._store_backend.set(self.key_to_tuple(key), self.serialize(key, value), **kwargs)
    ...
    def list_keys(self):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        keys_without_store_backend_id = [key for key in self._store_backend.list_keys() if (not (key == StoreBackend.STORE_BACKEND_ID_KEY))]
