...
GE_DATA_CONTEXT_PROJECT_CONFIG_FILE_NAME_ROOT: str = "great_expectations"
...
def create_using_s3_backend(func: Callable = None,) -> Callable:
    ...
    def initialize_using_s3_backend_wrapped_method(**kwargs):
        ...
        func_callee: Callable = create_s3_backend_data_context
        ...
        working_data_context_info: dict = func_callee(**kwargs_callee)

        working_data_context: ge.data_context.data_context.DataContext = working_data_context_info[
            "data_context"
        ]
        data_context_created: bool = working_data_context_info["data_context_created"]

        if data_context_created:
            kwargs_callee = copy.deepcopy(kwargs)
            kwargs_callee.update({"data_context": working_data_context})
...
def create_s3_backend_data_context(
    ...
    data_context_created: bool
    if overwrite_existing:
        project_config = create_minimal_project_config(
            usage_statistics_enabled=usage_statistics_enabled
        )
        data_context_created = True
    else:
        data_context_created = not found_existing_project_config
...
def create_standard_s3_backend_data_context(**kwargs,):
    func_callee: Callable = build_s3_backend_data_context
    # noinspection SpellCheckingInspection
    argspec: list = getfullargspec(func_callee)[0]
...
def build_s3_backend_data_context(
    ...
    if datasource_type == "spark":
        ...
        # noinspection PyUnusedLocal
        pandas_datasource: Datasource = data_context.add_pandas_datasource(
