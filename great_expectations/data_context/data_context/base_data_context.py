...
from great_expectations.rule_based_profiler.data_assistant.data_assistant_dispatcher import DataAssistantDispatcher
try:
...
from great_expectations.core.batch import Batch, BatchDefinition, BatchRequestBase, IDDict, get_batch_request_from_acceptable_arguments
...
from great_expectations.core.usage_statistics.usage_statistics import UsageStatisticsHandler, add_datasource_usage_statistics, get_batch_list_usage_statistics, run_validation_operator_usage_statistics, save_expectation_suite_usage_statistics, send_usage_message, usage_statistics_enabled_method
...
from great_expectations.data_context.types.base import CURRENT_GE_CONFIG_VERSION, DEFAULT_USAGE_STATISTICS_URL, AnonymizedUsageStatisticsConfig, CheckpointConfig, ConcurrencyConfig, DataContextConfig, DataContextConfigDefaults, DatasourceConfig, GeCloudConfig, ProgressBarsConfig, anonymizedUsageStatisticsSchema, dataContextConfigSchema, datasourceConfigSchema
from great_expectations.data_context.types.refs import GeCloudIdAwareRef
from great_expectations.data_context.types.resource_identifiers import ConfigurationIdentifier, ExpectationSuiteIdentifier, GeCloudIdentifier, ValidationResultIdentifier
from great_expectations.data_context.util import PasswordMasker, build_store_from_config, instantiate_class_from_config, load_class, parse_substitution_variable, substitute_all_config_variables, substitute_config_variable
...
class BaseDataContext(ConfigPeer):
    ...
    BASE_DIRECTORIES = [DataContextConfigDefaults.CHECKPOINTS_BASE_DIRECTORY.value, DataContextConfigDefaults.EXPECTATIONS_BASE_DIRECTORY.value, DataContextConfigDefaults.PLUGINS_BASE_DIRECTORY.value, DataContextConfigDefaults.PROFILERS_BASE_DIRECTORY.value, GE_UNCOMMITTED_DIR]
    GE_DIR = 'great_expectations'
    GE_YML = 'great_expectations.yml'
    GE_EDIT_NOTEBOOK_DIR = GE_UNCOMMITTED_DIR
    FALSEY_STRINGS = ['FALSE', 'false', 'False', 'f', 'F', '0']
    GLOBAL_CONFIG_PATHS = [os.path.expanduser('~/.great_expectations/great_expectations.conf'), '/etc/great_expectations.conf']
    DOLLAR_SIGN_ESCAPE_STRING = '\\$'
    TEST_YAML_CONFIG_SUPPORTED_STORE_TYPES = ['ExpectationsStore', 'ValidationsStore', 'HtmlSiteStore', 'EvaluationParameterStore', 'MetricStore', 'SqlAlchemyQueryStore', 'CheckpointStore', 'ProfilerStore']
    TEST_YAML_CONFIG_SUPPORTED_DATASOURCE_TYPES = ['Datasource', 'SimpleSqlalchemyDatasource']
    TEST_YAML_CONFIG_SUPPORTED_DATA_CONNECTOR_TYPES = ['InferredAssetFilesystemDataConnector', 'ConfiguredAssetFilesystemDataConnector', 'InferredAssetS3DataConnector', 'ConfiguredAssetS3DataConnector', 'InferredAssetAzureDataConnector', 'ConfiguredAssetAzureDataConnector', 'InferredAssetGCSDataConnector', 'ConfiguredAssetGCSDataConnector', 'InferredAssetSqlDataConnector', 'ConfiguredAssetSqlDataConnector']
    ...
    ALL_TEST_YAML_CONFIG_DIAGNOSTIC_INFO_TYPES = ['__substitution_error__', '__yaml_parse_error__', '__custom_subclass_not_core_ge__', '__class_name_not_provided__']
    ALL_TEST_YAML_CONFIG_SUPPORTED_TYPES = ((((TEST_YAML_CONFIG_SUPPORTED_STORE_TYPES + TEST_YAML_CONFIG_SUPPORTED_DATASOURCE_TYPES) + TEST_YAML_CONFIG_SUPPORTED_DATA_CONNECTOR_TYPES) + TEST_YAML_CONFIG_SUPPORTED_CHECKPOINT_TYPES) + TEST_YAML_CONFIG_SUPPORTED_PROFILER_TYPES)
    _data_context = None
    ...
    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT___INIT__.value)
    def __init__(self, project_config: Union[(DataContextConfig, Mapping)], context_root_dir: Optional[str]=None, runtime_environment: Optional[dict]=None, ge_cloud_mode: bool=False, ge_cloud_config: Optional[GeCloudConfig]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if ((self.plugins_directory is not None) and os.path.exists(self.plugins_directory)):
        ...
        self._initialize_usage_statistics(self.project_config_with_variables_substituted.anonymous_usage_statistics)
        self._cached_datasources = {}
        self._init_datasources(self.project_config_with_variables_substituted)
        self.validation_operators = {}
        if (('validation_operators' in self.get_config().commented_map) and self.config.validation_operators):
            for (validation_operator_name, validation_operator_config) in self.config.validation_operators.items():
                self.add_validation_operator(validation_operator_name, validation_operator_config)
    ...
    def _build_store_from_config(self, store_name: str, store_config: dict) -> Optional[Store]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if ((store_name == self.expectations_store_name) and store_config.get('store_backend')):
            store_config['store_backend'].update({'manually_initialize_store_backend_id': self.project_config_with_variables_substituted.anonymous_usage_statistics.data_context_id})
        if ((store_name not in [store['name'] for store in self.list_active_stores()]) and (store_config.get('store_backend') is not None)):
            store_config['store_backend'].update({'suppress_store_backend_id': True})
        new_store = build_store_from_config(store_name=store_name, store_config=store_config, module_name=module_name, runtime_environment={'root_directory': self.root_directory})
    ...
    def _init_datasources(self, config: DataContextConfig) -> None:
        ...
        for datasource_name in config.datasources:
            try:
                self._cached_datasources[datasource_name] = self.get_datasource(datasource_name=datasource_name)
    ...
    def _apply_global_config_overrides(self) -> None:
        ...
        global_data_context_id = self._get_global_config_value(environment_variable='GE_DATA_CONTEXT_ID', conf_file_section='anonymous_usage_statistics', conf_file_option='data_context_id')
        if global_data_context_id:
            ...
            if (not data_context_id_errors):
                ...
                self.config.anonymous_usage_statistics.data_context_id = global_data_context_id
            else:
                validation_errors.update(data_context_id_errors)
        global_usage_statistics_url = self._get_global_config_value(environment_variable='GE_USAGE_STATISTICS_URL', conf_file_section='anonymous_usage_statistics', conf_file_option='usage_statistics_url')
        if global_usage_statistics_url:
            usage_statistics_url_errors = anonymizedUsageStatisticsSchema.validate({'usage_statistics_url': global_usage_statistics_url})
            if (not usage_statistics_url_errors):
                logger.info('usage_statistics_url is defined globally. Applying override to project_config.')
                self.config.anonymous_usage_statistics.usage_statistics_url = global_usage_statistics_url
            else:
                validation_errors.update(usage_statistics_url_errors)
        if validation_errors:
            logger.warning('The following globally-defined config variables failed validation:\n{}\n\nPlease fix the variables if you would like to apply global values to project_config.'.format(json.dumps(validation_errors, indent=2)))

    @classmethod
    def _get_global_config_value(cls, environment_variable: Optional[str]=None, conf_file_section=None, conf_file_option=None) -> Optional[str]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        assert ((conf_file_section and conf_file_option) or ((not conf_file_section) and (not conf_file_option))), "Must pass both 'conf_file_section' and 'conf_file_option' or neither."
    ...
    def _construct_data_context_id(self) -> str:
        ...
        expectations_store = self._stores[self.project_config_with_variables_substituted.expectations_store_name]
        if isinstance(expectations_store.store_backend, TupleStoreBackend):
            return expectations_store.store_backend_id_warnings_suppressed
        else:
            return self.project_config_with_variables_substituted.anonymous_usage_statistics.data_context_id

    def _initialize_usage_statistics(self, usage_statistics_config: AnonymizedUsageStatisticsConfig) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not usage_statistics_config.enabled):
            ...
            return
        self._usage_statistics_handler = UsageStatisticsHandler(data_context=self, data_context_id=self._data_context_id, usage_statistics_url=usage_statistics_config.usage_statistics_url)
    ...
    def add_validation_operator(self, validation_operator_name: str, validation_operator_config: dict) -> 'ValidationOperator':
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        config = self.project_config_with_variables_substituted.validation_operators[validation_operator_name]
        module_name = 'great_expectations.validation_operators'
        new_validation_operator = instantiate_class_from_config(config=config, runtime_environment={'data_context': self, 'name': validation_operator_name}, config_defaults={'module_name': module_name})
        if (not new_validation_operator):
            raise ge_exceptions.ClassInstantiationError(module_name=module_name, package_name=None, class_name=config['class_name'])
    ...
    def get_site_names(self) -> List[str]:
        ...
        return list(self.project_config_with_variables_substituted.data_docs_sites.keys())

    def get_docs_sites_urls(self, resource_identifier=None, site_name: Optional[str]=None, only_if_exists=True, site_names: Optional[List[str]]=None) -> List[Dict[(str, str)]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        unfiltered_sites = self.project_config_with_variables_substituted.data_docs_sites
        sites = ({k: v for (k, v) in unfiltered_sites.items() if (k in site_names)} if site_names else unfiltered_sites)
        if (not sites):
    ...
    def _load_site_builder_from_site_config(self, site_config) -> SiteBuilder:
        ...
        site_builder = instantiate_class_from_config(config=site_config, runtime_environment={'data_context': self, 'root_directory': self.root_directory}, config_defaults={'module_name': default_module_name})
        if (not site_builder):
            raise ge_exceptions.ClassInstantiationError(module_name=default_module_name, package_name=None, class_name=site_config['class_name'])
        return site_builder

    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT_OPEN_DATA_DOCS.value)
    def open_data_docs(self, resource_identifier: Optional[str]=None, site_name: Optional[str]=None, only_if_exists: bool=True) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        data_docs_urls: List[Dict[(str, str)]] = self.get_docs_sites_urls(resource_identifier=resource_identifier, site_name=site_name, only_if_exists=only_if_exists)
    ...
    def plugins_directory(self):
        ...
        return self._normalize_absolute_or_relative_path(self.project_config_with_variables_substituted.plugins_directory)
    ...
    def checkpoint_store_name(self):
        ...
        try:
            ...
            if CheckpointStore.default_checkpoints_exist(directory_path=self.root_directory):
  ...
  Visit https://docs.greatexpectations.io/docs/guides/miscellaneous/migration_guide#migrating-to-the-batch-request-v3-api to learn more about the upgrade process.'''
    ...
    def checkpoint_store(self) -> 'CheckpointStore':
        ...
        try:
            ...
            from great_expectations.data_context.store.checkpoint_store import CheckpointStore
            if CheckpointStore.default_checkpoints_exist(directory_path=self.root_directory):
                logger.warning(f'''Checkpoint store named "{checkpoint_store_name}" is not a configured store, so will try to use default Checkpoint store.
  ...
  Visit https://docs.greatexpectations.io/docs/guides/miscellaneous/migration_guide#migrating-to-the-batch-request-v3-api to learn more about the upgrade process.''')
                return self._build_store_from_config(checkpoint_store_name, DataContextConfigDefaults.DEFAULT_STORES.value[checkpoint_store_name])
            raise ge_exceptions.StoreConfigurationError(f'Attempted to access the Checkpoint store named "{checkpoint_store_name}", which is not a configured store.')
  ...
  Visit https://docs.greatexpectations.io/docs/guides/miscellaneous/migration_guide#migrating-to-the-batch-request-v3-api to learn more about the upgrade process.'''
    ...
    def profiler_store(self) -> ProfilerStore:
        ...
        try:
            ...
            if BaseDataContext._default_profilers_exist(directory_path=self.root_directory):
                logger.warning(f'''Profiler store named "{profiler_store_name}" is not a configured store, so will try to use default Profiler store.
  ...
  Visit https://docs.greatexpectations.io/docs/guides/miscellaneous/migration_guide#migrating-to-the-batch-request-v3-api to learn more about the upgrade process.''')
                return self._build_store_from_config(profiler_store_name, DataContextConfigDefaults.DEFAULT_STORES.value[profiler_store_name])
            raise ge_exceptions.StoreConfigurationError(f'Attempted to access the Profiler store named "{profiler_store_name}", which is not a configured store.')
    ...
    def _default_profilers_exist(directory_path: Optional[str]) -> bool:
        ...
        if (not directory_path):
            return False
        profiler_directory_path: str = os.path.join(directory_path, DataContextConfigDefaults.DEFAULT_PROFILER_STORE_BASE_DIRECTORY_RELATIVE_NAME.value)
    ...
    def _load_config_variables_file(self):
        ...
        if self.ge_cloud_mode:
            return {}
        config_variables_file_path = cast(DataContextConfig, self.get_config()).config_variables_file_path
        if config_variables_file_path:
            try:
                defined_path = substitute_config_variable(config_variables_file_path, dict(os.environ))
    ...
    def get_config_with_variables_substituted(self, config=None) -> DataContextConfig:
        ...
        if (not config):
            config = self.config
        substituted_config_variables = substitute_all_config_variables(self.config_variables, dict(os.environ), self.DOLLAR_SIGN_ESCAPE_STRING)
        substitutions = {**substituted_config_variables, **dict(os.environ), **self.runtime_environment}
        if self.ge_cloud_mode:
            ge_cloud_config_variable_defaults = {'plugins_directory': self._normalize_absolute_or_relative_path(DataContextConfigDefaults.DEFAULT_PLUGINS_DIRECTORY.value), 'usage_statistics_url': DEFAULT_USAGE_STATISTICS_URL}
            for (config_variable, value) in ge_cloud_config_variable_defaults.items():
                if (substitutions.get(config_variable) is None):
                    logger.info(f'Config variable "{config_variable}" was not found in environment or global config ({self.GLOBAL_CONFIG_PATHS}). Using default value "{value}" instead. If you would like to use a different value, please specify it in an environment variable or in a great_expectations.conf file located at one of the above paths, in a section named "ge_cloud_config".')
                    substitutions[config_variable] = value
        return DataContextConfig(**substitute_all_config_variables(config, substitutions, self.DOLLAR_SIGN_ESCAPE_STRING))

    def escape_all_config_variables(self, value: Union[(str, dict, list)], dollar_sign_escape_string: str=DOLLAR_SIGN_ESCAPE_STRING, skip_if_substitution_variable: bool=True) -> Union[(str, dict, list)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (isinstance(value, dict) or isinstance(value, OrderedDict)):
            return {k: self.escape_all_config_variables(v, dollar_sign_escape_string, skip_if_substitution_variable) for (k, v) in value.items()}
        elif isinstance(value, list):
            return [self.escape_all_config_variables(v, dollar_sign_escape_string, skip_if_substitution_variable) for v in value]
    ...
    def save_config_variable(self, config_variable_name, value, skip_if_substitution_variable: bool=True) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        value = self.escape_all_config_variables(value, self.DOLLAR_SIGN_ESCAPE_STRING, skip_if_substitution_variable=skip_if_substitution_variable)
        config_variables[config_variable_name] = value
        config_variables_filepath = cast(DataContextConfig, self.get_config()).config_variables_file_path
        ...
        if (not os.path.isfile(config_variables_filepath)):
            logger.info('Creating new substitution_variables file at {config_variables_filepath}'.format(config_variables_filepath=config_variables_filepath))
    ...
    def get_available_data_asset_names(self, datasource_names=None, batch_kwargs_generator_names=None):
        ...
        if (datasource_names is None):
            ...
            raise ValueError('Datasource names must be a datasource name, list of datasource names or None (to list all datasources)')
        if (batch_kwargs_generator_names is not None):
            ...
            if (len(batch_kwargs_generator_names) == len(datasource_names)):
                for (idx, datasource_name) in enumerate(datasource_names):
                    ...
                    data_asset_names[datasource_name] = datasource.get_available_data_asset_names(batch_kwargs_generator_names[idx])
                ...
                raise ValueError('If providing batch kwargs generator, you must either specify one for each datasource or only one datasource.')
        else:
    ...
    def build_batch_kwargs(self, datasource, batch_kwargs_generator, data_asset_name=None, partition_id=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if kwargs.get('name'):
            if data_asset_name:
                raise ValueError("Cannot provide both 'name' and 'data_asset_name'. Please use 'data_asset_name' only.")
            warnings.warn('name is deprecated as a batch_parameter as of v0.11.2 and will be removed in v0.16. Please use data_asset_name instead.', DeprecationWarning)
            data_asset_name = kwargs.pop('name')
        datasource_obj = self.get_datasource(datasource)
        batch_kwargs = datasource_obj.build_batch_kwargs(batch_kwargs_generator=batch_kwargs_generator, data_asset_name=data_asset_name, partition_id=partition_id, **kwargs)
        return batch_kwargs

    def _get_batch_v2(self, batch_kwargs: Union[(dict, BatchKwargs)], expectation_suite_name: Union[(str, ExpectationSuite)], data_asset_type=None, batch_parameters=None) -> DataAsset:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not isinstance(expectation_suite_name, (ExpectationSuite, ExpectationSuiteIdentifier, str))):
            raise ge_exceptions.DataContextError('expectation_suite_name must be an ExpectationSuite, ExpectationSuiteIdentifier or string.')
        ...
        validator = BridgeValidator(batch=batch, expectation_suite=expectation_suite, expectation_engine=data_asset_type)
    ...
    def _get_batch_v3(self, datasource_name: Optional[str]=None, data_connector_name: Optional[str]=None, data_asset_name: Optional[str]=None, *, batch_request: Optional[BatchRequestBase]=None, batch_data: Optional[Any]=None, data_connector_query: Optional[Union[(IDDict, dict)]]=None, batch_identifiers: Optional[dict]=None, limit: Optional[int]=None, index: Optional[Union[(int, list, tuple, slice, str)]]=None, custom_filter_function: Optional[Callable]=None, batch_spec_passthrough: Optional[dict]=None, sampling_method: Optional[str]=None, sampling_kwargs: Optional[dict]=None, splitter_method: Optional[str]=None, splitter_kwargs: Optional[dict]=None, runtime_parameters: Optional[dict]=None, query: Optional[str]=None, path: Optional[str]=None, batch_filter_parameters: Optional[dict]=None, **kwargs) -> Union[(Batch, DataAsset)]:
        ...
        batch_list: List[Batch] = self.get_batch_list(datasource_name=datasource_name, data_connector_name=data_connector_name, data_asset_name=data_asset_name, batch_request=batch_request, batch_data=batch_data, data_connector_query=data_connector_query, batch_identifiers=batch_identifiers, limit=limit, index=index, custom_filter_function=custom_filter_function, batch_spec_passthrough=batch_spec_passthrough, sampling_method=sampling_method, sampling_kwargs=sampling_kwargs, splitter_method=splitter_method, splitter_kwargs=splitter_kwargs, runtime_parameters=runtime_parameters, query=query, path=path, batch_filter_parameters=batch_filter_parameters, **kwargs)
        warnings.warn('get_batch is deprecated for the V3 Batch Request API as of v0.13.20 and will be removed in v0.16. Please use get_batch_list instead.', DeprecationWarning)
        if (len(batch_list) != 1):
            raise ValueError(f'Got {len(batch_list)} batches instead of a single batch. If you would like to use a BatchRequest to return multiple batches, please use get_batch_list directly instead of calling get_batch')
        return batch_list[0]

    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT_RUN_VALIDATION_OPERATOR.value, args_payload_fn=run_validation_operator_usage_statistics)
    def run_validation_operator(self, validation_operator_name: str, assets_to_validate: List, run_id: Optional[Union[(str, RunIdentifier)]]=None, evaluation_parameters: Optional[dict]=None, run_name: Optional[str]=None, run_time: Optional[Union[(str, datetime.datetime)]]=None, result_format: Optional[Union[(str, dict)]]=None, **kwargs):
        ...
        for batch in assets_to_validate:
            if (not isinstance(batch, (tuple, DataAsset, Validator))):
                raise ge_exceptions.DataContextError('Batches are required to be of type DataAsset or Validator')
        try:
        ...
        if (evaluation_parameters is None):
            return validation_operator.run(assets_to_validate=assets_to_validate, run_id=run_id, run_name=run_name, run_time=run_time, result_format=result_format, **kwargs)
        else:
            return validation_operator.run(assets_to_validate=assets_to_validate, run_id=run_id, evaluation_parameters=evaluation_parameters, run_name=run_name, run_time=run_time, result_format=result_format, **kwargs)
    ...
    def _get_data_context_version(self, arg1: Any, **kwargs) -> Optional[str]:
        ...
        if {'datasource_name', 'data_connector_name', 'data_asset_name', 'batch_request', 'batch_data'}.intersection(set(kwargs.keys())):
            return 'v3'
        ...
        try:
            datasource: Union[(LegacyDatasource, BaseDatasource)] = self.get_datasource(datasource_name=datasource_name)
    ...
    def get_batch(self, arg1: Any=None, arg2: Any=None, arg3: Any=None, **kwargs) -> Union[(Batch, DataAsset)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (api_version == 'v3'):
            ...
            return self._get_batch_v3(datasource_name=datasource_name, data_connector_name=data_connector_name, data_asset_name=data_asset_name, **kwargs)
        if ('batch_kwargs' in kwargs):
            batch_kwargs = kwargs.get('batch_kwargs', None)
        else:
            batch_kwargs = arg1
        ...
        return self._get_batch_v2(batch_kwargs=batch_kwargs, expectation_suite_name=expectation_suite_name, data_asset_type=data_asset_type, batch_parameters=batch_parameters)

    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT_GET_BATCH_LIST.value, args_payload_fn=get_batch_list_usage_statistics)
    def get_batch_list(self, datasource_name: Optional[str]=None, data_connector_name: Optional[str]=None, data_asset_name: Optional[str]=None, *, batch_request: Optional[BatchRequestBase]=None, batch_data: Optional[Any]=None, data_connector_query: Optional[dict]=None, batch_identifiers: Optional[dict]=None, limit: Optional[int]=None, index: Optional[Union[(int, list, tuple, slice, str)]]=None, custom_filter_function: Optional[Callable]=None, sampling_method: Optional[str]=None, sampling_kwargs: Optional[dict]=None, splitter_method: Optional[str]=None, splitter_kwargs: Optional[dict]=None, runtime_parameters: Optional[dict]=None, query: Optional[str]=None, path: Optional[str]=None, batch_filter_parameters: Optional[dict]=None, batch_spec_passthrough: Optional[dict]=None, **kwargs) -> List[Batch]:
        ...
        batch_request = get_batch_request_from_acceptable_arguments(datasource_name=datasource_name, data_connector_name=data_connector_name, data_asset_name=data_asset_name, batch_request=batch_request, batch_data=batch_data, data_connector_query=data_connector_query, batch_identifiers=batch_identifiers, limit=limit, index=index, custom_filter_function=custom_filter_function, sampling_method=sampling_method, sampling_kwargs=sampling_kwargs, splitter_method=splitter_method, splitter_kwargs=splitter_kwargs, runtime_parameters=runtime_parameters, query=query, path=path, batch_filter_parameters=batch_filter_parameters, batch_spec_passthrough=batch_spec_passthrough, **kwargs)
        datasource_name = batch_request.datasource_name
        if (datasource_name in self.datasources):
            datasource: Datasource = cast(Datasource, self.datasources[datasource_name])
        else:
            raise ge_exceptions.DatasourceError(datasource_name, 'The given datasource could not be retrieved from the DataContext; please confirm that your configuration is accurate.')
        return datasource.get_batch_list_from_batch_request(batch_request=batch_request)

    def get_validator(self, datasource_name: Optional[str]=None, data_connector_name: Optional[str]=None, data_asset_name: Optional[str]=None, *, batch: Optional[Batch]=None, batch_list: Optional[List[Batch]]=None, batch_request: Optional[BatchRequestBase]=None, batch_request_list: List[Optional[BatchRequestBase]]=None, batch_data: Optional[Any]=None, data_connector_query: Optional[Union[(IDDict, dict)]]=None, batch_identifiers: Optional[dict]=None, limit: Optional[int]=None, index: Optional[Union[(int, list, tuple, slice, str)]]=None, custom_filter_function: Optional[Callable]=None, sampling_method: Optional[str]=None, sampling_kwargs: Optional[dict]=None, splitter_method: Optional[str]=None, splitter_kwargs: Optional[dict]=None, runtime_parameters: Optional[dict]=None, query: Optional[str]=None, path: Optional[str]=None, batch_filter_parameters: Optional[dict]=None, expectation_suite_ge_cloud_id: Optional[str]=None, batch_spec_passthrough: Optional[dict]=None, expectation_suite_name: Optional[str]=None, expectation_suite: Optional[ExpectationSuite]=None, create_expectation_suite_with_name: Optional[str]=None, **kwargs) -> Validator:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        '\n        This method applies only to the new (V3) Datasource schema.\n        '
        if (sum((bool(x) for x in [(expectation_suite is not None), (expectation_suite_name is not None), (create_expectation_suite_with_name is not None), (expectation_suite_ge_cloud_id is not None)])) > 1):
            raise ValueError(f"No more than one of expectation_suite_name,{('expectation_suite_ge_cloud_id,' if self.ge_cloud_mode else '')} expectation_suite, or create_expectation_suite_with_name can be specified")
        ...
        if (create_expectation_suite_with_name is not None):
            expectation_suite = self.create_expectation_suite(expectation_suite_name=create_expectation_suite_with_name)
        if (sum((bool(x) for x in [(batch is not None), (batch_list is not None), (batch_request is not None), (batch_request_list is not None)])) > 1):
            raise ValueError('No more than one of batch, batch_list, batch_request, or batch_request_list can be specified')
        if batch_list:
            pass
        elif batch:
            ...
            for batch_request in batch_request_list:
                batch_list.extend(self.get_batch_list(datasource_name=datasource_name, data_connector_name=data_connector_name, data_asset_name=data_asset_name, batch_request=batch_request, batch_data=batch_data, data_connector_query=data_connector_query, batch_identifiers=batch_identifiers, limit=limit, index=index, custom_filter_function=custom_filter_function, sampling_method=sampling_method, sampling_kwargs=sampling_kwargs, splitter_method=splitter_method, splitter_kwargs=splitter_kwargs, runtime_parameters=runtime_parameters, query=query, path=path, batch_filter_parameters=batch_filter_parameters, batch_spec_passthrough=batch_spec_passthrough, **kwargs))
        return self.get_validator_using_batch_list(expectation_suite=expectation_suite, batch_list=batch_list)

    def get_validator_using_batch_list(self, expectation_suite: ExpectationSuite, batch_list: List[Batch]) -> Validator:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        execution_engine: ExecutionEngine = self.datasources[batch_definition.datasource_name].execution_engine
        validator: Validator = Validator(execution_engine=execution_engine, interactive_evaluation=True, expectation_suite=expectation_suite, data_context=self, batches=batch_list)
        return validator
    ...
    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT_ADD_DATASOURCE.value, args_payload_fn=add_datasource_usage_statistics)
    def add_datasource(self, name, initialize=True, **kwargs) -> Optional[Dict[(str, Union[(LegacyDatasource, BaseDatasource)])]]:
        ...
        if hasattr(datasource_class, 'build_configuration'):
            ...
            config = kwargs
        return self._instantiate_datasource_from_config_and_update_project_config(name=name, config=config, initialize=initialize)

    def _instantiate_datasource_from_config_and_update_project_config(self, name: str, config: dict, initialize: bool=True) -> Optional[Union[(LegacyDatasource, BaseDatasource)]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        datasource_config = self.project_config_with_variables_substituted.datasources[name]
        config = dict(datasourceConfigSchema.dump(datasource_config))
        datasource: Optional[Union[(LegacyDatasource, BaseDatasource)]]
        if initialize:
            try:
                datasource = self._instantiate_datasource_from_config(name=name, config=config)
    ...
    def _instantiate_datasource_from_config(self, name: str, config: dict) -> Union[(LegacyDatasource, BaseDatasource)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        try:
            datasource: Union[(LegacyDatasource, BaseDatasource)] = self._build_datasource_from_config(name=name, config=config)
        except Exception as e:
            raise ge_exceptions.DatasourceInitializationError(datasource_name=name, message=str(e))
        return datasource
    ...
    def _build_datasource_from_config(self, name: str, config: Union[(dict, DatasourceConfig)]):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        datasource = instantiate_class_from_config(config=config, runtime_environment={'data_context': self, 'concurrency': self.concurrency}, config_defaults={'module_name': module_name})
        if (not datasource):
            raise ge_exceptions.ClassInstantiationError(module_name=module_name, package_name=None, class_name=config['class_name'])
        return datasource

    def get_datasource(self, datasource_name: str='default') -> Optional[Union[(LegacyDatasource, BaseDatasource)]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (datasource_name in self.project_config_with_variables_substituted.datasources):
            datasource_config: DatasourceConfig = copy.deepcopy(self.project_config_with_variables_substituted.datasources[datasource_name])
        else:
            raise ValueError(f'Unable to load datasource `{datasource_name}` -- no configuration found or invalid configuration.')
        config: dict = dict(datasourceConfigSchema.dump(datasource_config))
        datasource: Optional[Union[(LegacyDatasource, BaseDatasource)]] = self._instantiate_datasource_from_config(name=datasource_name, config=config)
    ...
    def list_expectation_suites(self):
        ...
        try:
            ...
            raise ge_exceptions.InvalidConfigError(f'Unable to find configured store: {str(e)}')
        return keys
    ...
    def list_datasources(self):
        ...
        datasources = []
        for (name, value) in self.project_config_with_variables_substituted.datasources.items():
    ...
    def list_stores(self):
        ...
        stores = []
        for (name, value) in self.project_config_with_variables_substituted.stores.items():
    ...
    def list_active_stores(self):
        ...
        active_store_names: List[str] = [self.expectations_store_name, self.validations_store_name, self.evaluation_parameter_store_name]
        try:
        ...
        try:
            ...
            logger.info('Profiler store is not configured; omitting it from active stores')
        return [store for store in self.list_stores() if (store['name'] in active_store_names)]
    ...
    def list_validation_operators(self):
        ...
        for (name, value) in self.project_config_with_variables_substituted.validation_operators.items():
            value['name'] = name
            validation_operators.append(value)
        return validation_operators

    def send_usage_message(self, event: str, event_payload: Optional[dict], success: Optional[bool]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def create_expectation_suite(self, expectation_suite_name: str, overwrite_existing: bool=False, ge_cloud_id: Optional[str]=None, **kwargs) -> ExpectationSuite:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        expectation_suite: ExpectationSuite = ExpectationSuite(expectation_suite_name=expectation_suite_name, data_context=self)
        if self.ge_cloud_mode:
            key: GeCloudIdentifier = GeCloudIdentifier(resource_type='expectation_suite', ge_cloud_id=ge_cloud_id)
            if (self.expectations_store.has_key(key) and (not overwrite_existing)):
                raise ge_exceptions.DataContextError('expectation_suite with GE Cloud ID {} already exists. If you would like to overwrite this expectation_suite, set overwrite_existing=True.'.format(ge_cloud_id))
        else:
            key: ExpectationSuiteIdentifier = ExpectationSuiteIdentifier(expectation_suite_name=expectation_suite_name)
            if (self.expectations_store.has_key(key) and (not overwrite_existing)):
                raise ge_exceptions.DataContextError('expectation_suite with name {} already exists. If you would like to overwrite this expectation_suite, set overwrite_existing=True.'.format(expectation_suite_name))
        self.expectations_store.set(key, expectation_suite, **kwargs)
        return expectation_suite

    def delete_expectation_suite(self, expectation_suite_name: Optional[str]=None, ge_cloud_id: Optional[str]=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if self.ge_cloud_mode:
            ...
            key: ExpectationSuiteIdentifier = ExpectationSuiteIdentifier(expectation_suite_name)
        if (not self.expectations_store.has_key(key)):
            raise ge_exceptions.DataContextError('expectation_suite with name {} does not exist.')
        else:
            self.expectations_store.remove_key(key)
            return True

    def get_expectation_suite(self, expectation_suite_name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> ExpectationSuite:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if self.ge_cloud_mode:
            ...
            key: Optional[ExpectationSuiteIdentifier] = ExpectationSuiteIdentifier(expectation_suite_name=expectation_suite_name)
        if self.expectations_store.has_key(key):
            ...
            raise ge_exceptions.DataContextError(f'expectation_suite {expectation_suite_name} not found')
    ...
    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT_SAVE_EXPECTATION_SUITE.value, args_payload_fn=save_expectation_suite_usage_statistics)
    def save_expectation_suite(self, expectation_suite: ExpectationSuite, expectation_suite_name: Optional[str]=None, overwrite_existing: bool=True, ge_cloud_id: Optional[str]=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if self.ge_cloud_mode:
            key: GeCloudIdentifier = GeCloudIdentifier(resource_type='expectation_suite', ge_cloud_id=(ge_cloud_id if (ge_cloud_id is not None) else str(expectation_suite.ge_cloud_id)))
            if (self.expectations_store.has_key(key) and (not overwrite_existing)):
                raise ge_exceptions.DataContextError('expectation_suite with GE Cloud ID {} already exists. If you would like to overwrite this expectation_suite, set overwrite_existing=True.'.format(ge_cloud_id))
        else:
            if (expectation_suite_name is None):
                key: ExpectationSuiteIdentifier = ExpectationSuiteIdentifier(expectation_suite_name=expectation_suite.expectation_suite_name)
            else:
    ...
    def _store_metrics(self, requested_metrics, validation_results, target_store_name) -> None:
        ...
        for (expectation_suite_dependency, metrics_list) in requested_metrics.items():
            ...
            if (not isinstance(metrics_list, list)):
                raise ge_exceptions.DataContextError('Invalid requested_metrics configuration: metrics requested for each expectation suite must be a list.')
            for metric_configuration in metrics_list:
                metric_configurations = BaseDataContext._get_metric_configuration_tuples(metric_configuration)
                for (metric_name, metric_kwargs) in metric_configurations:
                    try:
                        ...
                        self.stores[target_store_name].set(ValidationMetricIdentifier(run_id=run_id, data_asset_name=data_asset_name, expectation_suite_identifier=ExpectationSuiteIdentifier(expectation_suite_name), metric_name=metric_name, metric_kwargs_id=get_metric_kwargs_id(metric_name, metric_kwargs)), metric_value)
                    except ge_exceptions.UnavailableMetricError:
                        logger.debug('metric {} was requested by another expectation suite but is not available in this validation result.'.format(metric_name))

    def store_validation_result_metrics(self, requested_metrics, validation_results, target_store_name) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def evaluation_parameter_store_name(self):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return self.project_config_with_variables_substituted.evaluation_parameter_store_name
    ...
    def _compile_evaluation_parameter_dependencies(self) -> None:
        ...
        for key in self.expectations_store.list_keys():
            ...
            if (not expectation_suite_dict):
                continue
            expectation_suite: ExpectationSuite = ExpectationSuite(**expectation_suite_dict, data_context=self)
            dependencies = expectation_suite.get_evaluation_parameter_dependencies()
            if (len(dependencies) > 0):
                nested_update(self._evaluation_parameter_dependencies, dependencies)
        self._evaluation_parameter_dependencies_compiled = True

    def get_validation_result(self, expectation_suite_name, run_id=None, batch_identifier=None, validations_store_name=None, failed_only=False):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        key = ValidationResultIdentifier(expectation_suite_identifier=ExpectationSuiteIdentifier(expectation_suite_name=expectation_suite_name), run_id=run_id, batch_identifier=batch_identifier)
    ...
    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT_BUILD_DATA_DOCS.value)
    def build_data_docs(self, site_names=None, resource_identifiers=None, dry_run=False, build_index: bool=True):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if sites:
            ...
            for (site_name, site_config) in sites.items():
                ...
                if ((site_names and (site_name in site_names)) or (not site_names)):
                    ...
                    site_builder: SiteBuilder = instantiate_class_from_config(config=complete_site_config, runtime_environment={'data_context': self, 'root_directory': self.root_directory, 'site_name': site_name, 'ge_cloud_mode': self.ge_cloud_mode}, config_defaults={'module_name': module_name})
                    if (not site_builder):
                        raise ge_exceptions.ClassInstantiationError(module_name=module_name, package_name=None, class_name=complete_site_config['class_name'])
                    if dry_run:
                        index_page_locator_infos[site_name] = site_builder.get_resource_url(only_if_exists=False)
                    else:
                        index_page_resource_identifier_tuple = site_builder.build(resource_identifiers, build_index=(build_index and (not self.ge_cloud_mode)))
    ...
    def clean_data_docs(self, site_name=None) -> bool:
        ...
        if (not data_docs_sites):
            raise ge_exceptions.DataContextError('No data docs sites were found on this DataContext, therefore no sites will be cleaned.')
        data_docs_site_names = list(data_docs_sites.keys())
        if site_name:
            if (site_name not in data_docs_site_names):
                raise ge_exceptions.DataContextError(f'The specified site name `{site_name}` does not exist in this project.')
    ...
    def _clean_data_docs_site(self, site_name: str) -> bool:
        ...
        site_builder = instantiate_class_from_config(config=site_config, runtime_environment={'data_context': self, 'root_directory': self.root_directory}, config_defaults={'module_name': 'great_expectations.render.renderer.site_builder'})
        site_builder.clean_site()
        return True

    def profile_datasource(self, datasource_name, batch_kwargs_generator_name=None, data_assets=None, max_data_assets=20, profile_all_data_assets=True, profiler=BasicDatasetProfiler, profiler_configuration=None, dry_run=False, run_id=None, additional_batch_kwargs=None, run_name=None, run_time=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (batch_kwargs_generator_name is None):
            for name in datasource_data_asset_names_dict.keys():
                if (batch_kwargs_generator_name is not None):
                    profiling_results = {'success': False, 'error': {'code': BaseDataContext.PROFILING_ERROR_CODE_MULTIPLE_BATCH_KWARGS_GENERATORS_FOUND}}
            ...
            if (batch_kwargs_generator_name is None):
                profiling_results = {'success': False, 'error': {'code': BaseDataContext.PROFILING_ERROR_CODE_NO_BATCH_KWARGS_GENERATORS_FOUND}}
                return profiling_results
        else:
            try:
                available_data_asset_name_list = datasource_data_asset_names_dict[batch_kwargs_generator_name]['names']
            except KeyError:
                raise ge_exceptions.ProfilerError('batch kwargs Generator {} not found. Specify the name of a generator configured in this datasource'.format(batch_kwargs_generator_name))
        ...
        if (len(available_data_asset_name_list) == 0):
            raise ge_exceptions.ProfilerError('No Data Assets found in Datasource {}. Used batch kwargs generator: {}.'.format(datasource_name, batch_kwargs_generator_name))
        ...
        if (isinstance(data_assets, list) and (len(data_assets) > 0)):
            ...
            if (len(not_found_data_assets) > 0):
                profiling_results = {'success': False, 'error': {'code': BaseDataContext.PROFILING_ERROR_CODE_SPECIFIED_DATA_ASSETS_NOT_FOUND, 'not_found_data_assets': not_found_data_assets, 'data_assets': available_data_asset_name_list}}
                return profiling_results
            data_assets.sort()
            ...
            if (not profile_all_data_assets):
                if (total_data_assets > max_data_assets):
                    profiling_results = {'success': False, 'error': {'code': BaseDataContext.PROFILING_ERROR_CODE_TOO_MANY_DATA_ASSETS, 'num_data_assets': total_data_assets, 'data_assets': available_data_asset_name_list}}
                    return profiling_results
        ...
        if (not dry_run):
            ...
            for name in data_asset_names_to_profiled:
                ...
                try:
                    ...
                    logger.warning(f'SqlAlchemyError while profiling {name[1]}. Skipping.')
                    logger.debug(str(e))
                    skipped_data_assets += 1
            total_duration = (datetime.datetime.now() - total_start_time).total_seconds()
            logger.info(('\n    Profiled %d of %d named data assets, with %d total rows and %d columns in %.2f seconds.\n    Generated, evaluated, and stored %d Expectations during profiling. Please review results using data-docs.' % (len(data_asset_names_to_profiled), total_data_assets, total_rows, total_columns, total_duration, total_expectations)))
            if (skipped_data_assets > 0):
                logger.warning(('Skipped %d data assets due to errors.' % skipped_data_assets))
        profiling_results['success'] = True
        return profiling_results

    def profile_data_asset(self, datasource_name, batch_kwargs_generator_name=None, data_asset_name=None, batch_kwargs=None, expectation_suite_name=None, profiler=BasicDatasetProfiler, profiler_configuration=None, run_id=None, additional_batch_kwargs=None, run_name=None, run_time=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (batch_kwargs is None):
            try:
                generator = self.get_datasource(datasource_name=datasource_name).get_batch_kwargs_generator(name=batch_kwargs_generator_name)
                batch_kwargs = generator.build_batch_kwargs(data_asset_name, **additional_batch_kwargs)
            except ge_exceptions.BatchKwargsError:
                raise ge_exceptions.ProfilerError('Unable to build batch_kwargs for datasource {}, using batch kwargs generator {} for name {}'.format(datasource_name, batch_kwargs_generator_name, data_asset_name))
            except ValueError:
                raise ge_exceptions.ProfilerError('Unable to find datasource {} or batch kwargs generator {}.'.format(datasource_name, batch_kwargs_generator_name))
        else:
        ...
        if (expectation_suite_name is None):
            if ((batch_kwargs_generator_name is None) and (data_asset_name is None)):
                expectation_suite_name = ((((datasource_name + '.') + profiler.__name__) + '.') + BatchKwargs(batch_kwargs).to_id())
            else:
                expectation_suite_name = ((((((datasource_name + '.') + batch_kwargs_generator_name) + '.') + data_asset_name) + '.') + profiler.__name__)
        self.create_expectation_suite(expectation_suite_name=expectation_suite_name, overwrite_existing=True)
        batch = self.get_batch(expectation_suite_name=expectation_suite_name, batch_kwargs=batch_kwargs)
        if (not profiler.validate(batch)):
            raise ge_exceptions.ProfilerError(("batch '%s' is not a valid batch for the '%s' profiler" % (name, profiler.__name__)))
        ...
        validation_ref = self.validations_store.set(key=ValidationResultIdentifier(expectation_suite_identifier=ExpectationSuiteIdentifier(expectation_suite_name=expectation_suite_name), run_id=run_id, batch_identifier=batch.batch_id), value=validation_results)
        ...
        if isinstance(batch, Dataset):
            ...
            new_column_count = len({exp.kwargs['column'] for exp in expectation_suite.expectations if ('column' in exp.kwargs)})
            total_columns += new_column_count
        new_expectation_count = len(expectation_suite.expectations)
        total_expectations += new_expectation_count
        self.save_expectation_suite(expectation_suite)
        duration = (datetime.datetime.now() - start_time).total_seconds()
        logger.info(('\tProfiled %d columns using %d rows from %s (%.3f sec)' % (new_column_count, row_count, name, duration)))
        total_duration = (datetime.datetime.now() - total_start_time).total_seconds()
        logger.info(('\nProfiled the data asset, with %d total rows and %d columns in %.2f seconds.\nGenerated, evaluated, and stored %d Expectations during profiling. Please review results using data-docs.' % (total_rows, total_columns, total_duration, total_expectations)))
        profiling_results['success'] = True
        return profiling_results
    ...
    def add_checkpoint(self, name: str, config_version: Optional[Union[(int, float)]]=None, template_name: Optional[str]=None, module_name: Optional[str]=None, class_name: Optional[str]=None, run_name_template: Optional[str]=None, expectation_suite_name: Optional[str]=None, batch_request: Optional[dict]=None, action_list: Optional[List[dict]]=None, evaluation_parameters: Optional[dict]=None, runtime_configuration: Optional[dict]=None, validations: Optional[List[dict]]=None, profilers: Optional[List[dict]]=None, validation_operator_name: Optional[str]=None, batches: Optional[List[dict]]=None, site_names: Optional[Union[(str, List[str])]]=None, slack_webhook: Optional[str]=None, notify_on: Optional[str]=None, notify_with: Optional[Union[(str, List[str])]]=None, ge_cloud_id: Optional[str]=None, expectation_suite_ge_cloud_id: Optional[str]=None) -> Checkpoint:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        checkpoint: Checkpoint = Checkpoint.construct_from_config_args(data_context=self, checkpoint_store_name=self.checkpoint_store_name, name=name, config_version=config_version, template_name=template_name, module_name=module_name, class_name=class_name, run_name_template=run_name_template, expectation_suite_name=expectation_suite_name, batch_request=batch_request, action_list=action_list, evaluation_parameters=evaluation_parameters, runtime_configuration=runtime_configuration, validations=validations, profilers=profilers, validation_operator_name=validation_operator_name, batches=batches, site_names=site_names, slack_webhook=slack_webhook, notify_on=notify_on, notify_with=notify_with, ge_cloud_id=ge_cloud_id, expectation_suite_ge_cloud_id=expectation_suite_ge_cloud_id)
        self.checkpoint_store.add_checkpoint(checkpoint, name, ge_cloud_id)
        return checkpoint

    def get_checkpoint(self, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> Checkpoint:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        checkpoint_config: CheckpointConfig = self.checkpoint_store.get_checkpoint(name=name, ge_cloud_id=ge_cloud_id)
        checkpoint: Checkpoint = Checkpoint.instantiate_from_config_with_runtime_args(checkpoint_config=checkpoint_config, data_context=self, name=name)
        return checkpoint

    def delete_checkpoint(self, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def run_checkpoint(self, checkpoint_name: Optional[str]=None, ge_cloud_id: Optional[str]=None, template_name: Optional[str]=None, run_name_template: Optional[str]=None, expectation_suite_name: Optional[str]=None, batch_request: Optional[BatchRequestBase]=None, action_list: Optional[List[dict]]=None, evaluation_parameters: Optional[dict]=None, runtime_configuration: Optional[dict]=None, validations: Optional[List[dict]]=None, profilers: Optional[List[dict]]=None, run_id: Optional[Union[(str, int, float)]]=None, run_name: Optional[str]=None, run_time: Optional[datetime.datetime]=None, result_format: Optional[str]=None, expectation_suite_ge_cloud_id: Optional[str]=None, **kwargs) -> CheckpointResult:
        ...
        checkpoint: Checkpoint = self.get_checkpoint(name=checkpoint_name, ge_cloud_id=ge_cloud_id)
        result: CheckpointResult = checkpoint.run_with_runtime_args(template_name=template_name, run_name_template=run_name_template, expectation_suite_name=expectation_suite_name, batch_request=batch_request, action_list=action_list, evaluation_parameters=evaluation_parameters, runtime_configuration=runtime_configuration, validations=validations, profilers=profilers, run_id=run_id, run_name=run_name, run_time=run_time, result_format=result_format, expectation_suite_ge_cloud_id=expectation_suite_ge_cloud_id, **kwargs)
        return result

    def add_profiler(self, name: str, config_version: float, rules: Dict[(str, dict)], variables: Optional[dict]=None, ge_cloud_id: Optional[str]=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        config_data = {'name': name, 'config_version': config_version, 'rules': rules, 'variables': variables}
        ...
        return RuleBasedProfiler.add_profiler(config=config, data_context=self, profiler_store=self.profiler_store, ge_cloud_id=ge_cloud_id)

    def save_profiler(self, profiler: RuleBasedProfiler, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        key: Union[(GeCloudIdentifier, ConfigurationIdentifier)] = self.profiler_store.determine_key(name=name, ge_cloud_id=ge_cloud_id)
        self.profiler_store.set(key=key, value=profiler.config)

    def get_profiler(self, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> RuleBasedProfiler:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return RuleBasedProfiler.get_profiler(data_context=self, profiler_store=self.profiler_store, name=name, ge_cloud_id=ge_cloud_id)

    def delete_profiler(self, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        RuleBasedProfiler.delete_profiler(profiler_store=self.profiler_store, name=name, ge_cloud_id=ge_cloud_id)
    ...
    def list_profilers(self) -> List[str]:
        ...
        if (self.profiler_store is None):
            raise ge_exceptions.StoreConfigurationError('Attempted to list profilers from a Profiler Store, which is not a configured store.')
        return RuleBasedProfiler.list_profilers(profiler_store=self.profiler_store, ge_cloud_mode=self.ge_cloud_mode)

    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT_RUN_PROFILER_WITH_DYNAMIC_ARGUMENTS.value)
    def run_profiler_with_dynamic_arguments(self, name: Optional[str]=None, ge_cloud_id: Optional[str]=None, variables: Optional[dict]=None, rules: Optional[dict]=None) -> RuleBasedProfilerResult:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        return RuleBasedProfiler.run_profiler(data_context=self, profiler_store=self.profiler_store, name=name, ge_cloud_id=ge_cloud_id, variables=variables, rules=rules)

    @usage_statistics_enabled_method(event_name=UsageStatsEvents.DATA_CONTEXT_RUN_PROFILER_ON_DATA.value)
    def run_profiler_on_data(self, batch_list: Optional[List[Batch]]=None, batch_request: Optional[BatchRequestBase]=None, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> RuleBasedProfilerResult:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        return RuleBasedProfiler.run_profiler_on_data(data_context=self, profiler_store=self.profiler_store, batch_list=batch_list, batch_request=batch_request, name=name, ge_cloud_id=ge_cloud_id)

    def test_yaml_config(self, yaml_config: str, name: Optional[str]=None, class_name: Optional[str]=None, runtime_environment: Optional[dict]=None, pretty_print: bool=True, return_mode: Union[(Literal['instantiated_class'], Literal['report_object'])]='instantiated_class', shorten_tracebacks: bool=False):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        config = self._test_yaml_config_prepare_config(yaml_config, runtime_environment, usage_stats_event_name)
        ...
        try:
            if (class_name in self.TEST_YAML_CONFIG_SUPPORTED_STORE_TYPES):
                (instantiated_class, usage_stats_event_payload) = self._test_instantiation_of_store_from_yaml_config(name, class_name, config)
            elif (class_name in self.TEST_YAML_CONFIG_SUPPORTED_DATASOURCE_TYPES):
                (instantiated_class, usage_stats_event_payload) = self._test_instantiation_of_datasource_from_yaml_config(name, class_name, config)
            elif (class_name in self.TEST_YAML_CONFIG_SUPPORTED_CHECKPOINT_TYPES):
                (instantiated_class, usage_stats_event_payload) = self._test_instantiation_of_checkpoint_from_yaml_config(name, class_name, config)
            elif (class_name in self.TEST_YAML_CONFIG_SUPPORTED_DATA_CONNECTOR_TYPES):
                (instantiated_class, usage_stats_event_payload) = self._test_instantiation_of_data_connector_from_yaml_config(name, class_name, config, runtime_environment)
            elif (class_name in self.TEST_YAML_CONFIG_SUPPORTED_PROFILER_TYPES):
                (instantiated_class, usage_stats_event_payload) = self._test_instantiation_of_profiler_from_yaml_config(name, class_name, config)
            else:
                (instantiated_class, usage_stats_event_payload) = self._test_instantiation_of_misc_class_from_yaml_config(name, config, runtime_environment, usage_stats_event_payload)
            send_usage_message(data_context=self, event=usage_stats_event_name, event_payload=usage_stats_event_payload, success=True)
            if pretty_print:
...
''')
            ...
            if (class_name is None):
                usage_stats_event_payload['diagnostic_info'] = (usage_stats_event_payload.get('diagnostic_info', []) + ['__class_name_not_provided__'])
            elif ((usage_stats_event_payload.get('parent_class') is None) and (class_name in self.ALL_TEST_YAML_CONFIG_SUPPORTED_TYPES)):
                usage_stats_event_payload['parent_class'] = class_name
            send_usage_message(data_context=self, event=usage_stats_event_name, event_payload=usage_stats_event_payload, success=False)
            if shorten_tracebacks:
                traceback.print_exc(limit=1)
            else:
                raise e

    def _test_yaml_config_prepare_config(self, yaml_config: str, runtime_environment: dict, usage_stats_event_name: str) -> CommentedMap:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        try:
            substituted_config_variables: Union[(DataContextConfig, dict)] = substitute_all_config_variables(self.config_variables, dict(os.environ))
            substitutions: dict = {**substituted_config_variables, **dict(os.environ), **runtime_environment}
            config_str_with_substituted_variables: Union[(DataContextConfig, dict)] = substitute_all_config_variables(yaml_config, substitutions)
        except Exception as e:
            usage_stats_event_payload: dict = {'diagnostic_info': ['__substitution_error__']}
            send_usage_message(data_context=self, event=usage_stats_event_name, event_payload=usage_stats_event_payload, success=False)
            raise e
        try:
            ...
            usage_stats_event_payload: dict = {'diagnostic_info': ['__yaml_parse_error__']}
            send_usage_message(data_context=self, event=usage_stats_event_name, event_payload=usage_stats_event_payload, success=False)
            raise e

    def _test_instantiation_of_store_from_yaml_config(self, name: Optional[str], class_name: str, config: CommentedMap) -> Tuple[(Store, dict)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        instantiated_class = cast(Store, self._build_store_from_config(store_name=store_name, store_config=config))
        store_name = (instantiated_class.store_name or store_name)
        self.config['stores'][store_name] = config
        anonymizer = Anonymizer(self.data_context_id)
        usage_stats_event_payload = anonymizer.anonymize(store_name=store_name, store_obj=instantiated_class)
        return (instantiated_class, usage_stats_event_payload)

    def _test_instantiation_of_datasource_from_yaml_config(self, name: Optional[str], class_name: str, config: CommentedMap) -> Tuple[(Datasource, dict)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        instantiated_class = cast(Datasource, self._instantiate_datasource_from_config_and_update_project_config(name=datasource_name, config=config, initialize=True))
        anonymizer = Anonymizer(self.data_context_id)
        if (class_name == 'SimpleSqlalchemyDatasource'):
            usage_stats_event_payload = anonymizer.anonymize(obj=instantiated_class, name=datasource_name, config=config)
        else:
            ...
            usage_stats_event_payload = anonymizer.anonymize(obj=instantiated_class, name=datasource_name, config=full_datasource_config)
        return (instantiated_class, usage_stats_event_payload)

    def _test_instantiation_of_checkpoint_from_yaml_config(self, name: Optional[str], class_name: str, config: CommentedMap) -> Tuple[(Checkpoint, dict)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        checkpoint_class_args: dict = filter_properties_dict(properties=checkpoint_config, delete_fields={'class_name', 'module_name'}, clean_falsy=True)
        ...
        usage_stats_event_payload = anonymizer.anonymize(obj=instantiated_class, name=checkpoint_name, config=checkpoint_config)
        return (instantiated_class, usage_stats_event_payload)

    def _test_instantiation_of_data_connector_from_yaml_config(self, name: Optional[str], class_name: str, config: CommentedMap, runtime_environment: dict) -> Tuple[(DataConnector, dict)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        data_connector_name: str = (name or config.get('name') or 'my_temp_data_connector')
        instantiated_class = instantiate_class_from_config(config=config, runtime_environment={**runtime_environment, **{'root_directory': self.root_directory}}, config_defaults={})
        anonymizer = Anonymizer(self.data_context_id)
        usage_stats_event_payload = anonymizer.anonymize(obj=instantiated_class, name=data_connector_name, config=config)
        return (instantiated_class, usage_stats_event_payload)

    def _test_instantiation_of_profiler_from_yaml_config(self, name: Optional[str], class_name: str, config: CommentedMap) -> Tuple[(RuleBasedProfiler, dict)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        profiler_config: Union[(RuleBasedProfilerConfig, dict)] = RuleBasedProfilerConfig.from_commented_map(commented_map=config)
        profiler_config = profiler_config.to_json_dict()
        profiler_config.update({'name': profiler_name})
        instantiated_class = instantiate_class_from_config(config=profiler_config, runtime_environment={'data_context': self}, config_defaults={'module_name': 'great_expectations.rule_based_profiler', 'class_name': 'RuleBasedProfiler'})
        anonymizer: Anonymizer = Anonymizer(self.data_context_id)
        usage_stats_event_payload: dict = anonymizer.anonymize(obj=instantiated_class, name=profiler_name, config=profiler_config)
        return (instantiated_class, usage_stats_event_payload)

    def _test_instantiation_of_misc_class_from_yaml_config(self, name: Optional[str], config: CommentedMap, runtime_environment: dict, usage_stats_event_payload: dict) -> Tuple[(Any, dict)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        instantiated_class = instantiate_class_from_config(config=config, runtime_environment={**runtime_environment, **{'root_directory': self.root_directory}}, config_defaults={})
        anonymizer: Anonymizer = Anonymizer(self.data_context_id)
        parent_class_from_object = anonymizer.get_parent_class(object_=instantiated_class)
        parent_class_from_config = anonymizer.get_parent_class(object_config=config)
        if ((parent_class_from_object is not None) and parent_class_from_object.endswith('Store')):
            store_name: str = (name or config.get('name') or 'my_temp_store')
            store_name = (instantiated_class.store_name or store_name)
            usage_stats_event_payload = anonymizer.anonymize(store_name=store_name, store_obj=instantiated_class)
        elif ((parent_class_from_config is not None) and parent_class_from_config.endswith('Datasource')):
            datasource_name: str = (name or config.get('name') or 'my_temp_datasource')
            ...
            if (parent_class_from_config == 'SimpleSqlalchemyDatasource'):
                ...
                usage_stats_event_payload = anonymizer.anonymize(obj=instantiated_class, name=datasource_name, config=full_datasource_config)
        elif ((parent_class_from_config is not None) and parent_class_from_config.endswith('Checkpoint')):
            checkpoint_name: str = (name or config.get('name') or 'my_temp_checkpoint')
            checkpoint_config: Union[(CheckpointConfig, dict)]
            checkpoint_config = CheckpointConfig.from_commented_map(commented_map=config)
            checkpoint_config = checkpoint_config.to_json_dict()
            checkpoint_config.update({'name': checkpoint_name})
            usage_stats_event_payload = anonymizer.anonymize(obj=checkpoint_config, name=checkpoint_name, config=checkpoint_config)
        elif ((parent_class_from_config is not None) and parent_class_from_config.endswith('DataConnector')):
            data_connector_name: str = (name or config.get('name') or 'my_temp_data_connector')
            usage_stats_event_payload = anonymizer.anonymize(obj=instantiated_class, name=data_connector_name, config=config)
        else:
            usage_stats_event_payload['diagnostic_info'] = (usage_stats_event_payload.get('diagnostic_info', []) + ['__custom_subclass_not_core_ge__'])
        return (instantiated_class, usage_stats_event_payload)
    ...
    def _get_metric_configuration_tuples(metric_configuration, base_kwargs=None):
        ...
        for kwarg_name in metric_configuration.keys():
            if (not isinstance(metric_configuration[kwarg_name], dict)):
                raise ge_exceptions.DataContextError('Invalid metric_configuration: each key must contain a dictionary.')
            if (kwarg_name == 'metric_kwargs_id'):
                for metric_kwargs_id in metric_configuration[kwarg_name].keys():
                    if (base_kwargs != {}):
                        raise ge_exceptions.DataContextError('Invalid metric_configuration: when specifying metric_kwargs_id, no other keys or values may be defined.')
                    if (not isinstance(metric_configuration[kwarg_name][metric_kwargs_id], list)):
                        raise ge_exceptions.DataContextError('Invalid metric_configuration: each value must contain a list.')
                    metric_configurations_list += [(metric_name, {'metric_kwargs_id': metric_kwargs_id}) for metric_name in metric_configuration[kwarg_name][metric_kwargs_id]]
            else:
                for kwarg_value in metric_configuration[kwarg_name].keys():
                    ...
                    for nested_configuration in metric_configuration[kwarg_name][kwarg_value]:
                        metric_configurations_list += BaseDataContext._get_metric_configuration_tuples(nested_configuration, base_kwargs=base_kwargs)
