...
from great_expectations.data_context.templates import CONFIG_VARIABLES_TEMPLATE, PROJECT_TEMPLATE_USAGE_STATISTICS_DISABLED, PROJECT_TEMPLATE_USAGE_STATISTICS_ENABLED
from great_expectations.data_context.types.base import CURRENT_GE_CONFIG_VERSION, MINIMUM_SUPPORTED_CONFIG_VERSION, DataContextConfig, GeCloudConfig, dataContextConfigSchema
...
class DataContext(BaseDataContext):
    ...
    @classmethod
    def create(cls, project_root_dir: Optional[str]=None, usage_statistics_enabled: bool=True, runtime_environment: Optional[dict]=None) -> 'DataContext':
        import inspect
        ...
        if (not os.path.isdir(project_root_dir)):
            raise ge_exceptions.DataContextError('The project_root_dir must be an existing directory in which to initialize a new DataContext')
    ...
    @classmethod
    def write_project_template_to_disk(cls, ge_dir: str, usage_statistics_enabled: bool=True) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @classmethod
    def _get_ge_cloud_config_dict(cls, ge_cloud_base_url: Optional[str]=None, ge_cloud_account_id: Optional[str]=None, ge_cloud_access_token: Optional[str]=None, ge_cloud_organization_id: Optional[str]=None) -> Dict[(str, Optional[str])]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        ge_cloud_base_url = (ge_cloud_base_url or super()._get_global_config_value(environment_variable='GE_CLOUD_BASE_URL', conf_file_section='ge_cloud_config', conf_file_option='base_url') or 'https://app.greatexpectations.io/')
        if (ge_cloud_account_id is not None):
            logger.warning('The "ge_cloud_account_id" argument has been renamed "ge_cloud_organization_id" and will be deprecated in the next major release.')
        else:
            ge_cloud_account_id = super()._get_global_config_value(environment_variable='GE_CLOUD_ACCOUNT_ID', conf_file_section='ge_cloud_config', conf_file_option='account_id')
            logger.warning('If you have an environment variable named "GE_CLOUD_ACCOUNT_ID", please rename it to "GE_CLOUD_ORGANIZATION_ID". If you have a global config file with an "account_id" option, please rename it to "organization_id". "GE_CLOUD_ACCOUNT_ID" and "account_id" will be deprecated in the next major release.')
        if (ge_cloud_organization_id is None):
            ge_cloud_organization_id = super()._get_global_config_value(environment_variable='GE_CLOUD_ORGANIZATION_ID', conf_file_section='ge_cloud_config', conf_file_option='organization_id')
        ge_cloud_organization_id = (ge_cloud_organization_id or ge_cloud_account_id)
        ge_cloud_access_token = (ge_cloud_access_token or super()._get_global_config_value(environment_variable='GE_CLOUD_ACCESS_TOKEN', conf_file_section='ge_cloud_config', conf_file_option='access_token'))
        return {'base_url': ge_cloud_base_url, 'organization_id': ge_cloud_organization_id, 'access_token': ge_cloud_access_token}
    ...
    def get_ge_cloud_config(self, ge_cloud_base_url: Optional[str]=None, ge_cloud_account_id: Optional[str]=None, ge_cloud_access_token: Optional[str]=None, ge_cloud_organization_id: Optional[str]=None) -> GeCloudConfig:
        ...
        ge_cloud_config_dict = self._get_ge_cloud_config_dict(ge_cloud_base_url=ge_cloud_base_url, ge_cloud_account_id=ge_cloud_account_id, ge_cloud_access_token=ge_cloud_access_token, ge_cloud_organization_id=ge_cloud_organization_id)
        missing_keys = []
        ...
        if (len(missing_keys) > 0):
            ...
            raise DataContextError(f"{', '.join(missing_keys_str)} arg(s) required for ge_cloud_mode but neither provided nor found in environment or in global configs ({', '.join(global_config_path_str)}).")
        return GeCloudConfig(**ge_cloud_config_dict)

    def __init__(self, context_root_dir: Optional[str]=None, runtime_environment: Optional[dict]=None, ge_cloud_mode: bool=False, ge_cloud_base_url: Optional[str]=None, ge_cloud_account_id: Optional[str]=None, ge_cloud_access_token: Optional[str]=None, ge_cloud_organization_id: Optional[str]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if ge_cloud_mode:
            ge_cloud_config = self.get_ge_cloud_config(ge_cloud_base_url=ge_cloud_base_url, ge_cloud_account_id=ge_cloud_account_id, ge_cloud_access_token=ge_cloud_access_token, ge_cloud_organization_id=ge_cloud_organization_id)
            self._ge_cloud_config = ge_cloud_config
            if (context_root_dir is None):
                context_root_dir = os.getcwd()
                logger.info(f'context_root_dir was not provided - defaulting to current working directory "{context_root_dir}".')
        else:
            context_root_dir = (self.find_context_root_dir() if (context_root_dir is None) else context_root_dir)
        context_root_directory = os.path.abspath(os.path.expanduser(context_root_dir))
        self._context_root_directory = context_root_directory
        project_config = self._load_project_config()
        super().__init__(project_config, context_root_directory, runtime_environment, ge_cloud_mode=ge_cloud_mode, ge_cloud_config=ge_cloud_config)
        project_config_dict = dataContextConfigSchema.dump(project_config)
        if ((project_config.anonymous_usage_statistics.explicit_id is False) or (project_config_dict != dataContextConfigSchema.dump(self.config))):
    ...
    def _retrieve_data_context_config_from_ge_cloud(self) -> DataContextConfig:
        ...
        ge_cloud_url = (self.ge_cloud_config.base_url + f'/organizations/{self.ge_cloud_config.organization_id}/data-context-configuration')
        auth_headers = {'Content-Type': 'application/vnd.api+json', 'Authorization': f'Bearer {self.ge_cloud_config.access_token}'}
        response = requests.get(ge_cloud_url, headers=auth_headers)
        if (response.status_code != 200):
            raise ge_exceptions.GeCloudError(f'Bad request made to GE Cloud; {response.text}')
    ...
    def _load_project_config(self):
        ...
        try:
            ...
            raise ge_exceptions.InvalidConfigurationYamlError('Error: duplicate key found in project YAML file.')
        except OSError:
    ...
    def _save_project_config(self):
        ...
        if self.ge_cloud_mode:
            logger.debug('ge_cloud_mode detected - skipping DataContext._save_project_config')
            return
    ...
    def add_datasource(self, name, **kwargs) -> Optional[Union[(LegacyDatasource, BaseDatasource)]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        new_datasource: Optional[Union[(LegacyDatasource, BaseDatasource)]] = super().add_datasource(name=name, **kwargs)
        self._save_project_config()
    ...
    def find_context_root_dir(cls):
        ...
        if ge_home_environment:
            ...
            if (os.path.isdir(ge_home_environment) and os.path.isfile(os.path.join(ge_home_environment, 'great_expectations.yml'))):
    ...
    def set_ge_config_version(cls, config_version, context_root_dir=None, validate_config_version=True):
        ...
        if (not isinstance(config_version, (int, float))):
            raise ge_exceptions.UnsupportedConfigVersionError('The argument `config_version` must be a number.')
        if validate_config_version:
            if (config_version < MINIMUM_SUPPORTED_CONFIG_VERSION):
                raise ge_exceptions.UnsupportedConfigVersionError('Invalid config version ({}).\n    The version number must be at least {}. '.format(config_version, MINIMUM_SUPPORTED_CONFIG_VERSION))
    ...
    def is_project_initialized(cls, ge_dir):
        ...
        return (cls.does_config_exist_on_disk(ge_dir) and cls.all_uncommitted_directories_exist(ge_dir) and cls.config_variables_yml_exist(ge_dir) and cls._does_context_have_at_least_one_datasource(ge_dir) and cls._does_context_have_at_least_one_suite(ge_dir))
