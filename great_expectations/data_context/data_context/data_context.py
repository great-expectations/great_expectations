...
class DataContext(BaseDataContext):
    ...
    def create(
        ...
        if not os.path.isdir(project_root_dir):
            raise ge_exceptions.DataContextError(
                "The project_root_dir must be an existing directory in which to initialize a new DataContext"
            )
    ...
    def _get_ge_cloud_config_dict(
        ...
        if ge_cloud_account_id is not None:
            logger.warning(
                'The "ge_cloud_account_id" argument has been renamed "ge_cloud_organization_id" and will be deprecated in the next major release.'
            )
        else:
            ...
            logger.warning(
                'If you have an environment variable named "GE_CLOUD_ACCOUNT_ID", please rename it to "GE_CLOUD_ORGANIZATION_ID". If you have a global config file with an "account_id" option, please rename it to "organization_id". "GE_CLOUD_ACCOUNT_ID" and "account_id" will be deprecated in the next major release.'
            )
    ...
    def get_ge_cloud_config(
        ...
        if len(missing_keys) > 0:
            ...
            raise DataContextError(
                f"{', '.join(missing_keys_str)} arg(s) required for ge_cloud_mode but neither provided nor found in environment or in global configs ({', '.join(global_config_path_str)})."
            )
    ...
    def __init__(
        ...
        if ge_cloud_mode:
            ...
            if context_root_dir is None:
                ...
                logger.info(
                    f'context_root_dir was not provided - defaulting to current working directory "{context_root_dir}".'
                )
        else:
            context_root_dir = (
        ...
        if (project_config.anonymous_usage_statistics.explicit_id is False) or (
            project_config_dict != dataContextConfigSchema.dump(self.config)
        ):
