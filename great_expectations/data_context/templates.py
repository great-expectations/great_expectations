...
CONFIG_VARIABLES_TEMPLATE = f'{CONFIG_VARIABLES_INTRO}instance_id: {str(uuid.uuid4())}{os.linesep}'
EXPECTATIONS_STORE_STRING = yaml.dump({'expectations_store': DataContextConfigDefaults.DEFAULT_STORES.value['expectations_store']}).replace('\n', '\n  ')[:(- 2)]
VALIDATIONS_STORE_STRING = yaml.dump({'validations_store': DataContextConfigDefaults.DEFAULT_STORES.value['validations_store']}).replace('\n', '\n  ')[:(- 2)]
EVALUATION_PARAMETER_STORE_STRING = yaml.dump(DataContextConfigDefaults.DEFAULT_STORES.value['evaluation_parameter_store'])
CHECKPOINT_STORE_STRING = yaml.dump({'checkpoint_store': DataContextConfigDefaults.DEFAULT_STORES.value['checkpoint_store']}).replace('\n', '\n  ')[:(- 2)]
PROFILER_STORE_STRING = yaml.dump({'profiler_store': DataContextConfigDefaults.DEFAULT_STORES.value['profiler_store']}).replace('\n', '\n  ')[:(- 2)]
PROJECT_OPTIONAL_CONFIG_COMMENT = (CONFIG_VARIABLES_INTRO + f'''
