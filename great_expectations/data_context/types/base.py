...
continue.
        ...
        if (("bucket" in data) or ("max_keys" in data)) and (
            not (
                data["class_name"]
...
continue.
        ...
        if ("azure_options" in data) and (
            ...
            if not (("conn_str" in azure_options) ^ ("account_url" in azure_options)):
                raise ge_exceptions.InvalidConfigError(
                    "Your current configuration is either missing methods of authentication or is using too many for the Azure type of data connector. You must only select one between `conn_str` or `account_url`. Please update your configuration to continue.\n"
                )
        if (
            ("gcs_options" in data)
            ...
            not (
                data["class_name"]
                in ["InferredAssetGCSDataConnector", "ConfiguredAssetGCSDataConnector"]
            )
        ):
            raise ge_exceptions.InvalidConfigError(
                f"""Your current configuration uses one or more keys in a data connector that are required only by a
...
continue.
        ...
        if ("gcs_options" in data) and (
            ...
            if ("filename" in gcs_options) and ("info" in gcs_options):
                raise ge_exceptions.InvalidConfigError(
                    "Your current configuration can only use a single method of authentication for the GCS type of data connector. You must only select one between `filename` (from_service_account_file) and `info` (from_service_account_info). Please update your configuration to continue.\n"
                )
        if (
...
class ExecutionEngineConfigSchema(Schema):
    ...
    class_name = fields.String(required=True, allow_none=False)
    module_name = fields.String(
        required=False, allow_none=True, missing="great_expectations.execution_engine"
    )
    ...
    def validate_schema(self, data, **kwargs):
        ...
        if data["class_name"][0] == "$":
            return
        if (("connection_string" in data) or ("credentials" in data)) and (
            not (data["class_name"] == "SqlAlchemyExecutionEngine")
        ):
            raise ge_exceptions.InvalidConfigError(
...
configuration to continue.
        ...
        if ("spark_config" in data) and (
            not (data["class_name"] == "SparkDFExecutionEngine")
        ):
            raise ge_exceptions.InvalidConfigError(
...
class DatasourceConfigSchema(Schema):
    ...
    module_name = fields.String(
        required=False, allow_none=True, missing="great_expectations.datasource"
    )
    ...
    def validate_schema(self, data, **kwargs):
        ...
        if "generators" in data:
            raise ge_exceptions.InvalidConfigError(
                'Your current configuration uses the "generators" key in a datasource, but in version 0.10 of GE that key is renamed to "batch_kwargs_generators". Please update your configuration to continue.'
            )
        if data["class_name"][0] == "$":
            return
        if (
...
class GeCloudConfig(DictDot):
    def __init__(
        ...
        if account_id is not None:
            logger.warning(
                'The "account_id" argument has been renamed "organization_id" and will be deprecated in the next major release.'
            )
    ...
    def account_id(self):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f"<INTROSPECT> {__file}:{__func} - {k}:{v.__class__.__name__}")
        logger.warning(
            'The "account_id" attribute has been renamed to "organization_id" and will be deprecated in the next major release.'
        )
...
class DataContextConfigSchema(Schema):
    ...
    def validate_schema(self, data, **kwargs) -> None:
        ...
        if data["config_version"] < MINIMUM_SUPPORTED_CONFIG_VERSION:
            raise ge_exceptions.UnsupportedConfigVersionError(
                "You appear to have an invalid config version ({}).\n    The version number must be at least {}. Please see the migration guide at https://docs.greatexpectations.io/docs/guides/miscellaneous/migration_guide#migrating-to-the-batch-request-v3-api".format(
        ...
        if (
            ...
            logger.warning(
                f"""You appear to be using a legacy capability with the latest config version ({data['config_version']}).
...
class CheckpointConfigSchema(Schema):
    ...
    def validate_schema(self, data, **kwargs) -> None:
        ...
        if not (
            ("name" in data)
            or ("validation_operator_name" in data)
            or ("batches" in data)
        ):
            raise ge_exceptions.InvalidConfigError(
