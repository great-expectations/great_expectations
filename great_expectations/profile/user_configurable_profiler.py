...
from great_expectations.profile.base import OrderedProfilerCardinality, ProfilerSemanticTypes, profiler_data_types_with_mapping
...
class UserConfigurableProfiler():
    ...
    def __init__(self, profile_dataset: Union[(Batch, Dataset, Validator)], excluded_expectations: Optional[List[str]]=None, ignored_columns: Optional[List[str]]=None, not_null_only: bool=False, primary_or_compound_key: Optional[List[str]]=None, semantic_types_dict: Optional[Dict[(str, List[str])]]=None, table_expectations_only: bool=False, value_set_threshold: str='MANY') -> None:
        import inspect
        ...
        if isinstance(self.profile_dataset, Batch):
            ...
            self.profile_dataset = Validator(execution_engine=self.profile_dataset.data.execution_engine, batches=[self.profile_dataset])
        ...
        assert (self.value_set_threshold in OrderedProfilerCardinality.__members__), f'value_set_threshold must be one of {[i for i in OrderedProfilerCardinality.__members__]}'
        ...
        if (self.table_expectations_only is True):
            logger.info('table_expectations_only is set to True. When used to build a suite, this profiler will ignore allcolumns and create expectations only at the table level. If you would also like to create expectations at the column level, you can instantiate a new profiler with table_expectations_only set to False')
        ...
        if self.primary_or_compound_key:
            for column in self.primary_or_compound_key:
                if (column not in self.all_table_columns):
                    raise ValueError(f'''Column {column} not found. Please ensure that this column is in the {type(profile_dataset).__name__} if you would like to use it as a primary_or_compound_key.
''')
        included_columns = [column_name for column_name in self.all_table_columns if (column_name not in self.ignored_columns)]
        ...
        self.semantic_type_functions = {'DATETIME': self._build_expectations_datetime, 'NUMERIC': self._build_expectations_numeric, 'STRING': self._build_expectations_string, 'VALUE_SET': self._build_expectations_value_set, 'BOOLEAN': self._build_expectations_value_set}
    ...
    def build_suite(self) -> ExpectationSuite:
        ...
        if (len(self.profile_dataset.get_expectation_suite().expectations) > 0):
            suite_name: str = self.profile_dataset._expectation_suite.expectation_suite_name
            self.profile_dataset._expectation_suite = ExpectationSuite(expectation_suite_name=suite_name, data_context=None)
    ...
    def _send_usage_stats_message(self) -> None:
        ...
        if ('Dataset' in profile_dataset_type):
            ...
            raise ValueError(f'''The "profile_dataset_type" property must be one of "Dataset", "Batch", or "Validator" objects. The type detected is "{str(type(self.profile_dataset))}", which is illegal.
''')
        event_payload: dict = {'profile_dataset_type': profile_dataset_type, 'excluded_expectations_specified': ((self.excluded_expectations is not None) and isinstance(self.excluded_expectations, list) and (len(self.excluded_expectations) > 0)), 'ignored_columns_specified': ((self.ignored_columns is not None) and isinstance(self.ignored_columns, list) and (len(self.ignored_columns) > 0)), 'not_null_only': self.not_null_only, 'primary_or_compound_key_specified': ((self.primary_or_compound_key is not None) and isinstance(self.primary_or_compound_key, list) and (len(self.primary_or_compound_key) > 0)), 'semantic_types_dict_specified': ((self.semantic_types_dict is not None) and isinstance(self.semantic_types_dict, dict) and (len(self.semantic_types_dict.keys()) > 0)), 'table_expectations_only': self.table_expectations_only, 'value_set_threshold_specified': ((self.value_set_threshold is not None) and isinstance(self.value_set_threshold, str) and (len(self.value_set_threshold) > 0))}
        send_usage_message(data_context=self.profile_dataset._data_context, event=UsageStatsEvents.LEGACY_PROFILER_BUILD_SUITE.value, event_payload=event_payload, api_version='v2', success=True)
    ...
    def _profile_and_build_expectation_suite(self):
        ...
        if self.primary_or_compound_key:
            self._build_expectations_primary_or_compound_key(profile_dataset=self.profile_dataset, column_list=self.primary_or_compound_key)
        self._build_expectations_table(profile_dataset=self.profile_dataset)
        with tqdm(desc='Profiling', total=len(self.column_info), delay=5, disable=(not self._enable_progress_bars)) as pbar:
            for (column_name, column_info) in self.column_info.items():
                pbar.set_postfix_str(f'Column={column_name}')
                data_type = column_info.get('type')
                cardinality = column_info.get('cardinality')
                if (data_type in ('FLOAT', 'INT', 'NUMERIC')):
                    self._build_expectations_numeric(profile_dataset=self.profile_dataset, column=column_name)
                if (data_type == 'DATETIME'):
                    self._build_expectations_datetime(profile_dataset=self.profile_dataset, column=column_name)
    ...
    def _validate_semantic_types_dict(self):
        ...
        if (not isinstance(self.semantic_types_dict, dict)):
            raise ValueError(f'The semantic_types dict in the config must be a dictionary, but is currently a {type(self.semantic_types_dict)}. Please reformat.')
        for (k, v) in self.semantic_types_dict.items():
            assert isinstance(v, list), "Entries in semantic type dict must be lists of column names e.g. {'semantic_types': {'numeric': ['number_of_transactions']}}"
            if (not any(((k.upper() == semantic_type.value) for semantic_type in ProfilerSemanticTypes))):
                raise ValueError(f'{k} is not a recognized semantic_type. Please only include one of {[semantic_type.value for semantic_type in ProfilerSemanticTypes]}')
        ...
        if selected_columns:
            for column in selected_columns:
                if (column not in self.all_table_columns):
                    ...
                    raise ValueError(f'Column {column} is specified in both the semantic_types_dict and the list of ignored columns. Please remove one of these entries to proceed.')
        for (semantic_type, column_list) in self.semantic_types_dict.items():
            for column_name in column_list:
                ...
                if (semantic_type == 'datetime'):
                    assert (processed_column.get('type') in ('DATETIME', 'STRING')), f"Column {column_name} must be a datetime column or a string but appears to be {processed_column.get('type')}"
                elif (semantic_type == 'numeric'):
                    assert (processed_column.get('type') in ('INT', 'FLOAT', 'NUMERIC')), f"Column {column_name} must be an int or a float but appears to be {processed_column.get('type')}"
    ...
    def _add_column_type_to_column_info(self, profile_dataset, column_name):
        ...
        if ('expect_column_values_to_be_in_type_list' in self.excluded_expectations):
            logger.info('expect_column_values_to_be_in_type_list is in the excluded_expectations list. Thisexpectation is required to establish column data, so it will be run and then removed from theexpectation suite.')
    ...
    def _get_column_type(profile_dataset, column) -> str:
        ...
        try:
            if (profile_dataset.expect_column_values_to_be_in_type_list(column, type_list=sorted(list(ProfilerTypeMapping.INT_TYPE_NAMES))).success and profile_dataset.expect_column_values_to_be_in_type_list(column, type_list=sorted(list(ProfilerTypeMapping.FLOAT_TYPE_NAMES))).success):
                type_ = 'NUMERIC'
            elif profile_dataset.expect_column_values_to_be_in_type_list(column, type_list=sorted(list(ProfilerTypeMapping.INT_TYPE_NAMES))).success:
                type_ = 'INT'
            elif profile_dataset.expect_column_values_to_be_in_type_list(column, type_list=sorted(list(ProfilerTypeMapping.FLOAT_TYPE_NAMES))).success:
                type_ = 'FLOAT'
            elif profile_dataset.expect_column_values_to_be_in_type_list(column, type_list=sorted(list(ProfilerTypeMapping.STRING_TYPE_NAMES))).success:
                type_ = 'STRING'
            elif profile_dataset.expect_column_values_to_be_in_type_list(column, type_list=sorted(list(ProfilerTypeMapping.BOOLEAN_TYPE_NAMES))).success:
                type_ = 'BOOLEAN'
            elif profile_dataset.expect_column_values_to_be_in_type_list(column, type_list=sorted(list(ProfilerTypeMapping.DATETIME_TYPE_NAMES))).success:
                type_ = 'DATETIME'
            else:
                type_ = 'UNKNOWN'
            ...
            type_ = 'unknown'
        if (type_ == 'NUMERIC'):
            profile_dataset.expect_column_values_to_be_in_type_list(column, type_list=(sorted(list(ProfilerTypeMapping.INT_TYPE_NAMES)) + sorted(list(ProfilerTypeMapping.FLOAT_TYPE_NAMES))))
        profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_values_to_be_in_type_list', kwargs={'column': column}))
        return type_
    ...
    def _add_column_cardinality_to_column_info(self, profile_dataset, column_name):
        ...
        if (not column_cardinality):
            ...
            profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_unique_value_count_to_be_between', kwargs={'column': column_name}))
            profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_proportion_of_unique_values_to_be_between', kwargs={'column': column_name}))
    ...
    def _get_column_cardinality(profile_dataset, column) -> OrderedProfilerCardinality:
        ...
        pct_unique = None
        try:
            num_unique = profile_dataset.expect_column_unique_value_count_to_be_between(column, None, None).result['observed_value']
            pct_unique = profile_dataset.expect_column_proportion_of_unique_values_to_be_between(column, None, None).result['observed_value']
        except KeyError:
            logger.error(f'Failed to get cardinality of column {column:s} - continuing...')
        cardinality: OrderedProfilerCardinality = OrderedProfilerCardinality.get_basic_column_cardinality(num_unique, pct_unique)
    ...
    def _add_semantic_types_by_column_from_config_to_column_info(self, column_name):
        ...
        if (not semantic_types):
            assert isinstance(self.semantic_types_dict, dict), f'''The semantic_types dict in the config must be a dictionary, but is currently a {type(self.semantic_types_dict)}. Please reformat.
'''
            semantic_types = []
            for (semantic_type, column_list) in self.semantic_types_dict.items():
            ...
            if all(((i in column_info_entry.get('semantic_types')) for i in ['BOOLEAN', 'VALUE_SET'])):
                logger.info(f"Column {column_name} has both 'BOOLEAN' and 'VALUE_SET' specified as semantic_types.As these are currently the same in function, the 'VALUE_SET' type will be removed.")
    ...
    def _build_column_description_metadata(self, profile_dataset):
        ...
        expectation_suite = profile_dataset.get_expectation_suite(suppress_warnings=True, discard_failed_expectations=False)
        meta_columns = {}
    ...
    def _build_expectations_value_set(self, profile_dataset, column):
        ...
        if ('expect_column_values_to_be_in_set' not in self.excluded_expectations):
            value_set = profile_dataset.expect_column_distinct_values_to_be_in_set(column, value_set=None, result_format='SUMMARY').result['observed_value']
            profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_distinct_values_to_be_in_set', kwargs={'column': column}), match_type='domain')
    ...
    def _build_expectations_numeric(self, profile_dataset, column):
        ...
        if ('expect_column_min_to_be_between' not in self.excluded_expectations):
            observed_min = profile_dataset.expect_column_min_to_be_between(column, min_value=None, max_value=None, result_format='SUMMARY').result['observed_value']
            if (not is_nan(observed_min)):
                profile_dataset.expect_column_min_to_be_between(column, min_value=observed_min, max_value=observed_min)
            else:
                profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_min_to_be_between', kwargs={'column': column}), match_type='domain')
        ...
        if ('expect_column_max_to_be_between' not in self.excluded_expectations):
            observed_max = profile_dataset.expect_column_max_to_be_between(column, min_value=None, max_value=None, result_format='SUMMARY').result['observed_value']
            if (not is_nan(observed_max)):
                profile_dataset.expect_column_max_to_be_between(column, min_value=observed_max, max_value=observed_max)
            else:
                profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_max_to_be_between', kwargs={'column': column}), match_type='domain')
        ...
        if ('expect_column_mean_to_be_between' not in self.excluded_expectations):
            observed_mean = profile_dataset.expect_column_mean_to_be_between(column, min_value=None, max_value=None, result_format='SUMMARY').result['observed_value']
            if (not is_nan(observed_mean)):
                profile_dataset.expect_column_mean_to_be_between(column, min_value=observed_mean, max_value=observed_mean)
            else:
                profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_mean_to_be_between', kwargs={'column': column}), match_type='domain')
                logger.debug(f'Skipping expect_column_mean_to_be_between because observed value is nan: {observed_mean}')
        if ('expect_column_median_to_be_between' not in self.excluded_expectations):
            observed_median = profile_dataset.expect_column_median_to_be_between(column, min_value=None, max_value=None, result_format='SUMMARY').result['observed_value']
            if (not is_nan(observed_median)):
                profile_dataset.expect_column_median_to_be_between(column, min_value=observed_median, max_value=observed_median)
            else:
                profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_median_to_be_between', kwargs={'column': column}), match_type='domain')
                logger.debug(f'Skipping expect_column_median_to_be_between because observed value is nan: {observed_median}')
        ...
        if ('expect_column_quantile_values_to_be_between' not in self.excluded_expectations):
            if isinstance(profile_dataset, Dataset):
                ...
                if isinstance(profile_dataset.execution_engine, SqlAlchemyExecutionEngine):
                    sqlalchemy_execution_engine: SqlAlchemyExecutionEngine = cast(SqlAlchemyExecutionEngine, profile_dataset.execution_engine)
                    allow_relative_error = attempt_allowing_relative_error(sqlalchemy_execution_engine.engine.dialect)
            quantile_result = profile_dataset.expect_column_quantile_values_to_be_between(column, quantile_ranges={'quantiles': [0.05, 0.25, 0.5, 0.75, 0.95], 'value_ranges': [[None, None], [None, None], [None, None], [None, None], [None, None]]}, allow_relative_error=allow_relative_error, result_format='SUMMARY')
            if (quantile_result.exception_info and (quantile_result.exception_info['exception_traceback'] or quantile_result.exception_info['exception_message'])):
                profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_quantile_values_to_be_between', kwargs={'column': column}), match_type='domain')
                ...
                profile_dataset.expect_column_quantile_values_to_be_between(column, quantile_ranges={'quantiles': quantile_result.result['observed_value']['quantiles'], 'value_ranges': [[v, v] for v in quantile_result.result['observed_value']['values']]}, allow_relative_error=allow_relative_error)
        return profile_dataset
    ...
    def _build_expectations_datetime(self, profile_dataset, column):
        ...
        if ('expect_column_values_to_be_between' not in self.excluded_expectations):
            min_value = profile_dataset.expect_column_min_to_be_between(column, min_value=None, max_value=None, result_format='SUMMARY', parse_strings_as_datetimes=False).result['observed_value']
            if (min_value is not None):
                try:
                    ...
                    pass
            profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_min_to_be_between', kwargs={'column': column}), match_type='domain')
            max_value = profile_dataset.expect_column_max_to_be_between(column, min_value=None, max_value=None, result_format='SUMMARY', parse_strings_as_datetimes=False).result['observed_value']
            ...
            if ((min_value is not None) or (max_value is not None)):
                profile_dataset.expect_column_values_to_be_between(column, min_value=min_value, max_value=max_value, parse_strings_as_datetimes=False)
        return profile_dataset
    ...
    def _build_expectations_for_all_column_types(self, profile_dataset, column) -> None:
        ...
        if ('expect_column_values_to_not_be_null' not in self.excluded_expectations):
            ...
            if (not not_null_result.success):
                ...
                if ((unexpected_percent >= 50) and (not self.not_null_only)):
                    ...
                    profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_values_to_not_be_null', kwargs={'column': column}), match_type='domain')
        ...
        if ('expect_column_proportion_of_unique_values_to_be_between' not in self.excluded_expectations):
            ...
            if (not is_nan(pct_unique)):
                profile_dataset.expect_column_proportion_of_unique_values_to_be_between(column, min_value=pct_unique, max_value=pct_unique)
            else:
                profile_dataset._expectation_suite.remove_expectation(ExpectationConfiguration(expectation_type='expect_column_proportion_of_unique_values_to_be_between', kwargs={'column': column}), match_type='domain')
                logger.debug(f'''Skipping expect_column_proportion_of_unique_values_to_be_between because observed value is nan: {pct_unique}
''')
        if ('expect_column_values_to_be_in_type_list' not in self.excluded_expectations):
            ...
            if (col_type != 'UNKNOWN'):
                ...
                logger.info(f'Column type for column {column} is unknown. Skipping expect_column_values_to_be_in_type_list for this column.')
    ...
    def _build_expectations_table(self, profile_dataset) -> None:
        ...
        if ('expect_table_row_count_to_be_between' not in self.excluded_expectations):
            row_count = profile_dataset.expect_table_row_count_to_be_between(min_value=0, max_value=None).result['observed_value']
