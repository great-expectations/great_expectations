...
class JsonSchemaProfiler(Profiler):
    ...
    PROFILER_TYPE_LIST_BY_JSON_SCHEMA_TYPE = {JsonSchemaTypes.STRING.value: ProfilerTypeMapping.STRING_TYPE_NAMES, JsonSchemaTypes.INTEGER.value: ProfilerTypeMapping.INT_TYPE_NAMES, JsonSchemaTypes.NUMBER.value: ProfilerTypeMapping.FLOAT_TYPE_NAMES, JsonSchemaTypes.BOOLEAN.value: ProfilerTypeMapping.BOOLEAN_TYPE_NAMES}
    ...
    def validate(self, schema: dict) -> bool:
        ...
        if (not isinstance(schema, dict)):
            raise TypeError(f'This profiler requires a schema of type dict and was passed a {type(schema)}')
        if ('type' not in schema.keys()):
            raise KeyError('This profiler requires a json schema with a top level `type` key')
        if (schema['type'] != JsonSchemaTypes.OBJECT.value):
            raise TypeError('This profiler requires a json schema with a top level `type` of `object`')
    ...
    def _profile(self, schema: Dict, suite_name: str=None) -> ExpectationSuite:
        ...
        if (schema['type'] == JsonSchemaTypes.OBJECT.value):
            for (key, details) in schema['properties'].items():
                ...
                string_len_expectation = self._create_string_length_expectation(key, details)
                if string_len_expectation:
                    expectations.append(string_len_expectation)
                null_or_not_null_expectation = self._create_null_or_not_null_column_expectation(key, details)
                if null_or_not_null_expectation:
                    expectations.append(null_or_not_null_expectation)
        description = schema.get('description', None)
        meta = None
        if description:
            meta = {'notes': {'format': 'markdown', 'content': [f'''### Description:
{description}''']}}
        suite = ExpectationSuite(suite_name, expectations=expectations, meta=meta, data_context=None)
        suite.add_citation(comment=f'This suite was built by the {self.__class__.__name__}')
        return suite
    ...
    def _get_enum_list(self, details: dict) -> Optional[List[str]]:
        ...
        if enum:
            ...
            return None
        enum_list = [(JsonSchemaTypes.NULL.value if (item is None) else item) for item in enum_list]
        return enum_list

    def _create_existence_expectation(self, key: str, details: dict) -> ExpectationConfiguration:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
...
{description}''']}}
    ...
    def _create_type_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        object_types = list(filter((lambda object_type: (object_type not in [JsonSchemaTypes.NULL.value])), object_types))
        ...
        return ExpectationConfiguration('expect_column_values_to_be_in_type_list', kwargs)

    def _create_boolean_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _create_range_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if type_:
            ...
            for item in any_of:
                ...
                if (item_type in range_types):
                    ...
                    break
        if ((minimum is None) and (maximum is None) and (exclusive_minimum is None) and (exclusive_maximum is None)):
            return None
    ...
    def _create_string_length_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (minimum == maximum):
            ...
            return ExpectationConfiguration('expect_column_value_lengths_to_equal', kwargs)
        if (minimum is not None):
            kwargs['min_value'] = minimum
        if (maximum is not None):
            kwargs['max_value'] = maximum
        return ExpectationConfiguration('expect_column_value_lengths_to_be_between', kwargs)

    def _create_set_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not enum_list):
            return None
        enum_list = list(filter((lambda item: (item is not JsonSchemaTypes.NULL.value)), enum_list))
        kwargs = {'column': key, 'value_set': enum_list}
        return ExpectationConfiguration('expect_column_values_to_be_in_set', kwargs)

    def _create_null_or_not_null_column_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (JsonSchemaTypes.NULL.value not in object_types):
            return ExpectationConfiguration('expect_column_values_to_not_be_null', kwargs)
        if (len(object_types) == 1):
            return ExpectationConfiguration('expect_column_values_to_be_null', kwargs)
        return None

    def _create_regex_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _create_string_format_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _create_array_expectation(self, key: str, details: dict) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
