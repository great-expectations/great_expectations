...
class ProfilerCardinality(Enum):
    ...
    VERY_MANY = 'very many'
    UNIQUE = 'unique'
profiler_data_types_with_mapping = {'INT': list(ProfilerTypeMapping.INT_TYPE_NAMES), 'FLOAT': list(ProfilerTypeMapping.FLOAT_TYPE_NAMES), 'NUMERIC': (list(ProfilerTypeMapping.INT_TYPE_NAMES) + list(ProfilerTypeMapping.FLOAT_TYPE_NAMES)), 'STRING': list(ProfilerTypeMapping.STRING_TYPE_NAMES), 'BOOLEAN': list(ProfilerTypeMapping.BOOLEAN_TYPE_NAMES), 'DATETIME': list(ProfilerTypeMapping.DATETIME_TYPE_NAMES), 'UNKNOWN': ['unknown']}
...
class Profiler(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def _profile(self, item_to_profile: Any, suite_name: str=None) -> ExpectationSuite:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
...
class DatasetProfiler(DataAssetProfiler):
    ...
    def add_meta(cls, expectation_suite, batch_kwargs=None):
        ...
        expectation_suite.meta[class_name] = {'created_by': class_name, 'created_at': time.time()}
        if (batch_kwargs is not None):
            expectation_suite.meta[class_name]['batch_kwargs'] = batch_kwargs
        new_expectations = [cls.add_expectation_meta(exp) for exp in expectation_suite.expectations]
        expectation_suite.expectations = new_expectations
        if ('notes' not in expectation_suite.meta):
            expectation_suite.meta['notes'] = {'format': 'markdown', 'content': ['_To add additional notes, edit the <code>meta.notes.content</code> field in the appropriate Expectation json file._']}
        return expectation_suite
    ...
    def profile(cls, data_asset, run_id=None, profiler_configuration=None, run_name=None, run_time=None):
        ...
        expectation_suite.add_citation(comment=f'{str(cls.__name__)} added a citation based on the current batch.', batch_kwargs=data_asset.batch_kwargs, batch_markers=data_asset.batch_markers, batch_parameters=data_asset.batch_parameters)
