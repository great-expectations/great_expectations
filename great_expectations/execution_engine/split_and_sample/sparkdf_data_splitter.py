...
class SparkDataSplitter(DataSplitter):
    ...
    def split_on_year(self, df: DataFrame, column_name: str, batch_identifiers: dict) -> DataFrame:
        import inspect
    ...
    def split_on_year_and_month(self, df: DataFrame, column_name: str, batch_identifiers: dict) -> DataFrame:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def split_on_year_and_month_and_day(self, df: DataFrame, column_name: str, batch_identifiers: dict) -> DataFrame:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def split_on_date_parts(self, df: DataFrame, column_name: str, batch_identifiers: dict, date_parts: Union[(List[DatePart], List[str])]) -> DataFrame:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for (date_part, date_part_value) in date_parts_dict.items():
            df = df.filter((getattr(F, self._convert_date_part_to_spark_equivalent(date_part))(F.col(column_name)) == date_part_value))
        return df
    ...
    def _convert_date_part_to_spark_equivalent(date_part: [DatePart, str]) -> str:
        ...
        spark_date_part_decoder: dict = {DatePart.YEAR: 'year', DatePart.MONTH: 'month', DatePart.WEEK: 'weekofyear', DatePart.DAY: 'dayofmonth', DatePart.HOUR: 'hour', DatePart.MINUTE: 'minute', DatePart.SECOND: 'second'}
    ...
    @staticmethod
    def split_on_column_value(df, column_name: str, batch_identifiers: dict) -> DataFrame:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @staticmethod
    def split_on_converted_datetime(df, column_name: str, batch_identifiers: dict, date_format_string: str='yyyy-MM-dd') -> DataFrame:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        res = df.withColumn('date_time_tmp', F.from_unixtime(F.col(column_name), date_format_string)).filter((F.col('date_time_tmp') == matching_string)).drop('date_time_tmp')
        return res

    @staticmethod
    def split_on_divided_integer(df, column_name: str, divisor: int, batch_identifiers: dict):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        res = df.withColumn('div_temp', (F.col(column_name) / divisor).cast(sparktypes.IntegerType())).filter((F.col('div_temp') == matching_divisor)).drop('div_temp')
        return res
    ...
    def split_on_mod_integer(df, column_name: str, mod: int, batch_identifiers: dict):
        ...
        res = df.withColumn('mod_temp', (F.col(column_name) % mod).cast(sparktypes.IntegerType())).filter((F.col('mod_temp') == matching_mod_value)).drop('mod_temp')
        return res
    ...
    def split_on_multi_column_values(df, column_names: list, batch_identifiers: dict):
        ...
        for column_name in column_names:
            ...
            if (not value):
                raise ValueError(f'In order for SparkDFExecutionEngine to `_split_on_multi_column_values`, all values in  column_names must also exist in batch_identifiers. {column_name} was not found in batch_identifiers.')
            df = df.filter((F.col(column_name) == value))
        return df

    @staticmethod
    def split_on_hashed_column(df, column_name: str, hash_digits: int, batch_identifiers: dict, hash_function_name: str='sha256'):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        res = df.withColumn('encrypted_value', encrypt_udf(column_name)).filter((F.col('encrypted_value') == batch_identifiers['hash_value'])).drop('encrypted_value')
        return res
