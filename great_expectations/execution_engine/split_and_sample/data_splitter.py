...
class DataSplitter(abc.ABC):
    ...
    def _convert_date_parts(self, date_parts: Union[(List[DatePart], List[str])]) -> List[DatePart]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _validate_date_parts(date_parts: Union[(List[DatePart], List[str])]) -> None:
        ...
        if (len(date_parts) == 0):
            raise ge_exceptions.InvalidConfigError('date_parts are required when using split_on_date_parts.')
        if (not all(((isinstance(dp, DatePart) or isinstance(dp, str)) for dp in date_parts))):
            raise ge_exceptions.InvalidConfigError('date_parts should be of type DatePart or str.')
    ...
    def _verify_all_strings_are_valid_date_parts(date_part_strings: List[str]) -> None:
        ...
        try:
            ...
            raise ge_exceptions.InvalidConfigError(f'{e} please only specify strings that are supported in DatePart: {[dp.value for dp in DatePart]}')

    def _convert_datetime_batch_identifiers_to_date_parts_dict(self, column_batch_identifiers: Union[(datetime.datetime, str, dict)], date_parts: List[DatePart]) -> dict:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if isinstance(column_batch_identifiers, str):
            column_batch_identifiers: datetime.datetime = parse(column_batch_identifiers)
