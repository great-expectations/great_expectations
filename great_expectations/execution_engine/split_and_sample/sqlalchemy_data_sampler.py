...
from great_expectations.execution_engine.split_and_sample.data_sampler import DataSampler
try:
...
class SqlAlchemyDataSampler(DataSampler):
    ...
    def sample_using_limit(self, execution_engine: 'SqlAlchemyExecutionEngine', batch_spec: BatchSpec, where_clause: Optional[Selectable]=None) -> Union[(str, BinaryExpression, BooleanClauseList)]:
        import inspect
        ...
        if (dialect == 'oracle'):
            raw_query: Selectable = sa.select('*').select_from(sa.table(table_name, schema=batch_spec.get('schema_name', None))).where(where_clause)
            query: str = str(raw_query.compile(execution_engine, compile_kwargs={'literal_binds': True}))
            query += ('\nAND ROWNUM <= %d' % batch_spec['sampling_kwargs']['n'])
            return query
        elif (dialect == 'mssql'):
            selectable_query: Selectable = sa.select('*').select_from(sa.table(table_name, schema=batch_spec.get('schema_name', None))).where(where_clause).limit(batch_spec['sampling_kwargs']['n'])
            string_of_query: str = str(selectable_query.compile(execution_engine.engine, compile_kwargs={'literal_binds': True}))
            ...
            return sa.select('*').select_from(sa.table(table_name, schema=batch_spec.get('schema_name', None))).where(where_clause).limit(batch_spec['sampling_kwargs']['n'])
    ...
    def _validate_mssql_limit_param(self, n: Union[(str, int)]) -> None:
        ...
        if (not isinstance(n, (str, int))):
            raise ge_exceptions.InvalidConfigError("Please specify your sampling kwargs 'n' parameter as a string or int.")
        if (isinstance(n, str) and (not n.isdigit())):
            raise ge_exceptions.InvalidConfigError("If specifying your sampling kwargs 'n' parameter as a string please ensure it is parseable as an integer.")

    def sample_using_random(self, execution_engine: 'SqlAlchemyExecutionEngine', batch_spec: BatchSpec, where_clause: Optional[Selectable]=None) -> Selectable:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        table_name: str = batch_spec['table_name']
        num_rows: int = execution_engine.engine.execute(sa.select([sa.func.count()]).select_from(sa.table(table_name, schema=batch_spec.get('schema_name', None))).where(where_clause)).scalar()
        p: float = (batch_spec['sampling_kwargs']['p'] or 1.0)
        sample_size: int = round((p * num_rows))
        return sa.select('*').select_from(sa.table(table_name, schema=batch_spec.get('schema_name', None))).where(where_clause).order_by(sa.func.random()).limit(sample_size)
    ...
    def sample_using_md5(self, column_name: str, hash_digits: int=1, hash_value: str='f') -> bool:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
