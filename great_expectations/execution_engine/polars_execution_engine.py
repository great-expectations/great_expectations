...
from great_expectations.execution_engine.split_and_sample.polars_data_sampler import (
    PolarsDataSampler,
)
...
--ge-feature-maturity-info--
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._data_splitter = PolarsDataSplitter()
        self._data_sampler = PolarsDataSampler()
    ...
    def load_batch_data(
        self, batch_id: str, batch_data: Union[PolarsBatchData, pl_DataFrame]
    ) -> None:
        if isinstance(batch_data, pl_DataFrame):
        ...
        if isinstance(batch_spec, RuntimeDataBatchSpec):
            ...
            if isinstance(batch_data, str):
                raise ge_exceptions.ExecutionEngineError(
                    f"""PolarsExecutionEngine has been passed a string type batch_data, "{batch_data}", which is
                    illegal. Please check your config."""
                )

            if isinstance(batch_spec.batch_data, pl_DataFrame):
            ...
            try:
                ...
                raise ge_exceptions.ExecutionEngineError(
                    f"""PolarsExecutionEngine encountered the following error while trying to read data from S3
                    Bucket: {error}"""
                )
            logger.debug(
            ...
            if self._azure is None:
                raise ge_exceptions.ExecutionEngineError(
                    """PolarsExecutionEngine has been passed a AzureBatchSpec,
            ...
            try:
                ...
                raise ge_exceptions.ExecutionEngineError(
                    f"""PolarsExecutionEngine encountered the following error while trying to read data from GCS
                     Bucket: {error}"""
                )
            ...
            raise ge_exceptions.BatchSpecError(
                ...
                f"not {batch_spec.__class__.__name__}"
            )
    ...
    @property
    def dataframe(self) -> pl_DataFrame:
        ...
        return cast(PolarsBatchData, self.batch_manager.active_batch_data).dataframe
    ...
    def guess_reader_method_from_path(path):
        ...
        elif path.endswith(".feather") or path.endswith(".ipc"):
            return {"reader_method": "read_ipc"}
        elif path.endswith(".avro"):
            return {"reader_method": "read_avro"}
    ...
    def _get_reader_fn(self, reader_method=None, path=None):
        ...
        try:
            reader_fn = getattr(pl, reader_method)
            if reader_options:
    ...
    def get_domain_records(  # noqa: C901
        ...
        if batch_id is None:
            ...
            if self.batch_manager.active_batch_data_id is not None:
                data = cast(
                    PolarsBatchData, self.batch_manager.active_batch_data
                ).dataframe
            else:
            ...
            if batch_id in self.batch_manager.batch_data_cache:
                data = cast(
                    PolarsBatchData, self.batch_manager.batch_data_cache[batch_id]
                ).dataframe
            else:
                raise ge_exceptions.ValidationError(
        ...
        if row_condition:
            ...
            if condition_parser not in ["python", "polars"]:
                raise ValueError(
                    "condition_parser is required when setting a row_condition,"
                    " and must be 'python' or 'polars'"
                )
            if "pl.col" not in row_condition:
                raise ValueError(
                    "row_condtion must be structured with polars columns, e.g. `pl.col('foo') < 3`"
                )
            else:
                # Querying row condition

                data = data.filter(eval(row_condition))
        ...
        if (
            ...
            if ignore_row_if == "both_values_are_missing":
                data = data.filter(  # This isn't right
                    ~pl.fold(
                        acc=True, f=lambda acc, s: acc & s.is_null(), exprs=pl.all()
                    )
                )
