...
class SqlAlchemyBatchData(BatchData):
    ...
    def __init__(self, execution_engine, record_set_name: str=None, schema_name: str=None, table_name: str=None, query: str=None, selectable=None, create_temp_table: bool=True, temp_table_schema_name: str=None, use_quoted_name: bool=False, source_schema_name: str=None, source_table_name: str=None) -> None:
        import inspect
        ...
        if (sum((bool(x) for x in [table_name, query, (selectable is not None)])) != 1):
            ...
            raise ValueError('schema_name can only be used with table_name. Use temp_table_schema_name to provide a target schema for creating a temporary table.')
        if table_name:
            if use_quoted_name:
                table_name = quoted_name(table_name, quote=True)
            if (engine.dialect.name.lower() == 'bigquery'):
                if (schema_name is not None):
                    logger.warning('schema_name should not be used when passing a table_name for biquery. Instead, include the schema name in the table_name string.')
                self._selectable = sa.Table(table_name, sa.MetaData(), schema=None)
            else:
                self._selectable = sa.Table(table_name, sa.MetaData(), schema=schema_name)
            ...
            if (selectable is not None):
                if ((engine.dialect.name.lower() in ['oracle', 'mssql']) and isinstance(selectable, str)):
                    query = selectable
                else:
                    query = selectable.compile(dialect=self.sql_engine_dialect, compile_kwargs={'literal_binds': True})
            self._create_temporary_table(temp_table_name=generated_table_name, query=query, temp_table_schema_name=temp_table_schema_name)
            self._selectable = sa.Table(generated_table_name, sa.MetaData(), schema=temp_table_schema_name)
        elif query:
    ...
    def _create_temporary_table(self, temp_table_name, query, temp_table_schema_name=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (self.sql_engine_dialect.name.lower() == 'bigquery'):
            ...
            logger.warning(f'GE has created permanent TABLE {temp_table_name} as part of processing SqlAlchemyBatchData, which usually creates a TEMP TABLE.')
            stmt = f'CREATE TABLE {temp_table_name} AS {query}'
        elif (self.sql_engine_dialect.name.lower() == 'oracle'):
            stmt_1 = 'CREATE PRIVATE TEMPORARY TABLE {temp_table_name} ON COMMIT PRESERVE DEFINITION AS {query}'.format(temp_table_name=temp_table_name, query=query)
            stmt_2 = 'CREATE GLOBAL TEMPORARY TABLE {temp_table_name} ON COMMIT PRESERVE ROWS AS {query}'.format(temp_table_name=temp_table_name, query=query)
        elif (self.sql_engine_dialect.name.lower() == 'teradatasql'):
            stmt = 'CREATE VOLATILE TABLE "{temp_table_name}" AS ({query}) WITH DATA NO PRIMARY INDEX ON COMMIT PRESERVE ROWS'.format(temp_table_name=temp_table_name, query=query)
        else:
            stmt = f'CREATE TEMPORARY TABLE "{temp_table_name}" AS {query}'
