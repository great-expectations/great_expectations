...
def build_continuous_partition_object(
    ...
    if isinstance(bins, np.ndarray):  # noqa: SIM108
        bins = bins.tolist()
    else:
        bins = list(bins)
...
def infer_distribution_parameters(  # noqa: C901, PLR0912
    ...
    if "mean" not in params.keys():  # noqa: SIM118
        params["mean"] = data.mean()

    if "std_dev" not in params.keys():  # noqa: SIM118
        params["std_dev"] = data.std()

    if distribution == "beta":
        # scipy cdf(x, a, b, loc=0, scale=1)
        if "alpha" not in params.keys():  # noqa: SIM118
        ...
        if "beta" not in params.keys():  # noqa: SIM118
    ...
    #       params['lambda'] = params['mean']
        ...
        if "min" not in params.keys():  # noqa: SIM118
            if "loc" in params.keys():  # noqa: SIM118
                params["min"] = params["loc"]
            else:
                params["min"] = min(data)
        if "max" not in params.keys():  # noqa: SIM118
            if "scale" in params.keys():  # noqa: SIM118
        ...
        if "df" not in params.keys():  # noqa: SIM118
