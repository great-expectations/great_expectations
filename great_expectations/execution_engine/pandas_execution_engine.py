...
from great_expectations.core.batch_spec import AzureBatchSpec, BatchSpec, GCSBatchSpec, PathBatchSpec, RuntimeDataBatchSpec, S3BatchSpec
...
from great_expectations.execution_engine.split_and_sample.pandas_data_splitter import PandasDataSplitter
...
try:
    ...
    logger.debug('Unable to load AWS connection object; install optional boto3 dependency for support')
try:
    from azure.storage.blob import BlobServiceClient
except ImportError:
    BlobServiceClient = None
    logger.debug('Unable to load Azure connection object; install optional azure dependency for support')
try:
...
class PandasExecutionEngine(ExecutionEngine):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._config.update({'discard_subset_failing_expectations': self.discard_subset_failing_expectations, 'boto3_options': boto3_options, 'azure_options': azure_options, 'gcs_options': gcs_options})
    ...
    def load_batch_data(self, batch_id: str, batch_data: Any) -> None:
        ...
        if isinstance(batch_data, pd.DataFrame):
            ...
            raise ge_exceptions.GreatExpectationsError('PandasExecutionEngine requires batch data that is either a DataFrame or a PandasBatchData object')
    ...
    def get_batch_data_and_markers(self, batch_spec: BatchSpec) -> Tuple[(Any, BatchMarkers)]:
        ...
        if isinstance(batch_spec, RuntimeDataBatchSpec):
            ...
            if isinstance(batch_data, str):
                raise ge_exceptions.ExecutionEngineError(f'''PandasExecutionEngine has been passed a string type batch_data, "{batch_data}", which is illegal.
Please check your config.''')
            ...
            if (self._s3 is None):
                raise ge_exceptions.ExecutionEngineError('PandasExecutionEngine has been passed a S3BatchSpec,\n                        but the ExecutionEngine does not have a boto3 client configured. Please check your config.')
            s3_engine = self._s3
            try:
                ...
                raise ge_exceptions.ExecutionEngineError(f'PandasExecutionEngine encountered the following error while trying to read data from S3 Bucket: {error}')
            ...
            if (self._azure is None):
                raise ge_exceptions.ExecutionEngineError('PandasExecutionEngine has been passed a AzureBatchSpec,\n                        but the ExecutionEngine does not have an Azure client configured. Please check your config.')
            ...
            if (self._gcs is None):
                raise ge_exceptions.ExecutionEngineError('PandasExecutionEngine has been passed a GCSBatchSpec,\n                        but the ExecutionEngine does not have an GCS client configured. Please check your config.')
            ...
            try:
                ...
                raise ge_exceptions.ExecutionEngineError(f'PandasExecutionEngine encountered the following error while trying to read data from GCS Bucket: {error}')
            ...
            raise ge_exceptions.BatchSpecError(f'batch_spec must be of type RuntimeDataBatchSpec, PathBatchSpec, S3BatchSpec, or AzureBatchSpec, not {batch_spec.__class__.__name__}')
    ...
    def guess_reader_method_from_path(path):
        ...
        if (path.endswith('.csv') or path.endswith('.tsv')):
            ...
            raise ge_exceptions.ExecutionEngineError(f'Unable to determine reader method from path: "{path}".')
    ...
    def _get_reader_fn(self, reader_method=None, path=None):
        ...
        if ((reader_method is None) and (path is None)):
            raise ge_exceptions.ExecutionEngineError('Unable to determine pandas reader function without reader_method or path.')
        reader_options = {}
        ...
        try:
            ...
            raise ge_exceptions.ExecutionEngineError(f'Unable to find reader_method "{reader_method}" in pandas.')
    ...
    def get_domain_records(self, domain_kwargs: dict) -> pd.DataFrame:
        ...
        if (batch_id is None):
            if (self.active_batch_data_id is not None):
                ...
                raise ge_exceptions.ValidationError('No batch is specified, but could not identify a loaded batch.')
        elif (batch_id in self.loaded_batch_data_dict):
            data = self.loaded_batch_data_dict[batch_id].dataframe
        else:
            raise ge_exceptions.ValidationError(f'Unable to find batch with batch_id {batch_id}')
        ...
        if (('column_A' in domain_kwargs) and ('column_B' in domain_kwargs) and ('ignore_row_if' in domain_kwargs)):
            ...
            if (ignore_row_if == 'both_values_are_missing'):
                data = data.dropna(axis=0, how='all', subset=[column_A_name, column_B_name])
            elif (ignore_row_if == 'either_value_is_missing'):
                data = data.dropna(axis=0, how='any', subset=[column_A_name, column_B_name])
            else:
                if (ignore_row_if not in ['neither', 'never']):
                    raise ValueError(f'Unrecognized value of ignore_row_if ("{ignore_row_if}").')
                if (ignore_row_if == 'never'):
                    warnings.warn(f'''The correct "no-action" value of the "ignore_row_if" directive for the column pair case is "neither" (the use of "{ignore_row_if}" is deprecated as of v0.13.29 and will be removed in v0.16).  Please use "neither" instead.
''', DeprecationWarning)
            return data
    ...
    def get_compute_domain(self, domain_kwargs: dict, domain_type: Union[(str, MetricDomainTypes)], accessor_keys: Optional[Iterable[str]]=None) -> Tuple[(pd.DataFrame, dict, dict)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _sample_using_hash(df, column_name: str, hash_digits: int=1, hash_value: str='f', hash_function_name: str='md5'):
        ...
        try:
            ...
            raise ge_exceptions.ExecutionEngineError(f'''The sampling method used with PandasExecutionEngine has a reference to an invalid hash_function_name.
                    Reference to {hash_function_name} cannot be found.''')
        matches = df[column_name].map((lambda x: (hash_func(str(x).encode()).hexdigest()[((- 1) * hash_digits):] == hash_value)))
        return df[matches]
