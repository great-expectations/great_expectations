...
class PandasDataPartitioner(DataPartitioner):
    ...
    @staticmethod
    def partition_on_divided_integer(df, column_name: str, divisor: int, batch_identifiers: dict) -> pd.DataFrame:
    ...
    @staticmethod
    def partition_on_mod_integer(df, column_name: str, mod: int, batch_identifiers: dict) -> pd.DataFrame:
        """Divide the values in the named column by `divisor`, and partition on that"""
    ...
    @staticmethod
    def partition_on_multi_column_values(df, column_names: List[str], batch_identifiers: dict) -> pd.DataFrame:
    ...
    def partition_on_hashed_column(
        ...
        matching_rows = df[column_name].map(
            lambda x: hash_method(str(x).encode()).hexdigest()[-1 * hash_digits :] == batch_identifiers["hash_value"]
        )
        return df[matching_rows]
