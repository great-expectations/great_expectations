...
class SparkDataPartitioner(DataPartitioner):
    ...
    def partition_on_date_parts(
        ...
        for date_part, date_part_value in date_parts_dict.items():
            df = df.filter(
                getattr(F, self._convert_date_part_to_spark_equivalent(date_part))(F.col(column_name))
                == date_part_value
            )
        return df
    ...
    @staticmethod
    def partition_on_column_value(df, column_name: str, batch_identifiers: dict) -> pyspark.DataFrame:
