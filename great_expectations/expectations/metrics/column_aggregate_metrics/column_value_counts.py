...
class ColumnValueCounts(ColumnAggregateMetricProvider):
    ...
    def _pandas(cls, execution_engine: PandasExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        collate = metric_value_kwargs.get('collate', cls.default_kwarg_values['collate'])
        ...
        (df, _, accessor_domain_kwargs) = execution_engine.get_compute_domain(metric_domain_kwargs, MetricDomainTypes.COLUMN)
    ...
    def _sqlalchemy(cls, execution_engine: SqlAlchemyExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        if (collate is not None):
            raise ValueError('collate parameter is not supported in PandasDataset')
        (selectable, _, accessor_domain_kwargs) = execution_engine.get_compute_domain(metric_domain_kwargs, MetricDomainTypes.COLUMN)
        column = accessor_domain_kwargs['column']
        if (sort not in ['value', 'count', 'none']):
            raise ValueError("sort must be either 'value', 'count', or 'none'")
        ...
        results = execution_engine.engine.execute(query.select_from(selectable)).fetchall()
    ...
    def _spark(cls, execution_engine: SparkDFExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        (df, _, accessor_domain_kwargs) = execution_engine.get_compute_domain(metric_domain_kwargs, MetricDomainTypes.COLUMN)
        column = accessor_domain_kwargs['column']
        value_counts = df.select(column).where(F.col(column).isNotNull()).groupBy(column).count()
        if (sort == 'value'):
