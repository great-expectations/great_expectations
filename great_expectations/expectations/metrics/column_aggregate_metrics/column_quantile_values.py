...
class ColumnQuantileValues(ColumnAggregateMetricProvider):
    ...
    def _sqlalchemy(  # noqa: PLR0911, PLR0913
        ...
        ) = execution_engine.get_compute_domain(metric_domain_kwargs, domain_type=MetricDomainTypes.COLUMN)
    ...
    def _spark(  # noqa: PLR0913
        ...
        if not isinstance(allow_relative_error, float) or allow_relative_error < 0.0 or allow_relative_error > 1.0:
            raise ValueError(
...
def _get_column_quantiles_mssql(
    ...
    selects: list[sqlalchemy.WithinGroup] = [
        sa.func.percentile_disc(quantile).within_group(column.asc()).over() for quantile in quantiles
    ]
...
def _get_column_quantiles_bigquery(
    ...
    selects: list[sqlalchemy.WithinGroup] = [sa.func.percentile_disc(column, quantile).over() for quantile in quantiles]
    quantiles_query: sqlalchemy.Select = sa.select(*selects).select_from(selectable)
...
def _get_column_quantiles_mysql(
    ...
    for idx, quantile in enumerate(quantiles):
        ...
        quantile_column: sqlalchemy.Label = (
            ...
            .over(
                order_by=sa.case(
                    (
                        percent_rank_query.columns.p <= sa.cast(quantile, sa.dialects.mysql.DECIMAL(18, 15)),
    ...
    quantiles_query: sqlalchemy.Select = sa.select(*selects).distinct().order_by(percent_rank_query.columns.p.desc())
...
def _get_column_quantiles_clickhouse(column: str, quantiles: Iterable, selectable, execution_engine) -> list:
...
def _get_column_quantiles_sqlite(
    ...
    quantile_queries: list[sqlalchemy.Select] = [
        sa.select(column).order_by(column.asc()).offset(offset).limit(1).select_from(selectable) for offset in offsets
    ]

    try:
        quantiles_results = [
            execution_engine.execute_query(quantile_query).fetchone() for quantile_query in quantile_queries
        ]
