...
class ColumnValuesBetweenCount(MetricProvider):
    ...
    def _pandas(  # noqa: C901, PLR0913
        ...
        if min_value is not None and max_value is not None:
            ...
            series = val < max_value if strict_max else val <= max_value

        elif min_value is not None and max_value is None:
            series = min_value < val if strict_min else min_value <= val
        else:
    ...
    def _sqlalchemy(  # noqa: C901, PLR0912, PLR0913
        ...
        if min_value is None:
            condition = column < max_value if strict_max else column <= max_value

        elif max_value is None:
            condition = column > min_value if strict_min else column >= min_value
