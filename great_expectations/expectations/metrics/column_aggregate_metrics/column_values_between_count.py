...
class ColumnValuesBetweenCount(MetricProvider):
    ...
    def _pandas(cls, execution_engine: PandasExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        (df, compute_domain_kwargs, accessor_domain_kwargs) = execution_engine.get_compute_domain(domain_kwargs=metric_domain_kwargs, domain_type=MetricDomainTypes.COLUMN)
    ...
    def _sqlalchemy(cls, execution_engine: SqlAlchemyExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        if ((min_value == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=True)) or (min_value == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=True))):
            min_value = get_sql_dialect_floating_point_infinity_value(schema=dialect_name, negative=True)
        if ((min_value == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=False)) or (min_value == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=False))):
            min_value = get_sql_dialect_floating_point_infinity_value(schema=dialect_name, negative=False)
        if ((max_value == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=True)) or (max_value == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=True))):
            max_value = get_sql_dialect_floating_point_infinity_value(schema=dialect_name, negative=True)
        if ((max_value == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=False)) or (max_value == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=False))):
            max_value = get_sql_dialect_floating_point_infinity_value(schema=dialect_name, negative=False)
        (selectable, compute_domain_kwargs, accessor_domain_kwargs) = execution_engine.get_compute_domain(domain_kwargs=metric_domain_kwargs, domain_type=MetricDomainTypes.COLUMN)
        ...
        if (min_value is None):
            ...
            condition = sa.and_((column >= min_value), (column <= max_value))
        return execution_engine.engine.execute(sa.select([sa.func.count()]).select_from(selectable).where(condition)).scalar()
