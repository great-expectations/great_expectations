...
def _pandas_map_condition_index(
    ...
    **kwargs,
    ...
    domain_records_df: pd.DataFrame = execution_engine.get_domain_records(domain_kwargs=domain_kwargs)
    ...
    unexpected_index_list: Union[List[int], List[Dict[str, Any]]] = compute_unexpected_pandas_indices(
...
def _pandas_map_condition_query(
    ...
    **kwargs,
    ...
    return_unexpected_index_query: Optional[bool] = result_format.get("return_unexpected_index_query")
...
def _sqlalchemy_map_condition_unexpected_count_aggregate_fn(
    ...
    **kwargs,
    ...
    """Returns unexpected count for MapExpectations"""
    unexpected_condition, compute_domain_kwargs, accessor_domain_kwargs = metrics["unexpected_condition"]
    ...
    **kwargs,
    ...
    try:
        ...
        unexpected_count: Union[float, int] = execution_engine.execute_query(
            sa.select(
                unexpected_count_query.c[f"{SummarizationMetricNameSuffixes.UNEXPECTED_COUNT.value}"],
            )
        ).scalar()
...
def _sqlalchemy_map_condition_query(
    ...
    **kwargs,
    ...
    unexpected_index_column_names: List[str] | None = result_format.get("unexpected_index_column_names")
    ...
    final_select_statement: sa.select = unexpected_condition_query_with_selected_columns.select_from(
...
def _sqlalchemy_map_condition_index(
    ...
    **kwargs,
    ...
    unexpected_index_column_names: List[str] = result_format.get("unexpected_index_column_names", [])
    ...
    domain_records_as_selectable: sa.sql.Selectable = execution_engine.get_domain_records(domain_kwargs=domain_kwargs)
    unexpected_condition_query_with_selected_columns: sa.select = sa.select(*column_selector).where(
        unexpected_condition
    )
...
def _spark_map_condition_index(
    ...
    **kwargs,
    ...
    domain_kwargs = dict(**compute_domain_kwargs, **accessor_domain_kwargs)
    df: pyspark.sql.dataframe.DataFrame = execution_engine.get_domain_records(domain_kwargs=domain_kwargs)
    result_format = metric_value_kwargs["result_format"]
...
def _spark_map_condition_query(
    ...
    **kwargs,
    ...
    unexpected_condition_filtered: str = unexpected_condition_as_string.replace("Column<'(", "").replace(")'>", "")
...
def _get_sqlalchemy_customized_unexpected_index_list(
    ...
    if exclude_unexpected_values and len(query_result) != 0 and len(unexpected_index_column_names) != 0:
        primary_key_dict_list: dict[str, List[Any]] = {idx_col: [] for idx_col in unexpected_index_column_names}
...
def _get_spark_customized_unexpected_index_list(
    ...
    if exclude_unexpected_values and not filtered.isEmpty():
        dict_list_to_add: dict[str, List[Any]] = {idx_col: [] for idx_col in unexpected_index_column_names}
