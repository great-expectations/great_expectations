...
class ColumnTypes(TableMetricProvider):
    ...
    def _sqlalchemy(cls, execution_engine: SqlAlchemyExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        if (batch_id is None):
            if (execution_engine.active_batch_data_id is not None):
                ...
                raise GreatExpectationsError('batch_id could not be determined from domain kwargs and no active_batch_data is loaded into the execution engine')
        batch_data = execution_engine.loaded_batch_data_dict.get(batch_id)
        if (batch_data is None):
            raise GreatExpectationsError('the requested batch is not available; please load the batch into the execution engine.')
    ...
    def _spark(cls, execution_engine: SparkDFExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        (df, _, _) = execution_engine.get_compute_domain(metric_domain_kwargs, domain_type=MetricDomainTypes.TABLE)
        return _get_spark_column_metadata(df.schema, include_nested=metric_value_kwargs['include_nested'])
...
def _get_sqlalchemy_column_metadata(engine, batch_data: SqlAlchemyBatchData):
    ...
    if isinstance(batch_data.selectable, TextClause):
        ...
        table_selectable: str = (batch_data.source_table_name or batch_data.selectable.name)
        schema_name = (batch_data.source_schema_name or batch_data.selectable.schema)
    return get_sqlalchemy_column_metadata(engine=engine, table_selectable=table_selectable, schema_name=schema_name)
    ...
    if isinstance(field, sparktypes.StructType):
        ...
        if (include_nested and isinstance(field.dataType, sparktypes.StructType)):
            for child in field.dataType.fields:
                cols += _get_spark_column_metadata(child, parent_name=(parent_name + field.name), include_nested=include_nested)
    else:
