    ...
    def _sqlalchemy(cls, execution_engine: SqlAlchemyExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        (selectable, _, _) = execution_engine.get_compute_domain(metric_domain_kwargs, domain_type=MetricDomainTypes.TABLE)
        df = None
        ...
        if (table_name is None):
            try:
                if metric_value_kwargs['fetch_all']:
                    ...
                    df = next(pd.read_sql_query(sql=selectable, con=execution_engine.engine, chunksize=metric_value_kwargs['n_rows']))
            ...
            try:
                if metric_value_kwargs['fetch_all']:
                    df = pd.read_sql_table(table_name=getattr(selectable, 'name', None), schema=getattr(selectable, 'schema', None), con=execution_engine.engine)
                else:
                    df = next(pd.read_sql_table(table_name=getattr(selectable, 'name', None), schema=getattr(selectable, 'schema', None), con=execution_engine.engine, chunksize=metric_value_kwargs['n_rows']))
            except (ValueError, NotImplementedError):
                df = None
            except StopIteration:
                validator = Validator(execution_engine=execution_engine)
        ...
        if (df is None):
            ...
            if metric_value_kwargs['fetch_all']:
                sql = stmt.compile(dialect=execution_engine.engine.dialect, compile_kwargs={'literal_binds': True})
            elif (execution_engine.engine.dialect.name.lower() == 'mssql'):
                sql = str(stmt.compile(dialect=execution_engine.engine.dialect, compile_kwargs={'literal_binds': True}))
