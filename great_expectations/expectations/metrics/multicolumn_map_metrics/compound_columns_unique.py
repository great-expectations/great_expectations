...
from great_expectations.expectations.metrics.map_metric_provider import MulticolumnMapMetricProvider, multicolumn_condition_partial, multicolumn_function_partial
...
class CompoundColumnsUnique(MulticolumnMapMetricProvider):
    ...
    def _sqlalchemy_function(self, column_list, **kwargs):
        ...
        group_count_query = sa.select(count_selector).group_by(*column_list).select_from(original_table_clause).alias('group_counts_subquery')
        conditions = sa.and_(*((group_count_query.c[name] == original_table_clause.c[name]) for name in column_names))
        compound_columns_count_query = sa.select([original_table_clause, group_count_query.c._num_rows.label('_num_rows')]).select_from(original_table_clause.join(right=group_count_query, onclause=conditions, isouter=False)).alias('records_with_grouped_column_counts_subquery')
    ...
    def _spark(cls, column_list, **kwargs):
        ...
        row_wise_cond = (F.count(F.lit(1)).over(Window.partitionBy(F.struct(*column_names))) <= 1)
        return row_wise_cond
    ...
    def _get_evaluation_dependencies(cls, metric: MetricConfiguration, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        if isinstance(execution_engine, SqlAlchemyExecutionEngine):
            if (metric.metric_name == 'compound_columns.unique.condition'):
                dependencies['compound_columns.count.map'] = MetricConfiguration(metric_name='compound_columns.count.map', metric_domain_kwargs=metric.metric_domain_kwargs, metric_value_kwargs=None, metric_dependencies=None)
        return dependencies
