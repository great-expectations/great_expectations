...
from great_expectations.execution_engine.sqlalchemy_execution_engine import SqlAlchemyExecutionEngine, sa
...
def column_aggregate_value(engine: Type[ExecutionEngine], metric_fn_type='value', domain_type='column', **kwargs):
    ...
    if issubclass(engine, PandasExecutionEngine):
        ...
        def wrapper(metric_fn: Callable):
            ...
            @metric_value(engine=PandasExecutionEngine, metric_fn_type=metric_fn_type, domain_type=domain_type)
            @wraps(metric_fn)
            def inner_func(cls, execution_engine: PandasExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
                import inspect
                __frame = inspect.currentframe()
                __file = __frame.f_code.co_filename
                __func = __frame.f_code.co_name
                ...
                (df, _, accessor_domain_kwargs) = execution_engine.get_compute_domain(domain_kwargs=metric_domain_kwargs, domain_type=domain_type)
                column_name = accessor_domain_kwargs['column']
                if (column_name not in metrics['table.columns']):
                    raise ge_exceptions.InvalidMetricAccessorDomainKwargsKeyError(message=f'Error: The column "{column_name}" in BatchData does not exist.')
                if filter_column_isnull:
                    df = df[df[column_name].notnull()]
                return metric_fn(cls, column=df[column_name], **metric_value_kwargs, _metrics=metrics)
            return inner_func
        return wrapper
    else:
...
def column_aggregate_partial(engine: Type[ExecutionEngine], **kwargs):
    ...
    if issubclass(engine, SqlAlchemyExecutionEngine):
        ...
        def wrapper(metric_fn: Callable):
            ...
            @metric_partial(engine=SqlAlchemyExecutionEngine, partial_fn_type=partial_fn_type, domain_type=domain_type)
            @wraps(metric_fn)
            def inner_func(cls, execution_engine: SqlAlchemyExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
                import inspect
                __frame = inspect.currentframe()
                __file = __frame.f_code.co_filename
                __func = __frame.f_code.co_name
                ...
                (selectable, compute_domain_kwargs, accessor_domain_kwargs) = execution_engine.get_compute_domain(compute_domain_kwargs, domain_type=domain_type)
                column_name: str = accessor_domain_kwargs['column']
                sqlalchemy_engine: sa.engine.Engine = execution_engine.engine
                if (column_name not in metrics['table.columns']):
                    raise ge_exceptions.InvalidMetricAccessorDomainKwargsKeyError(message=f'Error: The column "{column_name}" in BatchData does not exist.')
                dialect = sqlalchemy_engine.dialect
                metric_aggregate = metric_fn(cls, column=sa.column(column_name), **metric_value_kwargs, _dialect=dialect, _table=selectable, _column_name=column_name, _sqlalchemy_engine=sqlalchemy_engine, _metrics=metrics)
        ...
        def wrapper(metric_fn: Callable):
            ...
            @metric_partial(engine=SparkDFExecutionEngine, partial_fn_type=partial_fn_type, domain_type=domain_type)
            @wraps(metric_fn)
            def inner_func(cls, execution_engine: SparkDFExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
                import inspect
                __frame = inspect.currentframe()
                __file = __frame.f_code.co_filename
                __func = __frame.f_code.co_name
                ...
                if filter_column_isnull:
                    ...
                    compute_domain_kwargs = metric_domain_kwargs
                (data, compute_domain_kwargs, accessor_domain_kwargs) = execution_engine.get_compute_domain(domain_kwargs=compute_domain_kwargs, domain_type=domain_type)
                column_name = accessor_domain_kwargs['column']
                if (column_name not in metrics['table.columns']):
                    raise ge_exceptions.InvalidMetricAccessorDomainKwargsKeyError(message=f'Error: The column "{column_name}" in BatchData does not exist.')
                column = data[column_name]
                metric_aggregate = metric_fn(cls, column=column, **metric_value_kwargs, _table=data, _column_name=column_name, _metrics=metrics)
                return (metric_aggregate, compute_domain_kwargs, accessor_domain_kwargs)
            return inner_func
        return wrapper
    else:
...
class ColumnAggregateMetricProvider(TableMetricProvider):
    ...
    def _get_evaluation_dependencies(cls, metric: MetricConfiguration, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        dependencies['table.columns'] = MetricConfiguration(metric_name='table.columns', metric_domain_kwargs=table_domain_kwargs, metric_value_kwargs=None, metric_dependencies=None)
        dependencies['table.row_count'] = MetricConfiguration(metric_name='table.row_count', metric_domain_kwargs=table_domain_kwargs, metric_value_kwargs=None, metric_dependencies=None)
        return dependencies
