...
class ColumnValuesZScore(ColumnMapMetricProvider):
    ...
    def _get_evaluation_dependencies(cls, metric: MetricConfiguration, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        if (metric.metric_name == 'column_values.z_score.under_threshold.condition'):
            dependencies['column_values.z_score.map'] = MetricConfiguration(metric_name='column_values.z_score.map', metric_domain_kwargs=metric.metric_domain_kwargs)
        if (metric.metric_name == 'column_values.z_score.map'):
            dependencies['column.mean'] = MetricConfiguration(metric_name='column.mean', metric_domain_kwargs=metric.metric_domain_kwargs)
            dependencies['column.standard_deviation'] = MetricConfiguration(metric_name='column.standard_deviation', metric_domain_kwargs=metric.metric_domain_kwargs)
        return dependencies
