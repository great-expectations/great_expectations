...
class ColumnValuesIncreasing(ColumnMapMetricProvider):
    ...
    @metric_partial(engine=SparkDFExecutionEngine, partial_fn_type=MetricPartialFunctionTypes.WINDOW_CONDITION_FN, domain_type=MetricDomainTypes.COLUMN)
    def _spark(cls, execution_engine: SparkDFExecutionEngine, metric_domain_kwargs: Dict, metric_value_kwargs: Dict, metrics: Dict[(str, Any)], runtime_configuration: Dict):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        parse_strings_as_datetimes: bool = (metric_value_kwargs.get('parse_strings_as_datetimes') or False)
        if parse_strings_as_datetimes:
            warnings.warn('The parameter "parse_strings_as_datetimes" is deprecated as of v0.13.41 in v0.16. As part of the V3 API transition, we\'ve moved away from input transformation. For more information, please see: https://greatexpectations.io/blog/why_we_dont_do_transformations_for_expectations/\n', DeprecationWarning)
        ...
        if isinstance(column_metadata['type'], (sparktypes.LongType, sparktypes.DoubleType, sparktypes.IntegerType)):
            compute_domain_kwargs = execution_engine.add_column_row_condition(metric_domain_kwargs, filter_null=cls.filter_column_isnull, filter_nan=True)
        else:
            compute_domain_kwargs = metric_domain_kwargs
        (df, compute_domain_kwargs, accessor_domain_kwargs) = execution_engine.get_compute_domain(compute_domain_kwargs, domain_type=MetricDomainTypes.COLUMN)
        column = F.col(column_name)
        if isinstance(column_metadata['type'], (sparktypes.TimestampType, sparktypes.DateType)):
            diff = F.datediff(column, F.lag(column).over(Window.orderBy(F.lit('constant'))))
        else:
        ...
        if (metric_value_kwargs['strictly'] is True):
            return (F.when((diff <= 0), F.lit(True)).otherwise(F.lit(False)), compute_domain_kwargs, accessor_domain_kwargs)
        else:
            return (F.when((diff < 0), F.lit(True)).otherwise(F.lit(False)), compute_domain_kwargs, accessor_domain_kwargs)
