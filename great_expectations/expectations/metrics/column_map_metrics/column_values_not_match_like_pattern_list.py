...
class ColumnValuesNotMatchLikePatternList(ColumnMapMetricProvider):
    ...
    def _sqlalchemy(cls, column, like_pattern_list, _dialect, **kwargs):
        ...
        like_pattern_expression = get_dialect_like_pattern_expression(column, _dialect, like_pattern_list[0], positive=False)
        if (like_pattern_expression is None):
            logger.warning(f'Like patterns are not supported for dialect {str(_dialect.name)}')
            raise NotImplementedError
        return sa.and_(*(get_dialect_like_pattern_expression(column, _dialect, like_pattern, positive=False) for like_pattern in like_pattern_list))
