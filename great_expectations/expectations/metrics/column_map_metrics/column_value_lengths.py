...
from great_expectations.expectations.metrics.map_metric_provider import ColumnMapMetricProvider, column_condition_partial, column_function_partial
...
class ColumnValuesValueLengthEquals(ColumnMapMetricProvider):
    ...
    def _get_evaluation_dependencies(cls, metric: MetricConfiguration, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        if (metric.metric_name == 'column_values.value_length.equals.condition'):
            dependencies['column_values.value_length.map'] = MetricConfiguration(metric_name='column_values.value_length.map', metric_domain_kwargs=metric.metric_domain_kwargs)
        return dependencies
...
class ColumnValuesValueLength(ColumnMapMetricProvider):
    ...
    @column_condition_partial(engine=PandasExecutionEngine)
    def _pandas(cls, column, _metrics, min_value=None, max_value=None, strict_min=None, strict_max=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if ((min_value is not None) and (max_value is not None)):
            if (strict_min and strict_max):
                ...
                metric_series = pandas_series_between_inclusive(series=column_lengths, min_value=min_value, max_value=max_value)
    ...
    @column_condition_partial(engine=SqlAlchemyExecutionEngine)
    def _sqlalchemy(cls, column, _metrics, min_value=None, max_value=None, strict_min=None, strict_max=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @column_condition_partial(engine=SparkDFExecutionEngine)
    def _spark(cls, column, _metrics, min_value=None, max_value=None, strict_min=None, strict_max=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
