...
from great_expectations.expectations.registry import get_metric_provider, register_metric, register_renderer
from great_expectations.validator.metric_configuration import MetricConfiguration
logger = logging.getLogger(__name__)

def metric_value(engine: Type[ExecutionEngine], metric_fn_type: Union[(str, MetricFunctionTypes)]=MetricFunctionTypes.VALUE, **kwargs):
    import inspect
...
def metric_partial(engine: Type[ExecutionEngine], partial_fn_type: Union[(str, MetricPartialFunctionTypes)], domain_type: Union[(str, MetricDomainTypes)], **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
class MetricProvider(metaclass=MetaMetricProvider):
    ...
    def _register_metric_functions(cls) -> None:
        ...
        for attr_name in dir(cls):
            ...
            if ((not hasattr(attr_obj, 'metric_engine')) and (not hasattr(attr_obj, '_renderer_type'))):
                continue
                ...
                declared_metric_name = (metric_name + metric_definition_kwargs.get('metric_name_suffix', ''))
                metric_fn_type = getattr(metric_fn, 'metric_fn_type', MetricFunctionTypes.VALUE)
                if (metric_fn_type == MetricFunctionTypes.VALUE):
                    register_metric(metric_name=declared_metric_name, metric_domain_keys=metric_domain_keys, metric_value_keys=metric_value_keys, execution_engine=engine, metric_class=cls, metric_provider=metric_fn, metric_fn_type=metric_fn_type)
                else:
                    register_metric(metric_name=((declared_metric_name + '.') + metric_fn_type.metric_suffix), metric_domain_keys=metric_domain_keys, metric_value_keys=metric_value_keys, execution_engine=engine, metric_class=cls, metric_provider=metric_fn, metric_fn_type=metric_fn_type)
                    register_metric(metric_name=declared_metric_name, metric_domain_keys=metric_domain_keys, metric_value_keys=metric_value_keys, execution_engine=engine, metric_class=cls, metric_provider=None, metric_fn_type=metric_fn_type)
            elif hasattr(attr_obj, '_renderer_type'):
                register_renderer(object_name=metric_name, parent_class=cls, renderer_fn=attr_obj)

    @classmethod
    def get_evaluation_dependencies(cls, metric: MetricConfiguration, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        return (cls._get_evaluation_dependencies(metric=metric, configuration=configuration, execution_engine=execution_engine, runtime_configuration=runtime_configuration) or {})
    ...
    def _get_evaluation_dependencies(cls, metric: MetricConfiguration, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        for metric_fn_type in MetricPartialFunctionTypes:
            ...
            if has_aggregate_fn:
                dependencies['metric_partial_fn'] = MetricConfiguration((metric_name + metric_suffix), metric.metric_domain_kwargs, metric.metric_value_kwargs)
        return dependencies
