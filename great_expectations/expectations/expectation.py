...
from great_expectations.core.expectation_configuration import ExpectationConfiguration, parse_result_format
from great_expectations.core.expectation_diagnostics.expectation_diagnostics import ExpectationDiagnostics
from great_expectations.core.expectation_diagnostics.expectation_test_data_cases import ExpectationLegacyTestCaseAdapter, ExpectationTestCase, ExpectationTestDataCases, TestBackend, TestData
from great_expectations.core.expectation_diagnostics.supporting_types import AugmentedLibraryMetadata, ExpectationBackendTestResultCounts, ExpectationDescriptionDiagnostics, ExpectationDiagnosticMaturityMessages, ExpectationErrorDiagnostics, ExpectationExecutionEngineDiagnostics, ExpectationMetricDiagnostics, ExpectationRendererDiagnostics, ExpectationTestDiagnostics, RendererTestDiagnostics
from great_expectations.core.expectation_validation_result import ExpectationValidationResult
from great_expectations.core.util import convert_to_json_serializable, nested_update
from great_expectations.exceptions import GreatExpectationsError, InvalidExpectationConfigurationError, InvalidExpectationKwargsError
from great_expectations.execution_engine import ExecutionEngine, PandasExecutionEngine
from great_expectations.execution_engine.execution_engine import MetricDomainTypes
from great_expectations.expectations.registry import _registered_metrics, _registered_renderers, get_metric_kwargs, register_expectation, register_renderer
from great_expectations.expectations.util import render_evaluation_parameter_string
from great_expectations.render.renderer.renderer import renderer
from great_expectations.render.types import CollapseContent, RenderedAtomicContent, RenderedContentBlockContainer, RenderedGraphContent, RenderedStringTemplateContent, RenderedTableContent, ValueListContent, renderedAtomicValueSchema
from great_expectations.render.util import num_to_str
from great_expectations.self_check.util import evaluate_json_test_cfe, generate_expectation_tests
from great_expectations.util import camel_to_snake, is_parseable_date
from great_expectations.validator.metric_configuration import MetricConfiguration
from great_expectations.validator.validator import Validator
logger = logging.getLogger(__name__)
_TEST_DEFS_DIR = os.path.join(os.path.dirname(__file__), '..', '..', 'tests', 'test_definitions')
...
class MetaExpectation(ABCMeta):
    ...
    def __new__(cls, clsname, bases, attrs):
        ...
        newclass.default_kwarg_values = nested_update(default_kwarg_values, attrs.get('default_kwarg_values', {}))
        return newclass
...
class Expectation(metaclass=MetaExpectation):
    ...
    default_kwarg_values = {'include_config': True, 'catch_exceptions': False, 'result_format': 'BASIC'}
    args_keys = None

    def __init__(self, configuration: Optional[ExpectationConfiguration]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @abstractmethod
    def _validate(self, configuration: ExpectationConfiguration, metrics: dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None) -> Union[(ExpectationValidationResult, dict)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = {'expectation_type': configuration.expectation_type, 'kwargs': configuration.kwargs}
        params_with_json_schema = {'expectation_type': {'schema': {'type': 'string'}, 'value': configuration.expectation_type}, 'kwargs': {'schema': {'type': 'string'}, 'value': configuration.kwargs}}
        return (template_str, params_with_json_schema, styling)

    @classmethod
    @renderer(renderer_type='atomic.prescriptive.summary')
    @render_evaluation_parameter_string
    def _prescriptive_summary(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        (template_str, params_with_json_schema, styling) = cls._atomic_prescriptive_template(configuration, result, language, runtime_configuration, **kwargs)
        value_obj = renderedAtomicValueSchema.load({'template': template_str, 'params': params_with_json_schema, 'schema': {'type': 'com.superconductive.rendered.string'}})
        rendered = RenderedAtomicContent(name='atomic.prescriptive.summary', value=value_obj, value_type='StringValueType')
        return rendered
    ...
    def _diagnostic_meta_properties_renderer(cls, result=None, **kwargs):
        ...
        meta_properties_to_render = result.expectation_config.kwargs.get('meta_properties_to_render', None)
    ...
    def _diagnostic_unexpected_statement_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if result.exception_info['raised_exception']:
            ...
            exception_message = RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': exception_message_template_str, 'params': {'expectation_type': result.expectation_config.expectation_type, 'exception_message': result.exception_info['exception_message']}, 'tag': 'strong', 'styling': {'classes': ['text-danger'], 'params': {'exception_message': {'tag': 'code'}, 'expectation_type': {'classes': ['badge', 'badge-danger', 'mb-2']}}}}})
            exception_traceback_collapse = CollapseContent(**{'collapse_toggle_link': 'Show exception traceback...', 'collapse': [RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': result.exception_info['exception_traceback'], 'tag': 'code'}})]})
            return [exception_message, exception_traceback_collapse]
        if (success or (not result_dict.get('unexpected_count'))):
            return []
        else:
            unexpected_count = num_to_str(result_dict['unexpected_count'], use_locale=True, precision=20)
            ...
            return [RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': template_str, 'params': {'unexpected_count': unexpected_count, 'unexpected_percent': unexpected_percent, 'element_count': element_count}, 'tag': 'strong', 'styling': {'classes': ['text-danger']}}})]

    @classmethod
    @renderer(renderer_type='renderer.diagnostic.unexpected_table')
    def _diagnostic_unexpected_table_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (result_dict is None):
            return None
        if ((not result_dict.get('partial_unexpected_list')) and (not result_dict.get('partial_unexpected_counts'))):
            return None
        table_rows = []
        ...
        unexpected_table_content_block = RenderedTableContent(**{'content_block_type': 'table', 'table': table_rows, 'header_row': header_row, 'styling': {'body': {'classes': ['table-bordered', 'table-sm', 'mt-3']}}})
    ...
    def _get_observed_value_from_evr(self, result: ExpectationValidationResult) -> str:
        ...
        if (result_dict.get('observed_value') is not None):
            ...
            return (num_to_str(result_dict.get('unexpected_percent'), precision=5) + '% unexpected')
        else:
            return '--'

    @classmethod
    @renderer(renderer_type='atomic.diagnostic.observed_value')
    def _atomic_diagnostic_observed_value(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        value_obj = renderedAtomicValueSchema.load({'template': observed_value, 'params': {}, 'schema': {'type': 'com.superconductive.rendered.string'}})
        rendered = RenderedAtomicContent(name='atomic.diagnostic.observed_value', value=value_obj, value_type='StringValueType')
        return rendered

    @classmethod
    @renderer(renderer_type='renderer.diagnostic.observed_value')
    def _diagnostic_observed_value_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def metrics_validate(self, metrics: Dict, configuration: Optional[ExpectationConfiguration]=None, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None) -> ExpectationValidationResult:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        validation_dependencies: dict = self.get_validation_dependencies(configuration, execution_engine=execution_engine, runtime_configuration=runtime_configuration)
        ...
        expectation_validation_result: Union[(ExpectationValidationResult, dict)] = self._validate(configuration=configuration, metrics=provided_metrics, runtime_configuration=runtime_configuration, execution_engine=execution_engine)
        evr: ExpectationValidationResult = self._build_evr(raw_response=expectation_validation_result, configuration=configuration)
        return evr
    ...
    def get_validation_dependencies(self, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None) -> dict:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        runtime_configuration = self.get_runtime_kwargs(configuration=configuration, runtime_configuration=runtime_configuration)
    ...
    def get_domain_kwargs(self, configuration: Optional[ExpectationConfiguration]=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not configuration):
            configuration = self.configuration
        domain_kwargs = {key: configuration.kwargs.get(key, self.default_kwarg_values.get(key)) for key in self.domain_keys}
    ...
    def get_success_kwargs(self, configuration: Optional[ExpectationConfiguration]=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        success_kwargs = {key: configuration.kwargs.get(key, self.default_kwarg_values.get(key)) for key in self.success_keys}
        success_kwargs.update(domain_kwargs)
        return success_kwargs

    def get_runtime_kwargs(self, configuration: Optional[ExpectationConfiguration]=None, runtime_configuration: dict=None) -> dict:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        runtime_kwargs = {key: configuration.kwargs.get(key, self.default_kwarg_values.get(key)) for key in self.runtime_keys}
    ...
    def get_result_format(self, configuration: ExpectationConfiguration, runtime_configuration: dict=None) -> dict:
        ...
        result_format: dict
        if runtime_configuration:
            result_format = runtime_configuration.get('result_format', configuration_result_format)
        else:
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        if (configuration is None):
            configuration = self.configuration
        try:
            assert (configuration.expectation_type == self.expectation_type), f'expectation configuration type {configuration.expectation_type} does not match expectation type {self.expectation_type}'
        except AssertionError as e:
            raise InvalidExpectationConfigurationError(str(e))

    def validate(self, validator: Validator, configuration: Optional[ExpectationConfiguration]=None, evaluation_parameters=None, interactive_evaluation=True, data_context=None, runtime_configuration=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        configuration.process_evaluation_parameters(evaluation_parameters, interactive_evaluation, data_context)
        evr = validator.graph_validate(configurations=[configuration], runtime_configuration=runtime_configuration)[0]
        return evr
    ...
    def configuration(self):
        ...
        if (self._configuration is None):
            raise InvalidExpectationConfigurationError('cannot access configuration: expectation has not yet been configured')
    ...
    def build_configuration(cls, *args, **kwargs):
        ...
        if ('meta' in kwargs):
            ...
            meta = None
        return ExpectationConfiguration(expectation_type=cls.expectation_type, kwargs=convert_to_json_serializable(deepcopy(all_args)), meta=meta)

    def run_diagnostics(self, raise_exceptions_for_backends: bool=False) -> ExpectationDiagnostics:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        _expectation_config: ExpectationConfiguration = self._get_expectation_configuration_from_examples(examples)
        metric_diagnostics_list: List[ExpectationMetricDiagnostics] = self._get_metric_diagnostics_list(expectation_config=_expectation_config)
        introspected_execution_engines: ExpectationExecutionEngineDiagnostics = self._get_execution_engine_diagnostics(metric_diagnostics_list=metric_diagnostics_list, registered_metrics=_registered_metrics)
        test_results: List[ExpectationTestDiagnostics] = self._get_test_results(expectation_type=description_diagnostics.snake_name, test_data_cases=examples, execution_engine_diagnostics=introspected_execution_engines, raise_exceptions_for_backends=raise_exceptions_for_backends)
        backend_test_result_counts: List[ExpectationBackendTestResultCounts] = ExpectationDiagnostics._get_backends_from_test_results(test_results)
        renderers: List[ExpectationRendererDiagnostics] = self._get_renderer_diagnostics(expectation_type=description_diagnostics.snake_name, test_diagnostics=test_results, registered_renderers=_registered_renderers)
        maturity_checklist: ExpectationDiagnosticMaturityMessages = self._get_maturity_checklist(library_metadata=library_metadata, description=description_diagnostics, examples=examples, tests=test_results, backend_test_result_counts=backend_test_result_counts, execution_engines=introspected_execution_engines)
        ...
        errors = [test_result.error_diagnostics for test_result in test_results if test_result.error_diagnostics]
        return ExpectationDiagnostics(library_metadata=library_metadata, examples=examples, gallery_examples=gallery_examples, description=description_diagnostics, renderers=renderers, metrics=metric_diagnostics_list, execution_engines=introspected_execution_engines, tests=test_results, backend_test_result_counts=backend_test_result_counts, maturity_checklist=maturity_checklist, errors=errors)

    def print_diagnostic_checklist(self, diagnostics: Optional[ExpectationDiagnostics]=None, show_failed_tests: bool=False) -> str:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
...
''')
    ...
    def _get_examples_from_json(self):
        ...
        results = []
        found = glob.glob(os.path.join(_TEST_DEFS_DIR, '**', f'{self.expectation_type}.json'), recursive=True)
        if found:
    ...
    def _get_examples(self, return_only_gallery_examples: bool=True) -> List[ExpectationTestDataCases]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for example in all_examples:
            ...
            for test in example['tests']:
                if ((test.get('include_in_gallery') == True) or (return_only_gallery_examples == False)):
            ...
            if (len(included_test_cases) > 0):
                ...
                if ('test_backends' in copied_example):
                    copied_example['test_backends'] = [TestBackend(**tb) for tb in copied_example['test_backends']]
    ...
    def _get_description_diagnostics(self) -> ExpectationDescriptionDiagnostics:
        ...
        return ExpectationDescriptionDiagnostics(**{'camel_name': camel_name, 'snake_name': snake_name, 'short_description': short_description, 'docstring': docstring})

    def _get_expectation_configuration_from_examples(self, examples: List[ExpectationTestDataCases]) -> ExpectationConfiguration:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
                ...
                if tests:
                    for test in tests:
                        if test.output.get('success'):
                            return ExpectationConfiguration(expectation_type=self.expectation_type, kwargs=test.input)

    @staticmethod
    def _choose_example(examples: List[ExpectationTestDataCases]) -> Tuple[(TestData, ExpectationTestCase)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @staticmethod
    def _get_registered_renderers(expectation_type: str, registered_renderers: dict) -> List[str]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @classmethod
    def _get_test_results(cls, expectation_type: str, test_data_cases: List[ExpectationTestDataCases], execution_engine_diagnostics: ExpectationExecutionEngineDiagnostics, raise_exceptions_for_backends: bool=False) -> List[ExpectationTestDiagnostics]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        test_results = []
        exp_tests = generate_expectation_tests(expectation_type=expectation_type, test_data_cases=test_data_cases, execution_engine_diagnostics=execution_engine_diagnostics, raise_exceptions_for_backends=raise_exceptions_for_backends)
        for exp_test in exp_tests:
            (validation_result, error_message, stack_trace) = evaluate_json_test_cfe(validator=exp_test['validator_with_data'], expectation_type=exp_test['expectation_type'], test=exp_test['test'], raise_exception=False)
            if (error_message is None):
                test_passed = True
                error_diagnostics = None
            else:
                error_diagnostics = ExpectationErrorDiagnostics(error_msg=error_message, stack_trace=stack_trace, test_title=exp_test['test']['title'], test_backend=exp_test['backend'])
                test_passed = False
            if validation_result:
                validation_result.expectation_config.raw_kwargs = validation_result.expectation_config._raw_kwargs
            test_results.append(ExpectationTestDiagnostics(test_title=exp_test['test']['title'], backend=exp_test['backend'], test_passed=test_passed, include_in_gallery=exp_test['test']['include_in_gallery'], validation_result=validation_result, error_diagnostics=error_diagnostics))
        return test_results
    ...
    def _get_renderer_diagnostics(self, expectation_type: str, test_diagnostics: List[ExpectationTestDiagnostics], registered_renderers: List[str], standard_renderers: List[str]=['renderer.answer', 'renderer.diagnostic.unexpected_statement', 'renderer.diagnostic.observed_value', 'renderer.diagnostic.status_icon', 'renderer.diagnostic.unexpected_table', 'renderer.prescriptive', 'renderer.question']) -> List[ExpectationRendererDiagnostics]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        supported_renderers = self._get_registered_renderers(expectation_type=expectation_type, registered_renderers=registered_renderers)
        ...
        for renderer_name in set(standard_renderers).union(set(supported_renderers)):
            ...
            if (renderer_name in supported_renderers):
                ...
                for test_diagnostic in test_diagnostics:
                    ...
                    try:
                        rendered_result = renderer(configuration=test_diagnostic['validation_result']['expectation_config'], result=test_diagnostic['validation_result'])
                        rendered_result_str = self._get_rendered_result_as_string(rendered_result)
                    except Exception as e:
                        new_sample = RendererTestDiagnostics(test_title=test_title, renderered_str=None, rendered_successfully=False, error_message=str(e), stack_trace=traceback.format_exc())
                    else:
                        new_sample = RendererTestDiagnostics(test_title=test_title, renderered_str=rendered_result_str, rendered_successfully=True)
                    finally:
                        samples.append(new_sample)
            new_renderer_diagnostics = ExpectationRendererDiagnostics(name=renderer_name, is_supported=(renderer_name in supported_renderers), is_standard=(renderer_name in standard_renderers), samples=samples)
    ...
    @staticmethod
    def _get_execution_engine_diagnostics(metric_diagnostics_list: List[ExpectationMetricDiagnostics], registered_metrics: dict, execution_engine_names: List[str]=['PandasExecutionEngine', 'SqlAlchemyExecutionEngine', 'SparkDFExecutionEngine']) -> ExpectationExecutionEngineDiagnostics:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _get_metric_diagnostics_list(self, expectation_config: ExpectationConfiguration) -> List[ExpectationMetricDiagnostics]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not expectation_config):
            return []
        validation_dependencies = self.get_validation_dependencies(configuration=expectation_config)
        metric_diagnostics_list = []
        for metric in validation_dependencies['metrics'].keys():
            new_metric_diagnostics = ExpectationMetricDiagnostics(name=metric, has_question_renderer=False)
    ...
    def _get_augmented_library_metadata(self):
        ...
        augmented_library_metadata = {'maturity': 'CONCEPT_ONLY', 'tags': [], 'contributors': [], 'requirements': [], 'library_metadata_passed_checks': False, 'has_full_test_suite': False, 'manually_reviewed_code': False}
        required_keys = {'contributors', 'tags'}
        allowed_keys = {'contributors', 'has_full_test_suite', 'manually_reviewed_code', 'maturity', 'requirements', 'tags'}
        problems = []
    ...
    def _get_maturity_checklist(self, library_metadata: AugmentedLibraryMetadata, description: ExpectationDescriptionDiagnostics, examples: List[ExpectationTestDataCases], tests: List[ExpectationTestDiagnostics], backend_test_result_counts: List[ExpectationBackendTestResultCounts], execution_engines: ExpectationExecutionEngineDiagnostics) -> ExpectationDiagnosticMaturityMessages:
        ...
        experimental_checks.append(ExpectationDiagnostics._check_core_logic_for_at_least_one_execution_engine(backend_test_result_counts))
        experimental_checks.append(ExpectationDiagnostics._check_linting(self))
        beta_checks.append(ExpectationDiagnostics._check_input_validation(self, examples))
        beta_checks.append(ExpectationDiagnostics._check_renderer_methods(self))
        beta_checks.append(ExpectationDiagnostics._check_core_logic_for_all_applicable_execution_engines(backend_test_result_counts))
        ...
        return ExpectationDiagnosticMaturityMessages(experimental=experimental_checks, beta=beta_checks, production=production_checks)
...
class TableExpectation(Expectation, ABC):
    ...
    def get_validation_dependencies(self, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for metric_name in self.metric_dependencies:
            metric_kwargs = get_metric_kwargs(metric_name=metric_name, configuration=configuration, runtime_configuration=runtime_configuration)
            dependencies['metrics'][metric_name] = MetricConfiguration(metric_name=metric_name, metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        return dependencies

    @staticmethod
    def validate_metric_value_between_configuration(configuration: Optional[ExpectationConfiguration]):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        try:
            assert ((min_val is None) or is_parseable_date(min_val) or isinstance(min_val, (float, int, dict))), 'Provided min threshold must be a datetime (for datetime columns) or number'
            ...
            assert ((max_val is None) or is_parseable_date(max_val) or isinstance(max_val, (float, int, dict))), 'Provided max threshold must be a datetime (for datetime columns) or number'
            if isinstance(max_val, dict):
                assert ('$PARAMETER' in max_val), 'Evaluation Parameter dict for max_value kwarg must have "$PARAMETER" key'
        except AssertionError as e:
            raise InvalidExpectationConfigurationError(str(e))
    ...
    def _validate_metric_value_between(self, metric_name, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
...
class ColumnExpectation(TableExpectation, ABC):
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        try:
            assert ('column' in configuration.kwargs), "'column' parameter is required for column expectations"
        except AssertionError as e:
            raise InvalidExpectationConfigurationError(str(e))
...
class ColumnMapExpectation(TableExpectation, ABC):
    ...
    domain_type = MetricDomainTypes.COLUMN
    success_keys = ('mostly',)
    default_kwarg_values = {'row_condition': None, 'condition_parser': None, 'mostly': 1, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': True}
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        try:
            assert ('column' in configuration.kwargs), "'column' parameter is required for column map expectations"
    ...
    def get_validation_dependencies(self, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        dependencies = super().get_validation_dependencies(configuration, execution_engine, runtime_configuration)
        assert isinstance(self.map_metric, str), 'ColumnMapExpectation must override get_validation_dependencies or declare exactly one map_metric'
        assert (self.metric_dependencies == tuple()), 'ColumnMapExpectation must be configured using map_metric, and cannot have metric_dependencies declared.'
        metric_dependencies = dependencies['metrics']
        metric_kwargs = get_metric_kwargs(metric_name='column_values.nonnull.unexpected_count', configuration=configuration, runtime_configuration=runtime_configuration)
        metric_dependencies['column_values.nonnull.unexpected_count'] = MetricConfiguration('column_values.nonnull.unexpected_count', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        metric_kwargs = get_metric_kwargs(metric_name=f'{self.map_metric}.unexpected_count', configuration=configuration, runtime_configuration=runtime_configuration)
        metric_dependencies[f'{self.map_metric}.unexpected_count'] = MetricConfiguration(f'{self.map_metric}.unexpected_count', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        metric_kwargs = get_metric_kwargs(metric_name='table.row_count', configuration=configuration, runtime_configuration=runtime_configuration)
        metric_dependencies['table.row_count'] = MetricConfiguration(metric_name='table.row_count', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        result_format_str = dependencies['result_format'].get('result_format')
        include_unexpected_rows = dependencies['result_format'].get('include_unexpected_rows')
        if (result_format_str == 'BOOLEAN_ONLY'):
            return dependencies
        metric_kwargs = get_metric_kwargs(f'{self.map_metric}.unexpected_values', configuration=configuration, runtime_configuration=runtime_configuration)
        metric_dependencies[f'{self.map_metric}.unexpected_values'] = MetricConfiguration(metric_name=f'{self.map_metric}.unexpected_values', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        if include_unexpected_rows:
            metric_kwargs = get_metric_kwargs(f'{self.map_metric}.unexpected_rows', configuration=configuration, runtime_configuration=runtime_configuration)
            metric_dependencies[f'{self.map_metric}.unexpected_rows'] = MetricConfiguration(metric_name=f'{self.map_metric}.unexpected_rows', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        ...
        if isinstance(execution_engine, PandasExecutionEngine):
            metric_kwargs = get_metric_kwargs(f'{self.map_metric}.unexpected_index_list', configuration=configuration, runtime_configuration=runtime_configuration)
            metric_dependencies[f'{self.map_metric}.unexpected_index_list'] = MetricConfiguration(metric_name=f'{self.map_metric}.unexpected_index_list', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        return dependencies
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        mostly = self.get_success_kwargs().get('mostly', self.default_kwarg_values.get('mostly'))
        ...
        return _format_map_output(result_format=parse_result_format(result_format), success=success, element_count=total_count, nonnull_count=nonnull_count, unexpected_count=unexpected_count, unexpected_list=unexpected_values, unexpected_index_list=unexpected_index_list, unexpected_rows=unexpected_rows)
...
class ColumnPairMapExpectation(TableExpectation, ABC):
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        super().validate_configuration(configuration)
        try:
            assert ('column_A' in configuration.kwargs), "'column_A' parameter is required for column pair map expectations"
            assert ('column_B' in configuration.kwargs), "'column_B' parameter is required for column pair map expectations"
            if ('mostly' in configuration.kwargs):
                mostly = configuration.kwargs['mostly']
                assert isinstance(mostly, (int, float)), "'mostly' parameter must be an integer or float"
                assert (0 <= mostly <= 1), "'mostly' parameter must be between 0 and 1"
    ...
    def get_validation_dependencies(self, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        dependencies = super().get_validation_dependencies(configuration, execution_engine, runtime_configuration)
        assert isinstance(self.map_metric, str), 'ColumnPairMapExpectation must override get_validation_dependencies or declare exactly one map_metric'
        assert (self.metric_dependencies == tuple()), 'ColumnPairMapExpectation must be configured using map_metric, and cannot have metric_dependencies declared.'
        metric_dependencies = dependencies['metrics']
        metric_kwargs = get_metric_kwargs(metric_name=f'{self.map_metric}.unexpected_count', configuration=configuration, runtime_configuration=runtime_configuration)
        metric_dependencies[f'{self.map_metric}.unexpected_count'] = MetricConfiguration(f'{self.map_metric}.unexpected_count', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        metric_kwargs = get_metric_kwargs(metric_name='table.row_count', configuration=configuration, runtime_configuration=runtime_configuration)
        metric_dependencies['table.row_count'] = MetricConfiguration(metric_name='table.row_count', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        metric_kwargs = get_metric_kwargs(f'{self.map_metric}.filtered_row_count', configuration=configuration, runtime_configuration=runtime_configuration)
        metric_dependencies[f'{self.map_metric}.filtered_row_count'] = MetricConfiguration(metric_name=f'{self.map_metric}.filtered_row_count', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        result_format_str = dependencies['result_format'].get('result_format')
        include_unexpected_rows = dependencies['result_format'].get('include_unexpected_rows')
        if (result_format_str == 'BOOLEAN_ONLY'):
            return dependencies
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        if ((total_count is None) or (filtered_row_count is None) or (total_count == 0) or (filtered_row_count == 0)):
            success = True
        else:
            success_ratio = (float((filtered_row_count - unexpected_count)) / filtered_row_count)
            success = (success_ratio >= mostly)
        return _format_map_output(result_format=parse_result_format(result_format), success=success, element_count=total_count, nonnull_count=filtered_row_count, unexpected_count=unexpected_count, unexpected_list=unexpected_values, unexpected_index_list=unexpected_index_list)

class MulticolumnMapExpectation(TableExpectation, ABC):
    map_metric = None
    domain_keys = ('batch_id', 'table', 'column_list', 'row_condition', 'condition_parser', 'ignore_row_if')
    domain_type = MetricDomainTypes.MULTICOLUMN
    success_keys = tuple()
    default_kwarg_values = {'row_condition': None, 'condition_parser': None, 'ignore_row_if': 'all_value_are_missing', 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': True}
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        super().validate_configuration(configuration)
        try:
            assert ('column_list' in configuration.kwargs), "'column_list' parameter is required for multicolumn map expectations"
        except AssertionError as e:
            raise InvalidExpectationConfigurationError(str(e))
    ...
    def get_validation_dependencies(self, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        dependencies = super().get_validation_dependencies(configuration, execution_engine, runtime_configuration)
        assert isinstance(self.map_metric, str), 'MulticolumnMapExpectation must override get_validation_dependencies or declare exactly one map_metric'
        assert (self.metric_dependencies == tuple()), 'MulticolumnMapExpectation must be configured using map_metric, and cannot have metric_dependencies declared.'
        metric_dependencies = dependencies['metrics']
        metric_kwargs = get_metric_kwargs(metric_name=f'{self.map_metric}.unexpected_count', configuration=configuration, runtime_configuration=runtime_configuration)
        metric_dependencies[f'{self.map_metric}.unexpected_count'] = MetricConfiguration(f'{self.map_metric}.unexpected_count', metric_domain_kwargs=metric_kwargs['metric_domain_kwargs'], metric_value_kwargs=metric_kwargs['metric_value_kwargs'])
        metric_kwargs = get_metric_kwargs(metric_name='table.row_count', configuration=configuration, runtime_configuration=runtime_configuration)
...
def _format_map_output(result_format, success, element_count, nonnull_count, unexpected_count, unexpected_list, unexpected_index_list, unexpected_rows=None):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (element_count > 0):
        ...
        unexpected_percent_nonmissing = None
    return_obj['result'] = {'element_count': element_count, 'unexpected_count': unexpected_count, 'unexpected_percent': unexpected_percent_nonmissing, 'partial_unexpected_list': unexpected_list[:result_format['partial_unexpected_count']]}
    if (not skip_missing):
    ...
    if (0 < result_format.get('partial_unexpected_count')):
        try:
            partial_unexpected_counts = [{'value': key, 'count': value} for (key, value) in sorted(Counter(immutable_unexpected_list).most_common(result_format['partial_unexpected_count']), key=(lambda x: ((- x[1]), x[0])))]
        except TypeError:
            partial_unexpected_counts = [{'error': 'partial_exception_counts requires a hashable type'}]
        finally:
