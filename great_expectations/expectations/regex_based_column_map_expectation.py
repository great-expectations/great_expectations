...
from great_expectations.expectations.expectation import ColumnMapExpectation, ExpectationConfiguration
...
class RegexColumnMapMetricProvider(ColumnMapMetricProvider):
    ...
    def _sqlalchemy(cls, column, _dialect, **kwargs):
        ...
        if (regex_expression is None):
            logger.warning(('Regex is not supported for dialect %s' % str(_dialect.dialect.name)))
...
class RegexBasedColumnMapExpectation(ColumnMapExpectation, ABC):
    ...
    def register_metric(regex_camel_name: str, regex_: str):
        ...
        new_column_regex_metric_provider = type(f'(ColumnValuesMatch{regex_camel_name}Regex', (RegexColumnMapMetricProvider,), {'condition_metric_name': map_metric, 'regex': regex_})
        return map_metric
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'mostly': {'schema': {'type': 'number'}, 'value': params.get('mostly')}, 'mostly_pct': {'schema': {'type': 'number'}, 'value': params.get('mostly_pct')}, 'regex': {'schema': {'type': 'string'}, 'value': params.get('regex')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
