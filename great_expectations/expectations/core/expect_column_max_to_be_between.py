...
class ExpectColumnMaxToBeBetween(ColumnExpectation):
    ...
    def _descriptive_stats_table_max_row_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        return [{'content_block_type': 'string_template', 'string_template': {'template': 'Maximum', 'tooltip': {'content': 'expect_column_max_to_be_between'}}}, f"{result.result['observed_value']:.2f}"]
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return self._validate_metric_value_between(metric_name='column.max', configuration=configuration, metrics=metrics, runtime_configuration=runtime_configuration, execution_engine=execution_engine)
