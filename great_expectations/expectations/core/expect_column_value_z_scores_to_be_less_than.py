...
class ExpectColumnValueZScoresToBeLessThan(ColumnMapExpectation):
    ...
    default_kwarg_values = {'row_condition': None, 'condition_parser': None, 'threshold': None, 'double_sided': True, 'mostly': 1, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False}
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        try:
            ...
            assert isinstance(configuration.kwargs['threshold'], (float, int, dict)), 'Provided threshold must be a number'
            ...
            assert isinstance(configuration.kwargs['double_sided'], (bool, dict)), 'Double sided parameter must be a boolean value'
