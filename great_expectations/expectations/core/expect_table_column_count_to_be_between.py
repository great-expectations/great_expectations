...
from great_expectations.render.util import handle_strict_min_max, substitute_none_for_missing
...
class ExpectTableColumnCountToBeBetween(TableExpectation):
    ...
    default_kwarg_values = {'min_value': None, 'max_value': None, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False, 'meta': None}
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['min_value', 'max_value', 'strict_min', 'strict_max'])
        ...
        params_with_json_schema = {'min_value': {'schema': {'type': 'number'}, 'value': params.get('min_value')}, 'max_value': {'schema': {'type': 'number'}, 'value': params.get('max_value')}, 'strict_min': {'schema': {'type': 'boolean'}, 'value': params.get('strict_min')}, 'strict_max': {'schema': {'type': 'boolean'}, 'value': params.get('strict_max')}}
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return self._validate_metric_value_between(metric_name='table.column_count', configuration=configuration, metrics=metrics, runtime_configuration=runtime_configuration, execution_engine=execution_engine)
