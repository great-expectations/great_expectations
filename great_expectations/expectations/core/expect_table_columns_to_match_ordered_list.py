...
from great_expectations.expectations.expectation import InvalidExpectationConfigurationError, TableExpectation
...
class ExpectTableColumnsToMatchOrderedList(TableExpectation):
    ...
    default_kwarg_values = {'row_condition': None, 'condition_parser': None, 'column_list': None, 'result_format': 'BASIC', 'column': None, 'column_index': None, 'include_config': True, 'catch_exceptions': False, 'meta': None}
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        try:
            ...
            assert (isinstance(configuration.kwargs['column_list'], (list, set, dict)) or (configuration.kwargs['column_list'] is None)), 'column_list must be a list, set, or None'
            if isinstance(configuration.kwargs['column_list'], dict):
                assert ('$PARAMETER' in configuration.kwargs['column_list']), 'Evaluation Parameter dict for column_list kwarg must have "$PARAMETER" key.'
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params_with_json_schema = {'column_list': {'schema': {'type': 'array'}, 'value': params.get('column_list')}}
        params_with_json_schema = add_values_with_json_schema_from_list_in_params(params=params, params_with_json_schema=params_with_json_schema, param_key_with_list='column_list')
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        if ((expected_column_list is None) or (list(actual_column_list) == list(expected_column_list))):
            ...
            return {'success': False, 'result': {'observed_value': list(actual_column_list), 'details': {'mismatched': mismatched}}}
