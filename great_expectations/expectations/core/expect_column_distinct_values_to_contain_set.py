...
from great_expectations.expectations.expectation import ColumnExpectation, InvalidExpectationConfigurationError
...
class ExpectColumnDistinctValuesToContainSet(ColumnExpectation):
    library_metadata = {'maturity': 'production', 'tags': ['core expectation', 'column aggregate expectation'], 'contributors': ['@great_expectations'], 'requirements': [], 'has_full_test_suite': True, 'manually_reviewed_code': False}
    metric_dependencies = ('column.value_counts',)
    success_keys = ('value_set', 'parse_strings_as_datetimes')
    default_kwarg_values = {'value_set': None, 'parse_strings_as_datetimes': False, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False}
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['column', 'value_set', 'parse_strings_as_datetimes', 'row_condition', 'condition_parser'])
        params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'value_set': {'schema': {'type': 'array'}, 'value': params.get('value_set')}, 'parse_strings_as_datetimes': {'schema': {'type': 'boolean'}, 'value': params.get('parse_strings_as_datetimes')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
