...
class ExpectColumnValuesToBeDecreasing(ColumnMapExpectation):
    ...
    default_kwarg_values = {'row_condition': None, 'condition_parser': None, 'strictly': None, 'mostly': 1, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False, 'parse_strings_as_datetimes': False}
    args_keys = ('column',)
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['column', 'strictly', 'mostly', 'parse_strings_as_datetimes', 'row_condition', 'condition_parser'])
        params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'strictly': {'schema': {'type': 'boolean'}, 'value': params.get('strictly')}, 'mostly': {'schema': {'type': 'number'}, 'value': params.get('mostly')}, 'mostly_pct': {'schema': {'type': 'string'}, 'value': params.get('mostly_pct')}, 'parse_strings_as_datetimes': {'schema': {'type': 'boolean'}, 'value': params.get('parse_strings_as_datetimes')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
