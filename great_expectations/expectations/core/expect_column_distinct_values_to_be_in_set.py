...
class ExpectColumnDistinctValuesToBeInSet(ColumnExpectation):
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['column', 'value_set', 'row_condition', 'condition_parser'])
        params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'value_set': {'schema': {'type': 'array'}, 'value': params.get('value_set')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
    ...
    def _descriptive_value_counts_bar_chart_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        bars = alt.Chart(df).mark_bar(**mark_bar_args).encode(y='count:Q', x='value:O', tooltip=['value', 'count']).properties(height=400, width=chart_pixel_width, autosize='fit')
        chart = bars.to_json()
        new_block = RenderedGraphContent(**{'content_block_type': 'graph', 'header': RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': 'Value Counts', 'tooltip': {'content': 'expect_column_distinct_values_to_be_in_set'}, 'tag': 'h6'}}), 'graph': chart, 'styling': {'classes': [f'col-{str(chart_container_col_width)}', 'mt-1']}})
        return new_block
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        try:
            ...
            assert (isinstance(configuration.kwargs['value_set'], (list, set, dict)) or (configuration.kwargs['value_set'] is None)), 'value_set must be a list, set, or None'
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        return {'success': success, 'result': {'observed_value': sorted(list(observed_value_set)), 'details': {'value_counts': observed_value_counts}}}
