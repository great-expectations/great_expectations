...
class ExpectColumnValuesToBeOfType(ColumnMapExpectation):
    ...
    default_kwarg_values = {'type_': None, 'mostly': 1, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False}
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['column', 'type_', 'mostly', 'row_condition', 'condition_parser'])
        params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'type_': {'schema': {'type': 'string'}, 'value': params.get('type_')}, 'mostly': {'schema': {'type': 'number'}, 'value': params.get('mostly')}, 'mostly_pct': {'schema': {'type': 'string'}, 'value': params.get('mostly_pct')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
    ...
    def _validate_sqlalchemy(self, actual_column_type, expected_type, execution_engine):
        ...
        if (expected_type is None):
            ...
            try:
                if ((expected_type.lower() == 'geography') and (execution_engine.engine.dialect.name.lower() == 'bigquery') and (not BIGQUERY_GEO_SUPPORT)):
                    logger.warning((('BigQuery GEOGRAPHY type is not supported by default. ' + 'To install support, please run:') + "  $ pip install 'sqlalchemy-bigquery[geography]'"))
    ...
    def get_validation_dependencies(self, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        if isinstance(execution_engine, PandasExecutionEngine):
            ...
            if (actual_column_type and (actual_column_type.type.__name__ == 'object_') and (expected_type not in ['object', 'object_', 'O', None])):
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        if isinstance(execution_engine, PandasExecutionEngine):
            ...
            return self._validate_sqlalchemy(actual_column_type=actual_column_type, expected_type=expected_type, execution_engine=execution_engine)
...
def _get_dialect_type_module(execution_engine):
    ...
    if (execution_engine.dialect_module is None):
        ...
        return sa
    try:
        if isinstance(execution_engine.dialect_module, sqlalchemy_redshift.dialect.RedshiftDialect):
            return execution_engine.dialect_module.sa
    except (TypeError, AttributeError):
        pass
    try:
        if (isinstance(execution_engine.dialect_module, sqla_bigquery.BigQueryDialect) and (bigquery_types_tuple is not None)):
            return bigquery_types_tuple
    except (TypeError, AttributeError):
        pass
    try:
        if (issubclass(execution_engine.dialect_module, teradatasqlalchemy.dialect.TeradataDialect) and (teradatatypes is not None)):
