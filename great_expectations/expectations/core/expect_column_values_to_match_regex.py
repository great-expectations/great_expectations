...
class ExpectColumnValuesToMatchRegex(ColumnMapExpectation):
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        regex = (configuration.kwargs.get('regex') or self.default_kwarg_values.get('regex'))
        try:
            ...
            if isinstance(regex, dict):
                assert ('$PARAMETER' in regex), 'Evaluation Parameter dict for regex kwarg must have "$PARAMETER" key.'
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'mostly': {'schema': {'type': 'number'}, 'value': params.get('mostly')}, 'mostly_pct': {'schema': {'type': 'string'}, 'value': params.get('mostly_pct')}, 'regex': {'schema': {'type': 'string'}, 'value': params.get('regex')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
