...
from great_expectations.execution_engine.util import is_valid_categorical_partition_object, is_valid_partition_object
from great_expectations.expectations.expectation import ColumnExpectation
from great_expectations.expectations.util import render_evaluation_parameter_string
from great_expectations.render.renderer.renderer import renderer
from great_expectations.render.types import RenderedAtomicContent, RenderedContentBlockContainer, RenderedGraphContent, RenderedStringTemplateContent, renderedAtomicValueSchema
...
class ExpectColumnKlDivergenceToBeLessThan(ColumnExpectation):
    ...
    default_kwarg_values = {'partition_object': None, 'threshold': None, 'tail_weight_holdout': 0, 'internal_weight_holdout': 0, 'bucketize_data': True, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False}
    ...
    def get_validation_dependencies(self, configuration: Optional[ExpectationConfiguration]=None, execution_engine: Optional[ExecutionEngine]=None, runtime_configuration: Optional[dict]=None):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        all_dependencies = super().get_validation_dependencies(configuration, execution_engine, runtime_configuration)
        ...
        if (partition_object is None):
            if configuration.kwargs.get('bucketize_data', self.default_kwarg_values['bucketize_data']):
                is_categorical = False
                partition_metric_configuration = MetricConfiguration('column.partition', metric_domain_kwargs=domain_kwargs, metric_value_kwargs={'bins': 'auto', 'allow_relative_error': False})
                validator = Validator(execution_engine=execution_engine)
                graph = ValidationGraph()
                validator.build_metric_dependency_graph(graph=graph, execution_engine=execution_engine, metric_configuration=partition_metric_configuration, configuration=configuration)
                resolved_metrics: Dict[(Tuple[(str, str, str)], Any)] = {}
                validator.resolve_validation_graph(graph=graph, metrics=resolved_metrics)
                ...
                nonnull_configuration = MetricConfiguration('column_values.nonnull.count', metric_domain_kwargs=domain_kwargs, metric_value_kwargs=None)
                ...
                is_categorical = True
                counts_configuration = MetricConfiguration('column.value_counts', metric_domain_kwargs=domain_kwargs, metric_value_kwargs={'sort': 'value'})
        ...
        if ((is_categorical is True) or is_valid_categorical_partition_object(partition_object)):
            dependencies['column.value_counts'] = MetricConfiguration('column.value_counts', metric_domain_kwargs=domain_kwargs, metric_value_kwargs={'sort': 'value'})
            dependencies['column_values.nonnull.count'] = MetricConfiguration('column_values.nonnull.count', domain_kwargs)
        else:
            if (bins is None):
                if (not is_valid_partition_object(partition_object)):
                    raise ValueError('Invalid partition_object provided')
                bins = partition_object['bins']
            hist_metric_configuration = MetricConfiguration('column.histogram', metric_domain_kwargs=domain_kwargs, metric_value_kwargs={'bins': bins})
            nonnull_configuration = MetricConfiguration('column_values.nonnull.count', metric_domain_kwargs=domain_kwargs, metric_value_kwargs=None)
            dependencies['column.histogram'] = hist_metric_configuration
            dependencies['column_values.nonnull.count'] = nonnull_configuration
            below_partition = MetricConfiguration('column_values.between.count', metric_domain_kwargs=domain_kwargs, metric_value_kwargs={'max_value': bins[0], 'strict_max': True})
            above_partition = MetricConfiguration('column_values.between.count', metric_domain_kwargs=domain_kwargs, metric_value_kwargs={'min_value': bins[(- 1)], 'strict_min': True})
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        bucketize_data = configuration.kwargs.get('bucketize_data', self.default_kwarg_values['bucketize_data'])
        partition_object = configuration.kwargs.get('partition_object', self.default_kwarg_values['partition_object'])
        threshold = configuration.kwargs.get('threshold', self.default_kwarg_values['threshold'])
        tail_weight_holdout = configuration.kwargs.get('tail_weight_holdout', self.default_kwarg_values['tail_weight_holdout'])
        internal_weight_holdout = configuration.kwargs.get('internal_weight_holdout', self.default_kwarg_values['internal_weight_holdout'])
        if (partition_object is None):
            if bucketize_data:
                ...
                partition_object = {'values': list(metrics['column.value_counts'].index), 'weights': list((np.array(metrics['column.value_counts']) / metrics['column_values.nonnull.count']))}
        if (not is_valid_partition_object(partition_object)):
        ...
        if is_valid_categorical_partition_object(partition_object):
            ...
            if ((partition_object['bins'][0] == (- np.inf)) and (partition_object['bins'][(- 1)] == np.inf)):
                ...
                comb_observed_weights = np.concatenate((observed_weights, [(above_partition / nonnull_count)]))
                observed_tail_weights = np.concatenate(([observed_weights[0]], [(above_partition / nonnull_count)]))
                ...
                expected_weights = expected_weights[:(- 1)]
                comb_observed_weights = np.concatenate(([(below_partition / nonnull_count)], observed_weights))
                observed_tail_weights = np.concatenate(([(below_partition / nonnull_count)], [observed_weights[(- 1)]]))
                observed_weights = observed_weights[:(- 1)]
            else:
                ...
                comb_observed_weights = np.concatenate(([(below_partition / nonnull_count)], observed_weights, [(above_partition / nonnull_count)]))
    ...
    def _get_kl_divergence_chart(cls, partition_object, header=None):
        ...
        if (len(weights) > 60):
            ...
            if partition_object.get('bins'):
                ...
                bars = alt.Chart(df).mark_bar().encode(x='bin_min:O', x2='bin_max:O', y='fraction:Q', tooltip=['bin_min', 'bin_max', 'fraction']).properties(width=chart_pixel_width, height=400, autosize='fit')
                chart = bars.to_json()
            elif partition_object.get('values'):
                values = partition_object['values']
                df = pd.DataFrame({'values': values, 'fraction': weights})
                bars = alt.Chart(df).mark_bar().encode(x='values:N', y='fraction:Q', tooltip=['values', 'fraction']).properties(width=chart_pixel_width, height=400, autosize='fit')
                chart = bars.to_json()
            if header:
                expected_distribution = RenderedGraphContent(**{'content_block_type': 'graph', 'graph': chart, 'header': header, 'styling': {'classes': [f'col-{str(chart_container_col_width)}', 'mt-2', 'pl-1', 'pr-1'], 'parent': {'styles': {'list-style-type': 'none'}}}})
            else:
                expected_distribution = RenderedGraphContent(**{'content_block_type': 'graph', 'graph': chart, 'styling': {'classes': [f'col-{str(chart_container_col_width)}', 'mt-2', 'pl-1', 'pr-1'], 'parent': {'styles': {'list-style-type': 'none'}}}})
    ...
    def _get_kl_divergence_partition_object_table(cls, partition_object, header=None):
        ...
        if partition_object.get('bins'):
            ...
            for (idx, fraction) in enumerate(fractions):
                if (idx == (len(fractions) - 1)):
                    table_rows.append([f'[{num_to_str(bins[idx])} - {num_to_str(bins[(idx + 1)])}]', num_to_str(fraction)])
                else:
                    table_rows.append([f'[{num_to_str(bins[idx])} - {num_to_str(bins[(idx + 1)])})', num_to_str(fraction)])
        else:
            values = partition_object['values']
        ...
        if header:
            return {'content_block_type': 'table', 'header': header, 'header_row': (['Interval', 'Fraction'] if partition_object.get('bins') else ['Value', 'Fraction']), 'table': table_rows, 'styling': {'classes': ['table-responsive'], 'body': {'classes': ['table', 'table-sm', 'table-bordered', 'mt-2', 'mb-2']}, 'parent': {'classes': ['show-scrollbars', 'p-2'], 'styles': {'list-style-type': 'none', 'overflow': 'auto', 'max-height': '80vh'}}}}
        else:
            return {'content_block_type': 'table', 'header_row': (['Interval', 'Fraction'] if partition_object.get('bins') else ['Value', 'Fraction']), 'table': table_rows, 'styling': {'classes': ['table-responsive'], 'body': {'classes': ['table', 'table-sm', 'table-bordered', 'mt-2', 'mb-2']}, 'parent': {'classes': ['show-scrollbars', 'p-2'], 'styles': {'list-style-type': 'none', 'overflow': 'auto', 'max-height': '80vh'}}}}
    ...
    def _atomic_partition_object_table_template(cls, partition_object: dict):
        ...
        if partition_object.get('bins'):
            ...
            for (idx, fraction) in enumerate(fractions):
                ...
                table_rows.append([{'type': 'string', 'value': f'[{interval_start} - {interval_end}{interval_closing_symbol}'}, {'type': 'string', 'value': num_to_str(fraction)}])
        else:
            values = partition_object['values']
            table_rows = [[{'schema': {'type': 'string'}, 'value': str(value)}, {'schema': {'type': 'string'}, 'value': num_to_str(fractions[idx])}] for (idx, value) in enumerate(values)]
        header_row = [{'schema': {'type': 'string'}, 'value': ('Interval' if partition_object.get('bins') else 'Value')}, {'schema': {'type': 'string'}, 'value': 'Fraction'}]
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['column', 'partition_object', 'threshold', 'row_condition', 'condition_parser'])
        header_params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'mostly': {'schema': {'type': 'number'}, 'value': params.get('mostly')}, 'threshold': {'schema': {'type': 'number'}, 'value': params.get('threshold')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
        ...
        if (len(weights) > 60):
            (distribution_table_header_row, distribution_table_rows) = cls._atomic_partition_object_table_template(partition_object=expected_partition_object)
        else:
            (chart, chart_container_col_width) = cls._atomic_kl_divergence_chart_template(partition_object=expected_partition_object)
        if (params['row_condition'] is not None):
            ...
            header_template_str = f'{conditional_template_str}, then {header_template_str}'
            header_params_with_json_schema.update(conditional_params)
        return (header_template_str, header_params_with_json_schema, chart, chart_container_col_width, distribution_table_header_row, distribution_table_rows, styling)
    ...
    def _prescriptive_summary(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        (header_template_str, header_params_with_json_schema, chart, _, distribution_table_header_row, distribution_table_rows, _) = cls._atomic_prescriptive_template(configuration, result, language, runtime_configuration, **kwargs)
        if (chart is not None):
            value_obj = renderedAtomicValueSchema.load({'header': {'schema': {'type': 'StringValueType'}, 'value': {'template': header_template_str, 'params': header_params_with_json_schema}}, 'graph': chart, 'schema': {'type': 'GraphType'}})
            value_type = 'GraphType'
        else:
            value_obj = renderedAtomicValueSchema.load({'header': {'schema': {'type': 'StringValueType'}, 'value': {'template': header_template_str, 'params': header_params_with_json_schema}}, 'header_row': distribution_table_header_row, 'table': distribution_table_rows, 'schema': {'type': 'TableType'}})
            value_type = 'TableType'
        return RenderedAtomicContent(name='atomic.prescriptive.summary', value=value_obj, value_type=value_type)
    ...
    def _prescriptive_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        expectation_string_obj = {'content_block_type': 'string_template', 'string_template': {'template': template_str, 'params': params}}
    ...
    @classmethod
    def _atomic_diagnostic_observed_value_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        observed_value = (num_to_str(result.result.get('observed_value')) if result.result.get('observed_value') else result.result.get('observed_value'))
        header_template_str = 'KL Divergence: $observed_value'
        header_params_with_json_schema = {'observed_value': {'schema': {'type': 'string'}, 'value': (str(observed_value) if observed_value else 'None (-infinity, infinity, or NaN)')}}
        chart = None
        chart_container_col_width = None
        distribution_table_header_row = None
        distribution_table_rows = None
        if (len(weights) > 60):
            (distribution_table_header_row, distribution_table_rows) = cls._atomic_partition_object_table_template(partition_object=observed_partition_object)
        else:
            (chart, chart_container_col_width) = cls._atomic_kl_divergence_chart_template(partition_object=observed_partition_object)
        return (header_template_str, header_params_with_json_schema, chart, chart_container_col_width, distribution_table_header_row, distribution_table_rows)
    ...
    def _atomic_diagnostic_observed_value(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        if (not result.result.get('details')):
            value_obj = renderedAtomicValueSchema.load({'template': '--', 'params': {}, 'schema': {'type': 'StringValueType'}})
            return RenderedAtomicContent(name='atomic.diagnostic.observed_value', value=value_obj, value_type='StringValueType')
        (header_template_str, header_params_with_json_schema, chart, chart_container_col_width, distribution_table_header_row, distribution_table_rows) = cls._atomic_diagnostic_observed_value_template(configuration, result, language, runtime_configuration, **kwargs)
        if (chart is not None):
            value_obj = renderedAtomicValueSchema.load({'header': {'schema': {'type': 'StringValueType'}, 'value': {'template': header_template_str, 'params': header_params_with_json_schema}}, 'graph': chart, 'schema': {'type': 'GraphType'}})
            value_type = 'GraphType'
        else:
            value_obj = renderedAtomicValueSchema.load({'header': {'schema': {'type': 'StringValueType'}, 'value': {'template': header_template_str, 'params': header_params_with_json_schema}}, 'header_row': distribution_table_header_row, 'table': distribution_table_rows, 'schema': {'type': 'TableType'}})
            value_type = 'TableType'
        return RenderedAtomicContent(name='atomic.diagnostic.observed_value', value=value_obj, value_type=value_type)
    ...
    def _diagnostic_observed_value_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        observed_value = (num_to_str(result.result.get('observed_value')) if result.result.get('observed_value') else result.result.get('observed_value'))
        observed_value_content_block = RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': 'KL Divergence: $observed_value', 'params': {'observed_value': (str(observed_value) if observed_value else 'None (-infinity, infinity, or NaN)')}, 'styling': {'classes': ['mb-2']}}})
        return RenderedContentBlockContainer(**{'content_block_type': 'content_block_container', 'content_blocks': [observed_value_content_block, observed_distribution]})

    @classmethod
    @renderer(renderer_type='renderer.descriptive.histogram')
    def _descriptive_histogram_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (len(weights) > 60):
            return None
        header = RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': 'Histogram', 'tooltip': {'content': 'expect_column_kl_divergence_to_be_less_than'}, 'tag': 'h6'}})
