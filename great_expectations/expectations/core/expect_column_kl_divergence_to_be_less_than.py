...
from great_expectations.execution_engine.util import (
    is_valid_categorical_distribution_object,
    is_valid_distribution_object,
)
...
class ExpectColumnKLDivergenceToBeLessThan(ColumnAggregateExpectation):
    """Expect the Kulback-Leibler (KL) divergence (relative entropy) of the specified column with respect to the distribution object to be lower than the provided threshold.
    ...
    distributed identically, when binned according to the provided distribution.

    In many practical contexts, choosing a value between 0.5 and 1 will provide a useful test.

    This expectation works on both categorical and continuous distributions. See notes below for details.
    ...
    Args:
        column (str): \
            The column name.
        distribution_object (dict or None): \
            The expected distribution object (see [distribution_object](https://docs.greatexpectations.io/docs/reference/expectations/distributional_expectations/#partition-objects)).
        ...
        internal_weight_holdout (float between 0 and 1 or None): \
            The amount of weight to split uniformly among zero-weighted distribution bins. internal_weight_holdout \
            provides a mechanisms to make the test less strict by assigning positive weights to values \
            observed in \
            the data for which the distribution explicitly expected zero weight. With no internal_weight_holdout, \
        ...
        tail_weight_holdout (float between 0 and 1 or None): \
            ...
            between (-Infinity, min(distribution_object['bins'])) and (max(distribution_object['bins']), +Infinity). \
            tail_weight_holdout provides a mechanism to make the test less strict by assigning positive \
            weights to \
            values observed in the data that are not present in the distribution. With no tail_weight_holdout, \
            any value observed outside the provided distribution_object will cause KL divergence to rise to +Infinity. \
            Defaults to 0.
        bucketize_data (boolean): If True, then continuous data will be bucketized before evaluation. Setting \
            this parameter to false allows evaluation of KL divergence with a None distribution object for \
    ...
    Notes:
        ...
        * details.observed_distribution in the result object is customized for this expectation to be a \
          dict representing the distribution observed in the data
        * details.expected_distribution in the result object is customized for this expectation to be a \
          dict representing the distribution against which the data were compared, after applying \
          specified weight holdouts

        If the distribution_object is categorical, this expectation will expect the values in column to also be \
        categorical.

        * If the column includes values that are not present in the distribution, the tail_weight_holdout \
        ...
        * If the distribution includes values that are not present in the column, the test will simply include \
          zero weight for that value.

        If the distribution_object is continuous, this expectation will discretize the values in the column according \
        to the bins specified in the distribution_object, and apply the test to the resulting distribution.
        ...
        * If internal_weight_holdout is specified, that value will be distributed equally among any intervals \
          with weight zero in the distribution_object.
    ...
    Code Examples:
        Passing Case:
            Input:
                ExpectColumnKLDivergenceToBeLessThan(
                    column="test",
                    distribution_object={"weights": [0.5, 0.3, 0.2], "values": ["A", "B", "C"]},
                    threshold=0.1
            )
        ...
        Failing Case:
            Input:
                ExpectColumnKLDivergenceToBeLessThan(
                    column="test",
                    distribution_object={"weights": [0.3333333333333333, 0.3333333333333333, 0.3333333333333333], "values": ["A", "B", "C"]},
                    threshold=0.01
            )
    ...
    distribution_object: Union[dict, None]
    ...
    def get_validation_dependencies(
        ...
        distribution_object = configuration.kwargs.get("distribution_object")
        ...
        if is_categorical is True or is_valid_categorical_distribution_object(distribution_object):
            validation_dependencies.set_metric_configuration(
                metric_name="column.value_counts",
            ...
            if (
                bins is None
            ):  # if the user did not supply a distribution_object, so we just computed it
                if not is_valid_distribution_object(distribution_object):
                    raise ValueError("Invalid distribution_object provided")  # noqa: TRY003
            ...
            below_distribution = MetricConfiguration(
                metric_name="column_values.between.count",
                metric_domain_kwargs=domain_kwargs,
                metric_value_kwargs={"max_value": bins[0], "strict_max": True},
            )
            validation_dependencies.set_metric_configuration(
                metric_name="below_distribution", metric_configuration=below_distribution
            )
            above_distribution = MetricConfiguration(
                metric_name="column_values.between.count",
                metric_domain_kwargs=domain_kwargs,
                metric_value_kwargs={"min_value": bins[-1], "strict_min": True},
            )
            validation_dependencies.set_metric_configuration(
                metric_name="above_distribution", metric_configuration=above_distribution
            )
    ...
    def _validate(  # noqa: C901, PLR0912, PLR0915
        ...
        distribution_object = configuration.kwargs.get(
            "distribution_object", self._get_default_value("distribution_object")
        )
        ...
        if distribution_object is None:
            if bucketize_data:
                # in this case, we have requested a distribution, histogram using said distribution, and nonnull count  # noqa: E501
        ...
        if not is_valid_distribution_object(distribution_object):
            raise ValueError("Invalid distribution object.")  # noqa: TRY003
        ...
        if tail_weight_holdout != 0 and "tail_weights" in distribution_object:
            raise ValueError(  # noqa: TRY003
                "tail_weight_holdout must be 0 when using tail_weights in distribution object"
            )

        # TODO: add checks for duplicate values in is_valid_categorical_distribution_object
        if is_valid_categorical_distribution_object(distribution_object):
            ...
            expected_weights = pd.Series(
                distribution_object["weights"],
                index=distribution_object["values"],
                name="expected",
            )
            ...
            if bucketize_data is False:
                raise ValueError(  # noqa: TRY003
                    "KL Divergence cannot be computed with a continuous distribution object and the bucketize_data "  # noqa: E501
            ...
            # Add in the frequencies observed above or below the provided distribution
            below_distribution = metrics["below_distribution"]
            above_distribution = metrics["above_distribution"]
            ...
            if "tail_weights" in distribution_object:
                distribution_tail_weight_holdout = np.sum(distribution_object["tail_weights"])
            else:
                distribution_tail_weight_holdout = 0  # noqa: F841 # unused

            expected_weights = np.array(distribution_object["weights"]) * (
            ...
            # We need to check cases to only add tail weight holdout if it makes sense based on the provided distribution.  # noqa: E501
            if (distribution_object["bins"][0] == -np.inf) and (
                distribution_object["bins"][-1]
            ) == np.inf:
                if tail_weight_holdout > 0:
                    raise ValueError(  # noqa: TRY003
                        "tail_weight_holdout cannot be used for "
                        "distribution with infinite endpoints."
                    )
                if "tail_weights" in distribution_object:
                    raise ValueError(  # noqa: TRY003
                        "There can be no tail weights for distributions with one or both endpoints at infinity"  # noqa: E501
                    )

                # Remove -inf and inf
                expected_bins = distribution_object["bins"][1:-1]
            ...
            elif distribution_object["bins"][0] == -np.inf:
                if "tail_weights" in distribution_object:
                    raise ValueError(  # noqa: TRY003
                        "There can be no tail weights for distributions with one or both endpoints at infinity"  # noqa: E501
                    )
                ...
                # Remove -inf
                expected_bins = distribution_object["bins"][1:]
                ...
                comb_observed_weights = np.concatenate(
                    (
                        observed_weights,
                        [above_distribution / nonnull_count],
                    )
                )
                # Set aside left tail weight and above distribution weight
                observed_tail_weights = np.concatenate(
                    (
                        [observed_weights[0]],
                        [above_distribution / nonnull_count],
            ...
            elif distribution_object["bins"][-1] == np.inf:
                if "tail_weights" in distribution_object:
                    raise ValueError(  # noqa: TRY003
                        "There can be no tail weights for distributions with one or both endpoints at infinity"  # noqa: E501
                    )
                ...
                comb_observed_weights = np.concatenate(
                    (
                        [below_distribution / nonnull_count],
                        observed_weights,
                    )
                )
                # Set aside right tail weight and below distribution weight
                observed_tail_weights = np.concatenate(
                    (
                        [below_distribution / nonnull_count],
                        [observed_weights[-1]],
                ...
                expected_bins = distribution_object["bins"]

                if "tail_weights" in distribution_object:
                    tail_weights = distribution_object["tail_weights"]
                ...
                # Tail weights are just the counts on either side of the distribution
                observed_tail_weights = (
                    np.concatenate(([below_distribution], [above_distribution])) / nonnull_count
                )
    ...
    def _get_kl_divergence_chart(  # noqa: PLR0912,C901 - 13
        cls, distribution_object, header=None
    ):
        weights = distribution_object["weights"]

        if len(weights) > 60:  # noqa: PLR2004
            expected_distribution = cls._get_kl_divergence_distribution_object_table(
                distribution_object, header=header
            )
        else:
            ...
            if distribution_object.get("bins"):
                bins = distribution_object["bins"]
                ...
                chart = bars.to_dict()
            elif distribution_object.get("values"):
                is_boolean_list = all(
                    isinstance(value, bool) for value in distribution_object["values"]
                )
                if is_boolean_list:
                    values = [str(value) for value in distribution_object["values"]]
                else:
                    values = distribution_object["values"]
    ...
    @classmethod
    def _atomic_kl_divergence_chart_template(cls, distribution_object: dict) -> tuple:  # noqa: C901 - too complex
        weights = distribution_object.get("weights", [])
    ...
    @classmethod
    def _get_kl_divergence_distribution_object_table(cls, distribution_object, header=None):
        table_rows = []
        fractions = distribution_object["weights"]
    ...
    @classmethod
    def _atomic_distribution_object_table_template(cls, distribution_object: dict):
        table_rows = []
        fractions = distribution_object["weights"]
        ...
        interval_or_value = "Interval" if distribution_object.get("bins") else "Value"
    ...
    def _prescriptive_template(
        ...
        expected_distribution_object = renderer_configuration.kwargs.get("distribution_object", {})
        weights = expected_distribution_object.get("weights", [])

        # generate template string for header
        if not expected_distribution_object:
        ...
        if len(weights) > 60:  # noqa: PLR2004
            ...
            ) = cls._atomic_distribution_object_table_template(
                distribution_object=expected_distribution_object
            )
        else:
    ...
    def _prescriptive_renderer(
        ...
        **kwargs,
        ...
        if not params.get("distribution_object"):
            template_str = "can match any distribution."
        else:
            template_str = (
                "Kullback-Leibler (KL) divergence with respect to the following distribution must be "  # noqa: E501
            ...
            expected_distribution = cls._get_kl_divergence_chart(params.get("distribution_object"))
    ...
    def _atomic_diagnostic_observed_value_template(
        ...
        observed_distribution_object = result.result.get("details", {}).get(
            "observed_distribution", {}
        )
        weights = observed_distribution_object.get("weights", [])
        ...
        if len(weights) > 60:  # noqa: PLR2004
            (
                ...
                distribution_object=observed_distribution_object
            )
        else:
    ...
    def _diagnostic_observed_value_renderer(
        ...
        **kwargs,
        ...
        observed_distribution_object = result.result["details"]["observed_distribution"]
        observed_distribution = cls._get_kl_divergence_chart(observed_distribution_object)
    ...
    def _descriptive_histogram_renderer(
        ...
        **kwargs,
        ...
        observed_distribution_object = result.result["details"]["observed_distribution"]
        weights = observed_distribution_object["weights"]
        if len(weights) > 60:  # noqa: PLR2004
            return None
        ...
        return cls._get_kl_divergence_chart(observed_distribution_object, header)
