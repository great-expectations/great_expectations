...
from great_expectations.render.types import RenderedAtomicContent, RenderedStringTemplateContent, RenderedTableContent, renderedAtomicValueSchema
from great_expectations.render.util import parse_row_condition_string_pandas_engine, substitute_none_for_missing
...
class ExpectColumnQuantileValuesToBeBetween(ColumnExpectation):
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        try:
            assert ('quantile_ranges' in configuration.kwargs), 'quantile_ranges must be provided'
            assert isinstance(configuration.kwargs['quantile_ranges'], dict), 'quantile_ranges should be a dictionary'
            assert all([(True if ((None in x) or (x == sorted(x))) else False) for x in configuration.kwargs['quantile_ranges']['value_ranges']]), 'quantile_ranges must consist of ordered pairs'
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration['kwargs'], ['column', 'quantile_ranges', 'row_condition', 'condition_parser'])
        header_params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'mostly': {'schema': {'type': 'number'}, 'value': params.get('mostly')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
        ...
        if (params['row_condition'] is not None):
            ...
            header_template_str = (((conditional_template_str + ', then ') + header_template_str[0].lower()) + header_template_str[1:])
            header_params_with_json_schema.update(conditional_params)
        quantile_ranges = (params.get('quantile_ranges') if params.get('quantile_ranges') else {})
        ...
        table_header_row = [{'schema': {'type': 'string'}, 'value': 'Quantile'}, {'schema': {'type': 'string'}, 'value': 'Min Value'}, {'schema': {'type': 'string'}, 'value': 'Max Value'}]
        table_rows = []
        ...
        for (quantile, value_range) in zip(quantiles, value_ranges):
            ...
            table_rows.append([{'value': quantile_string, 'schema': {'type': 'string'}}, {'value': (value_range[0] if (value_range[0] is not None) else 'Any'), 'schema': {'type': ('number' if (value_range[0] is not None) else 'string')}}, {'value': (value_range[1] if (value_range[1] is not None) else 'Any'), 'schema': {'type': ('number' if (value_range[1] is not None) else 'string')}}])
        return (header_template_str, header_params_with_json_schema, styling, table_header_row, table_rows)
    ...
    def _prescriptive_summary(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        (header_template_str, header_params_with_json_schema, _, table_header_row, table_rows) = cls._atomic_prescriptive_template(configuration, result, language, runtime_configuration, **kwargs)
        value_obj = renderedAtomicValueSchema.load({'header': {'schema': {'type': 'StringValueType'}, 'value': {'template': header_template_str, 'params': header_params_with_json_schema}}, 'header_row': table_header_row, 'table': table_rows, 'schema': {'type': 'TableType'}})
    ...
    def _prescriptive_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        for (quantile, value_range) in zip(quantiles, value_ranges):
            quantile_string = quantile_strings.get(quantile, f'{quantile:3.2f}')
            table_rows.append([quantile_string, (str(value_range[0]) if (value_range[0] is not None) else 'Any'), (str(value_range[1]) if (value_range[1] is not None) else 'Any')])
        quantile_range_table = RenderedTableContent(**{'content_block_type': 'table', 'header_row': table_header_row, 'table': table_rows, 'styling': {'body': {'classes': ['table', 'table-sm', 'table-unbordered', 'col-4', 'mt-2']}, 'parent': {'styles': {'list-style-type': 'none'}}}})
    ...
    def _diagnostic_observed_value_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        for (idx, quantile) in enumerate(quantiles):
            ...
            table_rows.append([(quantile_string if quantile_string else f'{quantile:3.2f}'), str(value_ranges[idx])])
        return RenderedTableContent(**{'content_block_type': 'table', 'header_row': table_header_row, 'table': table_rows, 'styling': {'body': {'classes': ['table', 'table-sm', 'table-unbordered', 'col-4']}}})
    ...
    def _atomic_diagnostic_observed_value_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        quantiles = result.result.get('observed_value', {}).get('quantiles', [])
        value_ranges = result.result.get('observed_value', {}).get('values', [])
        table_header_row = [{'schema': {'type': 'string'}, 'value': 'Quantile'}, {'schema': {'type': 'string'}, 'value': 'Value'}]
        table_rows = []
        quantile_strings = {0.25: 'Q1', 0.75: 'Q3', 0.5: 'Median'}
        for (idx, quantile) in enumerate(quantiles):
            quantile_string = quantile_strings.get(quantile)
            table_rows.append([{'value': (quantile_string if quantile_string else f'{quantile:3.2f}'), 'schema': {'type': 'string'}}, {'value': value_ranges[idx], 'schema': {'type': 'number'}}])
        return (template_string, params_with_json_schema, table_header_row, table_rows)
    ...
    def _atomic_diagnostic_observed_value(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        (template_string, params_with_json_schema, table_header_row, table_rows) = cls._atomic_diagnostic_observed_value_template(configuration, result, language, runtime_configuration, **kwargs)
        if (template_string is not None):
            value_obj = renderedAtomicValueSchema.load({'template': template_string, 'params': {}, 'schema': {'type': 'StringValueType'}})
            return RenderedAtomicContent(name='atomic.diagnostic.observed_value', value=value_obj, value_type='StringValueType')
        else:
            value_obj = renderedAtomicValueSchema.load({'header_row': table_header_row, 'table': table_rows, 'schema': {'type': 'TableType'}})
            return RenderedAtomicContent(name='atomic.diagnostic.observed_value', value=value_obj, value_type='TableType')

    @classmethod
    @renderer(renderer_type='renderer.descriptive.quantile_table')
    def _descriptive_quantile_table_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for (idx, quantile) in enumerate(quantiles):
            quantile_string = quantile_strings.get(quantile)
            table_rows.append([{'content_block_type': 'string_template', 'string_template': {'template': (quantile_string if quantile_string else f'{quantile:3.2f}'), 'tooltip': {'content': ('expect_column_quantile_values_to_be_between \n expect_column_median_to_be_between' if (quantile == 0.5) else 'expect_column_quantile_values_to_be_between')}}}, quantile_ranges[idx]])
        return RenderedTableContent(**{'content_block_type': 'table', 'header': RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': 'Quantiles', 'tag': 'h6'}}), 'table': table_rows, 'styling': {'classes': ['col-3', 'mt-1', 'pl-1', 'pr-1'], 'body': {'classes': ['table', 'table-sm', 'table-unbordered']}}})
