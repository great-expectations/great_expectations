...
class ExpectTableRowCountToEqual(TableExpectation):
    ...
    success_keys = ('value',)
    default_kwarg_values = {'value': None, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False, 'meta': None}
    args_keys = ('value',)
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['value', 'row_condition', 'condition_parser'])
        params_with_json_schema = {'value': {'schema': {'type': 'number'}, 'value': params.get('value')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        return {'success': (actual_table_row_count == expected_table_row_count), 'result': {'observed_value': actual_table_row_count}}
