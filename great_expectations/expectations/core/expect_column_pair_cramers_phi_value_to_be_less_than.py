...
class ExpectColumnPairCramersPhiValueToBeLessThan(TableExpectation):
    library_metadata = {'maturity': 'production', 'tags': ['core expectation', 'multi-column expectation', 'needs migration to modular expectations api'], 'contributors': ['@great_expectations'], 'requirements': []}
    metric_dependencies = tuple()
    success_keys = ('column_A', 'column_B', 'threshold')
    default_kwarg_values = {'column_A': None, 'column_B': None, 'bins_A': None, 'bins_B': None, 'n_bins_A': None, 'n_bins_B': None, 'threshold': 0.1, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False}
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params_with_json_schema = {'column_A': {'schema': {'type': 'string'}, 'value': params.get('column_A')}, 'column_B': {'schema': {'type': 'string'}, 'value': params.get('column_B')}}
    ...
    def _prescriptive_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        if ((params['column_A'] is None) or (params['column_B'] is None)):
            ...
            template_str = 'Values in $column_A and $column_B must be independent.'
        rendered_string_template_content = RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': template_str, 'params': params, 'styling': styling}})
    ...
    def _diagnostic_observed_value_renderer(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        if (observed_value is not None):
            ...
            if (crosstab is not None):
                ...
                return RenderedTableContent(**{'content_block_type': 'table', 'header': f'''Observed cramers phi of {observed_value}. 
Crosstab between {column_A} (rows) and {column_B} (columns):''', 'table': table, 'styling': {'body': {'classes': ['table', 'table-sm', 'table-unbordered', 'col-4', 'mt-2']}}})
            else:
