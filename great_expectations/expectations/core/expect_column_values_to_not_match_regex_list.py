...
class ExpectColumnValuesToNotMatchRegexList(ColumnMapExpectation):
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        try:
            ...
            assert isinstance(configuration.kwargs['regex_list'], (list, dict)), 'regex_list must be a list of regexes'
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['column', 'regex_list', 'mostly', 'row_condition', 'condition_parser'])
        params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'regex_list': {'schema': {'type': 'array'}, 'value': params.get('regex_list')}, 'mostly': {'schema': {'type': 'number'}, 'value': params.get('mostly')}, 'mostly_pct': {'schema': {'type': 'string'}, 'value': params.get('mostly_pct')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}}
        if ((not params.get('regex_list')) or (len(params.get('regex_list')) == 0)):
            values_string = '[ ]'
        else:
            for (i, v) in enumerate(params['regex_list']):
                params[f'v__{str(i)}'] = v
            values_string = ' '.join([f'$v__{str(i)}' for (i, v) in enumerate(params['regex_list'])])
        ...
        params_with_json_schema = add_values_with_json_schema_from_list_in_params(params=params, params_with_json_schema=params_with_json_schema, param_key_with_list='regex_list')
