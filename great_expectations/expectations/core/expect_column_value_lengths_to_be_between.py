...
from great_expectations.render.types import RenderedBulletListContent, RenderedGraphContent, RenderedStringTemplateContent, RenderedTableContent
...
class ExpectColumnValueLengthsToBeBetween(ColumnMapExpectation):
    ...
    default_kwarg_values = {'row_condition': None, 'condition_parser': None, 'min_value': None, 'max_value': None, 'strict_min': None, 'strict_max': None, 'mostly': 1, 'result_format': 'BASIC', 'include_config': True, 'catch_exceptions': False}
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]) -> None:
        ...
        try:
            assert ((configuration.kwargs.get('min_value') is not None) or (configuration.kwargs.get('max_value') is not None)), 'min_value and max_value cannot both be None'
            if configuration.kwargs.get('min_value'):
                assert (isinstance(configuration.kwargs['min_value'], dict) or float(configuration.kwargs.get('min_value')).is_integer()), 'min_value and max_value must be integers'
                if isinstance(configuration.kwargs.get('min_value'), dict):
                    assert ('$PARAMETER' in configuration.kwargs.get('min_value')), 'Evaluation Parameter dict for min_value kwarg must have "$PARAMETER" key.'
            if configuration.kwargs.get('max_value'):
                assert (isinstance(configuration.kwargs['max_value'], dict) or float(configuration.kwargs.get('max_value')).is_integer()), 'min_value and max_value must be integers'
                if isinstance(configuration.kwargs.get('max_value'), dict):
                    assert ('$PARAMETER' in configuration.kwargs.get('max_value')), 'Evaluation Parameter dict for max_value kwarg must have "$PARAMETER" key.'
    ...
    def _prescriptive_renderer(cls, configuration: ExpectationConfiguration=None, result: ExpectationValidationResult=None, language: str=None, runtime_configuration: dict=None, **kwargs) -> List[Union[(dict, str, RenderedStringTemplateContent, RenderedTableContent, RenderedBulletListContent, RenderedGraphContent, Any)]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
