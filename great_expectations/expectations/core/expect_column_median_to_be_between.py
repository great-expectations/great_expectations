...
class ExpectColumnMedianToBeBetween(ColumnExpectation):
    ...
    median_range_estimator_parameter_builder_config: ParameterBuilderConfig = ParameterBuilderConfig(module_name='great_expectations.rule_based_profiler.parameter_builder', class_name='NumericMetricRangeMultiBatchParameterBuilder', name='median_range_estimator', metric_name='column.median', metric_domain_kwargs=DOMAIN_KWARGS_PARAMETER_FULLY_QUALIFIED_NAME, metric_value_kwargs=None, enforce_numeric_metric=True, replace_nan_with_zero=True, reduce_scalar_metric=True, false_positive_rate=f'{VARIABLES_KEY}false_positive_rate', quantile_statistic_interpolation_method=f'{VARIABLES_KEY}quantile_statistic_interpolation_method', estimator=f'{VARIABLES_KEY}estimator', num_bootstrap_samples=f'{VARIABLES_KEY}num_bootstrap_samples', include_bootstrap_samples_histogram_in_details=f'{VARIABLES_KEY}include_bootstrap_samples_histogram_in_details', bootstrap_random_seed=f'{VARIABLES_KEY}bootstrap_random_seed', truncate_values=f'{VARIABLES_KEY}truncate_values', round_decimals=f'{VARIABLES_KEY}round_decimals', evaluation_parameter_builder_configs=None, json_serialize=True)
    validation_parameter_builder_configs: List[ParameterBuilderConfig] = [median_range_estimator_parameter_builder_config]
    default_profiler_config: RuleBasedProfilerConfig = RuleBasedProfilerConfig(name='expect_column_median_to_be_between', config_version=1.0, variables={}, rules={'default_expect_column_median_to_be_between_rule': {'variables': {'strict_min': False, 'strict_max': False, 'false_positive_rate': 0.05, 'quantile_statistic_interpolation_method': 'auto', 'estimator': 'bootstrap', 'num_bootstrap_samples': 9999, 'bootstrap_random_seed': None, 'include_bootstrap_samples_histogram_in_details': False, 'truncate_values': {'lower_bound': None, 'upper_bound': None}, 'round_decimals': 2}, 'domain_builder': {'class_name': 'ColumnDomainBuilder', 'module_name': 'great_expectations.rule_based_profiler.domain_builder'}, 'expectation_configuration_builders': [{'expectation_type': 'expect_column_median_to_be_between', 'class_name': 'DefaultExpectationConfigurationBuilder', 'module_name': 'great_expectations.rule_based_profiler.expectation_configuration_builder', 'validation_parameter_builder_configs': validation_parameter_builder_configs, 'column': f'{DOMAIN_KWARGS_PARAMETER_FULLY_QUALIFIED_NAME}{FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER}column', 'min_value': f'{PARAMETER_KEY}{median_range_estimator_parameter_builder_config.name}{FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER}{FULLY_QUALIFIED_PARAMETER_NAME_VALUE_KEY}[0]', 'max_value': f'{PARAMETER_KEY}{median_range_estimator_parameter_builder_config.name}{FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER}{FULLY_QUALIFIED_PARAMETER_NAME_VALUE_KEY}[1]', 'strict_min': f'{VARIABLES_KEY}strict_min', 'strict_max': f'{VARIABLES_KEY}strict_max', 'meta': {'profiler_details': f'{PARAMETER_KEY}{median_range_estimator_parameter_builder_config.name}{FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER}{FULLY_QUALIFIED_PARAMETER_NAME_METADATA_KEY}'}}]}})
    ...
    def _validate(self, configuration: ExpectationConfiguration, metrics: Dict, runtime_configuration: dict=None, execution_engine: ExecutionEngine=None):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return self._validate_metric_value_between(metric_name='column.median', configuration=configuration, metrics=metrics, runtime_configuration=runtime_configuration, execution_engine=execution_engine)
