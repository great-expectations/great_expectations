...
class SetBasedColumnMapExpectation(ColumnMapExpectation, ABC):
    ...
    def register_metric(set_camel_name: str, set_: str):
        ...
        new_column_set_metric_provider = type(f'(ColumnValuesMatch{set_camel_name}Set', (SetColumnMapMetricProvider,), {'condition_metric_name': map_metric, 'set_': set_})
        return map_metric
        ...
        if result.success:
            if ((mostly == 1) or (mostly is None)):
                if (set_semantic_name is not None):
                    ...
                    return f'All values in column "{column}" are in the set {str(set_)}.'
            elif (set_semantic_name is not None):
    ...
    def _atomic_prescriptive_template(cls, configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
        ...
        params = substitute_none_for_missing(configuration.kwargs, ['column', 'set_', 'mostly', 'row_condition', 'condition_parser', 'set_semantic_name'])
        params_with_json_schema = {'column': {'schema': {'type': 'string'}, 'value': params.get('column')}, 'mostly': {'schema': {'type': 'number'}, 'value': params.get('mostly')}, 'mostly_pct': {'schema': {'type': 'number'}, 'value': params.get('mostly_pct')}, 'set_': {'schema': {'type': 'string'}, 'value': params.get('set_')}, 'row_condition': {'schema': {'type': 'string'}, 'value': params.get('row_condition')}, 'condition_parser': {'schema': {'type': 'string'}, 'value': params.get('condition_parser')}, 'set_semantic_name': {'schema': {'type': 'string'}, 'value': params.get('set_semantic_name')}}
