            ...
            if lines:
                ...
                if element_count > sum(boolean_mapped_null_lines):
                    ...
                    boolean_mapped_success_lines = np.array(
                        func(self, *args, _lines=nonnull_lines, **kwargs)
                    )
    ...
    def expect_file_line_regex_match_count_to_be_between(
        ...
        if expected_min_count is not None:
            try:
                ...
                raise ValueError(
                    "expected_min_count must be a non-negative                                  integer or None"
                )
        if expected_max_count is not None:
            try:
                ...
                raise ValueError(
                    "expected_max_count must be a non-negative                                  integer or None"
                )
        if (expected_max_count is not None) and (expected_min_count is not None):
            try:
                ...
                raise ValueError(
                    "expected_max_count must be greater than or                                  equal to expected_min_count"
                )
        if (expected_max_count is not None) and (expected_min_count is not None):
            truth_list = [
                (
