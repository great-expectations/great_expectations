...
class QueryDataConnector(DataConnector):
    """Produce query-style batch_kwargs from sql files or defined queries.

    By default, a QueryDataConnector will look for queries in the
    ``execution_environments/execution_environment_name/data_connectors/data_connector_name`` directory, and look for files ending in ``.sql``.

    For example, a file stored in
    ``execution_environments/execution_environment_name/data_connectors/data_connector_name/movies_by_date.sql`` would allow you to access an
    ...
    def __init__(
        ...
        super().__init__(name=name, execution_environment=execution_environment)
        if (
            execution_environment
            and execution_environment.data_context
            and execution_environment.data_context.root_directory
        ):
            root_directory = execution_environment.data_context.root_directory
        else:
            root_directory = None
            ...
            if root_directory:
                query_store_backend = {
                    ...
                    "base_directory": os.path.join(
                        execution_environment.data_context.root_directory,
    ...
    def _build_batch_kwargs(self, batch_parameters):
        ...
        batch_kwargs = self._execution_environment.process_batch_parameters(
            **batch_parameters
        )
    ...
    def get_available_partition_ids(self, generator_asset=None, data_asset_name=None):
        ...
        raise BatchKwargsError("QueryDataConnector cannot identify partitions.", {})
