...
"{str(type(partition_name))}", which is illegal.
    ...
    partition_definition: Union[dict, None] = partition_request_dict.get(
...
"{str(type(data_asset_name))}", which is illegal.
    ...
    partition_index: Union[
        int, list, tuple, slice, str, None
    ] = partition_request_dict.get("partition_index")
            ...
            if self.partition_definition:
                ...
                for key in partition_definition_query_keys:
                    if not (
                        ...
                        and partition_definition[key] == self.partition_definition[key]
                    ):
                        return False
