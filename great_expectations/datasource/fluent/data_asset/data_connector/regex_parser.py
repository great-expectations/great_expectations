...
class RegExParser:
    def __init__(
        ...
        self._named_group_name_to_group_index_mapping: Dict[str, int] = dict(regex_pattern.groupindex)
    ...
    def get_all_group_names_to_group_indexes_bidirectional_mappings(
        ...
        common_group_indexes: List[int] = list(
            filter(
                lambda idx: idx not in self._named_group_name_to_group_index_mapping.values(),
        ...
        common_group_index_to_group_name_mapping: Dict[int, str] = {
            group_idx: f"{self._unnamed_regex_group_prefix}{group_idx}" for group_idx in common_group_indexes
        }
    ...
    def get_all_group_name_to_group_index_mapping(self) -> Dict[str, int]:
        all_group_names_to_group_indexes_bidirectional_mappings: Tuple[Dict[str, int], Dict[int, str]] = (
            self.get_all_group_names_to_group_indexes_bidirectional_mappings()
        )
        all_group_name_to_group_index_mapping: Dict[str, int] = all_group_names_to_group_indexes_bidirectional_mappings[
            0
        ]
    ...
    def get_all_group_index_to_group_name_mapping(self) -> Dict[int, str]:
        ...
        all_group_index_to_group_name_mapping: Dict[int, str] = all_group_names_to_group_indexes_bidirectional_mappings[
            1
        ]
        return all_group_index_to_group_name_mapping

    def get_all_group_names(self) -> List[str]:
        all_group_name_to_group_index_mapping: Dict[str, int] = self.get_all_group_name_to_group_index_mapping()
        all_group_names: List[str] = list(all_group_name_to_group_index_mapping.keys())
        return all_group_names

    def get_all_group_indexes(self) -> List[int]:
        all_group_index_to_group_name_mapping: Dict[int, str] = self.get_all_group_index_to_group_name_mapping()
        all_group_indexes: List[int] = list(all_group_index_to_group_name_mapping.keys())
    ...
    def get_group_name_to_group_value_mapping(
        ...
        all_matched_group_values: List[str] = self.get_all_matched_group_values(target=target) or []
        group_name_to_group_value_mapping: Dict[str, str] = dict(zip(all_group_names, all_matched_group_values))
    ...
    def get_group_index_to_group_value_mapping(
        ...
        all_matched_group_values: List[str] = self.get_all_matched_group_values(target=target) or []
        group_index_to_group_value_mapping: Dict[int, str] = dict(zip(all_group_indexes, all_matched_group_values))
