...
class GoogleCloudStorageDataConnector(FilePathDataConnector):
    ...
    def build_test_connection_error_message(  # noqa: PLR0913
        ...
        test_connection_error_message_template: str = (
            'No file in bucket "{bucket_or_name}" with prefix "{prefix}" and recursive file discovery set to "{recursive_file_discovery}" matched regular expressions pattern "{batching_regex}" using delimiter "{delimiter}" for DataAsset "{data_asset_name}".'
        )
