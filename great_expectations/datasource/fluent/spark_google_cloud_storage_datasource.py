...
class SparkGoogleCloudStorageDatasource(_SparkFilePathDatasource):
    # class attributes
    data_connector_type: ClassVar[Type[GoogleCloudStorageDataConnector]] = GoogleCloudStorageDataConnector
    ...
    def _get_gcs_client(self) -> google.Client:
        ...
        if not gcs_client:
            ...
            if google.service_account and google.storage:
                try:
                    credentials: Union[google.Client, None] = None  # If configured with gcloud CLI / env vars
                    _check_config_substitutions_needed(
                        self,
                        self.gcs_options,
                    ...
                    gcs_options: dict = self.dict(config_provider=self._config_provider).get("gcs_options", {})
