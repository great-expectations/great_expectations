...
class QueryGenerator(BatchGenerator):
    ...
    def __init__(self, datasource, name="default"):
        ...
        super(QueryGenerator, self).__init__(
            name=name, type_="queries", datasource=datasource)
        if (
        ...
        if datasource is not None:
            ...
            try:
                ...
                logger.warning(
                    "Unable to create inspector from engine in generator '%s'" % name)
                self.inspector = None
    ...
    def _get_iterator(self, data_asset_name, **kwargs):
        ...
        if self.engine is not None and self.inspector is not None:
            ...
            if len(split_data_asset_name) == 2:
                ...
                raise ValueError(
                    "Table name must be of shape '[SCHEMA.]TABLE'. Passed: " + data_asset_name)
    ...
    def get_available_data_asset_names(self):
        if self._queries_path:
            defined_queries = [path for path in os.walk(
                self._queries_path) if str(path).endswith(".sql")]
        else:
        ...
        if self.engine is not None and self.inspector is not None:
            for schema_name in self.inspector.get_schema_names():
                ...
                tables.extend([table_name if self.inspector.default_schema_name == schema_name else schema_name +
                               "." + table_name for table_name in self.inspector.get_table_names(schema=schema_name)])
