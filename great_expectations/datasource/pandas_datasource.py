...
from great_expectations.execution_engine.pandas_execution_engine import hash_pandas_dataframe
...
class PandasDatasource(LegacyDatasource):
    ...
    recognized_batch_parameters = {'reader_method', 'reader_options', 'limit', 'dataset_options', 'boto3_options'}

    @classmethod
    def build_configuration(cls, data_asset_type=None, batch_kwargs_generators=None, boto3_options=None, reader_method=None, reader_options=None, limit=None, **kwargs):
        import inspect
        ...
        if (boto3_options is not None):
            if isinstance(boto3_options, dict):
                ...
                raise ValueError('boto3_options must be a dictionary of key-value pairs to pass to boto3 upon initialization.')
    ...
    def __init__(self, name='pandas', data_context=None, data_asset_type=None, batch_kwargs_generators=None, boto3_options=None, reader_method=None, reader_options=None, limit=None, **kwargs) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        configuration_with_defaults = PandasDatasource.build_configuration(data_asset_type, batch_kwargs_generators, boto3_options, reader_method=reader_method, reader_options=reader_options, limit=limit, **kwargs)
    ...
    def get_batch(self, batch_kwargs, batch_parameters=None):
        ...
        if ('path' in batch_kwargs):
            ...
            raise BatchKwargsError('Invalid batch_kwargs: path, s3, or dataset is required for a PandasDatasource', batch_kwargs)
    ...
    def _get_reader_fn(self, reader_method=None, path=None):
        ...
        if ((reader_method is None) and (path is None)):
            raise BatchKwargsError('Unable to determine pandas reader function without reader_method or path.', {'reader_method': reader_method})
        reader_options = None
        ...
        try:
            ...
            raise BatchKwargsError(f'Unable to find reader_method {reader_method} in pandas.', {'reader_method': reader_method})
