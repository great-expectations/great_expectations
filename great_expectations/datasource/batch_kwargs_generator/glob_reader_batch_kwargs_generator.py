...
class GlobReaderBatchKwargsGenerator(BatchKwargsGenerator):
    ...
    def __init__(self, name='default', datasource=None, base_directory='/data', reader_options=None, asset_globs=None, reader_method=None) -> None:
        import inspect
        ...
        if (asset_globs is None):
            asset_globs = {'default': {'glob': '*', 'partition_regex': '^((19|20)\\d\\d[- /.]?(0[1-9]|1[012])[- /.]?(0[1-9]|[12][0-9]|3[01])_(.*))\\.csv', 'match_group_id': 1, 'reader_method': 'read_csv'}}
    ...
    def get_available_partition_ids(self, generator_asset=None, data_asset_name=None):
        ...
        batch_paths = self._get_data_asset_paths(data_asset_name=data_asset_name)
        partition_ids = [self._partitioner(path, glob_config) for path in batch_paths if (self._partitioner(path, glob_config) is not None)]
    ...
    def _get_iterator(self, data_asset_name, reader_method=None, reader_options=None, limit=None):
        ...
        return self._build_batch_kwargs_path_iter(paths, glob_config, reader_method=reader_method, reader_options=reader_options, limit=limit)

    def _build_batch_kwargs_path_iter(self, path_list, glob_config, reader_method=None, reader_options=None, limit=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for path in path_list:
            (yield self._build_batch_kwargs_from_path(path, glob_config, reader_method=reader_method, reader_options=reader_options, limit=limit))
    ...
    def _build_batch_kwargs_from_path(self, path, glob_config, reader_method=None, reader_options=None, limit=None):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        batch_kwargs = self._datasource.process_batch_parameters(reader_method=(reader_method or glob_config.get('reader_method') or self.reader_method), reader_options=(reader_options or glob_config.get('reader_options') or self.reader_options), limit=(limit or glob_config.get('limit')))
