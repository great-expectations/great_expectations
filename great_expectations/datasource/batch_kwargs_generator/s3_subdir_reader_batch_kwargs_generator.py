...
class S3SubdirReaderBatchKwargsGenerator(BatchKwargsGenerator):
    ...
    def __init__(self, name='default', datasource=None, bucket=None, boto3_options=None, base_directory='/data', reader_options=None, known_extensions=None, reader_method=None) -> None:
        import inspect
    ...
    def get_available_partition_ids(self, generator_asset=None, data_asset_name=None):
        ...
        for extension in self.known_extensions:
            if self.fs.isfile(os.path.join(self.base_directory, (data_asset_name + extension))):
    ...
    def _build_batch_kwargs(self, batch_parameters):
        ...
        if ('partition_id' in batch_parameters):
            ...
            path = None
            for extension in self.known_extensions:
                if self.fs.isfile(os.path.join(self.base_directory, data_asset_name, (partition_id + extension))):
    ...
    def _get_valid_file_options(self, base_directory=None):
        ...
        for file_option in file_options:
            ...
            file_option = file_option.split('/')[(- 1)]
            for extension in self.known_extensions:
