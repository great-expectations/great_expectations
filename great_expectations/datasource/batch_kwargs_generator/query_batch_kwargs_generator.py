...
class QueryBatchKwargsGenerator(BatchKwargsGenerator):
    ...
    def __init__(self, name='default', datasource=None, query_store_backend=None, queries=None) -> None:
        import inspect
        ...
        if (query_store_backend is None):
            if root_directory:
                query_store_backend = {'class_name': 'TupleFilesystemStoreBackend', 'base_directory': os.path.join(datasource.data_context.root_directory, 'datasources', datasource.name, 'generators', name), 'filepath_suffix': '.sql'}
            else:
                query_store_backend = {'class_name': 'InMemoryStoreBackend'}
        module_name = 'great_expectations.data_context.store'
        self._store_backend = instantiate_class_from_config(config=query_store_backend, runtime_environment={'root_directory': root_directory}, config_defaults={'module_name': module_name})
        if (not self._store_backend):
            raise ClassInstantiationError(module_name=module_name, package_name=None, class_name=query_store_backend['class_name'])
    ...
    def _get_iterator(self, data_asset_name, query_parameters=None):
        ...
        if (query_parameters is None):
            ...
            iter_ = iter([SqlAlchemyDatasourceQueryBatchKwargs(query=raw_query, query_parameters=query_parameters)])
        return iter_
