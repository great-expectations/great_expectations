...
class BatchKwargsGenerator():
    ...
    def reset_iterator(self, generator_asset=None, data_asset_name=None, **kwargs) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        self._data_asset_iterators[data_asset_name] = (self._get_iterator(data_asset_name=data_asset_name, **kwargs), kwargs)
    ...
    def get_iterator(self, generator_asset=None, data_asset_name=None, **kwargs):
        ...
        if (data_asset_name in self._data_asset_iterators):
            ...
            if (passed_kwargs != kwargs):
                logger.warning('Asked to yield batch_kwargs using different supplemental kwargs. Please reset iterator to use different supplemental kwargs.')
    ...
    def build_batch_kwargs(self, data_asset_name=None, partition_id=None, **kwargs):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        if (((not kwargs.get('name')) and (not data_asset_name)) or (kwargs.get('name') and data_asset_name)):
    ...
    def yield_batch_kwargs(self, data_asset_name=None, generator_asset=None, **kwargs):
        ...
        (data_asset_iterator, passed_kwargs) = self._data_asset_iterators[data_asset_name]
        if (passed_kwargs != kwargs):
            logger.warning('Asked to yield batch_kwargs using different supplemental kwargs. Resetting iterator to use new supplemental kwargs.')
            self.reset_iterator(data_asset_name=data_asset_name, **kwargs)
            (data_asset_iterator, passed_kwargs) = self._data_asset_iterators[data_asset_name]
        try:
            ...
            (data_asset_iterator, passed_kwargs) = self._data_asset_iterators[data_asset_name]
            if (passed_kwargs != kwargs):
                logger.warning('Asked to yield batch_kwargs using different batch parameters. Resetting iterator to use different batch parameters.')
                self.reset_iterator(data_asset_name=data_asset_name, **kwargs)
                (data_asset_iterator, passed_kwargs) = self._data_asset_iterators[data_asset_name]
            try:
                batch_kwargs = next(data_asset_iterator)
