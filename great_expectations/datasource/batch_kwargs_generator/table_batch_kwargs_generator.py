...
from great_expectations.marshmallow__shade import Schema, ValidationError, fields, post_load
...
class TableBatchKwargsGenerator(BatchKwargsGenerator):
    ...
    def __init__(self, name='default', datasource=None, assets=None) -> None:
        ...
        if (not assets):
            assets = {}
        try:
            self._assets = {asset_name: assetConfigurationSchema.load(asset_config) for (asset_name, asset_config) in assets.items()}
        except ValidationError as err:
            raise GreatExpectationsError(("Unable to load asset configuration in TableBatchKwargsGenerator '%s': validation error: %s." % (name, str(err))))
    ...
    def _get_iterator(self, data_asset_name, query_parameters=None, limit=None, offset=None, partition_id=None):
        ...
        if (data_asset_name in self._assets):
            ...
            try:
                ...
                raise BatchKwargsError((("Unable to generate batch kwargs for asset '" + data_asset_name) + "': missing template key"), {'data_asset_name': data_asset_name, 'table_template': asset_config.table, 'schema_template': asset_config.schema})
            ...
            if (len(split_data_asset_name) == 2):
            ...
            elif ((len(split_data_asset_name) == 3) and (self.engine.dialect.name.lower() == 'bigquery')):
                ...
                raise ValueError("Table name must be of shape '{}'. Passed: {}".format(shape, split_data_asset_name))
            try:
            ...
            if has_table(table_name, schema=schema_name):
                ...
                raise BatchKwargsError(f'TableBatchKwargsGenerator cannot access the following data:SCHEMA : {schema_name}TABLE : {table_name}', {})
        if (batch_kwargs is not None):
            if (partition_id is not None):
                logger.warning('table_generator cannot identify partitions; provided partition id will be recorded only')
    ...
    def get_available_data_asset_names(self):
        ...
        if ((self.engine is not None) and (self.inspector is not None)):
            for schema_name in self.inspector.get_schema_names():
                known_information_schemas = ['INFORMATION_SCHEMA', 'information_schema', 'performance_schema', 'sys', 'mysql']
                known_system_tables = ['sqlite_master']
                if (schema_name in known_information_schemas):
                    continue
                if (self.engine.dialect.name.lower() == 'bigquery'):
                    tables.extend([(table_name, 'table') for table_name in self.inspector.get_table_names(schema=schema_name) if (table_name not in known_system_tables)])
                else:
                    if (self.engine.dialect.name.lower() == 'sqlite'):
                        default_schema_name = None
                    else:
                        default_schema_name = self.inspector.default_schema_name
                    tables.extend([((table_name, 'table') if (default_schema_name == schema_name) else (f'{schema_name}.{table_name}', 'table')) for table_name in self.inspector.get_table_names(schema=schema_name) if (table_name not in known_system_tables)])
                try:
                    tables.extend([((table_name, 'view') if (default_schema_name == schema_name) else (f'{schema_name}.{table_name}', 'view')) for table_name in self.inspector.get_view_names(schema=schema_name) if (table_name not in known_system_tables)])
    ...
    def _build_batch_kwargs(self, batch_parameters):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return next(self._get_iterator(data_asset_name=batch_parameters.get('data_asset_name'), query_parameters=batch_parameters.get('query_parameters', {}), limit=batch_parameters.get('limit'), offset=batch_parameters.get('offset')))

    def get_available_partition_ids(self, generator_asset=None, data_asset_name=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        raise BatchKwargsError('TableBatchKwargsGenerator cannot identify partitions, however any existing table mayalready be referenced by accessing a data_asset with the name of the table or of the form SCHEMA.TABLE', {})
