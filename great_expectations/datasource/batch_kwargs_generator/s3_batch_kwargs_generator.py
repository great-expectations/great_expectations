...
class S3GlobReaderBatchKwargsGenerator(BatchKwargsGenerator):
    ...
    recognized_batch_parameters = {'data_asset_name', 'partition_id', 'reader_method', 'reader_options', 'limit'}

    def __init__(self, name='default', datasource=None, bucket=None, reader_options=None, assets=None, delimiter='/', reader_method=None, boto3_options=None, max_keys=1000) -> None:
        import inspect
    ...
    def _get_iterator(self, data_asset_name, reader_method=None, reader_options=None, limit=None):
        ...
        if (data_asset_name not in self._assets):
            batch_kwargs = {'data_asset_name': data_asset_name, 'reader_method': reader_method, 'reader_options': reader_options, 'limit': limit}
            raise BatchKwargsError(f'Unknown asset_name {data_asset_name}', batch_kwargs)
        if (data_asset_name not in self._iterators):
            self._iterators[data_asset_name] = {}
        asset_config = self._assets[data_asset_name]
        return self._build_asset_iterator(asset_config=asset_config, iterator_dict=self._iterators[data_asset_name], reader_method=reader_method, reader_options=reader_options, limit=limit)
    ...
    def _build_batch_kwargs(self, batch_parameters):
        ...
        if partition_id:
            try:
                ...
                raise GreatExpectationsError(f'No asset config found for asset {data_asset_name}')
            if (data_asset_name not in self._iterators):
                self._iterators[data_asset_name] = {}
            ...
            for key in self._get_asset_options(asset_config, iterator_dict):
                if (self._partitioner(key=key, asset_config=asset_config) == partition_id):
                    batch_kwargs = self._build_batch_kwargs_from_key(key=key, asset_config=asset_config, reader_options=batch_parameters.get('reader_options'), limit=batch_kwargs.get('limit'))
            if (batch_kwargs is None):
                raise BatchKwargsError(('Unable to identify partition %s for asset %s' % (partition_id, data_asset_name)), {data_asset_name: data_asset_name, partition_id: partition_id})
            return batch_kwargs
        else:
            return self.yield_batch_kwargs(data_asset_name=data_asset_name, **batch_parameters, **batch_kwargs)

    def _build_batch_kwargs_from_key(self, key, asset_config=None, reader_method=None, reader_options=None, limit=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        batch_kwargs = {'s3': f's3a://{self.bucket}/{key}', 'reader_options': self.reader_options}
    ...
    def _get_asset_options(self, asset_config, iterator_dict) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        query_options = {'Bucket': self.bucket, 'Delimiter': asset_config.get('delimiter', self._delimiter), 'Prefix': asset_config.get('prefix', None), 'MaxKeys': asset_config.get('max_keys', self._max_keys)}
    ...
    def _build_asset_iterator(self, asset_config, iterator_dict, reader_method=None, reader_options=None, limit=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        for key in self._get_asset_options(asset_config, iterator_dict):
            (yield self._build_batch_kwargs_from_key(key, asset_config, reader_method=None, reader_options=reader_options, limit=limit))
    ...
    def get_available_partition_ids(self, generator_asset=None, data_asset_name=None):
        ...
        iterator_dict = self._iterators[data_asset_name]
        asset_config = self._assets[data_asset_name]
        available_ids = [self._partitioner(key=key, asset_config=asset_config) for key in self._get_asset_options(asset_config, iterator_dict)]
    ...
    def _partitioner(self, key, asset_config):
        ...
        if ('partition_regex' in asset_config):
            ...
            if (matches is None):
                ...
                return (datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ') + '__unmatched')
            else:
                try:
                    return matches.group(match_group_id)
                except IndexError:
                    logger.warning(('No match group %d in key %s' % (match_group_id, key)))
                    return (datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ') + '__no_match_group')
        else:
