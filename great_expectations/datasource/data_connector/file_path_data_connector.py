...
from great_expectations.core.batch import BatchDefinition, BatchRequest, BatchRequestBase, BatchSpec
from great_expectations.core.batch_spec import PathBatchSpec
from great_expectations.datasource.data_connector.batch_filter import BatchFilter, build_batch_filter
from great_expectations.datasource.data_connector.data_connector import DataConnector
from great_expectations.datasource.data_connector.sorter import Sorter
from great_expectations.datasource.data_connector.util import batch_definition_matches_batch_request, build_sorters_from_config, map_batch_definition_to_data_reference_string_using_regex, map_data_reference_string_to_batch_definition_list_using_regex
...
class FilePathDataConnector(DataConnector):
    ...
    def __init__(self, name: str, datasource_name: str, execution_engine: Optional[ExecutionEngine]=None, default_regex: Optional[dict]=None, sorters: Optional[list]=None, batch_spec_passthrough: Optional[dict]=None) -> None:
        import inspect
    ...
    def _get_data_reference_list_from_cache_by_data_asset_name(self, data_asset_name: str) -> List[str]:
        ...
        batch_definition_list = self._get_batch_definition_list_from_batch_request(batch_request=BatchRequestBase(datasource_name=self.datasource_name, data_connector_name=self.name, data_asset_name=data_asset_name))
        if (len(self.sorters) > 0):
            batch_definition_list = self._sort_batch_definition_list(batch_definition_list=batch_definition_list)
        path_list: List[str] = [map_batch_definition_to_data_reference_string_using_regex(batch_definition=batch_definition, regex_pattern=pattern, group_names=group_names) for batch_definition in batch_definition_list]
        return path_list

    def get_batch_definition_list_from_batch_request(self, batch_request: BatchRequest) -> List[BatchDefinition]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _get_batch_definition_list_from_batch_request(self, batch_request: BatchRequestBase) -> List[BatchDefinition]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        batch_definition_list: List[BatchDefinition] = list(filter((lambda batch_definition: batch_definition_matches_batch_request(batch_definition=batch_definition, batch_request=batch_request)), self._get_batch_definition_list_from_cache()))
        if (len(self.sorters) > 0):
            batch_definition_list = self._sort_batch_definition_list(batch_definition_list=batch_definition_list)
        if (batch_request.data_connector_query is not None):
            ...
            batch_definition_list = batch_filter_obj.select_from_data_connector_query(batch_definition_list=batch_definition_list)
    ...
    def _map_data_reference_to_batch_definition_list(self, data_reference: str, data_asset_name: str=None) -> Optional[List[BatchDefinition]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        pattern: str = regex_config['pattern']
        group_names: List[str] = regex_config['group_names']
        return map_data_reference_string_to_batch_definition_list_using_regex(datasource_name=self.datasource_name, data_connector_name=self.name, data_asset_name=data_asset_name, data_reference=data_reference, regex_pattern=pattern, group_names=group_names)
    ...
    def _map_batch_definition_to_data_reference(self, batch_definition: BatchDefinition) -> str:
        ...
        pattern: str = regex_config['pattern']
        group_names: List[str] = regex_config['group_names']
        return map_batch_definition_to_data_reference_string_using_regex(batch_definition=batch_definition, regex_pattern=pattern, group_names=group_names)
    ...
    def _generate_batch_spec_parameters_from_batch_definition(self, batch_definition: BatchDefinition) -> dict:
        ...
        if (not path):
            raise ValueError(f'''No data reference for data asset name "{batch_definition.data_asset_name}" matches the given
    ...
    def _validate_sorters_configuration(self, data_asset_name: Optional[str]=None) -> None:
        ...
        if ((self.sorters is not None) and (len(self.sorters) > 0)):
            ...
            if any([(sorter_name not in group_names) for sorter_name in self.sorters.keys()]):
                raise ge_exceptions.DataConnectorError(f'''DataConnector "{self.name}" specifies one or more sort keys that do not appear among the
configured group_name.
                    ''')
            if (len(group_names) < len(self.sorters)):
                raise ge_exceptions.DataConnectorError(f'''DataConnector "{self.name}" is configured with {len(group_names)} group names;
