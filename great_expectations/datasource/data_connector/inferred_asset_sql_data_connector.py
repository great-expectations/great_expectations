...
class InferredAssetSqlDataConnector(ConfiguredAssetSqlDataConnector):
    ...
    def __init__(self, name: str, datasource_name: str, execution_engine: Optional[ExecutionEngine]=None, data_asset_name_prefix: str='', data_asset_name_suffix: str='', include_schema_name: bool=False, splitter_method: Optional[str]=None, splitter_kwargs: Optional[dict]=None, sampling_method: Optional[str]=None, sampling_kwargs: Optional[dict]=None, excluded_tables: Optional[list]=None, included_tables: Optional[list]=None, skip_inapplicable_tables: bool=True, introspection_directives: Optional[dict]=None, batch_spec_passthrough: Optional[dict]=None) -> None:
        ...
        super().__init__(name=name, datasource_name=datasource_name, execution_engine=execution_engine, assets=None, batch_spec_passthrough=batch_spec_passthrough)
        self._introspected_assets_cache = {}
        self._refresh_introspected_assets_cache(self._data_asset_name_prefix, self._data_asset_name_suffix, self._include_schema_name, self._splitter_method, self._splitter_kwargs, self._sampling_method, self._sampling_kwargs, self._excluded_tables, self._included_tables, self._skip_inapplicable_tables)
    ...
    def _refresh_introspected_assets_cache(self, data_asset_name_prefix: str=None, data_asset_name_suffix: str=None, include_schema_name: bool=False, splitter_method: str=None, splitter_kwargs: dict=None, sampling_method: str=None, sampling_kwargs: dict=None, excluded_tables: List=None, included_tables: List=None, skip_inapplicable_tables: bool=True) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for metadata in introspected_table_metadata:
            if ((excluded_tables is not None) and (f"{metadata['schema_name']}.{metadata['table_name']}" in excluded_tables)):
                continue
            if ((included_tables is not None) and (f"{metadata['schema_name']}.{metadata['table_name']}" not in included_tables)):
                continue
            ...
            data_asset_config = {'schema_name': metadata['schema_name'], 'table_name': metadata['table_name'], 'type': metadata['type']}
            ...
            try:
                self._get_batch_identifiers_list_from_data_asset_config(data_asset_name, data_asset_config)
    ...
    def _introspect_db(self, schema_name: str=None, ignore_information_schemas_and_system_tables: bool=True, information_schemas: List[str]=['INFORMATION_SCHEMA', 'information_schema', 'performance_schema', 'sys', 'mysql'], system_tables: List[str]=['sqlite_master'], include_views=True):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for schema_name in inspector.get_schema_names():
            ...
            for table_name in inspector.get_table_names(schema=schema_name):
                if (ignore_information_schemas_and_system_tables and (table_name in system_tables)):
                    continue
                tables.append({'schema_name': schema_name, 'table_name': table_name, 'type': 'table'})
            if include_views:
                try:
                    ...
                    for view_name in view_names:
                        if (ignore_information_schemas_and_system_tables and (view_name in system_tables)):
                            continue
                        tables.append({'schema_name': schema_name, 'table_name': view_name, 'type': 'view'})
        try:
            if ('redshift' == engine.dialect.name.lower()):
                result = engine.execute('select schemaname, tablename from svv_external_tables').fetchall()
                for row in result:
                    tables.append({'schema_name': row[0], 'table_name': row[1], 'type': 'table'})
