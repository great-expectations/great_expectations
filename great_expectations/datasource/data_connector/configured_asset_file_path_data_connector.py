...
class ConfiguredAssetFilePathDataConnector(FilePathDataConnector):
    ...
    def __init__(self, name: str, datasource_name: str, assets: dict, execution_engine: Optional[ExecutionEngine]=None, default_regex: Optional[dict]=None, sorters: Optional[list]=None, batch_spec_passthrough: Optional[dict]=None) -> None:
        import inspect
    ...
    def _refresh_data_references_cache(self) -> None:
        ...
        for data_asset_name in self.get_available_data_asset_names():
            ...
            for data_reference in self._get_data_reference_list(data_asset_name=data_asset_name):
                mapped_batch_definition_list: List[BatchDefinition] = self._map_data_reference_to_batch_definition_list(data_reference=data_reference, data_asset_name=data_asset_name)
    ...
    def get_data_reference_list_count(self) -> int:
        ...
        total_references: int = sum((len(self._data_references_cache[data_asset_name]) for data_asset_name in self._data_references_cache))
    ...
    def _get_batch_definition_list_from_cache(self) -> List[BatchDefinition]:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        batch_definition_list: List[BatchDefinition] = [batch_definitions[0] for data_reference_sub_cache in self._data_references_cache.values() for batch_definitions in data_reference_sub_cache.values() if (batch_definitions is not None)]
    ...
    def build_batch_spec(self, batch_definition: BatchDefinition) -> PathBatchSpec:
        ...
        if ((data_asset_name in self.assets) and self.assets[data_asset_name].batch_spec_passthrough and isinstance(self.assets[data_asset_name].batch_spec_passthrough, dict)):
