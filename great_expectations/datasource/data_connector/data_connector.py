...
from great_expectations.core.batch import BatchDefinition, BatchMarkers, BatchRequestBase
...
class DataConnector():
    ...
    def __init__(self, name: str, datasource_name: str, execution_engine: ExecutionEngine, batch_spec_passthrough: Optional[dict]=None) -> None:
        import inspect
        ...
        if (execution_engine is None):
            raise ge_exceptions.DataConnectorError('A non-existent/unknown ExecutionEngine instance was referenced.')
        self._name = name
    ...
    def get_batch_data_and_metadata(self, batch_definition: BatchDefinition) -> Tuple[(Any, BatchSpec, BatchMarkers)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def build_batch_spec(self, batch_definition: BatchDefinition) -> BatchSpec:
        ...
        batch_spec_params: dict = self._generate_batch_spec_parameters_from_batch_definition(batch_definition=batch_definition)
    ...
    def get_batch_definition_list_from_batch_request(self, batch_request: BatchRequestBase) -> List[BatchDefinition]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _map_data_reference_to_batch_definition_list(self, data_reference: Any, data_asset_name: Optional[str]=None) -> Optional[List[BatchDefinition]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def self_check(self, pretty_print=True, max_examples=3):
        ...
        report_obj = {'class_name': self.__class__.__name__, 'data_asset_count': len_asset_names, 'example_data_asset_names': asset_names[:max_examples], 'data_assets': {}}
        if pretty_print:
        ...
        for asset_name in asset_names[:max_examples]:
            data_reference_list = self._get_data_reference_list_from_cache_by_data_asset_name(data_asset_name=asset_name)
            ...
            report_obj['data_assets'][asset_name] = {'batch_definition_count': len_batch_definition_list, 'example_data_references': example_data_references}
...
''')
    ...
    def _self_check_fetch_batch(self, pretty_print: bool, example_data_reference: Any, data_asset_name: str):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if pretty_print:
            print('\n\t\tFetching batch data...')
        batch_definition_list: List[BatchDefinition] = self._map_data_reference_to_batch_definition_list(data_reference=example_data_reference, data_asset_name=data_asset_name)
        ...
        data: Any = validator.get_metric(metric=MetricConfiguration(metric_name='table.head', metric_domain_kwargs={'batch_id': batch_definition.id}, metric_value_kwargs={'n_rows': 5}))
        n_rows: int = validator.get_metric(metric=MetricConfiguration(metric_name='table.row_count', metric_domain_kwargs={'batch_id': batch_definition.id}))
    ...
    def _validate_batch_request(self, batch_request: BatchRequestBase) -> None:
        ...
        if (batch_request.datasource_name != self.datasource_name):
            raise ValueError(f'datasource_name in BatchRequest: "{batch_request.datasource_name}" does not match DataConnector datasource_name: "{self.datasource_name}".')
        if (batch_request.data_connector_name != self.name):
            raise ValueError(f'data_connector_name in BatchRequest: "{batch_request.data_connector_name}" does not match DataConnector name: "{self.name}".')
