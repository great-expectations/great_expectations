...
def batch_definition_matches_batch_request(
    ...
    if batch_request.datasource_name and (
        batch_request.datasource_name != batch_definition.datasource_name
    ):
        return False
    if batch_request.data_connector_name and (
        batch_request.data_connector_name != batch_definition.data_connector_name
    ):
        return False
    if batch_request.data_asset_name and (
        batch_request.data_asset_name != batch_definition.data_asset_name
    ):
        return False
...
def list_gcs_keys(gcs, query_options: dict, recursive: bool = False) -> List[str]:
    ...
    if (delimiter is None) and (not recursive):
        warnings.warn(
            'In order to access blobs with a ConfiguredAssetGCSDataConnector,             the delimiter that has been passed to gcs_options in your config cannot be empty;             please note that the value is being set to the default "/" in order to work with the Google SDK.'
        )
        query_options["delimiter"] = "/"
    elif (delimiter is not None) and recursive:
        warnings.warn(
            "In order to access blobs with an InferredAssetGCSDataConnector,             the delimiter that has been passed to gcs_options in your config must be empty;             please note that the value is being set to None in order to work with the Google SDK."
        )
...
def _build_asset_from_config(
    ...
    asset: Asset = instantiate_class_from_config(
        config=config, runtime_environment=runtime_environment, config_defaults={}
    )
    if not asset:
