...
class RuntimeDataConnector(DataConnector):
    ...
    def get_batch_data_and_metadata(
        self, batch_definition: BatchDefinition, runtime_parameters: dict
    ...
    def _validate_runtime_parameters(
        ...
        if len(keys_present) != 1:
            raise ge_exceptions.InvalidBatchRequestError(
                "The runtime_parameters dict must have one (and only one) of the following keys: 'batch_data', 'query', 'path'."
            )
