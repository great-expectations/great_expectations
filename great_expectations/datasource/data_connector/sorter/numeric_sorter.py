...
class NumericSorter(Sorter):
    ...
    def get_batch_key(self, batch_definition: BatchDefinition) -> Any:
        ...
        if (not is_numeric(value=batch_value)):
            raise ge_exceptions.SorterError(f'''BatchDefinition with IDDict "{self.name}" with value "{batch_value}" has value
    ...
    def __repr__(self) -> str:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        doc_fields_dict: dict = {'name': self.name, 'reverse': self.reverse, 'type': 'NumericSorter'}
