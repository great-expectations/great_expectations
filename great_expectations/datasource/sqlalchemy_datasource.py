        ...
        if "connection_string" in credentials:
            ...
            if "private_key_path" in credentials:
                (
                    options,
                    create_engine_kwargs,
                ) = self._get_sqlalchemy_key_pair_auth_url(drivername, credentials)
            else:
    ...
    def get_batch(self, batch_kwargs, batch_parameters=None):
        ...
        if "query" in batch_kwargs:
            if ("limit" in batch_kwargs) or ("offset" in batch_kwargs):
                logger.warning(
                    "Limit and offset parameters are ignored when using query-based batch_kwargs; consider adding limit and offset directly to the generated query."
                )
