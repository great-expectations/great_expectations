...
import logging
import warnings
from great_expectations.data_context.util import instantiate_class_from_config, load_class, verify_dynamic_loading_support
...
class LegacyDatasource():
    ...
    @classmethod
    def build_configuration(cls, class_name, module_name='great_expectations.datasource', data_asset_type=None, batch_kwargs_generators=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def __init__(self, name, data_context=None, data_asset_type=None, batch_kwargs_generators=None, **kwargs) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if isinstance(data_asset_type, str):
            warnings.warn('String-only configuration for data_asset_type is deprecated as of v0.7.11. As support will be removed in v0.16, please use module_name and class_name instead.', DeprecationWarning)
    ...
    def _build_batch_kwargs_generator(self, **kwargs):
        ...
        generator = instantiate_class_from_config(config=kwargs, runtime_environment={'datasource': self}, config_defaults={'module_name': 'great_expectations.datasource.batch_kwargs_generator'})
        if (not generator):
            raise ClassInstantiationError(module_name='great_expectations.datasource.batch_kwargs_generator', package_name=None, class_name=kwargs['class_name'])
        return generator
    ...
    def get_batch_kwargs_generator(self, name):
        ...
        if (name in self._batch_kwargs_generators):
            ...
            raise ValueError(('Unable to load batch kwargs generator %s -- no configuration found or invalid configuration.' % name))
