...
class InvalidBaseYamlConfigError(GreatExpectationsValidationError):
    ...
    def __init__(self, message, validation_error=None, field_name=None) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        if (validation_error is not None):
            if (validation_error and validation_error.messages and isinstance(validation_error.messages, dict) and all([(key is None) for key in validation_error.messages.keys()])):
                validation_error.messages = list(itertools.chain.from_iterable(validation_error.messages.values()))
...
class PluginClassNotFoundError(DataContextError, AttributeError):
    ...
    def __init__(self, module_name, class_name) -> None:
        ...
        if class_name_changes.get(class_name):
            ...
            self.cli_colored_message = colored_template.format(module_snippet, class_snippet, class_snippet, new_class_snippet)
        else:
            self.cli_colored_message = colored_template.format(module_snippet, class_snippet, class_snippet)
        super().__init__(self.message)
    ...
    def __init__(self, datasource_name, message) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.message = 'Cannot initialize datasource {}, error: {}'.format(datasource_name, message)
        super().__init__(self.message)
