...
class SuiteEditNotebookRenderer(BaseNotebookRenderer):
    ...
    def __init__(self, custom_templates_module: Optional[str]=None, header_markdown: Optional[NotebookTemplateConfig]=None, footer_markdown: Optional[NotebookTemplateConfig]=None, table_expectations_header_markdown: Optional[NotebookTemplateConfig]=None, column_expectations_header_markdown: Optional[NotebookTemplateConfig]=None, table_expectations_not_found_markdown: Optional[NotebookTemplateConfig]=None, column_expectations_not_found_markdown: Optional[NotebookTemplateConfig]=None, authoring_intro_markdown: Optional[NotebookTemplateConfig]=None, column_expectations_markdown: Optional[NotebookTemplateConfig]=None, header_code: Optional[NotebookTemplateConfig]=None, footer_code: Optional[NotebookTemplateConfig]=None, column_expectation_code: Optional[NotebookTemplateConfig]=None, table_expectation_code: Optional[NotebookTemplateConfig]=None, context: Optional[DataContext]=None) -> None:
        ...
        loaders = (custom_loader + [jinja2.PackageLoader('great_expectations.render.notebook_assets', 'suite_edit')])
    ...
    def from_data_context(data_context):
        ...
        return instantiate_class_from_config(config=(suite_edit_notebook_config.__dict__ if suite_edit_notebook_config else {'module_name': 'great_expectations.render.renderer.suite_edit_notebook_renderer', 'class_name': 'SuiteEditNotebookRenderer'}), runtime_environment={'context': data_context}, config_defaults={})
    ...
    def add_header(self, suite_name: str, batch_kwargs, suite_notes: Optional[dict]) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not batch_kwargs):
            batch_kwargs = {}
        code = self.render_with_overwrite(self.header_code, 'header.py.j2', suite_notes=suite_notes, suite_name=suite_name, batch_kwargs=batch_kwargs, env=os.environ)
    ...
    def _add_column_level_expectations(self, expectations_by_column):
        ...
        for (column, expectations) in expectations_by_column.items():
            ...
            self.add_markdown_cell(markdown)
            for exp in expectations:
                code = self.render_with_overwrite(self.column_expectation_code, 'column_expectation.py.j2', expectation=exp, kwargs_string=self._build_kwargs_string(exp), meta_args=self._build_meta_arguments(exp.meta))
                self.add_code_cell(code, lint=True)
    ...
    def _add_table_level_expectations(self, expectations_by_column):
        ...
        for exp in expectations_by_column['table_expectations']:
            code = self.render_with_overwrite(self.table_expectation_code, 'table_expectation.py.j2', expectation=exp, kwargs_string=self._build_kwargs_string(exp), meta_args=self._build_meta_arguments(exp.meta))
            self.add_code_cell(code, lint=True)
    ...
    def render(self, suite: ExpectationSuite, batch_kwargs=None) -> nbformat.NotebookNode:
        ...
        self.add_header(suite_name, batch_kwargs, suite_notes=suite.meta.get('notes', None))
