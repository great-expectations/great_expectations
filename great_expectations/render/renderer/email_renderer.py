...
class EmailRenderer(Renderer):
    ...
    def render(self, validation_result=None, data_docs_pages=None, notify_with=None):
        ...
        if validation_result:
            ...
            html = textwrap.dedent(f'''                <p><strong>Batch Validation Status</strong>: {status}</p>
                ...
                if (notify_with is not None):
                    for docs_link_key in notify_with:
                        if (docs_link_key in data_docs_pages.keys()):
                            ...
                            report_element = str(f'<strong>ERROR</strong>: The email is trying to provide a link to the following DataDocs: `{str(docs_link_key)}`, but it is not configured under data_docs_sites in the great_expectations.yml</br>')
        ...
        if docs_link:
            try:
                if ('file://' in docs_link):
                    report_element = str(f'<p><strong>DataDocs</strong> can be found here: <a href="{docs_link}">{docs_link}</a>.</br>(Please copy and paste link into a browser to view)</p>')
                else:
                    report_element = f'<p><strong>DataDocs</strong> can be found here: <a href="{docs_link}">{docs_link}</a>.</p>'
            except Exception as e:
                logger.warning(f'''EmailRenderer had a problem with generating the docs link.
