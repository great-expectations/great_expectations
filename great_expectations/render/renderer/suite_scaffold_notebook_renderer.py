...
class SuiteScaffoldNotebookRenderer(SuiteEditNotebookRenderer):
    ...
    def __init__(self, context: DataContext, suite: ExpectationSuite, batch_kwargs) -> None:
        import inspect
    ...
    def add_header(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell(f'''# Scaffold a new Expectation Suite (Experimental)
...
We'd love it if you'd **reach out to us on** the [**Great Expectations Slack Channel**](https://greatexpectations.io/slack)!''')
        if (not self.batch_kwargs):
            self.batch_kwargs = {}
        self.add_code_cell(f'''import great_expectations as ge
...
{columns}
    ...
    def add_footer(self) -> None:
        ...
        self.add_markdown_cell(f'''## Next steps
...
This can be done by running `great_expectations suite edit {self.suite_name}`.''')
    ...
    def load_batch(self):
        ...
        assert isinstance(batch, Dataset), 'Batch failed to load. Please check your batch_kwargs'
        return batch
    ...
    def _add_scaffold_cell(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_code_cell('profiler = UserConfigurableProfiler(profile_dataset=batch,\n    ignored_columns=ignored_columns,\n    excluded_expectations=None,\n    not_null_only=False,\n    primary_or_compound_key=False,\n    semantic_types_dict=None,\n    table_expectations_only=False,\n    value_set_threshold="MANY",\n    )\n\nsuite = profiler.build_suite()')
