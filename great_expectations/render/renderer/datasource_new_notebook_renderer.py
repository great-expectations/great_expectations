...
class DatasourceNewNotebookRenderer(BaseNotebookRenderer):
    ...
    def __init__(self, context: DataContext, datasource_type: DatasourceTypes, datasource_yaml: str, datasource_name: str='my_datasource', sql_credentials_snippet: Optional[str]=None) -> None:
        import inspect
    ...
    def _add_template_cell(self, lint: bool=True) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_code_cell(f'''example_yaml = {self.datasource_yaml}
print(example_yaml)''', lint=lint)
    ...
    def _add_test_yaml_cells(self, lint: bool=True) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell('# Test Your Datasource Configuration\nHere we will test your Datasource configuration to make sure it is valid.\n\nThis `test_yaml_config()` function is meant to enable fast dev loops. **If your\nconfiguration is correct, this cell will show you some snippets of the data\nassets in the data source.** You can continually edit your Datasource config\nyaml and re-run the cell to check until the new config is valid.\n\nIf you instead wish to use python instead of yaml to configure your Datasource,\nyou can use `context.add_datasource()` and specify all the required parameters.')
        self.add_code_cell('context.test_yaml_config(yaml_config=example_yaml)', lint=lint)
    ...
    def _add_save_datasource_cell(self, lint: bool=True) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell('## Save Your Datasource Configuration\nHere we will save your Datasource in your Data Context once you are satisfied with the configuration. Note that `overwrite_existing` defaults to False, but you may change it to True if you wish to overwrite. Please note that if you wish to include comments you must add them directly to your `great_expectations.yml`.')
        self.add_code_cell('sanitize_yaml_and_save_datasource(context, example_yaml, overwrite_existing=False)\ncontext.list_datasources()', lint=lint)
