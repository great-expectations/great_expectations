...
class Renderer():
    ...
    def _find_all_evrs_by_type(cls, evrs, type_, column_=None):
        ...
        ret = []
        for evr in evrs:
            if ((evr.expectation_config.expectation_type == type_) and ((not column_) or (column_ == evr.expectation_config.kwargs.get('column')))):
                ret.append(evr)
        return ret
    ...
    def _get_column_list_from_evrs(cls, evrs):
        ...
        sorted_columns = sorted(list({evr.expectation_config.kwargs['column'] for evr in evrs_ if ('column' in evr.expectation_config.kwargs)}))
