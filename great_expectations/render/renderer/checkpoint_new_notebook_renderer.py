        ...
        for (datasource_name, datasource) in self.context.datasources.items():
            for (data_connector_name, data_connector) in datasource.data_connectors.items():
                if (len(data_connector.get_available_data_asset_names()) > 0):
                    datasource_candidate = {'datasource_name': datasource_name, 'data_connector_name': data_connector_name, 'asset_name': list(data_connector.get_available_data_asset_names())[0]}
                    break
    ...
    def _add_header(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell(f'''# Create Your Checkpoint
...
**Checkpoint Name**: `{self.checkpoint_name}`''')
    ...
    def _add_imports(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_code_cell('from ruamel.yaml import YAML\nimport great_expectations as ge\nfrom pprint import pprint\n\nyaml = YAML()\ncontext = ge.get_context()\n', lint=True)
    ...
    def _add_optional_customize_your_config(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell('# Customize Your Configuration\nThe following cells show examples for listing your current configuration. You can replace values in the sample configuration with these values to customize your Checkpoint.')
        self.add_code_cell('# Run this cell to print out the names of your Datasources, Data Connectors and Data Assets\npprint(context.get_available_data_asset_names())', lint=True)
    ...
    def _add_sample_checkpoint_config(self) -> None:
        ...
        try:
            ...
            first_data_connector_name = first_datasource_with_asset['data_connector_name']
            first_asset_name = first_datasource_with_asset['asset_name']
            first_expectation_suite = self.context.list_expectation_suites()[0]
            first_expectation_suite_name = first_expectation_suite.expectation_suite_name
...
'''
            sample_yaml_str += '"""'
            sample_yaml_str += '\nprint(yaml_config)'
            self.add_code_cell(sample_yaml_str, lint=True)
        except:
            self.add_markdown_cell("Sorry, we were unable to create a sample configuration. Perhaps you don't have a Datasource or Expectation Suite configured.")
    ...
    def _add_test_and_save_your_checkpoint_configuration(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell('# Test Your Checkpoint Configuration\nHere we will test your Checkpoint configuration to make sure it is valid.\n\nThis `test_yaml_config()` function is meant to enable fast dev loops. If your configuration is correct, this cell will show a message that you successfully instantiated a Checkpoint. You can continually edit your Checkpoint config yaml and re-run the cell to check until the new config is valid.\n\nIf you instead wish to use python instead of yaml to configure your Checkpoint, you can use `context.add_checkpoint()` and specify all the required parameters.')
        self.add_code_cell('my_checkpoint = context.test_yaml_config(yaml_config=yaml_config)', lint=True)
    ...
    def _add_review_checkpoint(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell('# Review Your Checkpoint\n\nYou can run the following cell to print out the full yaml configuration. For example, if you used **SimpleCheckpoint**  this will show you the default action list.')
    ...
    def _add_add_checkpoint(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell('# Add Your Checkpoint\n\nRun the following cell to save this Checkpoint to your Checkpoint Store.')
        self.add_code_cell('context.add_checkpoint(**yaml.load(yaml_config))', lint=True)
    ...
    def _add_optional_run_checkpoint(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self.add_markdown_cell('# Run Your Checkpoint & Open Data Docs(Optional)\n\nYou may wish to run the Checkpoint now and review its output in Data Docs. If so uncomment and run the following cell.')
        self.add_code_cell('# context.run_checkpoint(checkpoint_name=my_checkpoint_name)\n# context.open_data_docs()', lint=True)
