...
def substitute_none_for_missing(kwargs, kwarg_list):
    ...
    """
    # Note: This is an exact dupe of bullet_list_content_block.substitute_none_for_missing
    # TODO: Factor this out into util.py
...
class EvrDescriptiveFailureStylableStringTemplates(object):

    @classmethod
    def _get_expectation_type(cls, evr):
        return evr["expectation_config"]["expectation_type"]

    @classmethod
    def render(cls, evr, **kwargs):
        #assert type(evr) == ExpectationValidationResult

        expectation_type = cls._get_expectation_type(evr)

        template_fn = getattr(cls, expectation_type, None)
        if template_fn is not None:
            new_stylable_string_template = template_fn(
                evr,
                # styling=cls._get_element_styling(),
                **kwargs
            )
            return new_stylable_string_template

        else:
            new_stylable_string_template = cls._missing_template_fn(
                evr,
                # cls._get_content_block_styling(),
                **kwargs
            )
            new_stylable_string_template += new_stylable_string_template
    ...
    def expect_column_values_to_be_in_type_list(cls, evr, styling=None, include_column_name=True):
        ...
        for i, v in enumerate(evr["result"]["partial_unexpected_list"]):
            params["v__"+str(i)] = v
        values_string = " ".join(
            ["$v__"+str(i) for i, v in enumerate(
                evr["result"]["partial_unexpected_list"])]
        )

        if include_column_name:
            # NOTE: Localization will be tricky for this template_str.
            template_str = "$column contains unexpected types: "+values_string+"."
        else:
            # NOTE: Localization will be tricky for this template_str.
            template_str = "values must belong to this set: "+values_string+"."
