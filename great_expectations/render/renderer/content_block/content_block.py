...
from great_expectations.expectations.registry import _registered_renderers, get_renderer_impl
from great_expectations.render.renderer.renderer import Renderer
from great_expectations.render.types import CollapseContent, RenderedMarkdownContent, RenderedStringTemplateContent, TextContent
...
class ContentBlockRenderer(Renderer):
    ...
    def render(cls, render_object: Any, **kwargs) -> Union[(_rendered_component_type, Any, None)]:
        ...
        runtime_configuration = {'styling': cls._get_element_styling(), 'include_column_name': kwargs.pop('include_column_name', None)}
        render_fn: Callable
        if isinstance(render_object, list):
            render_fn = cls._render_list
        else:
            render_fn = cls._render_other
        result = render_fn(render_object, exception_list_content_block, runtime_configuration, data_docs_exception_message, kwargs)
        return result

    @classmethod
    def _render_list(cls, render_object: list, exception_list_content_block: bool, runtime_configuration: dict, data_docs_exception_message: str, kwargs: dict) -> Optional[_rendered_component_type]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for obj_ in render_object:
            ...
            if ((content_block_fn is not None) and (not exception_list_content_block)):
                try:
                    if isinstance(obj_, ExpectationValidationResult):
                        ...
                        result = content_block_fn(configuration=expectation_config, result=obj_, runtime_configuration=runtime_configuration, **kwargs)
                    else:
                        result = content_block_fn(configuration=obj_, runtime_configuration=runtime_configuration, **kwargs)
                except Exception as e:
                    exception_traceback = traceback.format_exc()
                    exception_message = (data_docs_exception_message + f'{type(e).__name__}: "{str(e)}".  Traceback: "{exception_traceback}".')
                ...
                content_block_fn = (cls._missing_content_block_fn if exception_list_content_block else cls._get_content_block_fn('_missing_content_block_fn'))
                expectation_config = obj_.expectation_config
                result = content_block_fn(configuration=expectation_config, result=obj_, runtime_configuration=runtime_configuration, **kwargs)
            else:
                content_block_fn = cls._missing_content_block_fn
            ...
            if (result is not None):
                if isinstance(obj_, ExpectationConfiguration):
                    ...
                    horizontal_rule = RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': '', 'tag': 'hr', 'styling': {'classes': ['mt-1', 'mb-1']}}, 'styling': {'parent': {'styles': {'list-style-type': 'none'}}}})
                    result.append(horizontal_rule)
                blocks += result
        if (len(blocks) > 0):
            rendered_component_type_init_kwargs = {cls._content_block_type: blocks, 'styling': cls._get_content_block_styling()}
            rendered_component_type_default_init_kwargs = getattr(cls, '_rendered_component_default_init_kwargs', {})
            ...
            cls._process_content_block(content_block, has_failed_evr=has_failed_evr, render_object=render_object)
            return content_block
        else:
            return None

    @classmethod
    def _render_other(cls, render_object: Any, exception_list_content_block: bool, runtime_configuration: dict, data_docs_exception_message: str, kwargs: dict) -> Any:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if ((content_block_fn is not None) and (not exception_list_content_block)):
            try:
                if isinstance(render_object, ExpectationValidationResult):
                    result = content_block_fn(result=render_object, runtime_configuration=runtime_configuration, **kwargs)
                else:
                    result = content_block_fn(configuration=render_object, runtime_configuration=runtime_configuration, **kwargs)
            except Exception as e:
                exception_traceback = traceback.format_exc()
                exception_message = (data_docs_exception_message + f'{type(e).__name__}: "{str(e)}".  Traceback: "{exception_traceback}".')
                logger.error(exception_message)
    ...
    def _render_expectation_meta_notes(cls, expectation):
        ...
        if (not expectation.meta.get('notes')):
            ...
            collapse_link = RenderedStringTemplateContent(**{'content_block_type': 'string_template', 'string_template': {'template': '$icon', 'params': {'icon': ''}, 'styling': {'params': {'icon': {'classes': ['fas', 'fa-comment', 'text-info'], 'tag': 'i'}}}}})
            ...
            if isinstance(notes, str):
                ...
                if ('format' in notes):
                    if (notes['format'] == 'string'):
                        ...
                        if isinstance(notes['content'], str):
                            note_content = [RenderedMarkdownContent(**{'content_block_type': 'markdown', 'markdown': notes['content'], 'styling': {'parent': {'styles': {'color': 'red'}}}})]
                        elif isinstance(notes['content'], list):
                    ...
                    logger.warning('Unrecognized Expectation suite notes format. Skipping rendering.')
            notes_block = TextContent(**{'content_block_type': 'text', 'subheader': 'Notes:', 'text': note_content, 'styling': {'classes': ['col-12', 'mt-2', 'mb-2'], 'parent': {'styles': {'list-style-type': 'none'}}}})
            return CollapseContent(**{'collapse_toggle_link': collapse_link, 'collapse': [notes_block], 'inline_link': True, 'styling': {'body': {'classes': ['card', 'card-body', 'p-1']}, 'parent': {'styles': {'list-style-type': 'none'}}}})
