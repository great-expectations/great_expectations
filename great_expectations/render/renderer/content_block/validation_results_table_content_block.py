...
from great_expectations.expectations.core.expect_column_kl_divergence_to_be_less_than import ExpectColumnKlDivergenceToBeLessThan
...
class ValidationResultsTableContentBlockRenderer(ExpectationStringRenderer):
    ...
    _default_element_styling = {'default': {'classes': ['badge', 'badge-secondary']}, 'params': {'column': {'classes': ['badge', 'badge-primary']}}}
    _default_content_block_styling = {'body': {'classes': ['table']}, 'classes': ['ml-2', 'mr-2', 'mt-0', 'mb-0', 'table-responsive']}
    ...
    def _get_custom_columns(cls, validation_results):
        ...
        custom_columns = []
        if ((len(validation_results) > 0) and ('meta_properties_to_render' in validation_results[0].expectation_config.kwargs) and (validation_results[0].expectation_config.kwargs['meta_properties_to_render'] is not None)):
    ...
    def _get_content_block_fn(cls, expectation_type):
        ...
        def row_generator_fn(configuration=None, result=None, language=None, runtime_configuration=None, **kwargs):
            ...
            expectation_string_cell = expectation_string_fn(configuration=expectation, runtime_configuration=runtime_configuration)
            status_icon_renderer = get_renderer_impl(object_name=expectation_type, renderer_type='renderer.diagnostic.status_icon')
            status_cell = ([status_icon_renderer[1](result=result)] if status_icon_renderer else [getattr(cls, '_diagnostic_status_icon_renderer')(result=result)])
            ...
            try:
                unexpected_statement_renderer = get_renderer_impl(object_name=expectation_type, renderer_type='renderer.diagnostic.unexpected_statement')
                unexpected_statement = (unexpected_statement_renderer[1](result=result) if unexpected_statement_renderer else [])
            except Exception as e:
                exception_traceback = traceback.format_exc()
                exception_message = (data_docs_exception_message + f'{type(e).__name__}: "{str(e)}".  Traceback: "{exception_traceback}".')
                logger.error(exception_message)
            try:
                unexpected_table_renderer = get_renderer_impl(object_name=expectation_type, renderer_type='renderer.diagnostic.unexpected_table')
                ...
                logger.error(exception_message)
            try:
                observed_value_renderer = get_renderer_impl(object_name=expectation_type, renderer_type='renderer.diagnostic.observed_value')
                observed_value = [(observed_value_renderer[1](result=result) if observed_value_renderer else (cls._get_legacy_v2_api_observed_value(expectation_string_fn, result) or '--'))]
            except Exception as e:
                exception_traceback = traceback.format_exc()
                exception_message = (data_docs_exception_message + f'{type(e).__name__}: "{str(e)}".  Traceback: "{exception_traceback}".')
                logger.error(exception_message)
            ...
            if (len(expectation_string_cell) > 1):
                ...
                output_row = [((status_cell + expectation_string_cell) + observed_value)]
            meta_properties_renderer = get_renderer_impl(object_name=expectation_type, renderer_type='renderer.diagnostic.meta_properties')
    ...
    def _get_legacy_v2_api_style_expectation_string_fn(cls, expectation_type):
        ...
        if (legacy_expectation_string_fn is None):
            return None
        warnings.warn('V2 API style custom rendering is deprecated as of v0.13.28 and is not fully supported anymore; As it will be removed in v0.16, please transition to V3 API and associated rendering style', DeprecationWarning)

        def expectation_string_fn_with_legacy_translation(configuration: ExpectationConfiguration, runtime_configuration: dict):
            import inspect
            __frame = inspect.currentframe()
            __file = __frame.f_code.co_filename
            __func = __frame.f_code.co_name
            ...
            return legacy_expectation_string_fn(expectation=configuration, styling=runtime_configuration.get('styling', None), include_column_name=runtime_configuration.get('include_column_name', True))
