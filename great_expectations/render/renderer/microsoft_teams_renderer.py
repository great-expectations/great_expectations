...
class MicrosoftTeamsRenderer(Renderer):
    ...
    def render(self, validation_result=None, validation_result_suite_identifier=None, data_docs_pages=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        status = 'Failed :('
        query = {'type': 'message', 'attachments': [{'contentType': 'application/vnd.microsoft.card.adaptive', 'content': {'$schema': self.MICROSOFT_TEAMS_SCHEMA_URL, 'type': 'AdaptiveCard', 'version': '1.0', 'body': [{'type': 'Container', 'height': 'auto', 'separator': 'true', 'items': [{'type': 'ColumnSet', 'columns': [{'type': 'Column', 'width': 'stretch', 'items': [{'type': 'TextBlock', 'text': 'Validation results', 'weight': 'bolder', 'size': 'large', 'wrap': 'true'}]}]}]}, {'type': 'Container', 'height': 'auto', 'separator': 'true', 'items': [{'type': 'TextBlock', 'text': default_text, 'horizontalAlignment': 'left'}]}], 'actions': []}}]}
        validation_result_elements = []
        if validation_result:
            if validation_result.success:
                status = 'Success !!!'
            status_element = self._render_validation_result_element(key='Batch validation status', value=status, validation_result=validation_result)
            ...
            if validation_result_suite_identifier:
                ...
                data_asset_name_element = self._render_validation_result_element(key='Data asset name', value=data_asset_name)
                ...
                expectation_suite_name_element = self._render_validation_result_element(key='Expectation suite name', value=expectation_suite_name)
                ...
                if isinstance(run_id, RunIdentifier):
                    ...
                    run_name = run_id
                run_name_element = self._render_validation_result_element(key='Run name', value=run_name)
                validation_result_elements.append(run_name_element)
                batch_id_element = self._render_validation_result_element(key='Batch ID', value=batch_identifier)
                validation_result_elements.append(batch_id_element)
                query['attachments'][0]['content']['body'][0]['items'][0]['columns'][0]['items'].append({'type': 'TextBlock', 'spacing': 'none', 'text': run_time.strftime('%b %d %Y %H:%M:%S'), 'isSubtle': 'true', 'wrap': 'true'})
            n_checks_succeeded = validation_result.statistics['successful_expectations']
            n_checks = validation_result.statistics['evaluated_expectations']
            check_details_text = '*{}* of *{}* expectations were met'.format(n_checks_succeeded, n_checks)
            check_details_text_element = self._render_validation_result_element(key='Summary', value=check_details_text)
    ...
    def _get_report_element(docs_link):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        report_element = {'type': 'Action.OpenUrl', 'title': 'Open data docs', 'url': docs_link}
    ...
    def _render_validation_result_element(key, value, validation_result=None):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        validation_result_element = {'type': 'TextBlock', 'text': f'**{key}:** {value}', 'horizontalAlignment': 'left'}
