...
def resource_key_passes_run_name_filter(resource_key, run_name_filter):
    ...
    if run_name_filter.get('equals'):
        ...
        warnings.warn("The 'eq' key will is deprecated as of v0.11.9 and will be removed in v0.16. Please use the renamed 'equals' key.", DeprecationWarning)
        return (run_name_filter.get('eq') == run_name)
    elif run_name_filter.get('ne'):
        warnings.warn("The 'ne' key will is deprecated as of v0.11.9 and will be removed in v0.16. Please use the renamed 'not_equals' key.", DeprecationWarning)
...
def parse_row_condition_string_pandas_engine(condition_string: str, with_schema: bool=False) -> tuple:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    template_str = 'if '
    params = {}
    condition_string = condition_string.replace('&', ' AND ').replace(' and ', ' AND ').replace('|', ' OR ').replace(' or ', ' OR ').replace('~', ' NOT ').replace(' not ', ' NOT ')
    ...
    for (i, condition) in enumerate(conditions_list):
        ...
        if with_schema:
            params[f'row_condition__{str(i)}'] = {'schema': {'type': 'string'}, 'value': param_value}
        else:
            params[f'row_condition__{str(i)}'] = param_value
            condition_string = condition_string.replace(condition, f'$row_condition__{str(i)}')
