...
class Domain(IDDict, SerializableDotDict):
    ...
    def __init__(self, domain_kwargs: Optional[Dict[str, Any]] = None):
        domain_kwargs_dot_dict: SerializableDotDict = (
            self._convert_dictionaries_to_dot_dicts(source=domain_kwargs)
        )
    ...
    ) -> Optional[Union[Any, SerializableDotDict]]:
        if source is None:
            return None

        if isinstance(source, dict):
            if not isinstance(source, SerializableDotDict):
                source = SerializableDotDict(source)
            key: str
            value: Any
