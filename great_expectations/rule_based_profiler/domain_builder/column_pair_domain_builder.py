...
class ColumnPairDomainBuilder(ColumnDomainBuilder):
    ...
    def _get_domains(self, rule_name: str, variables: Optional[ParameterContainer]=None) -> List[Domain]:
        ...
        if (not (effective_column_names and (len(effective_column_names) == 2))):
            raise ge_exceptions.ProfilerExecutionError(message=f'''Error: Columns specified for {self.__class__.__name__} in sorted order must correspond to "column_A" and "column_B" (in this exact order).
''')
        ...
        domains: List[Domain] = [Domain(domain_type=self.domain_type, domain_kwargs=domain_kwargs, details={INFERRED_SEMANTIC_TYPE_KEY: semantic_types_by_column_name}, rule_name=rule_name)]
        return domains
