...
from great_expectations.core.batch import Batch, BatchRequestBase, batch_request_contains_batch_data
from great_expectations.core.config_peer import ConfigPeer
from great_expectations.core.expectation_configuration import ExpectationConfiguration
from great_expectations.core.usage_statistics.events import UsageStatsEvents
from great_expectations.core.usage_statistics.usage_statistics import UsageStatisticsHandler, get_profiler_run_usage_statistics, usage_statistics_enabled_method
...
from great_expectations.rule_based_profiler.config.base import DomainBuilderConfig, ExpectationConfigurationBuilderConfig, ParameterBuilderConfig, RuleBasedProfilerConfig, domainBuilderConfigSchema, expectationConfigurationBuilderConfigSchema, parameterBuilderConfigSchema
from great_expectations.rule_based_profiler.domain_builder.domain_builder import DomainBuilder
from great_expectations.rule_based_profiler.expectation_configuration_builder import ExpectationConfigurationBuilder, init_rule_expectation_configuration_builders
from great_expectations.rule_based_profiler.helpers.configuration_reconciliation import DEFAULT_RECONCILATION_DIRECTIVES, ReconciliationDirectives, ReconciliationStrategy, reconcile_rule_variables
from great_expectations.rule_based_profiler.helpers.util import convert_variables_to_dict
...
from great_expectations.rule_based_profiler.types import Domain, ParameterContainer, ParameterNode, RuleState, build_parameter_container_for_variables
...
class BaseRuleBasedProfiler(ConfigPeer):
    ...
    def __init__(self, profiler_config: RuleBasedProfilerConfig, data_context: Optional['BaseDataContext']=None, usage_statistics_handler: Optional[UsageStatisticsHandler]=None) -> None:
        import inspect
        ...
        if (variables is None):
            variables = {}
        _variables: ParameterContainer = build_parameter_container_for_variables(variables_configs=variables)
    ...
    def _init_rule(self, rule_name: str, rule_config: Dict[(str, Any)]) -> Rule:
        ...
        for attr in ('domain_builder', 'expectation_configuration_builders'):
            if (attr not in rule_config):
                raise ge_exceptions.ProfilerConfigurationError(message=f'Invalid rule "{rule_name}": missing mandatory {attr}.')
        variables: Dict[(str, Any)] = rule_config.get('variables', {})
        domain_builder: DomainBuilder = RuleBasedProfiler._init_rule_domain_builder(domain_builder_config=rule_config.get('domain_builder'), data_context=self._data_context)
        parameter_builders: Optional[List[ParameterBuilder]] = init_rule_parameter_builders(parameter_builder_configs=rule_config.get('parameter_builders'), data_context=self._data_context)
        expectation_configuration_builders: List[ExpectationConfigurationBuilder] = init_rule_expectation_configuration_builders(expectation_configuration_builder_configs=rule_config.get('expectation_configuration_builders'), data_context=self._data_context)
        return Rule(name=rule_name, variables=variables, domain_builder=domain_builder, parameter_builders=parameter_builders, expectation_configuration_builders=expectation_configuration_builders)

    @staticmethod
    def _init_rule_domain_builder(domain_builder_config: dict, data_context: Optional['BaseDataContext']=None) -> DomainBuilder:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        domain_builder: DomainBuilder = instantiate_class_from_config(config=domain_builder_config, runtime_environment={'data_context': data_context}, config_defaults={'module_name': 'great_expectations.rule_based_profiler.domain_builder'})
        return domain_builder

    @usage_statistics_enabled_method(event_name=UsageStatsEvents.PROFILER_RUN.value, args_payload_fn=get_profiler_run_usage_statistics)
    def run(self, variables: Optional[Dict[(str, Any)]]=None, rules: Optional[Dict[(str, Dict[(str, Any)])]]=None, batch_list: Optional[List[Batch]]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None, recompute_existing_parameter_values: bool=False, reconciliation_directives: ReconciliationDirectives=DEFAULT_RECONCILATION_DIRECTIVES) -> RuleBasedProfilerResult:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        effective_variables: Optional[ParameterContainer] = self.reconcile_profiler_variables(variables=variables, reconciliation_strategy=reconciliation_directives.variables)
        effective_rules: List[Rule] = self.reconcile_profiler_rules(rules=rules, reconciliation_directives=reconciliation_directives)
        rule: Rule
        effective_rules_configs: Optional[Dict[(str, Dict[(str, Any)])]] = {rule.name: rule.to_json_dict() for rule in effective_rules}
        progress_bar: tqdm = tqdm(total=len(effective_rules), desc='Profiling Dataset', disable=disable)
        ...
        for rule in effective_rules:
            rule_state = rule.run(variables=effective_variables, batch_list=batch_list, batch_request=batch_request, recompute_existing_parameter_values=recompute_existing_parameter_values, reconciliation_directives=reconciliation_directives)
            self.rule_states.append(rule_state)
            progress_bar.update(1)
            progress_bar.refresh()
        progress_bar.close()
        return RuleBasedProfilerResult(fully_qualified_parameter_names_by_domain=self.get_fully_qualified_parameter_names_by_domain(), parameter_values_for_fully_qualified_parameter_names_by_domain=self.get_parameter_values_for_fully_qualified_parameter_names_by_domain(), expectation_configurations=self.get_expectation_configurations(), citation={'citation_date': convert_to_json_serializable(data=datetime.datetime.now(datetime.timezone.utc)), 'comment': 'Suite created by Rule-Based Profiler with the configuration included.', 'profiler_config': {'name': self.name, 'config_version': self.config_version, 'variables': convert_variables_to_dict(variables=self.variables), 'rules': effective_rules_configs}})
    ...
    def get_fully_qualified_parameter_names_by_domain(self) -> Dict[(Domain, List[str])]:
        ...
        for rule_state in self.rule_states:
            rule_output = RuleOutput(rule_state=rule_state)
            fully_qualified_parameter_names_by_domain.update(rule_output.get_fully_qualified_parameter_names_by_domain())
    ...
    def get_fully_qualified_parameter_names_for_domain_id(self, domain_id: str) -> List[str]:
        ...
        for rule_state in self.rule_states:
            ...
            if (domain is not None):
                ...
                return rule_output.get_fully_qualified_parameter_names_for_domain_id(domain_id=domain_id)

    def get_parameter_values_for_fully_qualified_parameter_names_by_domain(self) -> Dict[(Domain, Dict[(str, ParameterNode)])]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        values_for_fully_qualified_parameter_names_by_domain: Dict[(Domain, Dict[(str, ParameterNode)])] = {}
        rule_state: RuleState
        rule_output: RuleOutput
        for rule_state in self.rule_states:
            rule_output = RuleOutput(rule_state=rule_state)
            values_for_fully_qualified_parameter_names_by_domain.update(rule_output.get_parameter_values_for_fully_qualified_parameter_names_by_domain())
        return values_for_fully_qualified_parameter_names_by_domain

    def get_parameter_values_for_fully_qualified_parameter_names_for_domain_id(self, domain_id: str) -> Dict[(str, ParameterNode)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for rule_state in self.rule_states:
            ...
            if (domain is not None):
                rule_output: RuleOutput = RuleOutput(rule_state=rule_state)
                return rule_output.get_parameter_values_for_fully_qualified_parameter_names_for_domain_id(domain_id=domain_id)
    ...
    def add_rule(self, rule: Rule) -> None:
        ...
        effective_rules: List[Rule] = self.reconcile_profiler_rules(rules=rules_dict, reconciliation_directives=ReconciliationDirectives(domain_builder=ReconciliationStrategy.UPDATE, parameter_builder=ReconciliationStrategy.UPDATE, expectation_configuration_builder=ReconciliationStrategy.UPDATE))
        self.rules = effective_rules
        updated_rules: Optional[Dict[(str, Dict[(str, Any)])]] = {rule.name: rule.to_json_dict() for rule in effective_rules}
        self._profiler_config.rules = updated_rules

    def reconcile_profiler_variables(self, variables: Optional[Dict[(str, Any)]]=None, reconciliation_strategy: ReconciliationStrategy=DEFAULT_RECONCILATION_DIRECTIVES.variables) -> Optional[ParameterContainer]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (variables and isinstance(variables, dict)):
            variables_configs: dict = self._reconcile_profiler_variables_as_dict(variables=variables, reconciliation_strategy=reconciliation_strategy)
            effective_variables = build_parameter_container_for_variables(variables_configs=variables_configs)
        else:
            effective_variables = self.variables
        return effective_variables

    def _reconcile_profiler_variables_as_dict(self, variables: Optional[Dict[(str, Any)]], reconciliation_strategy: ReconciliationStrategy=DEFAULT_RECONCILATION_DIRECTIVES.variables) -> dict:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (variables is None):
            variables = {}
        variables_configs: Optional[Dict[(str, Any)]] = convert_variables_to_dict(variables=self.variables)
    ...
    def reconcile_profiler_rules(self, rules: Optional[Dict[(str, Dict[(str, Any)])]]=None, reconciliation_directives: ReconciliationDirectives=DEFAULT_RECONCILATION_DIRECTIVES) -> List[Rule]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        effective_rules: Dict[(str, Rule)] = self._reconcile_profiler_rules_as_dict(rules=rules, reconciliation_directives=reconciliation_directives)
        return list(effective_rules.values())

    def _reconcile_profiler_rules_as_dict(self, rules: Optional[Dict[(str, Dict[(str, Any)])]]=None, reconciliation_directives: ReconciliationDirectives=DEFAULT_RECONCILATION_DIRECTIVES) -> Dict[(str, Rule)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        rule_name: str
        rule_config: dict
        override_rule_configs: Dict[(str, Dict[(str, Any)])] = {rule_name: RuleBasedProfiler._reconcile_rule_config(existing_rules=effective_rules, rule_name=rule_name, rule_config=rule_config, reconciliation_directives=reconciliation_directives) for (rule_name, rule_config) in rules.items()}
        override_rules: Dict[(str, Rule)] = {rule_name: self._init_rule(rule_name=rule_name, rule_config=rule_config) for (rule_name, rule_config) in override_rule_configs.items()}
        effective_rules.update(override_rules)
        return effective_rules

    @staticmethod
    def _reconcile_rule_config(existing_rules: Dict[(str, Rule)], rule_name: str, rule_config: dict, reconciliation_directives: ReconciliationDirectives=DEFAULT_RECONCILATION_DIRECTIVES) -> Dict[(str, Any)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (rule_name in existing_rules):
            ...
            effective_variables: dict = reconcile_rule_variables(variables=rule.variables, variables_config=variables_config, reconciliation_strategy=reconciliation_directives.variables)
            domain_builder_config: dict = rule_config.get('domain_builder', {})
            effective_domain_builder_config: dict = RuleBasedProfiler._reconcile_rule_domain_builder_config(domain_builder=rule.domain_builder, domain_builder_config=domain_builder_config, reconciliation_strategy=reconciliation_directives.domain_builder)
            parameter_builder_configs: List[dict] = rule_config.get('parameter_builders', [])
            effective_parameter_builder_configs: Optional[List[dict]] = RuleBasedProfiler._reconcile_rule_parameter_builder_configs(rule=rule, parameter_builder_configs=parameter_builder_configs, reconciliation_strategy=reconciliation_directives.parameter_builder)
            expectation_configuration_builder_configs: List[dict] = rule_config.get('expectation_configuration_builders', [])
            effective_expectation_configuration_builder_configs: List[dict] = RuleBasedProfiler._reconcile_rule_expectation_configuration_builder_configs(rule=rule, expectation_configuration_builder_configs=expectation_configuration_builder_configs, reconciliation_strategy=reconciliation_directives.expectation_configuration_builder)
            effective_rule_config = {'variables': effective_variables, 'domain_builder': effective_domain_builder_config, 'parameter_builders': effective_parameter_builder_configs, 'expectation_configuration_builders': effective_expectation_configuration_builder_configs}
        else:
            effective_rule_config = rule_config
        return effective_rule_config

    @staticmethod
    def _reconcile_rule_domain_builder_config(domain_builder: DomainBuilder, domain_builder_config: dict, reconciliation_strategy: ReconciliationStrategy=DEFAULT_RECONCILATION_DIRECTIVES.domain_builder) -> dict:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        deserialized_config: DomainBuilderConfig = domainBuilderConfigSchema.load(domain_builder_as_dict)
        ...
        if domain_builder_config:
            ...
            if (reconciliation_strategy == ReconciliationStrategy.NESTED_UPDATE):
                effective_domain_builder_config = nested_update(effective_domain_builder_config, domain_builder_config)
            elif (reconciliation_strategy == ReconciliationStrategy.REPLACE):
    ...
    @staticmethod
    def _reconcile_rule_parameter_builder_configs(rule: Rule, parameter_builder_configs: List[dict], reconciliation_strategy: ReconciliationStrategy=DEFAULT_RECONCILATION_DIRECTIVES.parameter_builder) -> Optional[List[dict]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        current_parameter_builders: Dict[(str, ParameterBuilder)] = rule._get_parameter_builders_as_dict()
        parameter_builder_name: str
        parameter_builder: ParameterBuilder
        parameter_builder_as_dict: dict
        for (parameter_builder_name, parameter_builder) in current_parameter_builders.items():
            ...
            deserialized_config: ParameterBuilderConfig = parameterBuilderConfigSchema.load(parameter_builder_as_dict)
            serialized_config: dict = deserialized_config.to_dict()
            effective_parameter_builder_configs[parameter_builder_name] = serialized_config
        parameter_builder_configs_override: Dict[(str, dict)] = {parameter_builder_config['name']: parameter_builder_config for parameter_builder_config in parameter_builder_configs}
        if (reconciliation_strategy == ReconciliationStrategy.NESTED_UPDATE):
            effective_parameter_builder_configs = nested_update(effective_parameter_builder_configs, parameter_builder_configs_override, dedup=True)
        elif (reconciliation_strategy == ReconciliationStrategy.REPLACE):
    ...
    @staticmethod
    def _reconcile_rule_expectation_configuration_builder_configs(rule: Rule, expectation_configuration_builder_configs: List[dict], reconciliation_strategy: ReconciliationStrategy=DEFAULT_RECONCILATION_DIRECTIVES.expectation_configuration_builder) -> List[dict]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for expectation_configuration_builder_config in expectation_configuration_builder_configs:
            _validate_builder_override_config(builder_config=expectation_configuration_builder_config)
        effective_expectation_configuration_builder_configs: Dict[(str, dict)] = {}
        current_expectation_configuration_builders: Dict[(str, ExpectationConfigurationBuilder)] = rule._get_expectation_configuration_builders_as_dict()
        expectation_configuration_builder_name: str
        expectation_configuration_builder: ExpectationConfigurationBuilder
        expectation_configuration_builder_as_dict: dict
        for (expectation_configuration_builder_name, expectation_configuration_builder) in current_expectation_configuration_builders.items():
            expectation_configuration_builder_as_dict = expectation_configuration_builder.to_json_dict()
            expectation_configuration_builder_as_dict['class_name'] = expectation_configuration_builder.__class__.__name__
            expectation_configuration_builder_as_dict['module_name'] = expectation_configuration_builder.__class__.__module__
            deserialized_config: ExpectationConfigurationBuilderConfig = expectationConfigurationBuilderConfigSchema.load(expectation_configuration_builder_as_dict)
            serialized_config: dict = deserialized_config.to_dict()
            effective_expectation_configuration_builder_configs[expectation_configuration_builder_name] = serialized_config
        expectation_configuration_builder_configs_override: Dict[(str, dict)] = {expectation_configuration_builder_config['expectation_type']: expectation_configuration_builder_config for expectation_configuration_builder_config in expectation_configuration_builder_configs}
        if (reconciliation_strategy == ReconciliationStrategy.NESTED_UPDATE):
            effective_expectation_configuration_builder_configs = nested_update(effective_expectation_configuration_builder_configs, expectation_configuration_builder_configs_override, dedup=True)
        elif (reconciliation_strategy == ReconciliationStrategy.REPLACE):
            effective_expectation_configuration_builder_configs = expectation_configuration_builder_configs_override
        elif (reconciliation_strategy == ReconciliationStrategy.UPDATE):
            effective_expectation_configuration_builder_configs.update(expectation_configuration_builder_configs_override)
    ...
    @staticmethod
    def run_profiler(data_context: 'BaseDataContext', profiler_store: ProfilerStore, name: Optional[str]=None, ge_cloud_id: Optional[str]=None, variables: Optional[dict]=None, rules: Optional[dict]=None) -> RuleBasedProfilerResult:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        profiler: RuleBasedProfiler = RuleBasedProfiler.get_profiler(data_context=data_context, profiler_store=profiler_store, name=name, ge_cloud_id=ge_cloud_id)
        return profiler.run(variables=variables, rules=rules, batch_list=None, batch_request=None, recompute_existing_parameter_values=False, reconciliation_directives=DEFAULT_RECONCILATION_DIRECTIVES)

    @staticmethod
    def run_profiler_on_data(data_context: 'BaseDataContext', profiler_store: ProfilerStore, batch_list: Optional[List[Batch]]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> RuleBasedProfilerResult:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        profiler: RuleBasedProfiler = RuleBasedProfiler.get_profiler(data_context=data_context, profiler_store=profiler_store, name=name, ge_cloud_id=ge_cloud_id)
        rule: Rule
        rules: Dict[(str, Dict[(str, Any)])] = {rule.name: rule.to_json_dict() for rule in profiler.rules}
        return profiler.run(variables=None, rules=rules, batch_list=batch_list, batch_request=batch_request, recompute_existing_parameter_values=False, reconciliation_directives=DEFAULT_RECONCILATION_DIRECTIVES)

    @staticmethod
    def add_profiler(config: RuleBasedProfilerConfig, data_context: 'BaseDataContext', profiler_store: ProfilerStore, ge_cloud_id: Optional[str]=None) -> 'RuleBasedProfiler':
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not RuleBasedProfiler._check_validity_of_batch_requests_in_config(config=config)):
            raise ge_exceptions.InvalidConfigError(f'batch_data found in batch_request cannot be saved to ProfilerStore "{profiler_store.store_name}"')
        new_profiler: 'RuleBasedProfiler' = instantiate_class_from_config(config=config.to_json_dict(), runtime_environment={'data_context': data_context}, config_defaults={'module_name': 'great_expectations.rule_based_profiler', 'class_name': 'RuleBasedProfiler'})
    ...
    @staticmethod
    def _check_validity_of_batch_requests_in_config(config: RuleBasedProfilerConfig) -> bool:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @staticmethod
    def get_profiler(data_context: 'BaseDataContext', profiler_store: ProfilerStore, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> 'RuleBasedProfiler':
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        assert (bool(name) ^ bool(ge_cloud_id)), 'Must provide either name or ge_cloud_id (but not both)'
        key: Union[(GeCloudIdentifier, ConfigurationIdentifier)]
        if ge_cloud_id:
            key = GeCloudIdentifier(resource_type='contract', ge_cloud_id=ge_cloud_id)
        else:
        ...
        try:
            ...
            raise ge_exceptions.ProfilerNotFoundError(message=f'''Non-existent Profiler configuration named "{id_}".
...
Details: {exc_ik}''')
        ...
        profiler: 'RuleBasedProfiler' = instantiate_class_from_config(config=config, runtime_environment={'data_context': data_context}, config_defaults={'module_name': 'great_expectations.rule_based_profiler', 'class_name': 'RuleBasedProfiler'})
        return profiler

    @staticmethod
    def delete_profiler(profiler_store: ProfilerStore, name: Optional[str]=None, ge_cloud_id: Optional[str]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
...
RuleBasedProfiler class name: {self.name}''')
            if (not self.variables):
                print('Your current RuleBasedProfiler configuration has an empty "variables" attribute.                     Please ensure you populate it if you\'d like to reference values in your "rules" attribute.')
    ...
    def to_json_dict(self) -> dict:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        variables_dict: Optional[Dict[(str, Any)]] = convert_variables_to_dict(variables=self.variables)
        rule: Rule
        serializeable_dict: dict = {'class_name': self.__class__.__name__, 'module_name': self.__class__.__module__, 'name': self.name, 'config_version': self.config_version, 'variables': variables_dict, 'rules': [rule.to_json_dict() for rule in self.rules]}
...
class RuleBasedProfiler(BaseRuleBasedProfiler):
    ...
    def __init__(self, name: str, config_version: float, variables: Optional[Dict[(str, Any)]]=None, rules: Optional[Dict[(str, Dict[(str, Any)])]]=None, data_context: Optional['BaseDataContext']=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        profiler_config: RuleBasedProfilerConfig = RuleBasedProfilerConfig(name=name, config_version=config_version, variables=variables, rules=rules)
        ...
        super().__init__(profiler_config=profiler_config, data_context=data_context, usage_statistics_handler=usage_statistics_handler)
...
def _validate_builder_override_config(builder_config: dict) -> None:
    ...
    if (not all((isinstance(builder_config, dict), ('class_name' in builder_config), ('module_name' in builder_config)))):
        raise ge_exceptions.ProfilerConfigurationError('Both "class_name" and "module_name" must be specified.')
