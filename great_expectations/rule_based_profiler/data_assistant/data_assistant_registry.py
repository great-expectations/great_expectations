...
class DataAssistantRegistry:
    """ """

    DATA_ASSISTANTS: Attributes = Attributes(
        {
            "volume": VolumeDataAssistant,
        }
    )

    def run_data_assistant_on_data(
        self,
        name: str,
        ...
        data_assistant_cls: type = DataAssistantRegistry.DATA_ASSISTANTS.get(
            name.lower()
        )
        if data_assistant_cls is None:
            raise ge_exceptions.DataAssistantNotFoundError(
                message=f'Non-existent DataAssistant named "{name}" invoked.'
            )

        data_assistant: DataAssistant = data_assistant_cls(
            name=name,
        )
...
#         name = name.lower()
#         if name in DataAssistantRegistry.DATA_ASSISTANTS:
#
