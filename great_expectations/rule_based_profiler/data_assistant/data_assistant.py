...
from great_expectations.rule_based_profiler.helpers.configuration_reconciliation import DEFAULT_RECONCILATION_DIRECTIVES
from great_expectations.rule_based_profiler.helpers.util import convert_variables_to_dict
from great_expectations.rule_based_profiler.parameter_builder import ParameterBuilder
from great_expectations.rule_based_profiler.rule import Rule
from great_expectations.rule_based_profiler.rule_based_profiler import BaseRuleBasedProfiler, RuleBasedProfiler
...
class DataAssistant(metaclass=MetaDataAssistant):
    ...
    def __init__(self, name: str, validator: Validator) -> None:
        ...
        self._profiler = RuleBasedProfiler(name=self.name, config_version=1.0, variables=None, data_context=self._validator.data_context)
    ...
    def _build_profiler(self) -> None:
        ...
        for (expectation_type, expectation_kwargs) in self.expectation_kwargs_by_expectation_type.items():
            profiler = self._validator.build_rule_based_profiler_for_expectation(expectation_type=expectation_type)(**expectation_kwargs)
            ...
            for rule in rules:
                ...
                parameter_builders.extend(self.metrics_parameter_builders_by_domain[Domain(domain_builder.domain_type)])
                expectation_configuration_builders = (rule.expectation_configuration_builders or [])
                self.profiler.add_rule(rule=Rule(name=rule.name, variables=rule.variables, domain_builder=domain_builder, parameter_builders=parameter_builders, expectation_configuration_builders=expectation_configuration_builders))
        self.profiler.variables = self.profiler.reconcile_profiler_variables(variables=variables, reconciliation_strategy=DEFAULT_RECONCILATION_DIRECTIVES.variables)
    ...
    def run(self) -> DataAssistantResult:
        ...
        data_assistant_result: DataAssistantResult = DataAssistantResult(batch_id_to_batch_identifier_display_name_map=self.batch_id_to_batch_identifier_display_name_map(), execution_time=0.0)
        run_profiler_on_data(data_assistant=self, data_assistant_result=data_assistant_result, profiler=self.profiler, variables=self.variables, rules=self.rules, batch_list=list(self._batches.values()), batch_request=None)
    ...
    def get_metrics_by_domain(self) -> Dict[(Domain, Dict[(str, ParameterNode)])]:
        ...
        domain_key: Domain
        parameter_values_for_fully_qualified_parameter_names_by_domain: Dict[(Domain, Dict[(str, ParameterNode)])] = dict(filter((lambda element: any((element[0].is_superset(other=domain_key) for domain_key in list(self.metrics_parameter_builders_by_domain.keys())))), self.profiler.get_parameter_values_for_fully_qualified_parameter_names_by_domain().items()))
        domain: Domain
        parameter_builders: List[ParameterBuilder]
        parameter_builder: ParameterBuilder
        fully_qualified_metrics_parameter_names_by_domain: Dict[(Domain, List[str])] = {domain: [parameter_builder.fully_qualified_parameter_name for parameter_builder in parameter_builders] for (domain, parameter_builders) in self.metrics_parameter_builders_by_domain.items()}
        parameter_values_for_fully_qualified_parameter_names: Dict[(str, ParameterNode)]
        fully_qualified_metrics_parameter_names: List[str]
        parameter_values_for_fully_qualified_parameter_names_by_domain = {domain: dict(filter((lambda element: (element[0] in fully_qualified_metrics_parameter_names_by_domain[domain_key])), parameter_values_for_fully_qualified_parameter_names.items())) for (domain_key, fully_qualified_metrics_parameter_names) in fully_qualified_metrics_parameter_names_by_domain.items() for (domain, parameter_values_for_fully_qualified_parameter_names) in parameter_values_for_fully_qualified_parameter_names_by_domain.items() if domain.is_superset(domain_key)}
        return parameter_values_for_fully_qualified_parameter_names_by_domain

    def batch_id_to_batch_identifier_display_name_map(self) -> Dict[(str, Set[Tuple[(str, Any)]])]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        batch_id: str
        batch: Batch
        return {batch_id: set(batch.batch_definition.batch_identifiers.items()) for (batch_id, batch) in self._batches.items()}

@measure_execution_time(execution_time_holder_object_reference_name='data_assistant_result', execution_time_property_name='execution_time', pretty_print=False)
def run_profiler_on_data(data_assistant: DataAssistant, data_assistant_result: DataAssistantResult, profiler: BaseRuleBasedProfiler, variables: Optional[Dict[(str, Any)]]=None, rules: Optional[Dict[(str, Dict[(str, Any)])]]=None, batch_list: Optional[List[Batch]]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (rules is None):
        rules = []
    rule: Rule
    rules_configs: Optional[Dict[(str, Dict[(str, Any)])]] = {rule.name: rule.to_json_dict() for rule in rules}
    rule_based_profiler_result: RuleBasedProfilerResult = profiler.run(variables=variables, rules=rules_configs, batch_list=batch_list, batch_request=batch_request, recompute_existing_parameter_values=False, reconciliation_directives=DEFAULT_RECONCILATION_DIRECTIVES)
