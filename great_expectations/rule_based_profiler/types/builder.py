...
from great_expectations.core.batch import Batch, BatchRequestBase, batch_request_contains_batch_data, get_batch_request_as_dict
...
class Builder(SerializableDictDot):
    ...
    exclude_field_names: Set[str] = {'batch_list', 'batch_request', 'data_context'}
    ...
    def set_batch_list_or_batch_request(self, batch_list: Optional[List[Batch]]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (self.batch_request is None):
            self.set_batch_data(batch_list=batch_list, batch_request=batch_request)

    def set_batch_data(self, batch_list: Optional[List[Batch]]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        arg: Any
        num_supplied_batch_specification_args: int = sum([(0 if (arg is None) else 1) for arg in (batch_list, batch_request)])
        if (num_supplied_batch_specification_args > 1):
            raise ge_exceptions.ProfilerConfigurationError(f'Please pass at most one of "batch_list" and "batch_request" arguments (you passed {num_supplied_batch_specification_args} arguments).')
