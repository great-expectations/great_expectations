    ...
    def __init__(
        ...
        if isinstance(domain_type, str):
            try:
                ...
                raise ValueError(
                    f""" {e}: Cannot instantiate Domain (domain_type "{str(domain_type)}" of type "{str(type(domain_type))}" is not supported).
"""
                )
        elif not isinstance(domain_type, MetricDomainTypes):
            raise ValueError(
                f"""Cannot instantiate Domain (domain_type "{str(domain_type)}" of type "{str(type(domain_type))}" is not supported).
"""
            )
        ...
        if inferred_semantic_domain_type:
            ...
            for semantic_domain_key in inferred_semantic_domain_type:
                ...
                if not is_consistent:
                    raise ValueError(
                        f"""Cannot instantiate Domain (domain_type "{str(domain_type)}" of type "{str(type(domain_type))}" -- key "{semantic_domain_key}", detected in "{INFERRED_SEMANTIC_TYPE_KEY}" dictionary, does not exist as value of appropriate key in "domain_kwargs" dictionary.
"""
                    )
        super().__init__(
        ...
        return (other is not None) and (
            ...
            or (
                ...
                and (
                    ...
                    == deep_filter_properties_iterable(
                        properties=other, clean_falsy=True
                    )
                )
            )
