...
from pyparsing import Literal, ParseException, ParseResults, Suppress, Word, ZeroOrMore, alphanums, alphas, nums
...
VARIABLES_KEY: str = f'{VARIABLES_PREFIX}{FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER}'
...
PARAMETER_KEY: str = f'{PARAMETER_PREFIX}{FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER}'
...
RESERVED_TERMINAL_LITERALS: Set[str] = {FULLY_QUALIFIED_PARAMETER_NAME_VALUE_KEY, FULLY_QUALIFIED_PARAMETER_NAME_ATTRIBUTED_VALUE_KEY, FULLY_QUALIFIED_PARAMETER_NAME_METADATA_KEY}
attribute_naming_pattern = (Word(alphas, (alphanums + '_.')) + ZeroOrMore(((((Suppress(Literal('["')) + Word(alphas, (alphanums + '_.'))) + Suppress(Literal('"]'))) ^ ((Suppress(Literal("['")) + Word(alphas, (alphanums + '_.'))) + Suppress(Literal("']")))) ^ ((Suppress(Literal('[')) + Word((nums + '-')).setParseAction((lambda s, l, t: [int(t[0])]))) + Suppress(Literal(']'))))))
...
def is_fully_qualified_parameter_name_literal_string_format(fully_qualified_parameter_name: str) -> bool:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def validate_fully_qualified_parameter_name(fully_qualified_parameter_name: str) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    if (not is_fully_qualified_parameter_name_literal_string_format(fully_qualified_parameter_name=fully_qualified_parameter_name)):
        raise ge_exceptions.ProfilerExecutionError(message=f'''Unable to get value for parameter name "{fully_qualified_parameter_name}" -- parameter names must start with {FULLY_QUALIFIED_PARAMETER_NAME_DELIMITER_CHARACTER} (e.g., "{FULLY_QUALIFIED_PARAMETER_NAME_DELIMITER_CHARACTER}{fully_qualified_parameter_name}").
''')
...
class ParameterContainer(SerializableDictDot):
    ...
    def get_parameter_node(self, parameter_name_root: str) -> Optional[ParameterNode]:
        ...
        if (self.parameter_nodes is None):
            return None
        parameter_node: ParameterNode = convert_dictionaries_to_parameter_nodes(source=self.parameter_nodes.get(parameter_name_root))
...
def convert_dictionaries_to_parameter_nodes(source: Optional[Any]) -> Optional[ParameterNode]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if isinstance(source, dict):
        ...
        value: Any
        return source_type([convert_dictionaries_to_parameter_nodes(source=value) for value in source])
    return source
...
def build_parameter_container_for_variables(variables_configs: Dict[(str, Any)]) -> ParameterContainer:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    build_parameter_container(parameter_container=parameter_container, parameter_values=parameter_values)
    return parameter_container

def build_parameter_container(parameter_container: ParameterContainer, parameter_values: Dict[(str, Any)]) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    for (fully_qualified_parameter_name, parameter_value) in parameter_values.items():
        validate_fully_qualified_parameter_name(fully_qualified_parameter_name=fully_qualified_parameter_name)
        fully_qualified_parameter_name_as_list = fully_qualified_parameter_name[1:].split(FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER)
        ...
        if (parameter_node is None):
            ...
            parameter_container.set_parameter_node(parameter_name_root=parameter_name_root, parameter_node=parameter_node)
        _build_parameter_node_tree_for_one_parameter(parameter_node=parameter_node, parameter_name_as_list=fully_qualified_parameter_name_as_list, parameter_value=parameter_value)

def _build_parameter_node_tree_for_one_parameter(parameter_node: ParameterNode, parameter_name_as_list: List[str], parameter_value: Any) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (len(parameter_name_as_list) > 1):
        ...
        _build_parameter_node_tree_for_one_parameter(parameter_node=parameter_node[parameter_name_part], parameter_name_as_list=parameter_name_as_list[1:], parameter_value=parameter_value)
    else:
        parameter_node[parameter_name_part] = convert_dictionaries_to_parameter_nodes(source=parameter_value)

def get_parameter_value_by_fully_qualified_parameter_name(fully_qualified_parameter_name: str, domain: Optional[Domain]=None, variables: Optional[ParameterContainer]=None, parameters: Optional[Dict[(str, ParameterContainer)]]=None) -> Optional[Union[(Any, ParameterNode)]]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if fully_qualified_parameter_name.startswith(DOMAIN_KWARGS_PARAMETER_FULLY_QUALIFIED_NAME):
        if (domain and domain[DOMAIN_KWARGS_PARAMETER_NAME]):
            return domain[DOMAIN_KWARGS_PARAMETER_NAME].get(fully_qualified_parameter_name[len(f'{DOMAIN_KWARGS_PARAMETER_FULLY_QUALIFIED_NAME}{FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER}'):])
        return None
    ...
    fully_qualified_parameter_name_as_list: List[str] = fully_qualified_parameter_name.split(FULLY_QUALIFIED_PARAMETER_NAME_SEPARATOR_CHARACTER)
    if (len(fully_qualified_parameter_name_as_list) == 0):
        return None
    return _get_parameter_value_from_parameter_container(fully_qualified_parameter_name=fully_qualified_parameter_name, fully_qualified_parameter_name_as_list=fully_qualified_parameter_name_as_list, parameter_container=parameter_container)

def _get_parameter_value_from_parameter_container(fully_qualified_parameter_name: str, fully_qualified_parameter_name_as_list: List[str], parameter_container: ParameterContainer) -> Optional[Union[(Any, ParameterNode)]]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    parameter_node: Optional[ParameterNode] = parameter_container.get_parameter_node(parameter_name_root=fully_qualified_parameter_name_as_list[0])
    if (parameter_node is None):
        return None
    ...
    try:
        for parameter_name_part in fully_qualified_parameter_name_as_list:
            ...
            if (len(parsed_attribute_name) < 1):
                raise KeyError(f'''Unable to get value for parameter name "{fully_qualified_parameter_name}": Part "{parameter_name_part}" in fully-qualified parameter name does not represent a valid expression.
''')
        ...
        raise KeyError(f'''Unable to find value for parameter name "{fully_qualified_parameter_name}": Part "{parameter_name_part}" does not exist in fully-qualified parameter name.
''')
    if (attribute_value_reference not in parent_parameter_node):
        raise KeyError(f'''Unable to find value for parameter name "{fully_qualified_parameter_name}": Part "{parameter_name_part}" of fully-qualified parameter name does not exist.
''')
    return return_value

def get_parameter_values_for_fully_qualified_parameter_names(domain: Optional[Domain]=None, variables: Optional[ParameterContainer]=None, parameters: Optional[Dict[(str, ParameterContainer)]]=None) -> Dict[(str, Any)]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        ...
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    fully_qualified_parameter_name: str
    return {fully_qualified_parameter_name: get_parameter_value_by_fully_qualified_parameter_name(fully_qualified_parameter_name=fully_qualified_parameter_name, domain=domain, variables=variables, parameters=parameters) for fully_qualified_parameter_name in get_fully_qualified_parameter_names(domain=domain, variables=variables, parameters=parameters)}

def get_fully_qualified_parameter_names(domain: Optional[Domain]=None, variables: Optional[ParameterContainer]=None, parameters: Optional[Dict[(str, ParameterContainer)]]=None) -> List[str]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (not ((variables is None) or (variables.parameter_nodes is None))):
        fully_qualified_parameter_names.extend(_get_parameter_node_attribute_names(parameter_name_root=PARAMETER_NAME_ROOT_FOR_VARIABLES, parameter_node=variables.parameter_nodes[PARAMETER_NAME_ROOT_FOR_VARIABLES]))
    if (parameters is not None):
        ...
        if (not ((parameter_container is None) or (parameter_container.parameter_nodes is None))):
            parameter_name_root: str
            parameter_node: ParameterNode
            for (parameter_name_root, parameter_node) in parameter_container.parameter_nodes.items():
                fully_qualified_parameter_names.extend(_get_parameter_node_attribute_names(parameter_name_root=PARAMETER_NAME_ROOT_FOR_PARAMETERS, parameter_node=parameter_node))
    return sorted(fully_qualified_parameter_names, reverse=True)

def _get_parameter_node_attribute_names(parameter_name_root: Optional[str]=None, parameter_node: Optional[ParameterNode]=None) -> List[str]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    _get_parameter_node_attribute_names_as_lists(attribute_names_as_lists=attribute_names_as_lists, parameter_name_root_as_list=parameter_name_root_as_list, parameter_node=parameter_node)
    attribute_names: Set[str] = set()
    attribute_name: str
    for attribute_name_as_list in attribute_names_as_lists:
        attribute_name_as_list = _get_parameter_name_parts_up_to_including_reserved_literal(attribute_name_as_list=attribute_name_as_list)
...
def _get_parameter_node_attribute_names_as_lists(attribute_names_as_lists: List[List[str]], parameter_name_root_as_list: Optional[List[str]]=None, parameter_node: Optional[ParameterNode]=None) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    for (attribute_name_part, attribute_value_part) in parameter_node.items():
        ...
        if isinstance(attribute_value_part, ParameterNode):
            _get_parameter_node_attribute_names_as_lists(attribute_names_as_lists=attribute_names_as_lists, parameter_name_root_as_list=partial_parameter_name_root_as_list, parameter_node=attribute_value_part)
        else:
            attribute_names_as_lists.append(partial_parameter_name_root_as_list)

def _get_parameter_name_parts_up_to_including_reserved_literal(attribute_name_as_list: List[str]) -> List[str]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
