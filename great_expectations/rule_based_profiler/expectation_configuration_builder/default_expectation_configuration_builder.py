...
from pyparsing import ParseException, ParseResults, Suppress, Word, alphanums, alphas, infixNotation, nums, oneOf, opAssoc
...
expr = infixNotation(operand, [(comparison_operator, 2, opAssoc.LEFT), (binary_operator, 2, opAssoc.LEFT)])

class ExpectationConfigurationConditionParserError(ge_exceptions.GreatExpectationsError):
    pass
...
class DefaultExpectationConfigurationBuilder(ExpectationConfigurationBuilder):
    ...
    exclude_field_names: Set[str] = (ExpectationConfigurationBuilder.exclude_field_names | {'kwargs'})

    def __init__(self, expectation_type: str, meta: Optional[Dict[(str, Any)]]=None, condition: Optional[str]=None, validation_parameter_builder_configs: Optional[List[ParameterBuilderConfig]]=None, data_context: Optional['BaseDataContext']=None, **kwargs) -> None:
        import inspect
        ...
        super().__init__(expectation_type=expectation_type, validation_parameter_builder_configs=validation_parameter_builder_configs, data_context=data_context, **kwargs)
        if (meta is None):
            meta = {}
        self._meta = meta
        if (not isinstance(meta, dict)):
            raise ge_exceptions.ProfilerExecutionError(message=f'''Argument "{meta}" in "{self.__class__.__name__}" must be of type "dictionary" (value of type "{str(type(meta))}" was encountered).
''')
        if (condition and (not isinstance(condition, str))):
            raise ge_exceptions.ProfilerExecutionError(message=f'''Argument "{condition}" in "{self.__class__.__name__}" must be of type "string" (value of type "{str(type(condition))}" was encountered).
''')
    ...
    @property
    def validation_parameter_builder_configs(self) -> Optional[List[ParameterBuilderConfig]]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _parse_condition(self) -> ParseResults:
        ...
        try:
            ...
            raise ExpectationConfigurationConditionParserError(f'Unable to parse Expectation Configuration Condition: "{self._condition}".')

    def _substitute_parameters_and_variables(self, term_list: Union[(str, ParseResults)], domain: Domain, variables: Optional[ParameterContainer]=None, parameters: Optional[Dict[(str, ParameterContainer)]]=None) -> ParseResults:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for (idx, token) in enumerate(term_list):
            if (isinstance(token, str) and token.startswith('$')):
                term_list[idx]: Dict[(str, Any)] = get_parameter_value_and_validate_return_type(domain=domain, parameter_reference=token, expected_return_type=None, variables=variables, parameters=parameters)
            elif isinstance(token, ParseResults):
                self._substitute_parameters_and_variables(term_list=token, domain=domain, variables=variables, parameters=parameters)
        return term_list

    def _build_binary_list(self, substituted_term_list: Union[(str, ParseResults)]) -> ParseResults:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _build_boolean_result(self, binary_list: Union[(ParseResults, List[Union[(bool, str)]])]) -> bool:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for (idx, token) in enumerate(binary_list):
            if ((not isinstance(token, bool)) and (not any([isinstance(t, ParseResults) for t in token])) and (len(token) > 1)):
    ...
    def _evaluate_condition(self, parsed_condition: ParseResults, domain: Domain, variables: Optional[ParameterContainer]=None, parameters: Optional[Dict[(str, ParameterContainer)]]=None) -> bool:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        substituted_term_list: ParseResults = self._substitute_parameters_and_variables(term_list=parsed_condition, domain=domain, variables=variables, parameters=parameters)
    ...
    def _build_expectation_configuration(self, domain: Domain, variables: Optional[ParameterContainer]=None, parameters: Optional[Dict[(str, ParameterContainer)]]=None) -> Optional[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        expectation_kwargs: Dict[(str, Any)] = {parameter_name: get_parameter_value_and_validate_return_type(domain=domain, parameter_reference=fully_qualified_parameter_name, expected_return_type=None, variables=variables, parameters=parameters) for (parameter_name, fully_qualified_parameter_name) in self.kwargs.items()}
        meta: Dict[(str, Any)] = get_parameter_value_and_validate_return_type(domain=domain, parameter_reference=self.meta, expected_return_type=dict, variables=variables, parameters=parameters)
        if self._condition:
            parsed_condition: ParseResults = self._parse_condition()
            condition: bool = self._evaluate_condition(parsed_condition=parsed_condition, domain=domain, variables=variables, parameters=parameters)
            if condition:
                return ExpectationConfiguration(expectation_type=self._expectation_type, kwargs=expectation_kwargs, meta=meta)
            else:
                return None
        else:
            return ExpectationConfiguration(expectation_type=self._expectation_type, kwargs=expectation_kwargs, meta=meta)
