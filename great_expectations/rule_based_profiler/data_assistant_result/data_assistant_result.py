...
PANDAS_210_OR_GREATER = packaging.version.parse(pd.__version__) >= packaging.version.Version("2.1.0")
...
class RuleStats(SerializableDictDot):
    ...
    num_domains: int = 0
    domains_count_by_domain_type: Dict[MetricDomainTypes, int] = field(default_factory=dict)
    domains_by_domain_type: Dict[MetricDomainTypes, List[dict]] = field(default_factory=dict)
...
class DataAssistantResult(SerializableDictDot):
    ...
    _batch_id_to_batch_identifier_display_name_map: Optional[Dict[str, Set[tuple[str, Any]]]] = field(default=None)
    ...
    def get_expectation_suite(
        ...
        expectation_suite.add_expectation_configurations(
            expectation_configurations=self.expectation_configurations if self.expectation_configurations else [],
            match_type="domain",
        ...
        if not include_profiler_config:
            key: str
            value: Any
            citation = {key: value for key, value in citation.items() if key != "profiler_config"}
        if citation:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "profiler_config": self.profiler_config.to_json_dict() if self.profiler_config else None,
            "profiler_execution_time": convert_to_json_serializable(data=self.profiler_execution_time),
            "rule_domain_builder_execution_time": convert_to_json_serializable(
                data=self.rule_domain_builder_execution_time
            ),
            "rule_execution_time": convert_to_json_serializable(data=self.rule_execution_time),
            "rule_exception_tracebacks": convert_to_json_serializable(data=self.rule_exception_tracebacks),
            "metrics_by_domain": [
                {
                    "domain_id": domain.id,
    ...
    def __repr__(self) -> str:
        ...
        if in_jupyter_notebook():
            ...
            value: Any
            json_dict = {
                key: value for key, value in json_dict.items() if key in DataAssistantResult.IN_JUPYTER_NOTEBOOK_KEYS
            }
            ...
            auxiliary_profiler_execution_details: dict = self._get_auxiliary_profiler_execution_details(verbose=verbose)
    ...
    def __str__(self) -> str:
        ...
        """
        json_dict: dict = self.to_json_dict()
        auxiliary_profiler_execution_details: dict = self._get_auxiliary_profiler_execution_details(verbose=True)
        json_dict.update(auxiliary_profiler_execution_details)
        return json.dumps(json_dict, indent=2)
    ...
    def _get_metric_expectation_map(self) -> dict[tuple[str, ...], str]:
        ...
        return {
            ((metric_names,) if isinstance(metric_names, str) else metric_names): expectation_name
    ...
    def _get_auxiliary_profiler_execution_details(self, verbose: bool) -> dict:
        ...
        if verbose:
            ...
            rule_domains: List[Domain] = list(self.metrics_by_domain.keys()) if self.metrics_by_domain else []
            ...
            for rule_name, rule_config in self.profiler_config.rules.items():
                ...
                if rule_stats is None:
                    rule_stats = RuleStats(
                        ...
                        num_expectation_configuration_builders=len(rule_config["expectation_configuration_builders"]),
                    )
                    ...
                    rule_stats.rule_execution_time = (
                        self.rule_execution_time[rule_name] if self.rule_execution_time else float(np.nan)
                    )

                if num_domains > 0:
                    for domain in domains:
                        if rule_stats.domains_count_by_domain_type.get(domain.domain_type) is None:
                            rule_stats.domains_count_by_domain_type[domain.domain_type] = 0

                        if rule_stats.domains_by_domain_type.get(domain.domain_type) is None:
                        ...
                        rule_stats.domains_by_domain_type[domain.domain_type].append(domain_as_json_dict)

                auxiliary_info.update(convert_to_json_serializable(data=rule_name_to_rule_stats_map))
    ...
    def _plot(  # noqa: PLR0913
        ...
        if include_column_names is not None and exclude_column_names is not None:
            raise ValueError("You may either use `include_column_names` or `exclude_column_names` (but not both).")

        display_charts: List[Union[alt.Chart, alt.LayerChart, alt.VConcatChart]] = []
        return_charts: List[Union[alt.Chart, alt.LayerChart]] = []

        expectation_configurations: list[ExpectationConfiguration] = self.expectation_configurations or []

        table_domain_charts: List[Union[alt.Chart, alt.LayerChart]] = self._plot_table_domain_charts(
    ...
    def _display(
        ...
        themed_charts: List[alt.Chart] = self._apply_theme(charts=charts, theme=altair_theme)
        ...
        if chart_titles:
            ...
            if plot_mode == plot_mode.DIAGNOSTIC:
                expectations_produced = len(self.expectation_configurations) if self.expectation_configurations else 0
                print(
...
Use DataAssistantResult.metrics_by_domain to show all calculated Metrics"""
            ...
            if gx_font_family:
                ...
                url_font_weights: str = ";".join([str(subtitle_font_weight), str(title_font_weight)])
            ...
            try:
                ...
                try:
                    for chart_layer in layer.layer:
                        chart_title = DataAssistantResult._get_chart_layer_title(layer=chart_layer)
    ...
    def _transform_column_lists_to_rows(
        ...
        list_column_names: List[str] = list(col_has_list[col_has_list["has_list"]]["column_name"])

        if "table_columns" in list_column_names and len(np.unique(df["table_columns"])) == 1:
            ...
            ilocations: List[int] = list(np.repeat(range(df.shape[0]), df[list_column_names[0]].apply(len)))
            # replicate rows and add flattened column of lists
            columns: List[int] = [idx for idx, col in enumerate(df.columns) if col not in list_column_names]
            ...
            if "table_columns" in list_column_names:
                ...
                df_transformed["column_number"] = pd.factorize(df_transformed["table_columns"])[0] + 1
            ...
            if "value_ranges" in list_column_names:
                ...
                df_transformed[["min_value", "max_value"]] = df_transformed["value_ranges"].values.tolist()
    ...
    def _get_sequential_isotype_chart(  # noqa: PLR0913
        ...
        tooltip: List[alt.Tooltip] = batch_plot_component.generate_tooltip() + [
            metric_plot_component.generate_tooltip() for metric_plot_component in metric_plot_components
        ]
    ...
    def _get_quantitative_metrics_chart(
        ...
        metric_plot_components: List[MetricPlotComponent] = []
        for sanitized_metric_name in sanitized_metric_names:
            metric_plot_component = MetricPlotComponent(name=sanitized_metric_name, alt_type=metric_type)
            metric_plot_components.append(metric_plot_component)
    ...
    def _get_expect_domain_values_to_be_between_chart(  # noqa: C901, PLR0912, PLR0915
        ...
        if sequential:
            return DataAssistantResult._get_expect_domain_values_to_be_between_line_chart(
                expectation_type=expectation_type,
                df=df,
                metric_plot_components=metric_plot_components,
                batch_plot_component=batch_plot_component,
    ...
    def _get_interactive_metrics_chart(
        ...
        batch_name: str = "batch"
        all_columns: List[str] = DataAssistantResult._get_all_columns_from_column_dfs(column_dfs=column_dfs)
        batch_identifiers: List[str] = [
            column for column in all_columns if column not in (sanitized_metric_names | {batch_name})
        ]
        batch_type: alt.StandardType
        if sequential:
            batch_type = AltairDataTypes.ORDINAL.value
        ...
        df_columns: List[str] = [batch_name, domain_name] + list(sanitized_metric_names) + batch_identifiers

        df: pd.DataFrame = pd.DataFrame(columns=df_columns)
        for column, column_df in column_dfs:
            column_df[domain_name] = column
            df = pd.concat([df, column_df[column_df.columns.intersection(df_columns)]], axis=0)
    ...
    def _get_interactive_expect_column_values_to_be_between_chart(  # noqa: C901, PLR0912, PLR0915
        ...
        for metric_plot_component in metric_plot_components:
            if strict_min_predicate:
                min_value_predicate = alt.datum.min_value < alt.datum[metric_plot_component.name]
            else:
                min_value_predicate = alt.datum.min_value <= alt.datum[metric_plot_component.name]

            if strict_max_predicate:
                max_value_predicate = alt.datum.max_value > alt.datum[metric_plot_component.name]
            else:
                max_value_predicate = alt.datum.max_value >= alt.datum[metric_plot_component.name]
    ...
    def _get_line_chart(
        ...
        tooltip: List[alt.Tooltip] = batch_plot_component.generate_tooltip() + [
            metric_plot_component.generate_tooltip(format=",") for metric_plot_component in metric_plot_components
        ]
    ...
    def _get_expect_domain_values_to_be_between_line_chart(  # noqa: PLR0913
        ...
        expectation_kwarg_line_color: alt.HexColor = alt.HexColor(TintsAndShades.ROYAL_BLUE_30)
        ...
        for expectation_kwarg_plot_component in expectation_kwarg_plot_components:
            expectation_kwarg_tooltips.append(expectation_kwarg_plot_component.generate_tooltip())
        ...
        band: alt.Chart = (
            ...
            .encode(
                ...
                y2=alt.Y2(max_value_plot_component.name, title=max_value_plot_component.title),
            )
            .properties(title=title)
        )
        ...
        for metric_plot_component in metric_plot_components:
            # encode point color based on anomalies
            assert metric_plot_component.name, f"Metric name must be set: {metric_plot_component}"
            metric_name = metric_plot_component.name
            predicate = (
                (alt.datum.min_value > alt.datum[metric_name]) & (alt.datum.max_value > alt.datum[metric_name])
            ) | ((alt.datum.min_value < alt.datum[metric_name]) & (alt.datum.max_value < alt.datum[metric_name]))
    ...
    def _get_interactive_line_chart(
        ...
        input_dropdown: alt.binding_select = alt.binding_select(options=columns, name="Select Column: ")
        ...
        for metric_plot_component in metric_plot_components:
            ...
            line_and_points_list.append(alt.layer(line, points).add_selection(selection).transform_filter(selection))

        return alt.layer(*line_and_points_list).add_selection(selection).transform_filter(selection)
    ...
    def _get_interactive_expect_column_values_to_be_between_line_chart(  # noqa: PLR0913
        ...
        lines_and_points: alt.LayerChart = DataAssistantResult._get_interactive_line_chart(
            expectation_type=expectation_type,
            df=df,
            metric_plot_components=metric_plot_components,
            batch_plot_component=batch_plot_component,
        ...
        for idx, line_layer in enumerate(lines_and_points.layer):
            ...
            anomaly_coded_points = points.encode(color=point_color_condition, tooltip=tooltip).transform_filter(
                selection
            )
        ...
        return (alt.layer(band, lower_limit, upper_limit, lines_and_points)).add_selection(selection)
    ...
    def _get_interactive_expect_column_values_to_be_between_bar_chart(  # noqa: PLR0913
        ...
        for idx, bar_layer in enumerate(bars.layer):
            ...
            bars.layer[idx] = bar_layer.encode(color=bar_color_condition, tooltip=tooltip).transform_filter(selection)

        bars.selection = alt.Undefined
        bars.transform = alt.Undefined
        bars = bars.transform_filter(selection)

        return (alt.layer(band, lower_limit, upper_limit, bars)).add_selection(selection)
    ...
    def _get_interactive_expect_column_values_to_be_between_range_chart(  # noqa: PLR0913
        ...
        max_value_plot_component: ExpectationKwargPlotComponent
        for expectation_kwarg_plot_component in expectation_kwarg_plot_components:
            expectation_kwargs_tooltip.append(expectation_kwarg_plot_component.generate_tooltip())
            expectation_kwargs_initial_dropdown_state.append(expectation_kwarg_plot_component.name)
        ...
        lines_and_points: alt.LayerChart = DataAssistantResult._get_interactive_range_chart(
            expectation_type=expectation_type,
            df=df,
            metric_plot_components=metric_plot_components,
            batch_plot_component=batch_plot_component,
    ...
    def _plot_table_domain_charts(  # noqa: C901, PLR0913
        ...
        metric_expectation_map: dict[tuple[str, ...], str] = self._get_metric_expectation_map()

        table_based_expectations: List[str] = [
            expectation for expectation in metric_expectation_map.values() if expectation.startswith("expect_table_")
        ]
        ...
        attributed_metrics_by_table_domain: Dict[Domain, Dict[str, List[ParameterNode]]] = (
        ...
        table_domain = Domain(domain_type=MetricDomainTypes.TABLE, rule_name="table_rule")
        attributed_metrics: Dict[str, List[ParameterNode]] = attributed_metrics_by_table_domain[table_domain]
        ...
        for metric_names in table_based_metric_names:
            ...
            attributed_values = [attributed_metrics[metric_name] for metric_name in metric_names]
        ...
        if len(charts) > 1:
            sorted_charts = [charts[first_chart_idx]] + [
                chart for idx, chart in enumerate(charts) if chart is not None and idx != first_chart_idx
            ]
        else:
    ...
    def _plot_column_domain_charts(  # noqa: PLR0913
        ...
        column_based_expectation_configurations_by_type: Dict[str, List[ExpectationConfiguration]] = (
        ...
        attributed_metrics_by_column_domain: Dict[Domain, Dict[str, List[ParameterNode]]] = (
        ...
        filtered_attributed_metrics_by_column_domain: Dict[Domain, Dict[str, List[ParameterNode]]]
        ...
        for metric_names in column_based_metric_names:
            expectation_type = metric_expectation_map[metric_names]
            column_based_expectation_configurations = column_based_expectation_configurations_by_type[expectation_type]

            filtered_attributed_metrics_by_column_domain = self._filter_attributed_metrics_by_metric_names(
    ...
    def _filter_expectation_configurations_by_column_type(
        ...
        column_based_expectations: Set[str] = {
            expectation for expectation in metric_expectation_map.values() if expectation.startswith("expect_column_")
        }
        ...
        column_based_expectation_configurations_by_type: Dict[str, List[ExpectationConfiguration]] = defaultdict(list)
        for expectation_configuration in column_based_expectation_configurations:
            ...
            column_based_expectation_configurations_by_type[type_].append(expectation_configuration)
        ...
        for domain, attributed_metric_values in attributed_metrics.items():
            ...
            for metric_name in metric_names:
                if metric_name in attributed_metric_values.keys():
                    filtered_attributed_metrics[domain][metric_name] = attributed_metric_values[metric_name]
    ...
    def _chart_domain_values(  # noqa: PLR0913
        ...
        sanitized_metric_names: Set[str] = self._get_sanitized_metric_names_from_metric_names(metric_names=metric_names)
        ...
        quantitative_metrics: Set[str] = self._get_sanitized_metric_names_from_altair_type(
        ...
        if plot_mode is PlotMode.DIAGNOSTIC:
            ...
            if DataAssistantResult._all_metric_names_in_iterable(
                ...
                expectation_plot_impl = self._get_expect_domain_values_to_match_set_chart
                ...
                expectation_plot_impl = self._get_expect_domain_values_to_be_between_chart
            elif DataAssistantResult._all_metric_names_in_iterable(
    ...
    def _create_return_charts_for_column_domain_expectation(  # noqa: PLR0913
        ...
        for domain, attributed_metrics in attributed_metrics_by_domain.items():
            for expectation_configuration in expectation_configurations:
                if (expectation_configuration.kwargs["column"] == domain.domain_kwargs.column) and (
                    metric_expectation_map.get(metric_names) == expectation_configuration.expectation_type
                ):
                    df = pd.DataFrame()
                    for metric_name in metric_names:
                        ...
                        if len(df.index) == 0:
                            ...
                            join_keys = [column for column in metric_df.columns if column not in sanitized_metric_names]
    ...
    def _chart_column_values(  # noqa: PLR0913
        ...
        sanitized_metric_names: Set[str] = DataAssistantResult._get_sanitized_metric_names_from_metric_names(
            metric_names=metric_names
        )
        ...
        if plot_mode is PlotMode.DIAGNOSTIC:
            ...
            if DataAssistantResult._all_metric_names_in_iterable(
                ...
                expectation_plot_impl = self._get_interactive_expect_column_values_nominal_chart
            elif DataAssistantResult._all_metric_names_in_iterable(
                metric_names=sanitized_metric_names, iterable=ordinal_metrics
            ):
                expectation_plot_impl = self._get_interactive_expect_column_values_ordinal_chart
            elif DataAssistantResult._all_metric_names_in_iterable(
                metric_names=sanitized_metric_names, iterable=quantitative_metrics
            ):
                expectation_plot_impl = self._get_interactive_expect_column_values_to_be_between_chart
            elif DataAssistantResult._all_metric_names_in_iterable(
                metric_names=sanitized_metric_names, iterable=temporal_metrics
            ):
    ...
    def _create_df_for_charting(  # noqa: C901, PLR0912
        ...
        metric_values: MetricValues = [
            value[0] if len(value) == 1 else value for value in attributed_values[0].values()
        ]

        sanitized_metric_name: str = sanitize_parameter_name(name=metric_name, suffix=None)

        df: pd.DataFrame = pd.DataFrame({sanitized_metric_name: metric_values})

        if metric_name == "column.quantile_values" and plot_mode == PlotMode.DESCRIPTIVE:
        ...
        if self._batch_id_to_batch_identifier_display_name_map is not None:
            batch_identifier_list = [
                self._batch_id_to_batch_identifier_display_name_map[batch_id] for batch_id in batch_ids
            ]
        ...
        batch_identifier_df: pd.DataFrame = pd.DataFrame(batch_identifier_records, columns=batch_identifier_keys_sorted)
            ...
            if expectation_configuration is not None:
                for kwarg_name in expectation_configuration.kwargs:
                    if isinstance(expectation_configuration.kwargs[kwarg_name], dict):
                        for key, value in expectation_configuration.kwargs[kwarg_name].items():
                        ...
                        df[kwarg_name] = [expectation_configuration.kwargs[kwarg_name] for _ in df.index]
                    else:
    ...
    def _create_column_dfs_for_charting(
        ...
        for metric_domain in metric_domains:
            attributed_values_by_metric_name: Dict[str, List[ParameterNode]] = attributed_metrics_by_domain[
                metric_domain
            ]
            column_name = metric_domain.domain_kwargs.column

            metric_domain_expectation_configuration: Optional[ExpectationConfiguration] = None
            for expectation_configuration in expectation_configurations:
    ...
    def _create_chart_for_table_domain_expectation(  # noqa: PLR0913
        ...
        if all(sanitized_metric_name in table_column_metrics for sanitized_metric_name in sanitized_metric_names):
            if (include_column_names is not None) or (exclude_column_names is not None):
                for sanitized_metric_name in sanitized_metric_names:
                    all_columns = df[sanitized_metric_name].apply(pd.Series).values.tolist()
                    for record in all_columns:
                        new_column_list = []
                        for column in record:
                            if (include_column_names is not None and column in include_column_names) or (
                                exclude_column_names is not None and column not in exclude_column_names
                            ):
    ...
    def _get_attributed_metrics_by_domain(
        ...
        metrics_attributed_values_by_domain: Dict[Domain, Dict[str, List[ParameterNode]]] = {}
        if self.metrics_by_domain:
            for (
                domain,
                ...
                for (
                    ...
                    if (
                        FULLY_QUALIFIED_PARAMETER_NAME_ATTRIBUTED_VALUE_KEY in parameter_node
                    ...
                    elif FULLY_QUALIFIED_PARAMETER_NAME_ATTRIBUTED_VALUE_KEY in parameter_node:
                        metrics_attributed_values_by_domain[domain].update(
                            {
                                parameter_node[
                                    FULLY_QUALIFIED_PARAMETER_NAME_METADATA_KEY
    ...
    def _determine_attributed_metrics_by_domain_type(
        ...
        attributed_metrics_by_domain: Dict[Domain, Dict[str, List[ParameterNode]]] = dict(
            filter(
    ...
    def _get_sanitized_metric_names_from_altair_type(self, altair_type: AltairDataTypes) -> Set[str]:
    ...
    def _get_sanitized_metric_names_from_metric_names(
        ...
        return {sanitize_parameter_name(name=metric_name, suffix=None) for metric_name in metric_names}

    @staticmethod
    def _all_metric_names_in_iterable(metric_names: Set[str], iterable: Iterable[str]) -> bool:
    ...
    def _clean_quantitative_metrics_column_dfs(
        ...
        for idx, (column_name, column_df) in enumerate(column_dfs):
            ...
            if len(cleaned_column_df.index) > 0:
                cleaned_column_dfs.append(ColumnDataFrame(column_name, cleaned_column_df))

        return cleaned_column_dfs

    @staticmethod
    def _clean_quantitative_metrics_df(df: pd.DataFrame, sanitized_metric_names: Set[str]) -> pd.DataFrame:
        sanitized_metric_names_list = list(sanitized_metric_names)
        df[sanitized_metric_names_list] = df[sanitized_metric_names_list].apply(pd.to_numeric, errors="coerce")
        return df.dropna()
    ...
    def _get_interactive_expect_column_values_nominal_chart(
        ...
        raise NotImplementedError("Nominal expectation charts have not been implemented for the column domain type.")
    ...
    def _get_interactive_nominal_metrics_chart(
        ...
        raise NotImplementedError("Nominal metric charts have not been implemented for the column domain type.")
