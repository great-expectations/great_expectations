...
class NumericMetricRangeMultiBatchParameterBuilder(MetricMultiBatchParameterBuilder):
    ...
    def __init__(
        ...
        false_positive_rate: Union[(str, float)] = 0.05,
...
"""
        ...
        if (
            ...
            raise ge_exceptions.ProfilerExecutionError(
                message=f"""The directive "quantile_statistic_interpolation_method" for {self.__class__.__name__} can be only one of {NumericMetricRangeMultiBatchParameterBuilder.RECOGNIZED_QUANTILE_STATISTIC_INTERPOLATION_METHODS} ("{quantile_statistic_interpolation_method}" was detected).
"""
            )
    ...
    def _estimate_metric_value_range(
        ...
        **kwargs,
        ...
        return NumericRangeEstimationResult(
            estimation_histogram=estimation_histogram, value_range=metric_value_range
        )
...
"""
    ...
    def _get_deterministic_estimate(
        metric_values: np.ndarray, **kwargs
    ) -> NumericRangeEstimationResult:
        import inspect
