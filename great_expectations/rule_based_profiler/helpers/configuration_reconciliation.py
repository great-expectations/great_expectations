...
DEFAULT_RECONCILATION_DIRECTIVES: ReconciliationDirectives = ReconciliationDirectives(variables=ReconciliationStrategy.UPDATE, domain_builder=ReconciliationStrategy.UPDATE, parameter_builder=ReconciliationStrategy.UPDATE, expectation_configuration_builder=ReconciliationStrategy.UPDATE)

def reconcile_rule_variables(variables: ParameterContainer, variables_config: dict, reconciliation_strategy: ReconciliationStrategy=DEFAULT_RECONCILATION_DIRECTIVES.variables) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if variables_config:
        if (reconciliation_strategy == ReconciliationStrategy.NESTED_UPDATE):
            effective_variables_config = nested_update(effective_variables_config, variables_config)
