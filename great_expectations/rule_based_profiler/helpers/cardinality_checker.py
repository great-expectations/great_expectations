...
class RelativeCardinalityLimit(CardinalityLimit):
    ...
    def to_json_dict(self) -> dict:
        ...
        return convert_to_json_serializable({'name': self.name, 'max_proportion_unique': self.max_proportion_unique, 'metric_name_defining_limit': self.metric_name_defining_limit})
    ...
    def to_json_dict(self) -> dict:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return convert_to_json_serializable({'name': self.name, 'max_proportion_unique': self.max_unique_values, 'metric_name_defining_limit': self.metric_name_defining_limit})
...
class CardinalityChecker():
    ...
    SUPPORTED_CARDINALITY_LIMIT_MODE_CLASSES: Tuple[Union[(AbsoluteCardinalityLimit, RelativeCardinalityLimit)]] = (AbsoluteCardinalityLimit, RelativeCardinalityLimit)
    SUPPORTED_LIMIT_MODE_CLASS_NAMES: Tuple[str] = (mode.__name__ for mode in SUPPORTED_CARDINALITY_LIMIT_MODE_CLASSES)
    SUPPORTED_CARDINALITY_LIMIT_MODE_STRINGS: Tuple[str] = (mode.name for mode in CardinalityLimitMode)

    def __init__(self, limit_mode: Optional[Union[(CardinalityLimitMode, str)]]=None, max_unique_values: Optional[int]=None, max_proportion_unique: Optional[float]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        self._limit_mode = self._convert_to_cardinality_mode(limit_mode=limit_mode, max_unique_values=max_unique_values, max_proportion_unique=max_proportion_unique)
    ...
    def _validate_metric_value(metric_value: float) -> None:
        ...
        if (not isinstance(metric_value, (int, float))):
            raise ProfilerConfigurationError(f'Value of measured cardinality must be of type int or float, you provided {type(metric_value)}')
        if (metric_value < 0.0):
            raise ProfilerConfigurationError(f'Value of cardinality (number of rows or percent unique) should be greater than 0.00, your value is {metric_value}')

    @staticmethod
    def _convert_to_cardinality_mode(limit_mode: Optional[Union[(CardinalityLimitMode, str)]]=None, max_unique_values: Optional[int]=None, max_proportion_unique: Optional[float]=None) -> Union[(AbsoluteCardinalityLimit, RelativeCardinalityLimit)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (limit_mode is not None):
            if isinstance(limit_mode, str):
                try:
                    ...
                    raise ProfilerConfigurationError(f'Please specify a supported cardinality mode. Supported cardinality modes are {[member.name for member in CardinalityLimitMode]}')
            else:
                return limit_mode.value
        if (max_unique_values is not None):
            return AbsoluteCardinalityLimit(name=f'CUSTOM_ABS_{max_unique_values}', max_unique_values=max_unique_values)
        if (max_proportion_unique is not None):
            return RelativeCardinalityLimit(name=f'CUSTOM_REL_{max_proportion_unique}', max_proportion_unique=max_proportion_unique)

def validate_input_parameters(limit_mode: Optional[Union[(CardinalityLimitMode, str)]]=None, max_unique_values: Optional[int]=None, max_proportion_unique: Optional[int]=None) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    num_supplied_params: int = sum([(0 if (param is None) else 1) for param in (limit_mode, max_unique_values, max_proportion_unique)])
    if (num_supplied_params != 1):
        raise ProfilerConfigurationError(f'Please pass ONE of the following parameters: limit_mode, max_unique_values, max_proportion_unique, you passed {num_supplied_params} parameters.')
    if (limit_mode is not None):
        if (not (isinstance(limit_mode, CardinalityLimitMode) or isinstance(limit_mode, str))):
            raise ProfilerConfigurationError(f"Please specify a supported cardinality limit type, supported classes are {','.join(CardinalityChecker.SUPPORTED_LIMIT_MODE_CLASS_NAMES)} and supported strings are {','.join(CardinalityChecker.SUPPORTED_CARDINALITY_LIMIT_MODE_STRINGS)}")
    if (max_unique_values is not None):
    ...
    if (max_proportion_unique is not None):
        if (not isinstance(max_proportion_unique, (float, int))):
            raise ProfilerConfigurationError(f'Please specify a float or int, you specified a {type(max_proportion_unique)}')
