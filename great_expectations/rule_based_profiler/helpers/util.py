...
def get_parameter_value_and_validate_return_type(
    ...
    if expected_return_type is not None:
        if not isinstance(parameter_reference, expected_return_type):
            raise ge_exceptions.ProfilerExecutionError(
                message=f"""Argument "{parameter_reference}" must be of type "{str(expected_return_type)}" (value of type "{str(type(parameter_reference))}" was encountered).
"""
            )
...
def get_resolved_metrics_by_key(
    ...
    resolved_metrics: Dict[(Tuple[(str, str, str)], Any)] = validator.compute_metrics(
        metric_configurations=[
            ...
            for (
                key,
                metric_configurations_for_key,
            ) in metric_configurations_by_key.items()
...
def build_domains_from_column_names(
    ...
    domains: List[Domain] = [
        Domain(
            ...
            domain_kwargs={"column": column_name},
            details={
                INFERRED_SEMANTIC_TYPE_KEY: (
                    {
...
def integer_semantic_domain_type(domain: Domain) -> bool:
    ...
    ] = domain.details.get(INFERRED_SEMANTIC_TYPE_KEY)
    semantic_domain_type: SemanticDomainTypes
    return inferred_semantic_domain_type and all(
        [
            (
                semantic_domain_type
                in [SemanticDomainTypes.LOGIC, SemanticDomainTypes.IDENTIFIER]
            )
...
def compute_bootstrap_quantiles_point_estimate(
    ...
    if (
        bootstrap_lower_quantile_bias / bootstrap_lower_quantile_standard_error
    ) <= 0.25:
    ...
    if (
        bootstrap_upper_quantile_bias / bootstrap_upper_quantile_standard_error
    ) <= 0.25:
...
def get_validator_with_expectation_suite(
    ...
    assert (expectation_suite is None) or isinstance(
