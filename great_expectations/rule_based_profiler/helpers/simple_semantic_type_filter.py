...
class SimpleSemanticTypeFilter(SemanticTypeFilter):
    ...
    def __init__(
        ...
        self._build_table_column_name_to_inferred_semantic_domain_type_map(
            batch_ids=batch_ids, validator=validator, column_names=column_names
        )
    ...
    def _build_table_column_name_to_inferred_semantic_domain_type_map(
        self, batch_ids: List[str], validator: "Validator", column_names: List[str]
    ) -> None:
        import inspect
        ...
        self._table_column_name_to_inferred_semantic_domain_type_map = {
            column_name: self._infer_semantic_domain_type_from_table_column_type(
                column_types_dict_list=column_types_dict_list, column_name=column_name
    ...
    def _infer_semantic_domain_type_from_table_column_type(
        column_types_dict_list: List[Dict[(str, Any)]], column_name: str
    ) -> InferredSemanticDomainType:
        import inspect
        ...
        if len(column_types_dict_list) != 1:
            raise ge_exceptions.ProfilerExecutionError(
                message=f"""Error: {len(column_types_dict_list)} columns were found while obtaining semantic type     information.  Please ensure that the specified column name refers to exactly one column.
    """
            )
