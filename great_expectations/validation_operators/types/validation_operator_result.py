...
class ValidationOperatorResult(DictDot):
    ...
    def __init__(self, run_id: RunIdentifier, run_results: Dict[(ValidationResultIdentifier, Dict[(str, Union[(ExpectationSuiteValidationResult, dict, str)])])], validation_operator_config: dict, evaluation_parameters: dict=None, success: Optional[bool]=None) -> None:
        import inspect
    ...
    def list_data_asset_names(self) -> List[str]:
        ...
        if (self._data_asset_names is None):
            self._data_asset_names = list({(data_asset['batch_kwargs'].get('data_asset_name') or '__none__') for data_asset in self.list_data_assets_validated()})
        ...
        if (self._validation_results_by_data_asset_name is None):
            ...
            for data_asset_name in self.list_data_asset_names():
                if (data_asset_name == '__none__'):
                    validation_results_by_data_asset_name[data_asset_name] = [data_asset['validation_results'] for data_asset in self.list_data_assets_validated() if (data_asset['batch_kwargs'].get('data_asset_name') is None)]
                else:
                    validation_results_by_data_asset_name[data_asset_name] = [data_asset['validation_results'] for data_asset in self.list_data_assets_validated() if (data_asset['batch_kwargs'].get('data_asset_name') == data_asset_name)]
        ...
        if (self._data_assets_validated_by_batch_id is None):
            ...
            for validation_result in self.list_validation_results():
                ...
                if (batch_id not in assets_validated_by_batch_id):
                    assets_validated_by_batch_id[batch_id] = {'batch_kwargs': batch_kwargs, 'validation_results': [validation_result], 'expectation_suite_names': [expectation_suite_name]}
                else:
