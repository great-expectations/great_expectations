...
class ValidationOperator():
    ...
    def run(self, assets_to_validate, run_id=None, evaluation_parameters=None, run_name=None, run_time=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
...
class ActionListValidationOperator(ValidationOperator):
    ...
    def __init__(self, data_context, action_list, name, result_format={'result_format': 'SUMMARY'}) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        assert (result_format['result_format'] in ['BOOLEAN_ONLY', 'BASIC', 'SUMMARY', 'COMPLETE'])
        ...
        for action_config in action_list:
            ...
            module_name = 'great_expectations.validation_operators'
            new_action = instantiate_class_from_config(config=config, runtime_environment={'data_context': self.data_context}, config_defaults={'module_name': module_name})
            if (not new_action):
    ...
    def validation_operator_config(self) -> dict:
        ...
        if (self._validation_operator_config is None):
            self._validation_operator_config = {'class_name': 'ActionListValidationOperator', 'module_name': 'great_expectations.validation_operators', 'name': self.name, 'kwargs': {'action_list': self.action_list, 'result_format': self.result_format}}
    ...
    def _build_batch_from_item(self, item):
        ...
        if isinstance(item, tuple):
            if (not ((len(item) == 2) and isinstance(item[0], dict) and isinstance(item[1], str))):
                raise ValueError('Unable to build batch from item.')
            batch = self.data_context.get_batch(batch_kwargs=item[0], expectation_suite_name=item[1])
        else:
            batch = item
        return batch

    def run(self, assets_to_validate, run_id=None, evaluation_parameters=None, run_name=None, run_time=None, catch_exceptions=None, result_format=None, checkpoint_identifier=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        with AsyncExecutor(self.data_context.concurrency, max_workers=len(assets_to_validate)) as async_executor:
            ...
            for item in assets_to_validate:
                ...
                batch_validate_arguments = {'run_id': run_id, 'result_format': result_format, 'evaluation_parameters': evaluation_parameters}
                if (catch_exceptions is not None):
                    batch_validate_arguments['catch_exceptions'] = catch_exceptions
                batch_and_async_result_tuples.append((batch, async_executor.submit(batch.validate, **batch_validate_arguments)))
            run_results = {}
            for (batch, async_batch_validation_result) in batch_and_async_result_tuples:
                if self.data_context.ge_cloud_mode:
                    expectation_suite_identifier = GeCloudIdentifier(resource_type='expectation_suite', ge_cloud_id=batch._expectation_suite.ge_cloud_id)
                    validation_result_id = GeCloudIdentifier(resource_type='suite_validation_result')
                else:
                    expectation_suite_identifier = ExpectationSuiteIdentifier(expectation_suite_name=batch._expectation_suite.expectation_suite_name)
                    validation_result_id = ValidationResultIdentifier(batch_identifier=batch_identifier, expectation_suite_identifier=expectation_suite_identifier, run_id=run_id)
                ...
                run_result_obj = {'validation_result': async_batch_validation_result.result(), 'actions_results': batch_actions_results}
                run_results[validation_result_id] = run_result_obj
        return ValidationOperatorResult(run_id=run_id, run_results=run_results, validation_operator_config=self.validation_operator_config, evaluation_parameters=evaluation_parameters)

    def _run_actions(self, batch: Union[(Batch, DataAsset)], expectation_suite_identifier: ExpectationSuiteIdentifier, expectation_suite, batch_validation_result, run_id, validation_result_id=None, checkpoint_identifier=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for action in self.action_list:
            ...
            if (validation_result_id is None):
                validation_result_id = ValidationResultIdentifier(expectation_suite_identifier=expectation_suite_identifier, run_id=run_id, batch_identifier=batch_identifier)
            try:
                action_result = self.actions[action['name']].run(validation_result_suite_identifier=validation_result_id, validation_result_suite=batch_validation_result, data_asset=batch, payload=batch_actions_results, expectation_suite_identifier=expectation_suite_identifier, checkpoint_identifier=checkpoint_identifier)
...
class WarningAndFailureExpectationSuitesValidationOperator(ActionListValidationOperator):
    ...
    def __init__(self, data_context, action_list, name, base_expectation_suite_name=None, expectation_suite_name_suffixes=None, stop_on_first_error=False, slack_webhook=None, notify_on='all', notify_with=None, result_format={'result_format': 'SUMMARY'}) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _build_slack_query(self, validation_operator_result: ValidationOperatorResult):
        ...
        failure_level_run_results = {validation_result_identifier: run_result for (validation_result_identifier, run_result) in run_results.items() if (run_result['expectation_suite_severity_level'] == 'failure')}
        if failure_level_run_results:
            failed_data_assets_msg_strings = [((validation_result_identifier.expectation_suite_identifier.expectation_suite_name + '-') + validation_result_identifier.batch_identifier) for (validation_result_identifier, run_result) in failure_level_run_results.items() if (not run_result['validation_result'].success)]
        title_block = {'type': 'section', 'text': {'type': 'mrkdwn', 'text': '*FailureVsWarning Validation Operator Completed.*'}}
        divider_block = {'type': 'divider'}
        query = {'blocks': [divider_block, title_block, divider_block]}
        status_element = {'type': 'section', 'text': {'type': 'mrkdwn', 'text': f'*Status*: {status_text}'}}
        query['blocks'].append(status_element)
        batch_identifiers_element = {'type': 'section', 'text': {'type': 'mrkdwn', 'text': f'*Batch Id List:* {batch_identifiers}'}}
        query['blocks'].append(batch_identifiers_element)
        if (not success):
            failed_data_assets_element = {'type': 'section', 'text': {'type': 'mrkdwn', 'text': f'*Failed Batches:* {failed_data_assets_msg_strings}'}}
            query['blocks'].append(failed_data_assets_element)
        run_name_element = {'type': 'section', 'text': {'type': 'mrkdwn', 'text': f'*Run Name:* {run_name}'}}
        query['blocks'].append(run_name_element)
        run_time_element = {'type': 'section', 'text': {'type': 'mrkdwn', 'text': f'*Run Time:* {run_time}'}}
        ...
        footer_section = {'type': 'context', 'elements': [{'type': 'mrkdwn', 'text': 'Learn about FailureVsWarning Validation Operators at {}'.format(documentation_url)}]}
        query['blocks'].append(footer_section)
        return query

    def run(self, assets_to_validate, run_id=None, base_expectation_suite_name=None, evaluation_parameters=None, run_name=None, run_time=None, result_format=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for item in assets_to_validate:
            ...
            failure_expectation_suite_identifier = ExpectationSuiteIdentifier(expectation_suite_name=(base_expectation_suite_name + self.expectation_suite_name_suffixes[0]))
            failure_validation_result_id = ValidationResultIdentifier(expectation_suite_identifier=failure_expectation_suite_identifier, run_id=run_id, batch_identifier=batch_id)
            failure_expectation_suite = None
            try:
                failure_expectation_suite = self.data_context.stores[self.data_context.expectations_store_name].get(failure_expectation_suite_identifier)
            except Exception:
            ...
            if failure_expectation_suite:
                ...
                failure_validation_result = batch.validate(failure_expectation_suite, run_id, result_format=(result_format if result_format else self.result_format), evaluation_parameters=evaluation_parameters)
                failure_run_result_obj['validation_result'] = failure_validation_result
                failure_actions_results = self._run_actions(batch, failure_expectation_suite_identifier, failure_expectation_suite, failure_validation_result, run_id)
                ...
                if ((not failure_validation_result.success) and self.stop_on_first_error):
                    break
            warning_expectation_suite_identifier = ExpectationSuiteIdentifier(expectation_suite_name=(base_expectation_suite_name + self.expectation_suite_name_suffixes[1]))
            warning_validation_result_id = ValidationResultIdentifier(expectation_suite_identifier=warning_expectation_suite_identifier, run_id=run_id, batch_identifier=batch.batch_id)
            warning_expectation_suite = None
            try:
                warning_expectation_suite = self.data_context.stores[self.data_context.expectations_store_name].get(warning_expectation_suite_identifier)
            except Exception:
            ...
            if warning_expectation_suite:
                ...
                warning_validation_result = batch.validate(warning_expectation_suite, run_id, result_format=(result_format if result_format else self.result_format), evaluation_parameters=evaluation_parameters)
                warning_run_result_obj['validation_result'] = warning_validation_result
                warning_actions_results = self._run_actions(batch, warning_expectation_suite_identifier, warning_expectation_suite, warning_validation_result, run_id)
        ...
        validation_operator_result = ValidationOperatorResult(run_id=run_id, run_results=run_results, validation_operator_config=self.validation_operator_config, evaluation_parameters=evaluation_parameters, success=all([run_result_obj['validation_result'].success for run_result_obj in run_results.values() if (run_result_obj['expectation_suite_severity_level'] == 'failure')]))
        if self.slack_webhook:
            if ((self.notify_on == 'all') or ((self.notify_on == 'success') and validation_operator_result.success) or ((self.notify_on == 'failure') and (not validation_operator_result.success))):
                slack_query = self._build_slack_query(validation_operator_result=validation_operator_result)
