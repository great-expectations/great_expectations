        ...
        if schemas and "spark" in schemas:
            ...
            try:
                spark_schema = pyspark.types.StructType(
                    [pyspark.types.StructField(column, spark_types[schema[column]](), True) for column in schema]
                )
...
def _get_test_validator_with_data_pandas(
    ...
    return build_pandas_validator_with_data(df=df, batch_definition=batch_definition, context=context)
...
def build_sa_validator_with_data(  # noqa: C901, PLR0912, PLR0913, PLR0915
    ...
    if schemas and sa_engine_name in schemas and isinstance(engine.dialect, dialect_classes[sa_engine_name]):
...
def build_sa_execution_engine(  # noqa: PLR0913
    ...
    execution_engine: SqlAlchemyExecutionEngine = SqlAlchemyExecutionEngine(engine=sqlalchemy_engine)
    batch_data = SqlAlchemyBatchData(execution_engine=execution_engine, table_name=table_name)
    ...
    execution_engine = SqlAlchemyExecutionEngine(engine=sqlalchemy_engine, batch_data_dict={batch_id: batch_data})
...
def candidate_test_is_on_temporary_notimplemented_list_v3_api(context, expectation_type):
    ...
    if context in ["trino"]:
        return expectation_type in set(candidate_test_is_on_temporary_notimplemented_list_v3_api_trino).union(
    ...
    if include_sqlalchemy:
        ...
        if include_clickhouse:
            ...
            try:
                ...
                if raise_exceptions_for_backends is True:
                    raise ImportError("clickhouse tests are requested, but unable to connect") from e
                else:
            ...
            try:
                ...
                if "data_alt" in d and d["data_alt"] is not None:
                    ...
                    try:
                        ...
                        _debug(f"Needed to use data_alt for backend {c}, but it worked for {expectation_type}")
                else:
                    _error(
...
def should_we_generate_this_test(  # noqa: C901, PLR0911, PLR0912, PLR0913
    ...
    if backend in expectation_test_case.suppress_test_for:
        if ignore_suppress:
            ...
            _debug(f"Backend {backend} is suppressed for test {expectation_test_case.title}: | {extra_debug_info}")
            return False
        ...
        if "result" in result and "observed_value" in result["result"]:
            if isinstance(result["result"]["observed_value"], (np.floating, float)):
                assert np.allclose(
                    ...
                    expectationValidationResultSchema.load(test["output"])["result"]["observed_value"],
                    rtol=RTOL,
                    atol=ATOL,
            ...
            # any of multiple possible observed_values. expect_column_values_to_be_of_type is one such expectation.
                ...
                try:
                    assert result["result"]["unexpected_list"] == value, (
                        "expected " + str(value) + " but got " + str(result["result"]["unexpected_list"])
                    )
                ...
                assert result["result"]["partial_unexpected_list"] == value, (
                    "expected " + str(value) + " but got " + str(result["result"]["partial_unexpected_list"])
                )
                ...
                if "raised_exception" not in result["exception_info"]:
                    ...
                    assert value in result["exception_info"]["exception_traceback"], (
                        "expected to find " + value + " in " + result["exception_info"]["exception_traceback"]
                    )
...
def generate_dataset_name_from_expectation_name(
    ...
    if not sub_index:
        ...
        dataset_name = dataset.get("dataset_name", f"{expectation_type}_dataset_{index}_{sub_index}")
...
def _get_clickhouse_connection_string(hostname: str = "localhost", schema_name: str = "test") -> str:
    return f"clickhouse+native://{hostname}:9000/{schema_name}"


def _create_clickhouse_engine(hostname: str = "localhost", schema_name: str = "schema") -> sqlalchemy.Engine:
    engine = sa.create_engine(_get_clickhouse_connection_string(hostname=hostname, schema_name=schema_name))
    from clickhouse_sqlalchemy.exceptions import DatabaseException
    from sqlalchemy import text  # noqa: TID251

    with engine.begin() as conn:
        try:
            schemas = conn.execute(text(f"show schemas from memory like {schema_name!r}")).fetchall()
...
def _create_trino_engine(hostname: str = "localhost", schema_name: str = "schema") -> sqlalchemy.Engine:
    engine = sa.create_engine(_get_trino_connection_string(hostname=hostname, schema_name=schema_name))

    with engine.begin() as conn:
        try:
            schemas = conn.execute(sa.text(f"show schemas from memory like {schema_name!r}")).fetchall()
            if (schema_name,) not in schemas:
...
def _get_redshift_connection_string() -> str:
    ...
    if not host:
        raise ValueError("Environment Variable REDSHIFT_HOST is required to run integration tests against Redshift")
    if not port:
        raise ValueError("Environment Variable REDSHIFT_PORT is required to run integration tests against Redshift")
    if not user:
        raise ValueError("Environment Variable REDSHIFT_USERNAME is required to run integration tests against Redshift")
    if not pswd:
        raise ValueError("Environment Variable REDSHIFT_PASSWORD is required to run integration tests against Redshift")
    if not db:
        raise ValueError("Environment Variable REDSHIFT_DATABASE is required to run integration tests against Redshift")
    if not ssl:
        raise ValueError("Environment Variable REDSHIFT_SSLMODE is required to run integration tests against Redshift")
...
def generate_sqlite_db_path():
    ...
    abspath = os.path.abspath(  # noqa: PTH100
        os.path.join(  # noqa: PTH118
            tmp_dir,
            "sqlite_db" + "".join([random.choice(string.ascii_letters + string.digits) for _ in range(8)]) + ".db",
        )
    )
    return abspath
