...
from great_expectations.core.batch import BatchRequest, BatchRequestBase, RuntimeBatchRequest, get_batch_request_as_dict, materialize_batch_request
...
def send_slack_notification(query, slack_webhook=None, slack_channel=None, slack_token=None):
    ...
    try:
        ...
        if (response.status_code != 200):
            logger.warning(f'Request to Slack webhook returned error {response.status_code}: {response.text}')
        else:
...
def send_opsgenie_alert(query, suite_name, settings):
    ...
    payload = {'message': f'Great Expectations suite {suite_name} failed', 'description': query, 'priority': settings['priority']}
    session = requests.Session()
    try:
        ...
        if (response.status_code != 202):
            logger.warning(f'Request to Opsgenie API returned error {response.status_code}: {response.text}')
        else:
            return 'success'
    return 'error'
...
def send_microsoft_teams_notifications(query, microsoft_teams_webhook):
    ...
    try:
        ...
        if (response.status_code != 200):
            logger.warning(f'Request to Microsoft Teams webhook returned error {response.status_code}: {response.text}')
            return
        else:
...
def send_webhook_notifications(query, webhook, target_platform):
    ...
    try:
        ...
        if (response.status_code != 200):
            logger.warning(f'Request to {target_platform} webhook returned error {response.status_code}: {response.text}')
        else:
            return f'{target_platform} notification succeeded.'

def send_email(title, html, smtp_address, smtp_port, sender_login, sender_password, sender_alias, receiver_emails_list, use_tls, use_ssl):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def get_substituted_validation_dict(substituted_runtime_config: dict, validation_dict: dict) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def get_substituted_batch_request(substituted_runtime_config: dict, validation_batch_request: Optional[Union[(BatchRequestBase, dict)]]=None) -> Optional[Union[(BatchRequest, RuntimeBatchRequest)]]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    for (key, value) in validation_batch_request.items():
        ...
        if ((value is not None) and (substituted_value is not None)):
            raise ge_exceptions.CheckpointError(f'BatchRequest attribute "{key}" was specified in both validation and top-level CheckpointConfig.')
    effective_batch_request: dict = dict(**substituted_runtime_batch_request, **validation_batch_request)
...
def substitute_template_config(source_config: dict, template_config: dict) -> dict:
    ...
    if (source_config.get('expectation_suite_ge_cloud_id') is not None):
        dest_config['expectation_suite_ge_cloud_id'] = source_config['expectation_suite_ge_cloud_id']
    if (source_config.get('batch_request') is not None):
        batch_request = (dest_config.get('batch_request') or {})
        updated_batch_request = nested_update(batch_request, source_config['batch_request'], dedup=True)
    ...
    if (source_config.get('action_list') is not None):
        ...
        dest_config['action_list'] = get_updated_action_list(base_action_list=action_list, other_action_list=source_config['action_list'])
    if (source_config.get('evaluation_parameters') is not None):
        evaluation_parameters = (dest_config.get('evaluation_parameters') or {})
        updated_evaluation_parameters = nested_update(evaluation_parameters, source_config['evaluation_parameters'], dedup=True)
    ...
    if (source_config.get('runtime_configuration') is not None):
        ...
        updated_runtime_configuration = nested_update(runtime_configuration, source_config['runtime_configuration'], dedup=True)
    ...
    if (source_config.get('validations') is not None):
        ...
        validations.extend(filter((lambda v: (v not in existing_validations)), source_config['validations']))
    ...
    if (source_config.get('profilers') is not None):
        ...
        profilers.extend(filter((lambda v: (v not in existing_profilers)), source_config['profilers']))
...
def substitute_runtime_config(source_config: dict, runtime_kwargs: dict) -> dict:
    ...
    if (runtime_kwargs.get('batch_request') is not None):
        ...
        updated_batch_request = nested_update(batch_request, batch_request_from_runtime_kwargs, dedup=True)
        dest_config['batch_request'] = updated_batch_request
    if (runtime_kwargs.get('action_list') is not None):
        action_list = (dest_config.get('action_list') or [])
        dest_config['action_list'] = get_updated_action_list(base_action_list=action_list, other_action_list=runtime_kwargs['action_list'])
    if (runtime_kwargs.get('evaluation_parameters') is not None):
        evaluation_parameters = (dest_config.get('evaluation_parameters') or {})
        updated_evaluation_parameters = nested_update(evaluation_parameters, runtime_kwargs['evaluation_parameters'], dedup=True)
        dest_config['evaluation_parameters'] = updated_evaluation_parameters
    if (runtime_kwargs.get('runtime_configuration') is not None):
        runtime_configuration = (dest_config.get('runtime_configuration') or {})
        updated_runtime_configuration = nested_update(runtime_configuration, runtime_kwargs['runtime_configuration'], dedup=True)
        dest_config['runtime_configuration'] = updated_runtime_configuration
    if (runtime_kwargs.get('validations') is not None):
        validations = (dest_config.get('validations') or [])
        existing_validations = (source_config.get('validations') or [])
        validations.extend(filter((lambda v: (v not in existing_validations)), runtime_kwargs['validations']))
        dest_config['validations'] = validations
    ...
    for other_action in other_action_list:
        ...
        if (other_action_name in base_action_list_dict):
            if other_action['action']:
                nested_update(base_action_list_dict[other_action_name], other_action, dedup=True)
            else:
...
def batch_request_in_validations_contains_batch_data(validations: Optional[List[dict]]=None) -> bool:
    ...
    if (validations is not None):
        for (idx, val) in enumerate(validations):
            if ((val.get('batch_request') is not None) and isinstance(val.get('batch_request'), (dict, DictDot)) and (val['batch_request'].get('runtime_parameters') is not None) and (val['batch_request']['runtime_parameters'].get('batch_data') is not None)):
                return True
    return False

def get_validations_with_batch_request_as_dict(validations: Optional[list]=None) -> Optional[list]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def validate_validation_dict(validation_dict: dict) -> None:
    ...
    if (not validation_dict.get('expectation_suite_name')):
        raise ge_exceptions.CheckpointError('validation expectation_suite_name must be specified')
...
def send_sns_notification(sns_topic_arn: str, sns_subject: str, validation_results: str, **kwargs) -> str:
    ...
    message_dict = {'TopicArn': sns_topic_arn, 'Subject': sns_subject, 'Message': json.dumps(validation_results), 'MessageAttributes': {'String': {'DataType': 'String.Array', 'StringValue': 'ValidationResults'}}, 'MessageStructure': 'json'}
