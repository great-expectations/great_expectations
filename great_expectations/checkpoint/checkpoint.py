...
class BaseCheckpoint(ConfigPeer):
    ...
    def run(  # noqa: C901, PLR0913, PLR0912, PLR0915
        ...
        if self._validator:
            ...
            if expectation_suite_name or _does_validation_contain_expectation_suite_name(validations=validations):
                raise gx_exceptions.CheckpointError(
            ...
            for run_result in run_results.values():
                ...
                if not validation_result_url:
                    if (
                        ...
                        and "validation_result_url" in run_result["actions_results"]["store_validation_result"]
                    ):
                        validation_result_url = run_result["actions_results"]["store_validation_result"][
    ...
    def _get_substituted_config(
        ...
        substituted_config: dict = substitute_runtime_config(source_config=config_kwargs, runtime_kwargs=runtime_kwargs)
    ...
    def _run_validation(  # noqa: PLR0913
        ...
        if validation_dict is None:
            validation_dict = CheckpointValidationConfig(id=substituted_runtime_config.get("default_validation_id"))
        ...
        try:
            ...
            expectation_suite_name: str | None = substituted_validation_dict.get("expectation_suite_name")
            expectation_suite_id: str | None = substituted_validation_dict.get("expectation_suite_id")
            include_rendered_content: bool | None = substituted_validation_dict.get("include_rendered_content")
            ...
            validator: Validator = self._validator or context.get_validator(
                ...
                expectation_suite_name=expectation_suite_name if not self._using_cloud_context else None,
                expectation_suite_id=(expectation_suite_id if self._using_cloud_context else None),
                include_rendered_content=include_rendered_content,
            )

            action_list: Sequence[ActionDict] | None = substituted_validation_dict.get("action_list")
...
"""
    ...
    def _reconcile_validations(
        ...
        if validator:
            if validations:
                raise ValueError("Please provide either a validator or validations list (but not both).")
