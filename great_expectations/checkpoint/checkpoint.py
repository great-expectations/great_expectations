...
class Checkpoint:
    ...
    def __init__(
        ...
        # self._config = copy.deepcopy(checkpoint_config)
        # TODO: <Alex>ALEX</Alex>
        # TODO: <Alex>ALEX</Alex>
    ...
    def get_substituted_config(
        ...
        print(f'\n[ALEX_TEST] [CHECKPOINT.GET_SUBSTITUTED_CONFIG()] CONFIG-0:\n{config} ; TYPE: {str(type(config))}')
        if config is None:
            config = self.config
        print(f'\n[ALEX_TEST] [CHECKPOINT.GET_SUBSTITUTED_CONFIG()] CONFIG-1:\n{config} ; TYPE: {str(type(config))}')
        ...
        print(f'\n[ALEX_TEST] [CHECKPOINT.GET_SUBSTITUTED_CONFIG()] CONFIG-2:\n{config} ; TYPE: {str(type(config))}')
        substituted_config: Union[CheckpointConfig, dict]

        template_name = config.template_name
        if not template_name:
            print(f'\n[ALEX_TEST] [CHECKPOINT.GET_SUBSTITUTED_CONFIG()] NO_TEMPLATE_NAME-CONFIG-1:\n{config} ; TYPE: {str(type(config))}')
            substituted_config = copy.deepcopy(config)
            print(f'\n[ALEX_TEST] [CHECKPOINT.GET_SUBSTITUTED_CONFIG()] NO_TEMPLATE_NAME-SUBSTITUTED_CONFIG-2:\n{substituted_config} ; TYPE: {str(type(substituted_config))}')
        ...
        if self.data_context.ge_cloud_mode:
            print(f'\n[ALEX_TEST] [CHECKPOINT.GET_SUBSTITUTED_CONFIG()] CLOUD_MODE-RETURNING_SUBSTITUTED_CONFIG-3:\n{substituted_config} ; TYPE: {str(type(substituted_config))}')
    ...
    def _substitute_config_variables(
        ...
        print(f'\n[ALEX_TEST] [CHECKPOINT._SUBSTITUTE_CONFIG_VARIABLES()] CONFIG-0:\n{config} ; TYPE: {str(type(config))}')
    ...
    def run(
        ...
        # # TODO: <Alex>ALEX</Alex>
        # # substituted_runtime_config = None
        # # try:
        # #     substituted_runtime_config = self.get_substituted_config(
        # #         runtime_kwargs=runtime_kwargs
        # #     )
        # #     print(f'\n[ALEX_TEST] [CHECKPOINT.RUN()] SUBSTITUTED_RUNTIME_CONFIG:\n{substituted_runtime_config} ; TYPE: {str(type(substituted_runtime_config))}')
        # # except Exception as e0:
        # #     print(f'\n[ALEX_TEST] [CHECKPOINT.RUN()] SUBSTITUTED_RUNTIME_CONFIG: EXCEPTION!!!!!!!!!!!!!!!! {e0} ; TYPE: {str(type(e0))}')
        # TODO: <Alex>ALEX</Alex>
        # substituted_runtime_config: CheckpointConfig = self.resolve_config_using_acceptable_arguments(
        #     template_name=template_name,
        #     run_name_template=run_name_template,
        #     expectation_suite_name=expectation_suite_name,
        #     batch_request=batch_request,
