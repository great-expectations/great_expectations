...
from great_expectations.checkpoint.util import batch_request_in_validations_contains_batch_data, get_substituted_validation_dict, get_validations_with_batch_request_as_dict, substitute_runtime_config, substitute_template_config
from great_expectations.core import RunIdentifier
from great_expectations.core.async_executor import AsyncExecutor, AsyncResult
from great_expectations.core.batch import BatchRequest, BatchRequestBase, RuntimeBatchRequest, batch_request_contains_batch_data, get_batch_request_as_dict
from great_expectations.core.config_peer import ConfigOutputModes, ConfigPeer
from great_expectations.core.usage_statistics.events import UsageStatsEvents
from great_expectations.core.usage_statistics.usage_statistics import get_checkpoint_run_usage_statistics, usage_statistics_enabled_method
...
from great_expectations.data_context.util import instantiate_class_from_config, substitute_all_config_variables
...
class BaseCheckpoint(ConfigPeer):
    ...
    def __init__(self, checkpoint_config: CheckpointConfig, data_context: 'DataContext') -> None:
        import inspect
    ...
    @usage_statistics_enabled_method(event_name=UsageStatsEvents.CHECKPOINT_RUN.value, args_payload_fn=get_checkpoint_run_usage_statistics)
    def run(self, template_name: Optional[str]=None, run_name_template: Optional[str]=None, expectation_suite_name: Optional[str]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None, action_list: Optional[List[dict]]=None, evaluation_parameters: Optional[dict]=None, runtime_configuration: Optional[dict]=None, validations: Optional[List[dict]]=None, profilers: Optional[List[dict]]=None, run_id: Optional[Union[(str, RunIdentifier)]]=None, run_name: Optional[str]=None, run_time: Optional[Union[(str, datetime.datetime)]]=None, result_format: Optional[Union[(str, dict)]]=None, expectation_suite_ge_cloud_id: Optional[str]=None) -> CheckpointResult:
        ...
        if ((len(validations) == 0) and (not batch_request)):
            raise ge_exceptions.CheckpointError(f'Checkpoint "{self.name}" must contain either a batch_request or validations.')
        ...
        with AsyncExecutor(self.data_context.concurrency, max_workers=len(validations)) as async_executor:
            async_validation_operator_results: List[AsyncResult[ValidationOperatorResult]] = []
            if (len(validations) > 0):
                for (idx, validation_dict) in enumerate(validations):
                    self._run_validation(substituted_runtime_config=substituted_runtime_config, async_validation_operator_results=async_validation_operator_results, async_executor=async_executor, result_format=result_format, run_id=run_id, idx=idx, validation_dict=validation_dict)
            else:
                self._run_validation(substituted_runtime_config=substituted_runtime_config, async_validation_operator_results=async_validation_operator_results, async_executor=async_executor, result_format=result_format, run_id=run_id)
            run_results: dict = {}
            for async_validation_operator_result in async_validation_operator_results:
                run_results.update(async_validation_operator_result.result().run_results)
    ...
    def get_substituted_config(self, runtime_kwargs: Optional[dict]=None) -> dict:
        ...
        substituted_runtime_config: dict = self._get_substituted_template(source_config=config_kwargs)
        substituted_runtime_config = self._get_substituted_runtime_kwargs(source_config=substituted_runtime_config, runtime_kwargs=runtime_kwargs)
    ...
    def _get_substituted_template(self, source_config: dict) -> dict:
        ...
        if template_name:
            ...
            if (template_config['config_version'] != source_config['config_version']):
                raise ge_exceptions.CheckpointError(f"Invalid template '{template_name}' (ver. {template_config['config_version']}) for Checkpoint '{source_config}' (ver. {source_config['config_version']}. Checkpoints can only use templates with the same config_version.")
            substituted_template_config: dict = self._get_substituted_template(source_config=template_config)
    ...
    def _get_substituted_runtime_kwargs(self, source_config: dict, runtime_kwargs: Optional[dict]=None) -> dict:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _substitute_config_variables(self, config: dict) -> dict:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        substituted_config_variables = substitute_all_config_variables(self.data_context.config_variables, dict(os.environ), self.data_context.DOLLAR_SIGN_ESCAPE_STRING)
        substitutions = {**substituted_config_variables, **dict(os.environ), **self.data_context.runtime_environment}
    ...
    def _run_validation(self, substituted_runtime_config: dict, async_validation_operator_results: List[AsyncResult], async_executor: AsyncExecutor, result_format: Optional[dict], run_id: Optional[Union[(str, RunIdentifier)]], idx: Optional[int]=0, validation_dict: Optional[dict]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        try:
            ...
            batch_request: Union[(BatchRequest, RuntimeBatchRequest)] = substituted_validation_dict.get('batch_request')
            expectation_suite_name: str = substituted_validation_dict.get('expectation_suite_name')
            expectation_suite_ge_cloud_id: str = substituted_validation_dict.get('expectation_suite_ge_cloud_id')
            validator: Validator = self.data_context.get_validator(batch_request=batch_request, expectation_suite_name=(expectation_suite_name if (not self.data_context.ge_cloud_mode) else None), expectation_suite_ge_cloud_id=(expectation_suite_ge_cloud_id if self.data_context.ge_cloud_mode else None))
            action_list: list = substituted_validation_dict.get('action_list')
            runtime_configuration_validation = substituted_validation_dict.get('runtime_configuration', {})
            ...
            action_list_validation_operator: ActionListValidationOperator = ActionListValidationOperator(data_context=self.data_context, action_list=action_list, result_format=result_format, name=f'{self.name}-checkpoint-validation[{idx}]')
            checkpoint_identifier = None
            if self.data_context.ge_cloud_mode:
                checkpoint_identifier = GeCloudIdentifier(resource_type='contract', ge_cloud_id=str(self.ge_cloud_id))
            operator_run_kwargs = {}
            if (catch_exceptions_validation is not None):
                operator_run_kwargs['catch_exceptions'] = catch_exceptions_validation
            async_validation_operator_results.append(async_executor.submit(action_list_validation_operator.run, assets_to_validate=[validator], run_id=run_id, evaluation_parameters=substituted_validation_dict.get('evaluation_parameters'), result_format=result_format, checkpoint_identifier=checkpoint_identifier, **operator_run_kwargs))
        except (ge_exceptions.CheckpointError, ge_exceptions.ExecutionEngineError, ge_exceptions.MetricError) as e:
            raise ge_exceptions.CheckpointError(f"Exception occurred while running validation[{idx}] of Checkpoint '{self.name}': {e.message}.")
...
Checkpoint class name: {self.__class__.__name__}''')
        validations_present: bool = (self.validations and isinstance(self.validations, list) and (len(self.validations) > 0))
        action_list: Optional[list] = self.action_list
        action_list_present: bool = (((action_list is not None) and isinstance(action_list, list) and (len(action_list) > 0)) or (validations_present and all([(validation.get('action_list') and isinstance(validation['action_list'], list) and (len(validation['action_list']) > 0)) for validation in self.validations])))
        if pretty_print:
            if (not validations_present):
                print('Your current Checkpoint configuration has an empty or missing "validations" attribute.  This\nmeans you must either update your Checkpoint configuration or provide an appropriate validations\nlist programmatically (i.e., when your Checkpoint is run).\n                    ')
            if (not action_list_present):
                print('Your current Checkpoint configuration has an empty or missing "action_list" attribute.  This\nmeans you must provide an appropriate validations list programmatically (i.e., when your Checkpoint\nis run), with each validation having its own defined "action_list" attribute.\n                    ')
...
class Checkpoint(BaseCheckpoint):
    ...
    def __init__(self, name: str, data_context: 'DataContext', config_version: Optional[Union[(int, float)]]=None, template_name: Optional[str]=None, run_name_template: Optional[str]=None, expectation_suite_name: Optional[str]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None, action_list: Optional[List[dict]]=None, evaluation_parameters: Optional[dict]=None, runtime_configuration: Optional[dict]=None, validations: Optional[List[dict]]=None, profilers: Optional[List[dict]]=None, validation_operator_name: Optional[str]=None, batches: Optional[List[dict]]=None, ge_cloud_id: Optional[UUID]=None, expectation_suite_ge_cloud_id: Optional[UUID]=None) -> None:
        ...
        if batch_request_contains_batch_data(batch_request=batch_request):
            raise ValueError(f'''Error: batch_data found in batch_request -- only primitive types are allowed as Checkpoint constructor arguments.
''')
        if batch_request_in_validations_contains_batch_data(validations=validations):
            raise ValueError(f'''Error: batch_data found in batch_request -- only primitive types are allowed as Checkpoint constructor arguments.
''')
        checkpoint_config: CheckpointConfig = CheckpointConfig(name=name, config_version=config_version, template_name=template_name, run_name_template=run_name_template, expectation_suite_name=expectation_suite_name, batch_request=batch_request, action_list=action_list, evaluation_parameters=evaluation_parameters, runtime_configuration=runtime_configuration, validations=validations, profilers=profilers, validation_operator_name=validation_operator_name, batches=batches, ge_cloud_id=ge_cloud_id, expectation_suite_ge_cloud_id=expectation_suite_ge_cloud_id)
    ...
    def run_with_runtime_args(self, template_name: Optional[str]=None, run_name_template: Optional[str]=None, expectation_suite_name: Optional[str]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None, action_list: Optional[List[dict]]=None, evaluation_parameters: Optional[dict]=None, runtime_configuration: Optional[dict]=None, validations: Optional[List[dict]]=None, profilers: Optional[List[dict]]=None, run_id: Optional[Union[(str, int, float)]]=None, run_name: Optional[str]=None, run_time: Optional[datetime.datetime]=None, result_format: Optional[str]=None, expectation_suite_ge_cloud_id: Optional[str]=None, **kwargs) -> CheckpointResult:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        checkpoint_config_from_store: CheckpointConfig = cast(CheckpointConfig, self.get_config())
        if (('runtime_configuration' in checkpoint_config_from_store) and checkpoint_config_from_store.runtime_configuration and ('result_format' in checkpoint_config_from_store.runtime_configuration)):
            result_format = (result_format or checkpoint_config_from_store.runtime_configuration.get('result_format'))
        if (result_format is None):
            result_format = {'result_format': 'SUMMARY'}
        batch_request = get_batch_request_as_dict(batch_request=batch_request)
        validations = get_validations_with_batch_request_as_dict(validations=validations)
        checkpoint_config_from_call_args: dict = {'template_name': template_name, 'run_name_template': run_name_template, 'expectation_suite_name': expectation_suite_name, 'batch_request': batch_request, 'action_list': action_list, 'evaluation_parameters': evaluation_parameters, 'runtime_configuration': runtime_configuration, 'validations': validations, 'profilers': profilers, 'run_id': run_id, 'run_name': run_name, 'run_time': run_time, 'result_format': result_format, 'expectation_suite_ge_cloud_id': expectation_suite_ge_cloud_id}
        checkpoint_config: dict = {key: value for (key, value) in checkpoint_config_from_store.items() if (key in checkpoint_config_from_call_args)}
        checkpoint_config.update(checkpoint_config_from_call_args)
        checkpoint_run_arguments: dict = dict(**checkpoint_config, **kwargs)
        filter_properties_dict(properties=checkpoint_run_arguments, clean_falsy=True, inplace=True)
        return self.run(**checkpoint_run_arguments)

    @staticmethod
    def construct_from_config_args(data_context: 'DataContext', checkpoint_store_name: str, name: str, config_version: Optional[Union[(int, float)]]=None, template_name: Optional[str]=None, module_name: Optional[str]=None, class_name: Optional[str]=None, run_name_template: Optional[str]=None, expectation_suite_name: Optional[str]=None, batch_request: Optional[dict]=None, action_list: Optional[List[dict]]=None, evaluation_parameters: Optional[dict]=None, runtime_configuration: Optional[dict]=None, validations: Optional[List[dict]]=None, profilers: Optional[List[dict]]=None, validation_operator_name: Optional[str]=None, batches: Optional[List[dict]]=None, site_names: Optional[Union[(str, List[str])]]=None, slack_webhook: Optional[str]=None, notify_on: Optional[str]=None, notify_with: Optional[Union[(str, List[str])]]=None, ge_cloud_id: Optional[str]=None, expectation_suite_ge_cloud_id: Optional[str]=None) -> 'Checkpoint':
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        validations = get_validations_with_batch_request_as_dict(validations=validations)
        if batch_request_contains_batch_data(batch_request=batch_request):
            raise ge_exceptions.InvalidConfigError(f'batch_data found in batch_request cannot be saved to CheckpointStore "{checkpoint_store_name}"')
        if batch_request_in_validations_contains_batch_data(validations=validations):
            raise ge_exceptions.InvalidConfigError(f'batch_data found in validations cannot be saved to CheckpointStore "{checkpoint_store_name}"')
        ...
        checkpoint_config = deep_filter_properties_iterable(properties=checkpoint_config, clean_falsy=True)
        new_checkpoint: Checkpoint = instantiate_class_from_config(config=checkpoint_config, runtime_environment={'data_context': data_context}, config_defaults={'module_name': 'great_expectations.checkpoint'})
        return new_checkpoint

    @staticmethod
    def instantiate_from_config_with_runtime_args(checkpoint_config: CheckpointConfig, data_context: 'DataContext', **runtime_kwargs) -> 'Checkpoint':
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        checkpoint: Checkpoint = instantiate_class_from_config(config=config, runtime_environment={'data_context': data_context}, config_defaults={'module_name': 'great_expectations.checkpoint'})
        return checkpoint
...
class LegacyCheckpoint(Checkpoint):
    ...
    def __init__(self, name: str, data_context, validation_operator_name: Optional[str]=None, batches: Optional[List[dict]]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        super().__init__(name=name, data_context=data_context, validation_operator_name=validation_operator_name, batches=batches)
    ...
    def _run_default_validation_operator(self, assets_to_validate: List, run_id: Optional[Union[(str, RunIdentifier)]]=None, evaluation_parameters: Optional[dict]=None, run_name: Optional[str]=None, run_time: Optional[Union[(str, datetime.datetime)]]=None, result_format: Optional[Union[(str, dict)]]=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        default_validation_operator = ActionListValidationOperator(data_context=self.data_context, action_list=[{'name': 'store_validation_result', 'action': {'class_name': 'StoreValidationResultAction'}}, {'name': 'store_evaluation_params', 'action': {'class_name': 'StoreEvaluationParametersAction'}}, {'name': 'update_data_docs', 'action': {'class_name': 'UpdateDataDocsAction', 'site_names': []}}], result_format=result_format, name='default-action-list-validation-operator')
        if (evaluation_parameters is None):
            return default_validation_operator.run(assets_to_validate=assets_to_validate, run_id=run_id, run_name=run_name, run_time=run_time, result_format=result_format)
        else:
            return default_validation_operator.run(assets_to_validate=assets_to_validate, run_id=run_id, evaluation_parameters=evaluation_parameters, run_name=run_name, run_time=run_time, result_format=result_format)

    def run(self, run_id=None, evaluation_parameters=None, run_name=None, run_time=None, result_format=None, **kwargs):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (self.validation_operator_name and self.data_context.validation_operators.get(self.validation_operator_name)):
            results = self.data_context.run_validation_operator(self.validation_operator_name, assets_to_validate=batches_to_validate, run_id=run_id, evaluation_parameters=evaluation_parameters, run_name=run_name, run_time=run_time, result_format=result_format, **kwargs)
        else:
            if self.validation_operator_name:
                logger.warning(f'Could not find Validation Operator "{self.validation_operator_name}" when running Checkpoint "{self.name}". Using default action_list_operator.')
            results = self._run_default_validation_operator(assets_to_validate=batches_to_validate, run_id=run_id, evaluation_parameters=evaluation_parameters, run_name=run_name, run_time=run_time, result_format=result_format)
        return results
...
class SimpleCheckpoint(Checkpoint):
    ...
    def __init__(self, name: str, data_context, config_version: Optional[Union[(int, float)]]=None, template_name: Optional[str]=None, run_name_template: Optional[str]=None, expectation_suite_name: Optional[str]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None, action_list: Optional[List[dict]]=None, evaluation_parameters: Optional[dict]=None, runtime_configuration: Optional[dict]=None, validations: Optional[List[dict]]=None, profilers: Optional[List[dict]]=None, ge_cloud_id: Optional[UUID]=None, site_names: Union[(str, List[str])]='all', slack_webhook: Optional[str]=None, notify_on: str='all', notify_with: Union[(str, List[str])]='all', expectation_suite_ge_cloud_id: Optional[str]=None, **kwargs) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        checkpoint_config: CheckpointConfig = self._configurator_class(name=name, data_context=data_context, config_version=config_version, template_name=template_name, run_name_template=run_name_template, expectation_suite_name=expectation_suite_name, batch_request=batch_request, action_list=action_list, evaluation_parameters=evaluation_parameters, runtime_configuration=runtime_configuration, validations=validations, profilers=profilers, site_names=site_names, slack_webhook=slack_webhook, notify_on=notify_on, notify_with=notify_with, ge_cloud_id=ge_cloud_id, expectation_suite_ge_cloud_id=expectation_suite_ge_cloud_id).build()
        super().__init__(name=checkpoint_config.name, data_context=data_context, config_version=checkpoint_config.config_version, template_name=checkpoint_config.template_name, run_name_template=checkpoint_config.run_name_template, expectation_suite_name=checkpoint_config.expectation_suite_name, batch_request=batch_request, action_list=checkpoint_config.action_list, evaluation_parameters=checkpoint_config.evaluation_parameters, runtime_configuration=checkpoint_config.runtime_configuration, validations=validations, profilers=checkpoint_config.profilers, ge_cloud_id=checkpoint_config.ge_cloud_id, expectation_suite_ge_cloud_id=checkpoint_config.expectation_suite_ge_cloud_id)
    ...
    def run(self, template_name: Optional[str]=None, run_name_template: Optional[str]=None, expectation_suite_name: Optional[str]=None, batch_request: Optional[Union[(BatchRequestBase, dict)]]=None, action_list: Optional[List[dict]]=None, evaluation_parameters: Optional[dict]=None, runtime_configuration: Optional[dict]=None, validations: Optional[List[dict]]=None, profilers: Optional[List[dict]]=None, run_id: Optional[Union[(str, RunIdentifier)]]=None, run_name: Optional[str]=None, run_time: Optional[Union[(str, datetime.datetime)]]=None, result_format: Optional[str]=None, site_names: Union[(str, List[str])]='all', slack_webhook: Optional[str]=None, notify_on: str='all', notify_with: Union[(str, List[str])]='all', expectation_suite_ge_cloud_id: Optional[str]=None) -> CheckpointResult:
        ...
        if any((site_names, slack_webhook, notify_on, notify_with)):
            new_baseline_config = self._configurator_class(name=self.name, data_context=self.data_context, action_list=action_list, site_names=site_names, slack_webhook=slack_webhook, notify_on=notify_on, notify_with=notify_with).build()
        return super().run(template_name=template_name, run_name_template=run_name_template, expectation_suite_name=expectation_suite_name, batch_request=batch_request, action_list=(new_baseline_config.action_list if new_baseline_config else action_list), evaluation_parameters=evaluation_parameters, runtime_configuration=runtime_configuration, validations=validations, profilers=profilers, run_id=run_id, run_name=run_name, run_time=run_time, result_format=result_format, expectation_suite_ge_cloud_id=expectation_suite_ge_cloud_id)
