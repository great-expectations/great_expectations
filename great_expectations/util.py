...
def get_context(
    ...
    3. If gx.get_context() is passed in an in-memory `project_config` then it will return a EphemeralDataContext.
    ...
    4. If GX is being run in the Cloud, and the information needed for ge_cloud_config (ie ge_cloud_base_url,
        ...
        environment variables, or in a .conf file, then gx.get_context() will return a CloudDataContext.
    ...
    # Validate project config and if applicable, convert from mapping to DataContextConfig
    if project_config is None:
        raise ValueError(
            "If instantiating a local context type, please pass in a valid project_config"
        )
