...
from inspect import ArgInfo, BoundArguments, Parameter, Signature, currentframe, getargvalues, getclosurevars, getmodule, signature
...
from great_expectations.exceptions import GreatExpectationsError, PluginClassNotFoundError, PluginModuleNotFoundError
...
try:
    ...
    Table = None
    Select = None
SINGULAR_TO_PLURAL_LOOKUP_DICT: dict = {'batch': 'batches', 'checkpoint': 'checkpoints', 'data_asset': 'data_assets', 'expectation': 'expectations', 'expectation_suite': 'expectation_suites', 'suite_validation_result': 'suite_validation_results', 'expectation_validation_result': 'expectation_validation_results', 'contract': 'contracts', 'rendered_data_doc': 'rendered_data_docs'}
PLURAL_TO_SINGULAR_LOOKUP_DICT: dict = {'batches': 'batch', 'checkpoints': 'checkpoint', 'data_assets': 'data_asset', 'expectations': 'expectation', 'expectation_suites': 'expectation_suite', 'suite_validation_results': 'suite_validation_result', 'expectation_validation_results': 'expectation_validation_result', 'contracts': 'contract', 'rendered_data_docs': 'rendered_data_doc'}
...
def pluralize(singular_ge_noun):
    ...
    try:
        ...
        raise GreatExpectationsError(f"Unable to pluralize '{singular_ge_noun}'. Please update great_expectations.util.SINGULAR_TO_PLURAL_LOOKUP_DICT")
...
def singularize(plural_ge_noun):
    ...
    try:
        ...
        raise GreatExpectationsError(f"Unable to singularize '{plural_ge_noun}'. Please update great_expectations.util.PLURAL_TO_SINGULAR_LOOKUP_DICT.")
...
def measure_execution_time(execution_time_holder_object_reference_name: str='execution_time_holder', execution_time_property_name: str='execution_time', pretty_print: bool=True) -> Callable:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    def execution_time_decorator(func: Callable) -> Callable:
        ...
        def compute_delta_t(*args, **kwargs) -> Any:
            ...
            try:
                ...
                if (kwargs is None):
                    kwargs = {}
                execution_time_holder: type = kwargs.get(execution_time_holder_object_reference_name)
                ...
                if pretty_print:
                    ...
                    print(f'Total execution time of function {func.__name__}({str(dict(call_args))}): {delta_t} seconds.')
...
def get_currently_executing_function() -> Callable:
    ...
    func_obj: Callable = [referer for referer in get_referrers(fc) if ((getattr(referer, '__code__', None) is fc) and (getclosurevars(referer).nonlocals.items() <= fb.f_locals.items()))][0]
    return func_obj

def get_currently_executing_function_call_arguments(include_module_name: bool=False, include_caller_names: bool=False, **kwargs) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    fc: CodeType = fb.f_code
    cur_func_obj: Callable = [referer for referer in get_referrers(fc) if ((getattr(referer, '__code__', None) is fc) and (getclosurevars(referer).nonlocals.items() <= fb.f_locals.items()))][0]
...
def verify_dynamic_loading_support(module_name: str, package_name: str=None) -> None:
    ...
    try:
        module_spec: importlib.machinery.ModuleSpec = importlib.util.find_spec(module_name, package=package_name)
    except ModuleNotFoundError:
        module_spec = None
    if (not module_spec):
        if (not package_name):
...
def read_csv(filename, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, profiler=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (dataset_class is not None):
        return _convert_to_dataset_class(df=df, dataset_class=dataset_class, expectation_suite=expectation_suite, profiler=profiler)
    else:
        return _load_and_convert_to_dataset_class(df=df, class_name=class_name, module_name=module_name, expectation_suite=expectation_suite, profiler=profiler)

def read_json(filename, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, accessor_func=None, profiler=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def read_excel(filename, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, profiler=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    try:
        ...
        raise ImportError("Pandas now requires 'openpyxl' as an optional-dependency to read Excel files. Please use pip or conda to install openpyxl and try again")
    if (dataset_class is None):
        verify_dynamic_loading_support(module_name=module_name)
        dataset_class = load_class(class_name=class_name, module_name=module_name)
    if isinstance(df, dict):
        for key in df:
            df[key] = _convert_to_dataset_class(df=df[key], dataset_class=dataset_class, expectation_suite=expectation_suite, profiler=profiler)
    else:
        df = _convert_to_dataset_class(df=df, dataset_class=dataset_class, expectation_suite=expectation_suite, profiler=profiler)
    return df

def read_table(filename, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, profiler=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def read_feather(filename, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, profiler=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def read_parquet(filename, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, profiler=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def from_pandas(pandas_df, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, profiler=None):
    ...
    if (dataset_class is not None):
        return _convert_to_dataset_class(df=pandas_df, dataset_class=dataset_class, expectation_suite=expectation_suite, profiler=profiler)
    else:
        return _load_and_convert_to_dataset_class(df=pandas_df, class_name=class_name, module_name=module_name, expectation_suite=expectation_suite, profiler=profiler)

def read_pickle(filename, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, profiler=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def read_sas(filename, class_name='PandasDataset', module_name='great_expectations.dataset', dataset_class=None, expectation_suite=None, profiler=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def validate(data_asset, expectation_suite=None, data_asset_name=None, expectation_suite_name=None, data_context=None, data_asset_class_name=None, data_asset_module_name='great_expectations.dataset', data_asset_class=None, *args, **kwargs):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if ((expectation_suite is None) and (data_context is None)):
        raise ValueError('Either an expectation suite or a DataContext is required for validation.')
    if (expectation_suite is None):
        ...
        if isinstance(expectation_suite, dict):
            ...
            expectation_suite: ExpectationSuite = ExpectationSuite(**expectation_suite_dict, data_context=data_context)
        ...
        logger.info(('Validating data_asset_name %s with expectation_suite_name %s' % (data_asset_name, expectation_suite.expectation_suite_name)))
    if ((data_asset_class_name is None) and (data_asset_class is None)):
        return data_asset.validate(*args, expectation_suite=expectation_suite, data_context=data_context, **kwargs)
    ...
    data_asset_ = _convert_to_dataset_class(data_asset, dataset_class=data_asset_class, expectation_suite=expectation_suite)
...
def lint_code(code: str) -> str:
    ...
    if (black is None):
        logger.warning("Please install the optional dependency 'black' to enable linting. Returning input with no changes.")
        return code
...
def filter_properties_dict(properties: Optional[dict]=None, keep_fields: Optional[Set[str]]=None, delete_fields: Optional[Set[str]]=None, clean_nulls: bool=True, clean_falsy: bool=False, keep_falsy_numerics: bool=True, inplace: bool=False) -> Optional[dict]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (not isinstance(properties, dict)):
        raise ValueError(f'Source "properties" must be a dictionary (illegal type "{str(type(properties))}" detected).')
    if (not inplace):
    ...
    if clean_nulls:
        keys_for_deletion.extend([key for (key, value) in properties.items() if (not ((keep_fields and (key in keep_fields)) or (delete_fields and (key in delete_fields)) or (value is not None)))])
    if clean_falsy:
        if keep_falsy_numerics:
            keys_for_deletion.extend([key for (key, value) in properties.items() if (not ((keep_fields and (key in keep_fields)) or (delete_fields and (key in delete_fields)) or is_truthy(value=value) or is_numeric(value=value)))])
        else:
            keys_for_deletion.extend([key for (key, value) in properties.items() if (not ((keep_fields and (key in keep_fields)) or (delete_fields and (key in delete_fields)) or is_truthy(value=value)))])
...
def deep_filter_properties_iterable(properties: Optional[Union[(dict, list, set, tuple)]]=None, keep_fields: Optional[Set[str]]=None, delete_fields: Optional[Set[str]]=None, clean_nulls: bool=True, clean_falsy: bool=False, keep_falsy_numerics: bool=True, inplace: bool=False) -> Optional[Union[(dict, list, set)]]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if isinstance(properties, dict):
        if (not inplace):
            properties = copy.deepcopy(properties)
        filter_properties_dict(properties=properties, keep_fields=keep_fields, delete_fields=delete_fields, clean_nulls=clean_nulls, clean_falsy=clean_falsy, keep_falsy_numerics=keep_falsy_numerics, inplace=True)
        key: str
        value: Any
        for (key, value) in properties.items():
            deep_filter_properties_iterable(properties=value, keep_fields=keep_fields, delete_fields=delete_fields, clean_nulls=clean_nulls, clean_falsy=clean_falsy, keep_falsy_numerics=keep_falsy_numerics, inplace=True)
        keys_to_delete: List[str] = list(filter((lambda k: _is_to_be_removed_from_deep_filter_properties_iterable(value=properties[k], clean_nulls=clean_nulls, clean_falsy=clean_falsy, keep_falsy_numerics=keep_falsy_numerics)), properties.keys()))
        ...
        for value in properties:
            deep_filter_properties_iterable(properties=value, keep_fields=keep_fields, delete_fields=delete_fields, clean_nulls=clean_nulls, clean_falsy=clean_falsy, keep_falsy_numerics=keep_falsy_numerics, inplace=True)
        properties = list(filter((lambda v: (not _is_to_be_removed_from_deep_filter_properties_iterable(value=v, clean_nulls=clean_nulls, clean_falsy=clean_falsy, keep_falsy_numerics=keep_falsy_numerics))), properties))
    if inplace:
        return None
    return properties

def _is_to_be_removed_from_deep_filter_properties_iterable(value: Any, clean_nulls: bool, clean_falsy: bool, keep_falsy_numerics: bool) -> bool:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    conditions: Tuple[(bool, ...)] = ((clean_nulls and (value is None)), ((not keep_falsy_numerics) and is_numeric(value) and (value == 0)), (clean_falsy and (not is_numeric(value)) and (not value)))
...
def is_candidate_subset_of_target(candidate: Any, target: Any) -> bool:
    ...
    if isinstance(candidate, dict):
        key: Any
        value: Any
        return all((((key in target) and is_candidate_subset_of_target(candidate=val, target=target[key])) for (key, val) in candidate.items()))
    if isinstance(candidate, (list, set, tuple)):
        subitem: Any
        superitem: Any
        return all((any((is_candidate_subset_of_target(subitem, superitem) for superitem in target)) for subitem in candidate))
...
def generate_temporary_table_name(default_table_name_prefix: str='ge_temp_', num_digits: int=8) -> str:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
