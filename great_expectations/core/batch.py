...
"{str(type(data_asset_name))}", which is illegal.
        ...
        if batch_identifiers and (not isinstance(batch_identifiers, IDDict)):
            raise TypeError(
                f"""The type of batch_identifiers must be an IDDict object.  The type given is "{str(type(batch_identifiers))}", which is illegal.
"""
            )
...
class RuntimeBatchRequest(BatchRequestBase):
    ...
    def _validate_runtime_batch_request_specific_init_parameters(
        ...
        if batch_spec_passthrough and (not isinstance(batch_spec_passthrough, dict)):
            raise TypeError(
                f"""The type for batch_spec_passthrough must be a dict object. The type given is "{str(type(batch_spec_passthrough))}", which is illegal.
"""
            )
...
class Batch(SerializableDictDot):
    ...
    def to_json_dict(self) -> dict:
        ...
        deep_filter_properties_iterable(
            properties=json_dict["batch_request"], inplace=True
        )
        return json_dict
...
def batch_request_contains_batch_data(
    ...
    for (k, v) in __frame.f_locals.items():
        if any((var in k) for var in ("__frame", "__file", "__func")):
            continue
        print(f"<INTROSPECT> {__file}:{__func} - {k}:{v.__class__.__name__}")
    return batch_request_contains_runtime_parameters(batch_request=batch_request) and (
        batch_request["runtime_parameters"].get("batch_data") is not None
    )
...
def get_batch_request_from_acceptable_arguments(
    ...
    **kwargs,
    ...
    if any(
        ...
        raise ValueError(
            "If batch_data, query, or path arguments are provided, the same keys cannot appear in the runtime_parameters argument."
        )
    if batch_request:
        return batch_request
    batch_request_class: type
    ...
    if any([(batch_data is not None), query, path, runtime_parameters]):
        ...
        if data_connector_query is None:
            if (batch_filter_parameters is not None) and (
                batch_identifiers is not None
            ):
                raise ValueError(
            ...
            if (batch_filter_parameters is None) and (batch_identifiers is not None):
                logger.warning(
                    'Attempting to build data_connector_query but "batch_identifiers" was provided instead of "batch_filter_parameters". The "batch_identifiers" key on data_connector_query has been renamed to "batch_filter_parameters". Please update your code. Falling back on provided "batch_identifiers".'
                )
