...
def deep_convert_properties_iterable_to_id_dict(
    ...
    if isinstance(source, (list, set, tuple)):
        ...
        element: Any
        return data_type([deep_convert_properties_iterable_to_id_dict(source=element) for element in source])
...
def _deep_convert_properties_iterable_to_id_dict(source: dict) -> IDDict:
    ...
    for key, value in source.items():
        if isinstance(value, dict):
            ...
            element: Any
            source[key] = data_type([deep_convert_properties_iterable_to_id_dict(source=element) for element in value])
