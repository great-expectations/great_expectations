...
class AugmentedLibraryMetadata(SerializableDictDot):
    ...
    legacy_maturity_level_substitutions = {'experimental': 'EXPERIMENTAL', 'beta': 'BETA', 'production': 'PRODUCTION'}
    ...
    def from_legacy_dict(cls, dict):
        ...
        for (k, v) in dict.items():
            if (k in inspect.signature(cls).parameters):
                ...
                logging.warning(f'WARNING: Got extra parameter: {k} while instantiating AugmentedLibraryMetadata.This parameter will be ignored.You probably need to clean up a library_metadata object.')
