    ...
    def from_legacy_dict(cls, dict):
        ...
        for (k, v) in dict.items():
            if k in inspect.signature(cls).parameters:
                ...
                logging.warning(
                    f"WARNING: Got extra parameter: {k} while instantiating AugmentedLibraryMetadata.This parameter will be ignored.You probably need to clean up a library_metadata object."
                )
            if ("maturity" in temp_dict) and (
                temp_dict["maturity"] in cls.legacy_maturity_level_substitutions
            ):
