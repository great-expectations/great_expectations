...
from great_expectations.core.batch import BatchRequest, RuntimeBatchRequest, get_batch_request_as_dict
from great_expectations.core.usage_statistics.anonymizers.base import BaseAnonymizer
from great_expectations.core.usage_statistics.anonymizers.types.base import CHECKPOINT_OPTIONAL_TOP_LEVEL_KEYS
...
class CheckpointAnonymizer(BaseAnonymizer):
    ...
    def _anonymize_checkpoint_config(self, name: str, config: dict) -> dict:
        ...
        self._anonymize_object_info(anonymized_info_dict=anonymized_info_dict, object_config=checkpoint_config_dict)
    ...
    def _anonymize_checkpoint_run(self, obj: object, **kwargs) -> dict:
        ...
        anonymized_run_name_template: Optional[str] = self._anonymize_string(run_name_template)
        expectation_suite_name: Optional[str] = kwargs.get('expectation_suite_name')
        anonymized_expectation_suite_name: Optional[str] = self._anonymize_string(expectation_suite_name)
        batch_request: Optional[Union[(BatchRequest, RuntimeBatchRequest, dict)]] = kwargs.get('batch_request')
        if (batch_request is None):
            batch_request = {}
        anonymized_batch_request: Optional[Dict[(str, List[str])]] = self._aggregate_anonymizer.anonymize(*(), **batch_request)
        action_list: Optional[List[dict]] = kwargs.get('action_list')
        anonymized_action_list: Optional[List[dict]] = None
        if action_list:
            try:
                anonymized_action_list = [self._aggregate_anonymizer.anonymize(action_name=action_config_dict['name'], action_config=action_config_dict['action']) for action_config_dict in action_list]
            except Exception:
                logger.debug('anonymize_checkpoint_run: Unable to create anonymized_action_list payload field')
        ...
        if validations:
            for validation_obj in validations:
                validation_batch_request: Optional[Union[(BatchRequest, RuntimeBatchRequest, dict)]] = validation_obj.get('batch_request')
                ...
                anonymized_validation_batch_request: Optional[Optional[Dict[(str, List[str])]]] = self._aggregate_anonymizer.anonymize(*(), **validation_batch_request)
                validation_expectation_suite_name: Optional[str] = validation_obj.get('expectation_suite_name')
                anonymized_validation_expectation_suite_name: Optional[str] = self._anonymize_string(validation_expectation_suite_name)
                validation_action_list: Optional[List[dict]] = validation_obj.get('action_list')
                anonymized_validation_action_list: Optional[List[dict]] = None
                if validation_action_list:
                    try:
                        anonymized_validation_action_list = [self._aggregate_anonymizer.anonymize(action_name=action_config_dict['name'], action_config=action_config_dict['action']) for action_config_dict in validation_action_list]
                    except Exception:
                        logger.debug('anonymize_checkpoint_run: Unable to create anonymized_validation_action_list payload field')
                ...
                anonymized_validation: Dict[(str, Dict[(str, Any)])] = {'anonymized_batch_request': anonymized_validation_batch_request, 'anonymized_expectation_suite_name': anonymized_validation_expectation_suite_name, 'anonymized_action_list': anonymized_validation_action_list}
        ...
        expectation_suite_ge_cloud_id: Optional[str] = kwargs.get('expectation_suite_ge_cloud_id')
        ...
        if (expectation_suite_ge_cloud_id is None):
            ...
            anonymized_expectation_suite_ge_cloud_id = self._anonymize_string(str(expectation_suite_ge_cloud_id))
        ...
        anonymized_checkpoint_run_properties_dict: Dict[(str, List[str])] = {'anonymized_name': anonymized_name, 'config_version': config_version, 'anonymized_template_name': anonymized_template_name, 'anonymized_run_name_template': anonymized_run_name_template, 'anonymized_expectation_suite_name': anonymized_expectation_suite_name, 'anonymized_batch_request': anonymized_batch_request, 'anonymized_action_list': anonymized_action_list, 'anonymized_validations': anonymized_validations, 'anonymized_run_id': anonymized_run_id, 'anonymized_run_name': anonymized_run_name, 'anonymized_run_time': anonymized_run_time, 'anonymized_expectation_suite_ge_cloud_id': anonymized_expectation_suite_ge_cloud_id, 'checkpoint_optional_top_level_keys': checkpoint_optional_top_level_keys}
        deep_filter_properties_iterable(properties=anonymized_checkpoint_run_properties_dict, clean_falsy=True, inplace=True)
