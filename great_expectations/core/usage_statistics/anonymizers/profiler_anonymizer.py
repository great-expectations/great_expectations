...
class ProfilerAnonymizer(BaseAnonymizer):
    ...
    def _anonymize_profiler_info(self, name: str, config: dict) -> dict:
        ...
        anonymized_info_dict: dict = {'anonymized_name': self._anonymize_string(name)}
        self._anonymize_object_info(anonymized_info_dict=anonymized_info_dict, object_config=config)
    ...
    def _anonymize_profiler_run(self, obj: object, **kwargs) -> dict:
        ...
        assert isinstance(obj, RuleBasedProfilerConfig), 'ProfilerAnonymizer can only handle objects of type RuleBasedProfilerConfig'
        ...
        anonymized_profiler_run_properties_dict: dict = {'anonymized_name': anonymized_name, 'config_version': config_version, 'anonymized_rules': anonymized_rules, 'rule_count': rule_count, 'variable_count': variable_count}
        deep_filter_properties_iterable(properties=anonymized_profiler_run_properties_dict, clean_falsy=True, inplace=True)
    ...
    def _anonymize_rule(self, name: str, rule: dict) -> dict:
        ...
        if (domain_builder is not None):
            anonymized_rule['anonymized_domain_builder'] = self._anonymize_domain_builder(domain_builder)
        parameter_builders: List[dict] = rule.get('parameter_builders', [])
        anonymized_rule['anonymized_parameter_builders'] = self._anonymize_parameter_builders(parameter_builders)
        expectation_configuration_builders: List[dict] = rule.get('expectation_configuration_builders', [])
        anonymized_rule['anonymized_expectation_configuration_builders'] = self._anonymize_expectation_configuration_builders(expectation_configuration_builders)
    ...
    def _anonymize_domain_builder(self, domain_builder: dict) -> dict:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        anonymized_domain_builder: dict = self._anonymize_object_info(object_config=domain_builder, anonymized_info_dict={}, runtime_environment={'module_name': 'great_expectations.rule_based_profiler.domain_builder'})
        batch_request: Optional[dict] = domain_builder.get('batch_request')
        if batch_request:
            anonymized_batch_request: Optional[dict] = self._aggregate_anonymizer.anonymize(**batch_request)
            anonymized_domain_builder['anonymized_batch_request'] = anonymized_batch_request
            logger.debug('Anonymized batch request in DomainBuilder')
        return anonymized_domain_builder

    def _anonymize_parameter_builders(self, parameter_builders: List[dict]) -> List[dict]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for parameter_builder in parameter_builders:
            anonymized_parameter_builder: dict = self._anonymize_parameter_builder(parameter_builder)
    ...
    def _anonymize_parameter_builder(self, parameter_builder: dict) -> dict:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        anonymized_parameter_builder: dict = self._anonymize_object_info(object_config=parameter_builder, anonymized_info_dict={}, runtime_environment={'module_name': 'great_expectations.rule_based_profiler.parameter_builder'})
        anonymized_parameter_builder['anonymized_name'] = self._anonymize_string(parameter_builder.get('name'))
        batch_request: Optional[dict] = parameter_builder.get('batch_request')
        if batch_request:
            anonymized_batch_request: Optional[dict] = self._aggregate_anonymizer.anonymize(**batch_request)
            anonymized_parameter_builder['anonymized_batch_request'] = anonymized_batch_request
            logger.debug('Anonymized batch request in ParameterBuilder')
        return anonymized_parameter_builder

    def _anonymize_expectation_configuration_builders(self, expectation_configuration_builders: List[dict]) -> List[dict]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for expectation_configuration_builder in expectation_configuration_builders:
            anonymized_expectation_configuration_builder: dict = self._anonymize_expectation_configuration_builder(expectation_configuration_builder)
            anonymized_expectation_configuration_builders.append(anonymized_expectation_configuration_builder)
        return anonymized_expectation_configuration_builders

    def _anonymize_expectation_configuration_builder(self, expectation_configuration_builder: dict) -> dict:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        anonymized_expectation_configuration_builder: dict = self._anonymize_object_info(object_config=expectation_configuration_builder, anonymized_info_dict={}, runtime_environment={'module_name': 'great_expectations.rule_based_profiler.expectation_configuration_builder'})
        expectation_type: Optional[str] = expectation_configuration_builder.get('expectation_type')
        self._anonymize_expectation(expectation_type, anonymized_expectation_configuration_builder)
        condition: Optional[str] = expectation_configuration_builder.get('condition')
        if condition:
            anonymized_expectation_configuration_builder['anonymized_condition'] = self._anonymize_string(condition)
            logger.debug('Anonymized condition in ExpectationConfigurationBuilder')
        return anonymized_expectation_configuration_builder

    def _anonymize_expectation(self, expectation_type: Optional[str], info_dict: dict) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (expectation_type in self.CORE_GE_EXPECTATION_TYPES):
            ...
            info_dict['anonymized_expectation_type'] = self._anonymize_string(expectation_type)
    ...
    def can_handle(self, obj: Optional[object]=None, **kwargs) -> bool:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return ((obj is not None) and isinstance(obj, (RuleBasedProfilerConfig, RuleBasedProfiler)))
