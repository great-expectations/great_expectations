...
class BatchAnonymizer(BaseAnonymizer):
    ...
    def anonymize(self, obj: Union[(Tuple[(dict, str)], 'DataAsset', 'Validator')], **kwargs) -> Any:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if batch_kwargs:
            anonymized_info_dict['anonymized_batch_kwarg_keys'] = self._anonymize_batch_kwargs(batch_kwargs)
        else:
            anonymized_info_dict['anonymized_batch_kwarg_keys'] = []
        if expectation_suite_name:
            anonymized_info_dict['anonymized_expectation_suite_name'] = self._anonymize_string(expectation_suite_name)
        else:
            anonymized_info_dict['anonymized_expectation_suite_name'] = '__not_found__'
        if datasource_name:
            anonymized_info_dict['anonymized_datasource_name'] = self._anonymize_string(datasource_name)
        else:
    ...
    def _anonymize_batch_kwargs(self, batch_kwargs: dict) -> List[str]:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        ge_batch_kwarg_keys = ['datasource', 'reader_method', 'reader_options', 'path', 's3', 'dataset', 'PandasInMemoryDF', 'ge_batch_id', 'query', 'table', 'SparkDFRef', 'limit', 'query_parameters', 'offset', 'snowflake_transient_table', 'data_asset_name']
        ...
        for batch_kwarg_key in batch_kwargs.keys():
            if (batch_kwarg_key in ge_batch_kwarg_keys):
                ...
                anonymized_batch_kwarg_keys.append(self._anonymize_string(batch_kwarg_key))
    ...
    def can_handle(self, obj: Optional[object]=None, **kwargs) -> bool:
        ...
        if (obj is None):
            return False
        return (isinstance(obj, (Validator, DataAsset)) or (isinstance(obj, tuple) and (len(obj) == 2)))
