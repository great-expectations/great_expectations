...
class GEExecutionEnvironment():
    ...
    def _build_required_dependencies(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        dependency_list: List[PackageInfo] = self._build_dependencies_info(self._ge_dependencies.get_required_dependency_names(), install_environment=InstallEnvironment.REQUIRED)
    ...
    def _build_dev_dependencies(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        dependency_list: List[PackageInfo] = self._build_dependencies_info(self._ge_dependencies.get_dev_dependency_names(), install_environment=InstallEnvironment.DEV)
        self._dependencies.extend(dependency_list)
    ...
    def _get_all_installed_packages(self) -> List[str]:
        ...
        if (self._all_installed_packages is None):
            self._all_installed_packages = [item.metadata.get('Name') for item in metadata.distributions() if (item.metadata.get('Name') is not None)]
        return self._all_installed_packages

    def _build_dependencies_info(self, dependency_names: List[str], install_environment: InstallEnvironment) -> List[PackageInfo]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for dependency_name in dependency_names:
            ...
            dependencies.append(PackageInfo(package_name=dependency_name, installed=installed, install_environment=install_environment, version=package_version))
        return dependencies
