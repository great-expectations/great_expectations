...
from great_expectations.core.usage_statistics.execution_environment import GEExecutionEnvironment, PackageInfo, PackageInfoSchema
from great_expectations.core.usage_statistics.schemas import anonymized_usage_statistics_record_schema
...
class UsageStatisticsHandler():
    ...
    def __init__(self, data_context: 'DataContext', data_context_id: str, usage_statistics_url: str) -> None:
        import inspect
    ...
    def build_init_payload(self) -> dict:
        ...
        expectation_suites: List[ExpectationSuite] = [self._data_context.get_expectation_suite(expectation_suite_name) for expectation_suite_name in self._data_context.list_expectation_suite_names()]
        init_payload = {'platform.system': platform.system(), 'platform.release': platform.release(), 'version_info': str(sys.version_info), 'datasources': self._data_context.project_config_with_variables_substituted.datasources, 'stores': self._data_context.stores, 'validation_operators': self._data_context.validation_operators, 'data_docs_sites': self._data_context.project_config_with_variables_substituted.data_docs_sites, 'expectation_suites': expectation_suites, 'dependencies': self._get_serialized_dependencies()}
        anonymized_init_payload = self._anonymizer.anonymize_init_payload(init_payload=init_payload)
    ...
    def _get_serialized_dependencies(self) -> List[dict]:
        ...
        serialized_dependencies: List[dict] = [schema.dump(package_info) for package_info in dependencies]
    ...
    def build_envelope(self, message: dict) -> dict:
        ...
        message['event_time'] = (datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:(- 3)] + 'Z')
        event_duration_property_name: str = f"{message['event']}.duration".replace('.', '_')
    ...
    def validate_message(message: dict, schema: dict) -> bool:
        ...
        try:
            ...
            logger.debug((f'{UsageStatsExceptionPrefix.INVALID_MESSAGE.value} invalid message: ' + str(e)))
            return False

    def send_usage_message(self, event: str, event_payload: Optional[dict]=None, success: Optional[bool]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        try:
            message: dict = {'event': event, 'event_payload': (event_payload or {}), 'success': success}
            self.emit(message)
        except Exception:
            pass
    ...
    def emit(self, message: dict) -> None:
        ...
        try:
            ...
            if (not self.validate_message(message, schema=anonymized_usage_statistics_record_schema)):
                return
            self._message_queue.put(message)
        except Exception as e:
            log_message: str = f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}'
...
def usage_statistics_enabled_method(func: Optional[Callable]=None, event_name: Optional[str]=None, args_payload_fn: Optional[Callable]=None, result_payload_fn: Optional[Callable]=None) -> Callable:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if callable(func):
        ...
        def usage_statistics_wrapped_method(*args, **kwargs):
            ...
            try:
                ...
                if handler:
                    event_duration_property_name: str = f'{event_name}.duration'.replace('.', '_')
        ...
        def usage_statistics_wrapped_method_partial(func):
            ...
            for (k, v) in __frame.f_locals.items():
                if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                    continue
                print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
            return usage_statistics_enabled_method(func, event_name=event_name, args_payload_fn=args_payload_fn, result_payload_fn=result_payload_fn)
        return usage_statistics_wrapped_method_partial

def run_validation_operator_usage_statistics(data_context: 'DataContext', validation_operator_name: str, assets_to_validate: list, **kwargs) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    payload = {}
    try:
        payload['anonymized_operator_name'] = anonymizer.anonymize(obj=validation_operator_name)
    except TypeError as e:
        logger.debug(f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}, run_validation_operator_usage_statistics: Unable to create validation_operator_name hash')
    if data_context._usage_statistics_handler:
        try:
            anonymizer = data_context._usage_statistics_handler.anonymizer
            payload['anonymized_batches'] = [anonymizer.anonymize(obj=batch) for batch in assets_to_validate]
        except Exception as e:
            logger.debug(f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}, run_validation_operator_usage_statistics: Unable to create anonymized_batches payload field')
    return payload

def save_expectation_suite_usage_statistics(data_context: 'DataContext', expectation_suite: ExpectationSuite, expectation_suite_name: Optional[str]=None, **kwargs) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    try:
        payload['anonymized_expectation_suite_name'] = anonymizer.anonymize(obj=expectation_suite_name)
    except Exception as e:
        logger.debug(f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}, save_expectation_suite_usage_statistics: Unable to create anonymized_expectation_suite_name payload field')
    return payload

def edit_expectation_suite_usage_statistics(data_context: 'DataContext', expectation_suite_name: str, interactive_mode: Optional[CLISuiteInteractiveFlagCombinations]=None) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    try:
        ...
        logger.debug(f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}, edit_expectation_suite_usage_statistics: Unable to create anonymized_expectation_suite_name payload field')
    return payload

def add_datasource_usage_statistics(data_context: 'DataContext', name: str, **kwargs) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    datasource_anonymizer = DatasourceAnonymizer(salt=data_context_id, aggregate_anonymizer=aggregate_anonymizer)
    payload = {}
    try:
        payload = datasource_anonymizer._anonymize_datasource_info(name, kwargs)
    except Exception as e:
        logger.debug(f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}, add_datasource_usage_statistics: Unable to create add_datasource_usage_statistics payload field')
    return payload
...
def get_batch_list_usage_statistics(data_context: 'DataContext', *args, **kwargs) -> dict:
    ...
    if data_context._usage_statistics_handler:
        try:
            ...
            logger.debug(f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}, get_batch_list_usage_statistics: Unable to create anonymized_batch_request payload field')
    return payload

def get_checkpoint_run_usage_statistics(checkpoint: 'Checkpoint', *args, **kwargs) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    usage_statistics_handler: Optional[UsageStatisticsHandler] = checkpoint._usage_statistics_handler
    ...
    if usage_statistics_handler:
        try:
            ...
            resolved_runtime_kwargs: dict = CheckpointConfig.resolve_config_using_acceptable_arguments(*(checkpoint,), **kwargs)
            payload: dict = anonymizer.anonymize(*(checkpoint,), **resolved_runtime_kwargs)
        except Exception as e:
            logger.debug(f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}, get_checkpoint_run_usage_statistics: Unable to create anonymized_checkpoint_run payload field')
    return payload

def get_profiler_run_usage_statistics(profiler: 'RuleBasedProfiler', variables: Optional[dict]=None, rules: Optional[dict]=None, *args, **kwargs) -> dict:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    usage_statistics_handler: Optional[UsageStatisticsHandler] = profiler._usage_statistics_handler
    data_context_id: Optional[str] = None
    if usage_statistics_handler:
    ...
    if usage_statistics_handler:
        try:
            anonymizer = usage_statistics_handler.anonymizer
            resolved_runtime_config: 'RuleBasedProfilerConfig' = RuleBasedProfilerConfig.resolve_config_using_acceptable_arguments(profiler=profiler, variables=variables, rules=rules)
            payload: dict = anonymizer.anonymize(obj=resolved_runtime_config)
        except Exception as e:
            logger.debug(f'{UsageStatsExceptionPrefix.EMIT_EXCEPTION.value}: {e} type: {type(e)}, get_profiler_run_usage_statistics: Unable to create anonymized_profiler_run payload field')
    return payload

def send_usage_message(data_context: 'DataContext', event: str, event_payload: Optional[dict]=None, success: Optional[bool]=None) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    'send a usage statistics message.'
    try:
        handler: UsageStatisticsHandler = getattr(data_context, '_usage_statistics_handler', None)
        message: dict = {'event': event, 'event_payload': event_payload, 'success': success}
