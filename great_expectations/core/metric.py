...
class MetricIdentifier(DataContextKey):
    ...
    def from_object(cls, metric):
        ...
        if (not isinstance(metric, Metric)):
            raise GreatExpectationsError('Unable to build MetricIdentifier from object of type {} when Metric is expected.'.format(type(metric)))
...
class BatchMetric(Metric):
    ...
    def __init__(self, metric_name, metric_kwargs, batch_identifier, metric_value) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
...
class ValidationMetric(Metric):
    ...
    def __init__(self, run_id, data_asset_name, expectation_suite_identifier, metric_name, metric_kwargs, metric_value) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not isinstance(expectation_suite_identifier, ExpectationSuiteIdentifier)):
            expectation_suite_identifier = ExpectationSuiteIdentifier(expectation_suite_name=expectation_suite_identifier)
...
class ValidationMetricIdentifier(MetricIdentifier):
    ...
    def __init__(self, run_id, data_asset_name, expectation_suite_identifier, metric_name, metric_kwargs_id) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def from_object(cls, validation_metric):
        ...
        if (not isinstance(validation_metric, ValidationMetric)):
            raise GreatExpectationsError('Unable to build ValidationMetricIdentifier from object of type {} when ValidationMetric is expected.'.format(type(validation_metric)))
        return cls(run_id=validation_metric.run_id, data_asset_name=validation_metric.data_asset_name, expectation_suite_identifier=validation_metric.expectation_suite_identifier, metric_name=validation_metric.metric_name, metric_kwargs_id=validation_metric.metric_kwargs_id)
    ...
    def to_tuple(self):
        ...
        return tuple((((list(self.run_id.to_tuple()) + [tuple_data_asset_name]) + list(self.expectation_suite_identifier.to_tuple())) + [self.metric_name, (self.metric_kwargs_id or '__')]))
    ...
    def to_fixed_length_tuple(self):
        ...
        if (self.data_asset_name is None):
            ...
            tuple_data_asset_name = self.data_asset_name
        return tuple((((list(self.run_id.to_tuple()) + [tuple_data_asset_name]) + list(self.expectation_suite_identifier.to_fixed_length_tuple())) + [self.metric_name, (self.metric_kwargs_id or '__')]))
    ...
    def to_evaluation_parameter_urn(self):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        if (self._metric_kwargs_id is None):
            return ('urn:great_expectations:validations:' + ':'.join((list(self.expectation_suite_identifier.to_fixed_length_tuple()) + [self.metric_name])))
        else:
            return ('urn:great_expectations:validations:' + ':'.join((list(self.expectation_suite_identifier.to_fixed_length_tuple()) + [self.metric_name, self._metric_kwargs_id])))
    ...
    def from_tuple(cls, tuple_):
        ...
        return cls(run_id=RunIdentifier.from_tuple((tuple_[0], tuple_[1])), data_asset_name=tuple_data_asset_name, expectation_suite_identifier=ExpectationSuiteIdentifier.from_tuple(tuple_[3:(- 2)]), metric_name=metric_id.metric_name, metric_kwargs_id=metric_id.metric_kwargs_id)
    ...
    def from_fixed_length_tuple(cls, tuple_):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        if (len(tuple_) != 6):
            raise GreatExpectationsError('ValidationMetricIdentifier fixed length tuple must have exactly six components.')
        if (tuple_[2] == '__'):
            tuple_data_asset_name = None
        else:
            tuple_data_asset_name = tuple_[2]
        metric_id = MetricIdentifier.from_tuple(tuple_[(- 2):])
        return cls(run_id=RunIdentifier.from_fixed_length_tuple((tuple_[0], tuple_[1])), data_asset_name=tuple_data_asset_name, expectation_suite_identifier=ExpectationSuiteIdentifier.from_fixed_length_tuple(tuple((tuple_[3],))), metric_name=metric_id.metric_name, metric_kwargs_id=metric_id.metric_kwargs_id)
