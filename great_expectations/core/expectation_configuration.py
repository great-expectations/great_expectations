...
from great_expectations.core.evaluation_parameters import _deduplicate_evaluation_parameter_dependencies, build_evaluation_parameters, find_evaluation_parameter_dependencies
from great_expectations.core.urn import ge_urn
from great_expectations.core.util import convert_to_json_serializable, ensure_json_serializable, nested_update
from great_expectations.exceptions import InvalidExpectationConfigurationError, InvalidExpectationKwargsError, ParserError
from great_expectations.expectations.registry import get_expectation_impl
from great_expectations.marshmallow__shade import Schema, ValidationError, fields, post_dump, post_load
...
def parse_result_format(result_format: Union[(str, dict)]) -> dict:
    ...
    if isinstance(result_format, str):
        result_format = {'result_format': result_format, 'partial_unexpected_count': 20, 'include_unexpected_rows': False}
    else:
        if (('include_unexpected_rows' in result_format) and ('result_format' not in result_format)):
            raise ValueError('When using `include_unexpected_rows`, `result_format` must be explicitly specified')
...
class ExpectationConfiguration(SerializableDictDot):
    ...
    def __init__(self, expectation_type: str, kwargs: dict, meta: Optional[dict]=None, success_on_last_run: Optional[bool]=None, ge_cloud_id: Optional[str]=None, expectation_context: Optional[ExpectationContext]=None) -> None:
        ...
        if (not isinstance(kwargs, dict)):
            raise InvalidExpectationConfigurationError('expectation configuration kwargs must be a dict.')
        self._kwargs = kwargs
    ...
    def process_evaluation_parameters(self, evaluation_parameters, interactive_evaluation: bool=True, data_context: Optional[Any]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        (evaluation_args, substituted_parameters) = build_evaluation_parameters(self._kwargs, evaluation_parameters, interactive_evaluation, data_context)
    ...
    def patch(self, op: str, path: str, value: Any) -> 'ExpectationConfiguration':
        ...
        try:
            ...
            raise IndexError("Ensure you have a valid jsonpatch path of the form '/path/foo' (see http://jsonpatch.com/)")
    ...
    def _get_default_custom_kwargs(self) -> dict:
        ...
        if self.expectation_type.startswith('expect_column_pair'):
            return {'domain_kwargs': ['column_A', 'column_B', 'row_condition', 'condition_parser'], 'success_kwargs': [], 'default_kwarg_values': {'column_A': None, 'column_B': None, 'row_condition': None, 'condition_parser': None}}
        elif self.expectation_type.startswith('expect_column'):
            return {'domain_kwargs': ['column', 'row_condition', 'condition_parser'], 'success_kwargs': [], 'default_kwarg_values': {'column': None, 'row_condition': None, 'condition_parser': None}}
    ...
    def get_domain_kwargs(self) -> dict:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        expectation_kwargs_dict = self.kwarg_lookup_dict.get(self.expectation_type, None)
        if (expectation_kwargs_dict is None):
            ...
            default_kwarg_values = expectation_kwargs_dict.get('default_kwarg_values', {})
            domain_keys = expectation_kwargs_dict['domain_kwargs']
        domain_kwargs = {key: self.kwargs.get(key, default_kwarg_values.get(key)) for key in domain_keys}
    ...
    def get_success_kwargs(self) -> dict:
        ...
        success_kwargs = {key: self.kwargs.get(key, default_kwarg_values.get(key)) for key in success_keys}
    ...
    def get_runtime_kwargs(self, runtime_configuration: Optional[dict]=None) -> dict:
        ...
        runtime_kwargs = {key: lookup_kwargs.get(key, default_kwarg_values.get(key)) for key in runtime_keys}
        runtime_kwargs['result_format'] = parse_result_format(runtime_kwargs['result_format'])
        runtime_kwargs.update(success_kwargs)
        return runtime_kwargs

    def applies_to_same_domain(self, other_expectation_configuration: 'ExpectationConfiguration') -> bool:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def isEquivalentTo(self, other: Union[(dict, 'ExpectationConfiguration')], match_type: str='success') -> bool:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not isinstance(other, self.__class__)):
            if isinstance(other, dict):
                try:
                    ...
                    logger.debug('Unable to evaluate equivalence of ExpectationConfiguration object with dict because dict other could not be instantiated as an ExpectationConfiguration')
                    return NotImplemented
            else:
                return NotImplemented
        if (match_type == 'domain'):
            return all(((self.expectation_type == other.expectation_type), (self.get_domain_kwargs() == other.get_domain_kwargs())))
        elif (match_type == 'success'):
            return all(((self.expectation_type == other.expectation_type), (self.get_success_kwargs() == other.get_success_kwargs())))
        elif (match_type == 'runtime'):
            return all(((self.expectation_type == other.expectation_type), (self.kwargs == other.kwargs)))
        return False
    ...
    def __eq__(self, other):
        ...
        return all(((self.expectation_type == other.expectation_type), (this_kwargs == other_kwargs), (this_meta == other_meta)))
    ...
    def get_evaluation_parameter_dependencies(self) -> dict:
        ...
        for value in self.kwargs.values():
            if (isinstance(value, dict) and ('$PARAMETER' in value)):
                param_string_dependencies = find_evaluation_parameter_dependencies(value['$PARAMETER'])
        ...
        for string_urn in urns:
            try:
                ...
                logger.warning(f"Unable to parse great_expectations urn {value['$PARAMETER']}")
                continue
    ...
    @staticmethod
    def _update_dependencies_with_expectation_suite_urn(dependencies: dict, urn: ParseResults) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (not urn.get('metric_kwargs')):
            nested_update(dependencies, {urn['expectation_suite_name']: [urn['metric_name']]})
        else:
            nested_update(dependencies, {urn['expectation_suite_name']: [{'metric_kwargs_id': {urn['metric_kwargs']: [urn['metric_name']]}}]})
    ...
    def validate(self, validator: Any, runtime_configuration=None):
        ...
        return expectation_impl(self).validate(validator=validator, runtime_configuration=runtime_configuration)

    def metrics_validate(self, metrics: Dict, runtime_configuration: dict=None, execution_engine=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        expectation_impl = self._get_expectation_impl()
        return expectation_impl(self).metrics_validate(metrics, runtime_configuration=runtime_configuration, execution_engine=execution_engine)

class ExpectationConfigurationSchema(Schema):
    expectation_type = fields.Str(required=True, error_messages={'required': 'expectation_type missing in expectation configuration'})
    kwargs = fields.Dict(required=False, allow_none=True)
    meta = fields.Dict(required=False, allow_none=True)
    ge_cloud_id = fields.UUID(required=False, allow_none=True)
    expectation_context = fields.Nested((lambda : ExpectationContextSchema), required=False, allow_none=True)
