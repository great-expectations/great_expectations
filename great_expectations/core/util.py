...
SCHEMAS = {'api_np': {'NegativeInfinity': (- np.inf), 'PositiveInfinity': np.inf}, 'api_cast': {'NegativeInfinity': (- float('inf')), 'PositiveInfinity': float('inf')}, 'mysql': {'NegativeInfinity': (- 1.79e+308), 'PositiveInfinity': 1.79e+308}, 'mssql': {'NegativeInfinity': (- 1.79e+308), 'PositiveInfinity': 1.79e+308}}
try:
    import pyspark
except ImportError:
    pyspark = None
...
def nested_update(d: Union[(Iterable, dict)], u: Union[(Iterable, dict)], dedup: bool=False, concat_lists: bool=True):
    import inspect
...
def convert_to_json_serializable(data):
    ...
    if isinstance(data, pd.Series):
        ...
        return [{index_name: convert_to_json_serializable(idx), value_name: convert_to_json_serializable(val)} for (idx, val) in data.iteritems()]
    ...
    if isinstance(data, decimal.Decimal):
        if requires_lossy_conversion(data):
            logger.warning(f'Using lossy conversion for decimal {data} to float object to support serialization.')
        return float(data)
    if isinstance(data, RunIdentifier):
        return data.to_json_dict()
    else:
        raise TypeError(f'{str(data)} is of type {type(data).__name__} which cannot be serialized.')
...
def ensure_json_serializable(data):
    ...
    if isinstance(data, pd.Series):
        index_name = (data.index.name or 'index')
        value_name = (data.name or 'value')
        _ = [{index_name: ensure_json_serializable(idx), value_name: ensure_json_serializable(val)} for (idx, val) in data.iteritems()]
        return
    if (pyspark and isinstance(data, pyspark.sql.DataFrame)):
    ...
    if isinstance(data, RunIdentifier):
        ...
        raise InvalidExpectationConfigurationError(('%s is of type %s which cannot be serialized to json' % (str(data), type(data).__name__)))
...
def substitute_all_strftime_format_strings(data: Union[(dict, list, str, Any)], datetime_obj: Optional[datetime.datetime]=None) -> Union[(str, Any)]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def get_datetime_string_from_strftime_format(format_str: str, datetime_obj: Optional[datetime.datetime]=None) -> str:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def parse_string_to_datetime(datetime_string: str, datetime_format_string: Optional[str]=None) -> datetime.date:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (not isinstance(datetime_string, str)):
        raise ge_exceptions.SorterError(f'''Source "datetime_string" must have string type (actual type is "{str(type(datetime_string))}").
            ''')
...
class AzureUrl():
    ...
    def __init__(self, url: str) -> None:
        ...
        if (search is None):
            ...
            assert (search is not None), 'The provided URL does not adhere to the format specified by the Azure SDK (<ACCOUNT_NAME>.blob.core.windows.net/<CONTAINER>/<BLOB>)'
            ...
            assert (search is not None), 'The provided URL does not adhere to the format specified by the Azure SDK (wasbs://<CONTAINER>@<ACCOUNT_NAME>.blob.core.windows.net/<BLOB>)'
            self._protocol = search.group(1)
...
class GCSUrl():
    ...
    def __init__(self, url: str) -> None:
        ...
        assert (search is not None), 'The provided URL does not adhere to the format specified by the GCS SDK (gs://<BUCKET_OR_NAME>/<BLOB>)'
...
def get_or_create_spark_application(spark_config: Optional[Dict[(str, str)]]=None, force_reuse_spark_context: bool=False):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    spark_session: Optional[SparkSession] = get_or_create_spark_session(spark_config=spark_config)
    if (spark_session is None):
        raise ValueError('SparkContext could not be started.')
    sc_stopped: bool = spark_session.sparkContext._jsc.sc().isStopped()
    if ((not force_reuse_spark_context) and spark_restart_required(current_spark_config=spark_session.sparkContext.getConf().getAll(), desired_spark_config=spark_config)):
        if (not sc_stopped):
            try:
...
def spark_restart_required(current_spark_config: List[tuple], desired_spark_config: dict) -> bool:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (desired_spark_config.get('spark.app.name') != current_spark_config_dict.get('spark.app.name')):
        return True
    if (not {(k, v) for (k, v) in desired_spark_config.items()}.issubset(current_spark_config)):
        return True
    return False
