...
class MetricFunctionTypes(enum.Enum):
    """Enum type, whose members depict the nature of return value of a metric implementation function (defined for
    a specified "ExecutionEngine" subclass) that is the final result (rather than a Callable for deferred execution).
    ...
    - MAP_VALUES (unused in current version) -- metric implementation function returns a mapping between every "Domain"
      value and the result of a transformation of the corresponding "Domain" value.
...
class MetricPartialFunctionTypes(enum.Enum):
    """Enum type, whose members depict the nature of return value of a metric implementation function (defined for
    a specified "ExecutionEngine" subclass) that is a (partial) Callable to be executed once execution plan is complete.

    The available types are:
    - MAP_FN -- metric implementation function returns a mapping transformation for "Domain" values that evaluates to a
      quantity (rather than a condition statement, or a series, etc.).
    ...
    - WINDOW_FN -- metric implementation function returns specified windowing operation over "Domain" values (currently
      applicable only to "SparkDFExecutionEngine").
