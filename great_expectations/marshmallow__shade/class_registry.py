...
def register(classname: str, cls: "SchemaType") -> None:
    ...
    if (classname in _registry) and (
        not any((each.__module__ == module) for each in _registry[classname])
    ):
...
def get_class(
    ...
    try:
        ...
        raise RegistryError(
            "Class with name {!r} was not found. You may need to import the class.".format(
                classname
            )
        ) from error
    if len(classes) > 1:
        ...
        raise RegistryError(
            "Multiple classes with name {!r} were found. Please use the full, module-qualified path.".format(
                classname
            )
        )
    else:
