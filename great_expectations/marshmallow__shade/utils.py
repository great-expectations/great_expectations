...
def is_iterable_but_not_string(obj) -> bool:
    ...
    return (hasattr(obj, "__iter__") and (not hasattr(obj, "strip"))) or is_generator(
        obj
    )
...
def is_aware(datetime: dt.datetime) -> bool:
    ...
    for (k, v) in __frame.f_locals.items():
        if any((var in k) for var in ("__frame", "__file", "__func")):
            continue
        print(f"<INTROSPECT> {__file}:{__func} - {k}:{v.__class__.__name__}")
    return (datetime.tzinfo is not None) and (
        datetime.tzinfo.utcoffset(datetime) is not None
    )
