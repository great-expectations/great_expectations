...
from great_expectations.dataset.util import check_sql_engine_dialect, get_approximate_percentile_disc_sql
from great_expectations.util import generate_temporary_table_name, get_pyathena_potential_type, get_sqlalchemy_inspector, import_library_module
...
class SqlAlchemyBatchReference():
    ...
    def get_init_kwargs(self):
        ...
        if (self._table_name and self._query):
            kwargs = {'engine': self._engine, 'table_name': self._table_name, 'custom_sql': self._query}
...
class MetaSqlAlchemyDataset(Dataset):
    ...
    def column_map_expectation(cls, func):
        ...
        def inner_wrapper(self, column, mostly=None, result_format=None, *args, **kwargs):
            ...
            if (((len(ignore_values) > 0) and (None not in ignore_values)) or ((len(ignore_values) > 1) and (None in ignore_values))):
                ignore_values_conditions += [sa.column(column).in_([val for val in ignore_values if (val is not None)])]
            ...
            if (len(ignore_values_conditions) > 1):
                ...
                ignore_values_condition = BinaryExpression(sa.literal(False), sa.literal(True), custom_op('='))
            count_query: Select
            if (self.sql_engine_dialect.name.lower() == 'mssql'):
                count_query = self._get_count_query_mssql(expected_condition=expected_condition, ignore_values_condition=ignore_values_condition)
            else:
                count_query = self._get_count_query_generic_sqlalchemy(expected_condition=expected_condition, ignore_values_condition=ignore_values_condition)
            ...
            if (self.engine.dialect.name.lower() == 'oracle'):
                raw_query = sa.select([sa.column(column)]).select_from(self._table).where(sa.and_(sa.not_(expected_condition), sa.not_(ignore_values_condition)))
                query = str(raw_query.compile(self.engine, compile_kwargs={'literal_binds': True}))
                query += ('\nAND ROWNUM <= %d' % unexpected_count_limit)
            else:
                query = sa.select([sa.column(column)]).select_from(self._table).where(sa.and_(sa.not_(expected_condition), sa.not_(ignore_values_condition))).limit(unexpected_count_limit)
            unexpected_query_results = self.engine.execute(query)
            nonnull_count: int = (count_results['element_count'] - count_results['null_count'])
            ...
            return_obj = self._format_map_output(result_format, success, count_results['element_count'], nonnull_count, count_results['unexpected_count'], maybe_limited_unexpected_list, None)
            if (func.__name__ in ['expect_column_values_to_not_be_null', 'expect_column_values_to_be_null']):
    ...
    def _get_count_query_mssql(self, expected_condition: BinaryExpression, ignore_values_condition: BinaryExpression) -> Select:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        with self.engine.begin():
            ...
            count_case_statement: List[sa.sql.elements.Label] = [sa.case([(sa.and_(sa.not_(expected_condition), sa.not_(ignore_values_condition)), 1)], else_=0).label('condition')]
            inner_case_query: sa.sql.dml.Insert = temp_table_obj.insert().from_select(count_case_statement, sa.select(count_case_statement).select_from(self._table))
            self.engine.execute(inner_case_query)
        element_count_query: Select = sa.select([sa.func.count().label('element_count'), sa.func.sum(sa.case([(ignore_values_condition, 1)], else_=0)).label('null_count')]).select_from(self._table).alias('ElementAndNullCountsSubquery')
        unexpected_count_query: Select = sa.select([sa.func.sum(sa.column('condition')).label('unexpected_count')]).select_from(temp_table_obj).alias('UnexpectedCountSubquery')
        count_query: Select = sa.select([element_count_query.c.element_count, element_count_query.c.null_count, unexpected_count_query.c.unexpected_count])
        return count_query

    def _get_count_query_generic_sqlalchemy(self, expected_condition: BinaryExpression, ignore_values_condition: BinaryExpression) -> Select:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return sa.select([sa.func.count().label('element_count'), sa.func.sum(sa.case([(ignore_values_condition, 1)], else_=0)).label('null_count'), sa.func.sum(sa.case([(sa.and_(sa.not_(expected_condition), sa.not_(ignore_values_condition)), 1)], else_=0)).label('unexpected_count')]).select_from(self._table)
...
class SqlAlchemyDataset(MetaSqlAlchemyDataset):
    ...
    def __init__(self, table_name=None, engine=None, connection_string=None, custom_sql=None, schema=None, *args, **kwargs) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (dialect_name in ['postgresql', 'mysql', 'sqlite', 'oracle', 'mssql', 'hive']):
            self.dialect = import_library_module(module_name=f'sqlalchemy.dialects.{self.engine.dialect.name}')
        elif (dialect_name == 'snowflake'):
            self.dialect = import_library_module(module_name='snowflake.sqlalchemy.snowdialect')
        elif (self.engine.dialect.name.lower() == 'dremio'):
            self.dialect = import_library_module(module_name='sqlalchemy_dremio.pyodbc.dialect')
        elif (dialect_name == 'redshift'):
            self.dialect = import_library_module(module_name='sqlalchemy_redshift.dialect')
            ...
            self.dialect = import_library_module(module_name='pyathena.sqlalchemy_athena')
        elif (dialect_name == 'teradatasql'):
            self.dialect = import_library_module(module_name='teradatasqlalchemy.dialect')
        else:
            self.dialect = None
        ...
        if ((custom_sql is not None) and (self.engine.dialect.name.lower() == 'bigquery')):
            if ((self.generated_table_name is not None) and (self.engine.dialect.dataset_id is None)):
                raise ValueError('No BigQuery dataset specified. Please specify a default dataset in engine url')
        if custom_sql:
            self.create_temporary_table(table_name, custom_sql, schema_name=temp_table_schema_name)
    ...
    def attempt_allowing_relative_error(self):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        detected_redshift: bool = ((sqlalchemy_redshift is not None) and check_sql_engine_dialect(actual_sql_engine_dialect=self.sql_engine_dialect, candidate_sql_engine_dialect=sqlalchemy_redshift.dialect.RedshiftDialect))
        detected_psycopg2: bool = ((sqlalchemy_psycopg2 is not None) and check_sql_engine_dialect(actual_sql_engine_dialect=self.sql_engine_dialect, candidate_sql_engine_dialect=sqlalchemy_psycopg2.PGDialect_psycopg2))
    ...
    def head(self, n=5):
        ...
        try:
            df = next(pd.read_sql_table(table_name=self._table.name, schema=self._table.schema, con=self.engine, chunksize=n))
        ...
        return PandasDataset(df, expectation_suite=self.get_expectation_suite(discard_failed_expectations=False, discard_result_format_kwargs=False, discard_catch_exceptions_kwargs=False, discard_include_config_kwargs=False))
    ...
    def get_column_nonnull_count(self, column):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        ignore_values = [None]
        count_query = sa.select([sa.func.count().label('element_count'), sa.func.sum(sa.case([(sa.or_((sa.column(column).in_(ignore_values) if (self.engine.dialect.name.lower() != 'teradatasql') else False), (sa.column(column).is_(None) if (None in ignore_values) else False)), 1)], else_=0)).label('null_count')]).select_from(self._table)
    ...
    def get_column_sum(self, column):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return convert_to_json_serializable(self.engine.execute(sa.select([sa.func.sum(sa.column(column))]).select_from(self._table)).scalar())
    ...
    def get_column_max(self, column, parse_strings_as_datetimes=False):
        ...
        return convert_to_json_serializable(self.engine.execute(sa.select([sa.func.max(sa.column(column))]).select_from(self._table)).scalar())
    ...
    def get_column_min(self, column, parse_strings_as_datetimes=False):
        ...
        if parse_strings_as_datetimes:
            raise NotImplementedError
        return convert_to_json_serializable(self.engine.execute(sa.select([sa.func.min(sa.column(column))]).select_from(self._table)).scalar())
    ...
    def get_column_value_counts(self, column, sort='value', collate=None):
        ...
        query = sa.select([sa.column(column).label('value'), sa.func.count(sa.column(column)).label('count')]).where((sa.column(column) != None)).group_by(sa.column(column))
        if (sort == 'value'):
        ...
        series = pd.Series([row[1] for row in results], index=pd.Index(data=[row[0] for row in results], name='value'), name='count')
        return series
    ...
    def get_column_mean(self, column):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return convert_to_json_serializable(self.engine.execute(sa.select([sa.func.avg((sa.column(column) * 1.0))]).select_from(self._table)).scalar())
    ...
    def get_column_unique_count(self, column):
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return convert_to_json_serializable(self.engine.execute(sa.select([sa.func.count(sa.func.distinct(sa.column(column)))]).select_from(self._table)).scalar())
    ...
    def get_column_median(self, column):
        ...
        if (self.sql_engine_dialect.name.lower() == 'awsathena'):
            element_values = self.engine.execute(f'SELECT approx_percentile({column},  0.5) FROM {self._table}')
            return convert_to_json_serializable(element_values.fetchone()[0])
        else:
            nonnull_count = self.get_column_nonnull_count(column)
            element_values = self.engine.execute(sa.select([sa.column(column)]).order_by(sa.column(column)).where((sa.column(column) != None)).offset(max(((nonnull_count // 2) - 1), 0)).limit(2).select_from(self._table))
    ...
    def get_column_quantiles(self, column: str, quantiles: Iterable, allow_relative_error: bool=False) -> list:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (self.sql_engine_dialect.name.lower() == 'mssql'):
            ...
            return self._get_column_quantiles_bigquery(column=column, quantiles=quantiles)
            ...
            return self._get_column_quantiles_generic_sqlalchemy(column=column, quantiles=quantiles, allow_relative_error=allow_relative_error)
        elif (self.sql_engine_dialect.name.lower() == 'sqlite'):
            return self._get_column_quantiles_sqlite(column=column, quantiles=quantiles)
        else:
            return convert_to_json_serializable(self._get_column_quantiles_generic_sqlalchemy(column=column, quantiles=quantiles, allow_relative_error=allow_relative_error))
    ...
    def _get_column_quantiles_mssql(self, column: str, quantiles: Iterable) -> list:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        selects: List[WithinGroup] = [sa.func.percentile_disc(quantile).within_group(sa.column(column).asc()).over() for quantile in quantiles]
    ...
    def _get_column_quantiles_bigquery(self, column: str, quantiles: Iterable) -> list:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        selects: List[WithinGroup] = [sa.func.percentile_disc(sa.column(column), quantile).over() for quantile in quantiles]
        quantiles_query: Select = sa.select(selects).select_from(self._table)
        try:
    ...
    def _get_column_quantiles_mysql(self, column: str, quantiles: Iterable) -> list:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        percent_rank_query: CTE = sa.select([sa.column(column), sa.cast(sa.func.percent_rank().over(order_by=sa.column(column).asc()), sa.dialects.mysql.DECIMAL(18, 15)).label('p')]).order_by(sa.column('p').asc()).select_from(self._table).cte('t')
        selects: List[WithinGroup] = []
        for (idx, quantile) in enumerate(quantiles):
            if np.issubdtype(type(quantile), np.float_):
                quantile = float(quantile)
            quantile_column: Label = sa.func.first_value(sa.column(column)).over(order_by=sa.case([((percent_rank_query.c.p <= sa.cast(quantile, sa.dialects.mysql.DECIMAL(18, 15))), percent_rank_query.c.p)], else_=None).desc()).label(f'q_{idx}')
    ...
    def _get_column_quantiles_sqlite(self, column, quantiles: Iterable) -> list:
        ...
        table_row_count_query: Select = sa.select([sa.func.count()]).select_from(self._table)
        table_row_count_result: Optional[Row] = None
        try:
            table_row_count_result = self.engine.execute(table_row_count_query).fetchone()
        except ProgrammingError as pe:
            self._treat_quantiles_exception(pe)
        table_row_count: int
        ...
        quantile_queries: List[Select] = [sa.select([sa.column(column)]).order_by(sa.column(column).asc()).offset(offset).limit(1).select_from(self._table) for offset in offsets]
        quantile_result: Row
        quantile_query: Select
        try:
            quantiles_results: List[Row] = [self.engine.execute(quantile_query).fetchone() for quantile_query in quantile_queries]
            return list(itertools.chain.from_iterable([list(quantile_result) for quantile_result in quantiles_results]))
        except ProgrammingError as pe:
            self._treat_quantiles_exception(pe)

    def _get_column_quantiles_generic_sqlalchemy(self, column: str, quantiles: Iterable, allow_relative_error: bool) -> list:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        selects: List[WithinGroup] = [sa.func.percentile_disc(quantile).within_group(sa.column(column).asc()) for quantile in quantiles]
        quantiles_query: Select = sa.select(selects).select_from(self._table)
        try:
            quantiles_results: Row = self.engine.execute(quantiles_query).fetchone()
            return list(quantiles_results)
            ...
            if self.attempt_allowing_relative_error():
                sql_approx: str = get_approximate_percentile_disc_sql(selects=selects, sql_engine_dialect=self.sql_engine_dialect)
                selects_approx: List[TextClause] = [sa.text(sql_approx)]
                quantiles_query_approx: Select = sa.select(selects_approx).select_from(self._table)
                if allow_relative_error:
                    try:
                        quantiles_results: Row = self.engine.execute(quantiles_query_approx).fetchone()
                        return list(quantiles_results)
                    except ProgrammingError as pe:
                        self._treat_quantiles_exception(pe)
                else:
                    raise ValueError(f'The SQL engine dialect "{str(self.sql_engine_dialect)}" does not support computing quantiles without approximation error; set allow_relative_error to True to allow approximate quantiles.')
            else:
                raise ValueError(f'The SQL engine dialect "{str(self.sql_engine_dialect)}" does not support computing quantiles with approximation error; set allow_relative_error to False to disable approximate quantiles.')
    ...
    def get_column_stdev(self, column):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        if (self.sql_engine_dialect.name.lower() == 'mssql'):
            res = self.engine.execute(sa.select([sa.func.stdev(sa.column(column))]).select_from(self._table).where((sa.column(column) is not None))).fetchone()
        else:
            res = self.engine.execute(sa.select([sa.func.stddev_samp(sa.column(column))]).select_from(self._table).where((sa.column(column) is not None))).fetchone()
        return float(res[0])
    ...
    def get_column_hist(self, column, bins):
        ...
        for idx in range(idx, (len(bins) - 2)):
            case_conditions.append(sa.func.sum(sa.case([(sa.and_((sa.column(column) >= bins[idx]), (sa.column(column) < bins[(idx + 1)])), 1)], else_=0)).label(f'bin_{str(idx)}'))
        if ((bins[(- 1)] == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=False)) or (bins[(- 1)] == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=False))):
            case_conditions.append(sa.func.sum(sa.case([((sa.column(column) >= bins[(- 2)]), 1)], else_=0)).label(f'bin_{str((len(bins) - 1))}'))
        else:
            case_conditions.append(sa.func.sum(sa.case([(sa.and_((sa.column(column) >= bins[(- 2)]), (sa.column(column) <= bins[(- 1)])), 1)], else_=0)).label(f'bin_{str((len(bins) - 1))}'))
        query = sa.select(case_conditions).where((sa.column(column) != None)).select_from(self._table)
    ...
    def get_column_count_in_range(self, column, min_val=None, max_val=None, strict_min=False, strict_max=True):
        ...
        if ((min_val == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=True)) or (min_val == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=True))):
            min_val = get_sql_dialect_floating_point_infinity_value(schema=self.sql_engine_dialect.name.lower(), negative=True)
        if ((min_val == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=False)) or (min_val == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=False))):
            min_val = get_sql_dialect_floating_point_infinity_value(schema=self.sql_engine_dialect.name.lower(), negative=False)
        if ((max_val == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=True)) or (max_val == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=True))):
            max_val = get_sql_dialect_floating_point_infinity_value(schema=self.sql_engine_dialect.name.lower(), negative=True)
        if ((max_val == get_sql_dialect_floating_point_infinity_value(schema='api_np', negative=False)) or (max_val == get_sql_dialect_floating_point_infinity_value(schema='api_cast', negative=False))):
            max_val = get_sql_dialect_floating_point_infinity_value(schema=self.sql_engine_dialect.name.lower(), negative=False)
        min_condition = None
        max_condition = None
        ...
        query = sa.select([sa.func.count(sa.column(column))]).where(sa.and_((sa.column(column) != None), condition)).select_from(self._table)
    ...
    def create_temporary_table(self, table_name, custom_sql, schema_name=None) -> None:
        ...
        if (engine_dialect == 'bigquery'):
            ...
            stmt = f'{custom_sqlmod[0]}into {{table_name}} from{custom_sqlmod[1]}'.format(table_name=table_name)
        elif (engine_dialect == 'awsathena'):
            stmt = f'CREATE TABLE {table_name} AS {custom_sql}'
        elif (engine_dialect == 'oracle'):
            stmt_1 = 'CREATE PRIVATE TEMPORARY TABLE {table_name} ON COMMIT PRESERVE DEFINITION AS {custom_sql}'.format(table_name=table_name, custom_sql=custom_sql)
            stmt_2 = 'CREATE GLOBAL TEMPORARY TABLE {table_name} ON COMMIT PRESERVE ROWS AS {custom_sql}'.format(table_name=table_name, custom_sql=custom_sql)
        elif (engine_dialect == 'teradatasql'):
            stmt = 'CREATE VOLATILE TABLE "{table_name}" AS ({custom_sql}) WITH DATA NO PRIMARY INDEX ON COMMIT PRESERVE ROWS'.format(table_name=table_name, custom_sql=custom_sql)
        elif (self.sql_engine_dialect.name.lower() in ('hive', b'hive')):
            stmt = 'CREATE TEMPORARY TABLE {schema_name}.{table_name} AS {custom_sql}'.format(schema_name=(schema_name if (schema_name is not None) else 'default'), table_name=table_name, custom_sql=custom_sql)
        else:
            stmt = f'CREATE TEMPORARY TABLE "{table_name}" AS {custom_sql}'
    ...
    def column_reflection_fallback(self):
        ...
        if (self.sql_engine_dialect.name.lower() == 'mssql'):
            ...
            col_info_query: TextClause = sa.text(f'''
SELECT
    cols.NAME, ty.NAME
FROM
...
WHERE
    object_id = OBJECT_ID('tempdb..{self._table}')
            ...
            col_info_dict_list = [{'name': col_name, 'type': getattr(type_module, col_type.upper())()} for (col_name, col_type) in col_info_tuples_list]
        else:
    ...
    def expect_table_row_count_to_equal_other_table(self, other_table_name, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def expect_compound_columns_to_be_unique(self, column_list, ignore_row_if='all_values_are_missing', result_format=None, row_condition=None, condition_parser=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        columns = [sa.column(col['name']) for col in self.columns if (col['name'] in column_list)]
        query = sa.select([sa.func.count()]).group_by(*columns).having((sa.func.count() > 1)).select_from(self._table)
        if (ignore_row_if == 'all_values_are_missing'):
            ...
            raise ValueError(f'ignore_row_if was set to an unexpected value: {ignore_row_if}')
        ...
        return {'success': (unexpected_count == 0), 'result': {'unexpected_percent': unexpected_percent}}

    @DocInherit
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_be_null(self, column, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_not_be_null(self, column, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def _get_dialect_type_module(self):
        ...
        try:
            ...
            pass
        try:
            if (isinstance(self.sql_engine_dialect, sqla_bigquery.BigQueryDialect) and (bigquery_types_tuple is not None)):
                return bigquery_types_tuple
        except (TypeError, AttributeError):
            pass
        try:
            if (isinstance(self.sql_engine_dialect, teradatasqlalchemy.dialect.TeradataDialect) and (teradatatypes is not None)):
    ...
    def expect_column_values_to_be_of_type(self, column, type_, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def expect_column_values_to_be_in_type_list(self, column, type_list, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_be_in_set(self, column, value_set, mostly=None, parse_strings_as_datetimes=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_not_be_in_set(self, column, value_set, mostly=None, parse_strings_as_datetimes=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_be_between(self, column, min_value=None, max_value=None, strict_min=False, strict_max=False, allow_cross_type_comparisons=None, parse_strings_as_datetimes=None, output_strftime_format=None, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (min_value is None):
            ...
            return sa.and_((sa.column(column) > min_value), (sa.column(column) < max_value))
        elif strict_min:
            return sa.and_((sa.column(column) > min_value), (sa.column(column) <= max_value))
        elif strict_max:
            return sa.and_((sa.column(column) >= min_value), (sa.column(column) < max_value))
    ...
    @DocInherit
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_value_lengths_to_equal(self, column, value, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_value_lengths_to_be_between(self, column, min_value=None, max_value=None, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if ((min_value is not None) and (max_value is not None)):
            return sa.and_((sa.func.length(sa.column(column)) >= min_value), (sa.func.length(sa.column(column)) <= max_value))
    ...
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_be_unique(self, column, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        dup_query = sa.select([sa.column(column)]).select_from(self._table).group_by(sa.column(column)).having((sa.func.count(sa.column(column)) > 1))
        if (self.sql_engine_dialect.name.lower() == 'mysql'):
            temp_table_name = generate_temporary_table_name()
            temp_table_stmt = 'CREATE TEMPORARY TABLE {new_temp_table} AS SELECT tmp.{column_name} FROM {source_table} tmp'.format(new_temp_table=temp_table_name, source_table=self._table, column_name=column)
            self.engine.execute(temp_table_stmt)
            dup_query = sa.select([sa.column(column)]).select_from(sa.text(temp_table_name)).group_by(sa.column(column)).having((sa.func.count(sa.column(column)) > 1))
    ...
    def _get_dialect_regex_expression(self, column, regex, positive=True):
        ...
        try:
            ...
            pass
        try:
            if isinstance(self.sql_engine_dialect, snowflake.sqlalchemy.snowdialect.SnowflakeDialect):
                if positive:
        ...
        try:
            if isinstance(self.sql_engine_dialect, sqlalchemy_dremio.pyodbc.dialect):
                if positive:
                    ...
                    return sa.not_(sa.func.REGEXP_MATCHES(sa.column(column), literal(regex)))
        except (AttributeError, TypeError):
            pass
        try:
            if isinstance(self.sql_engine_dialect, teradatasqlalchemy.dialect.TeradataDialect):
                if positive:
                    ...
                    return (sa.func.REGEXP_SIMILAR(sa.column(column), literal(regex), literal('i')) == 0)
        except (AttributeError, TypeError):
            pass
        try:
            if isinstance(self.sql_engine_dialect, pyathena.sqlalchemy_athena.AthenaDialect):
                if positive:
                    return sa.func.REGEXP_LIKE(sa.column(column), literal(regex))
                else:
                    return sa.not_(sa.func.REGEXP_LIKE(sa.column(column), literal(regex)))
        except (AttributeError, TypeError):
            pass
        return None

    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_match_regex(self, column, regex, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (regex_expression is None):
            logger.warning(f'Regex is not supported for dialect {str(self.sql_engine_dialect)}')
            raise NotImplementedError
        return regex_expression

    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_not_match_regex(self, column, regex, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_match_regex_list(self, column, regex_list, match_on='any', mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (regex_expression is None):
            ...
            raise NotImplementedError
        if (match_on == 'any'):
            condition = sa.or_(*(self._get_dialect_regex_expression(column, regex) for regex in regex_list))
        else:
            condition = sa.and_(*(self._get_dialect_regex_expression(column, regex) for regex in regex_list))
        return condition

    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_not_match_regex_list(self, column, regex_list, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (regex_expression is None):
            logger.warning(f'Regex is not supported for dialect {str(self.sql_engine_dialect)}')
            raise NotImplementedError
        return sa.and_(*(self._get_dialect_regex_expression(column, regex, positive=False) for regex in regex_list))
    ...
    def _get_dialect_like_pattern_expression(self, column, like_pattern, positive=True):
        ...
        try:
            ...
            logger.debug('Unable to load BigQueryDialect dialect while running _get_dialect_like_pattern_expression', exc_info=True)
            pass
        if isinstance(self.sql_engine_dialect, (sa.dialects.sqlite.dialect, sa.dialects.postgresql.dialect, sa.dialects.mysql.dialect, sa.dialects.mssql.dialect)):
            dialect_supported = True
        try:
            if isinstance(self.sql_engine_dialect, sqlalchemy_redshift.dialect.RedshiftDialect):
                dialect_supported = True
    ...
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_match_like_pattern(self, column, like_pattern, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        like_pattern_expression = self._get_dialect_like_pattern_expression(column, like_pattern)
        if (like_pattern_expression is None):
            logger.warning(('Like patterns are not supported for dialect %s' % str(self.sql_engine_dialect)))
            raise NotImplementedError
        return like_pattern_expression

    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_not_match_like_pattern(self, column, like_pattern, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        like_pattern_expression = self._get_dialect_like_pattern_expression(column, like_pattern, positive=False)
        if (like_pattern_expression is None):
            logger.warning(('Like patterns are not supported for dialect %s' % str(self.sql_engine_dialect)))
            raise NotImplementedError
        return like_pattern_expression
    ...
    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_match_like_pattern_list(self, column, like_pattern_list, match_on='any', mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        like_pattern_expression = self._get_dialect_like_pattern_expression(column, like_pattern_list[0])
        if (like_pattern_expression is None):
            logger.warning(('Like patterns are not supported for dialect %s' % str(self.sql_engine_dialect)))
            raise NotImplementedError
        if (match_on == 'any'):
            condition = sa.or_(*(self._get_dialect_like_pattern_expression(column, like_pattern) for like_pattern in like_pattern_list))
        else:
            condition = sa.and_(*(self._get_dialect_like_pattern_expression(column, like_pattern) for like_pattern in like_pattern_list))
        return condition

    @MetaSqlAlchemyDataset.column_map_expectation
    def expect_column_values_to_not_match_like_pattern_list(self, column, like_pattern_list, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (len(like_pattern_list) == 0):
            raise ValueError('At least one like_pattern must be supplied in the like_pattern_list.')
        like_pattern_expression = self._get_dialect_like_pattern_expression(column, like_pattern_list[0], positive=False)
        if (like_pattern_expression is None):
            logger.warning(('Like patterns are not supported for dialect %s' % str(self.sql_engine_dialect)))
            raise NotImplementedError
        return sa.and_(*(self._get_dialect_like_pattern_expression(column, like_pattern, positive=False) for like_pattern in like_pattern_list))
