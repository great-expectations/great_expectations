...
def partition_data(data, bins="auto", n_bins=10):
    ...
    for (k, v) in __frame.f_locals.items():
        ...
        print(f"<INTROSPECT> {__file}:{__func} - {k}:{v.__class__.__name__}")
    warnings.warn(
        "partition_data is deprecated as of v0.10.10 and will be removed is a future release. Use either continuous_partition_data or                     categorical_partition_data instead.",
        DeprecationWarning,
    )
...
def continuous_partition_data(data, bins="auto", n_bins=10, **kwargs):
    ...
    try:
        ...
        raise ValueError(
            "Unable to compute histogram. Did you know you can pass additional kwargs to numpy histogram,such as a range? Numpy error was: "
            + str(e)
        )
