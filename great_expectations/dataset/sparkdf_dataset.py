...
try:
    ...
    from pyspark.sql.functions import array, col, count, countDistinct, datediff, desc, expr, isnan, lag
    from pyspark.sql.functions import length as length_
    from pyspark.sql.functions import lit, monotonically_increasing_id, stddev_samp, struct, udf, when
...
class MetaSparkDFDataset(Dataset):
    ...
    def column_map_expectation(cls, func):
        ...
        def inner_wrapper(self, column, mostly=None, result_format=None, *args, **kwargs):
            ...
            if (unexpected_count == 0):
                ...
                maybe_limited_unexpected_list = [row[eval_col] for row in unexpected_df.collect()]
            ...
            return_obj = self._format_map_output(result_format, success, element_count, nonnull_count, unexpected_count, maybe_limited_unexpected_list, unexpected_index_list=None)
    ...
    def column_pair_map_expectation(cls, func):
        ...
        def inner_wrapper(self, column_A, column_B, mostly=None, ignore_row_if='both_values_are_missing', result_format=None, *args, **kwargs):
            ...
            self.spark_df = self.spark_df.withColumn(eval_col_A, col(column_A)).withColumn(eval_col_B, col(column_B))
            if (result_format is None):
                result_format = self.default_expectation_args['result_format']
            result_format = parse_result_format(result_format)
            if (result_format['result_format'] == 'COMPLETE'):
                ...
                unexpected_count_limit = result_format['partial_unexpected_count']
            cols_df = self.spark_df.select(eval_col_A, eval_col_B).withColumn('__row', monotonically_increasing_id())
            cols_df.cache()
            element_count = self.get_row_count()
            if (ignore_row_if == 'both_values_are_missing'):
                boolean_mapped_null_values = cols_df.selectExpr('`__row`', '`{0}` AS `A_{0}`'.format(eval_col_A), '`{0}` AS `B_{0}`'.format(eval_col_B), 'ISNULL(`{}`) AND ISNULL(`{}`) AS `__null_val`'.format(eval_col_A, eval_col_B))
            elif (ignore_row_if == 'either_value_is_missing'):
                boolean_mapped_null_values = cols_df.selectExpr('`__row`', '`{0}` AS `A_{0}`'.format(eval_col_A), '`{0}` AS `B_{0}`'.format(eval_col_B), f'ISNULL(`{eval_col_A}`) OR ISNULL(`{eval_col_B}`) AS `__null_val`')
                ...
                boolean_mapped_null_values = cols_df.selectExpr('`__row`', '`{0}` AS `A_{0}`'.format(eval_col_A), '`{0}` AS `B_{0}`'.format(eval_col_B), lit(False).alias('__null_val'))
            else:
            ...
            if (unexpected_count == 0):
                ...
                if unexpected_count_limit:
                    unexpected_df = unexpected_df.limit(unexpected_count_limit)
                maybe_limited_unexpected_list = [(row[f'A_{eval_col_A}'], row[f'B_{eval_col_B}']) for row in unexpected_df.collect()]
                if ('output_strftime_format' in kwargs):
                    output_strftime_format = kwargs['output_strftime_format']
                    parsed_maybe_limited_unexpected_list = []
                    for val in maybe_limited_unexpected_list:
    ...
    def multicolumn_map_expectation(cls, func):
        ...
        @cls.expectation(argspec)
        @wraps(func)
        def inner_wrapper(self, column_list, mostly=None, ignore_row_if='all_values_are_missing', result_format=None, *args, **kwargs):
            import inspect
            __frame = inspect.currentframe()
            __file = __frame.f_code.co_filename
            __func = __frame.f_code.co_name
            ...
            if (ignore_row_if == 'all_values_are_missing'):
                boolean_mapped_skip_values = temp_df.select([*eval_cols, reduce((lambda a, b: (a & b)), [col(c).isNull() for c in eval_cols]).alias('__null_val')])
            elif (ignore_row_if == 'any_value_is_missing'):
                boolean_mapped_skip_values = temp_df.select([*eval_cols, reduce((lambda a, b: (a | b)), [col(c).isNull() for c in eval_cols]).alias('__null_val')])
            elif (ignore_row_if == 'never'):
                boolean_mapped_skip_values = temp_df.select([*eval_cols, lit(False).alias('__null_val')])
            else:
                raise ValueError('Unknown value of ignore_row_if: %s', (ignore_row_if,))
            ...
            if (unexpected_count == 0):
                ...
                if unexpected_count_limit:
                    unexpected_df = unexpected_df.limit(unexpected_count_limit)
                maybe_limited_unexpected_list = [OrderedDict(((col_name, row[eval_col_name]) for (col_name, eval_col_name) in zip(column_list, eval_cols))) for row in unexpected_df.collect()]
                if ('output_strftime_format' in kwargs):
                    output_strftime_format = kwargs['output_strftime_format']
                    parsed_maybe_limited_unexpected_list = []
                    for val in maybe_limited_unexpected_list:
...
class SparkDFDataset(MetaSparkDFDataset):
    ...
    def head(self, n=5):
        ...
        return PandasDataset(self.spark_df.limit(n).toPandas(), expectation_suite=self.get_expectation_suite(discard_failed_expectations=False, discard_result_format_kwargs=False, discard_catch_exceptions_kwargs=False, discard_include_config_kwargs=False))
    ...
    def get_column_value_counts(self, column, sort='value', collate=None):
        ...
        value_counts = self.spark_df.select(column).where(col(column).isNotNull()).groupBy(column).count()
        if (sort == 'value'):
            value_counts = value_counts.orderBy(column)
        elif (sort == 'count'):
            value_counts = value_counts.orderBy(desc('count'))
        value_counts = value_counts.collect()
        series = pd.Series([row['count'] for row in value_counts], index=pd.Index(data=[row[column] for row in value_counts], name='value'), name='count')
        return series
    ...
    def expect_column_values_to_be_in_set(self, column, value_set, mostly=None, parse_strings_as_datetimes=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        ...
        if (None in value_set):
            ...
            raise ValueError('expect_column_values_to_be_in_set cannot support a None in the value_set in spark')
        return column.withColumn('__success', column[0].isin(value_set))

    @DocInherit
    @MetaSparkDFDataset.column_map_expectation
    def expect_column_values_to_not_be_in_set(self, column, value_set, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSparkDFDataset.column_map_expectation
    def expect_column_values_to_be_between(self, column, min_value=None, max_value=None, strict_min=False, strict_max=False, parse_strings_as_datetimes=None, output_strftime_format=None, allow_cross_type_comparisons=None, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSparkDFDataset.column_map_expectation
    def expect_column_values_to_match_strftime_format(self, column, strftime_format, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSparkDFDataset.column_map_expectation
    def expect_column_values_to_match_json_schema(self, column, json_schema, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    @DocInherit
    @MetaSparkDFDataset.column_map_expectation
    def expect_column_values_to_be_json_parseable(self, column, mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def expect_column_values_to_be_in_type_list(self, column, type_list: List[str], mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def expect_column_values_to_match_regex_list(self, column, regex_list, match_on='any', mostly=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        ...
        if (match_on == 'any'):
            ...
            return column.withColumn('__success', column[0].rlike(''.join(formatted_regex_list)))
        else:
    ...
    def expect_column_pair_values_to_be_equal(self, column_A, column_B, ignore_row_if='both_values_are_missing', result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        return join_df.withColumn('__success', when((col(column_A_name) == col(column_B_name)), True).otherwise(False))

    @DocInherit
    @MetaSparkDFDataset.column_pair_map_expectation
    def expect_column_pair_values_A_to_be_greater_than_B(self, column_A, column_B, or_equal=None, parse_strings_as_datetimes=None, allow_cross_type_comparisons=None, ignore_row_if='both_values_are_missing', result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if parse_strings_as_datetimes:
            _udf = udf(parse, sparktypes.TimestampType())
            (timestamp_column_A, timestamp_column_B) = (f'__ts_{column_A_name}', f'__ts_{column_B_name}')
        ...
        join_df = temp_column_A.join(temp_column_B, (temp_column_A['__row'] == temp_column_B['__row']), how='inner')
        if or_equal:
            return join_df.withColumn('__success', when((col(column_A_name) >= col(column_B_name)), True).otherwise(False))
        else:
            return join_df.withColumn('__success', when((col(column_A_name) > col(column_B_name)), True).otherwise(False))
    ...
    @DocInherit
    @MetaSparkDFDataset.column_pair_map_expectation
    def expect_column_pair_values_to_be_in_set(self, column_A, column_B, value_pairs_set, ignore_row_if='both_values_are_missing', result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        column_B_name = column_B.schema.names[1]
        join_df = column_A.join(column_B, (column_A['__row'] == column_B['__row']), how='inner')
        join_df = join_df.withColumn('combine_AB', array(col(column_A_name), col(column_B_name)))
        value_set_df = SQLContext(self.spark_df._sc).createDataFrame(value_pairs_set, ['col_A', 'col_B']).select(array('col_A', 'col_B').alias('set_AB'))
        return join_df.join(value_set_df, (join_df['combine_AB'] == value_set_df['set_AB']), 'left').withColumn('__success', when(col('set_AB').isNull(), lit(False)).otherwise(lit(True)))
    ...
    def expect_select_column_values_to_be_unique_within_record(self, column_list, mostly=None, ignore_row_if='all_values_are_missing', result_format=None, include_config=True, catch_exceptions=None, meta=None):
        ...
        for i in range(0, (len(column_names) - 1)):
            conditions.append((~ col(column_names[i]).eqNullSafe(col(column_names[(i + 1)]))))
    ...
    def expect_compound_columns_to_be_unique(self, column_list, mostly=None, ignore_row_if='all_values_are_missing', result_format=None, include_config=True, catch_exceptions=None, meta=None):
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        column_names = column_list.schema.names[:]
        return column_list.withColumn('__success', (count(lit(1)).over(Window.partitionBy(struct(*column_names))) <= 1))

    @DocInherit
    @MetaSparkDFDataset.column_map_expectation
    def expect_column_values_to_be_increasing(self, column, strictly=False, mostly=None, parse_strings_as_datetimes=None, output_strftime_format=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if parse_strings_as_datetimes:
            column = self._apply_dateutil_parse(column)
            column = column.withColumn('constant', lit('constant')).withColumn('lag', lag(column[0]).over(Window.orderBy(col('constant'))))
            column = column.withColumn('diff', datediff(col(column_name), col('lag')))
        else:
            column = column.withColumn('constant', lit('constant')).withColumn('lag', lag(column[0]).over(Window.orderBy(col('constant')))).withColumn('diff', (column[0] - col('lag')))
    ...
    @DocInherit
    @MetaSparkDFDataset.column_map_expectation
    def expect_column_values_to_be_decreasing(self, column, strictly=False, mostly=None, parse_strings_as_datetimes=None, output_strftime_format=None, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def expect_multicolumn_sum_to_equal(self, column_list, sum_total, result_format=None, include_config=True, catch_exceptions=None, meta=None):
        ...
        return column_list.withColumn('__success', when((col('actual_total') == sum_total), lit(True)).otherwise(lit(False)))
