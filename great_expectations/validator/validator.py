...
class Validator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(batches) > 1:
            logger.debug(
                f"{len(batches)} batches will be added to this Validator. The batch_identifiers for the active batch are {self.active_batch.batch_definition['batch_identifiers'].items()}"
            )
    ...
    def _build_expectation_configuration(
        ...
        if profiler is not None:
            ...
            configuration = ExpectationConfiguration(
                expectation_type=expectation_type, kwargs=expectation_kwargs, meta=meta
            )
        else:
            configuration = ExpectationConfiguration(
                expectation_type=expectation_type, kwargs=expectation_kwargs, meta=meta
    ...
    def _build_rule_based_profiler_from_config_and_runtime_args(
        ...
        profiler: BaseRuleBasedProfiler = BaseRuleBasedProfiler(
            profiler_config=profiler_config, data_context=self.data_context
        )
    ...
    def compute_metrics(
        ...
        ] = self.resolve_validation_graph(graph=graph, metrics=resolved_metrics)
...
occurred while resolving metrics."""
    ...
    def _get_default_domain_kwargs(
        ...
        for key in metric_provider_cls.domain_keys:
            if (key not in metric_configuration.metric_domain_kwargs) and (
                key in metric_provider_cls.default_kwarg_values
            ):
    ...
    def _get_default_value_kwargs(
        ...
        for key in metric_provider_cls.value_keys:
            if (key not in metric_configuration.metric_value_kwargs) and (
                key in metric_provider_cls.default_kwarg_values
            ):
    ...
    def _catch_exceptions_in_failing_expectation_validations(
        ...
        exception_info: ExceptionInfo = ExceptionInfo(
            exception_traceback=exception_traceback, exception_message=exception_message
        )
        configuration: ExpectationConfiguration
        result: ExpectationValidationResult
        ...
        if len(metric_dependencies) == 0:
            ...
            for metric_dependency in metric_dependencies.values():
                if metric_dependency.id == metric_configuration.id:
                    ...
                    continue
                graph.add(
                    MetricEdge(left=metric_configuration, right=metric_dependency)
                )
                self.build_metric_dependency_graph(
                    graph=graph,
                    execution_engine=execution_engine,
    ...
    def resolve_validation_graph(
        ...
        while not done:
            ...
            for metric in ready_metrics:
                if (metric.id in failed_metric_info) and (
                    failed_metric_info[metric.id]["num_failures"]
...
"""
    ...
    def active_batch_id(self, batch_id: str) -> None:
        ...
        if batch_id not in available_batch_ids:
            raise ValueError(
                f"""batch_id {batch_id} not found in loaded batches.  Batches must first be loaded before they can be set as active.
"""
            )
        else:
    ...
    def validate(
        ...
        try:
            ...
            if isinstance(run_id, str) and (not run_name):
                warnings.warn(
                    "String run_ids are deprecated as of v0.13.0 and support will be removed in v0.16. Please provide a run_id of type RunIdentifier(run_name=None, run_time=None), or a dictionary containing run_name and run_time (both optional). Instead of providing a run_id, you may also providerun_name and run_time separately.",
                    DeprecationWarning,
                )
                try:
    ...
    def columns(self, domain_kwargs: Optional[Dict[(str, Any)]] = None) -> List[str]:
        ...
        columns: List[str] = self.get_metric(
            metric=MetricConfiguration(
                metric_name="table.columns", metric_domain_kwargs=domain_kwargs
            )
        )
        return columns
    ...
    def _parse_validation_graph(
        validation_graph: ValidationGraph, metrics: Dict[(Tuple[(str, str, str)], Any)]
