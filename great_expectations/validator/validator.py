...
from great_expectations.exceptions import GreatExpectationsError, InvalidExpectationConfigurationError, MetricResolutionError
...
from great_expectations.expectations.registry import get_expectation_impl, get_metric_provider, list_registered_expectation_implementations
...
from great_expectations.rule_based_profiler.helpers.configuration_reconciliation import DEFAULT_RECONCILATION_DIRECTIVES, ReconciliationDirectives, ReconciliationStrategy
...
from great_expectations.validator.validation_graph import ExpectationValidationGraph, MetricEdge, ValidationGraph
...
def _calc_validation_statistics(validation_results: List[ExpectationValidationResult]) -> ValidationStatistics:
    import inspect
...
class Validator():
    DEFAULT_RUNTIME_CONFIGURATION = {'include_config': True, 'catch_exceptions': False, 'result_format': 'BASIC'}
    RUNTIME_KEYS = DEFAULT_RUNTIME_CONFIGURATION.keys()

    def __init__(self, execution_engine: ExecutionEngine, interactive_evaluation: bool=True, expectation_suite: Optional[ExpectationSuite]=None, expectation_suite_name: Optional[str]=None, data_context: Optional[Any]=None, batches: Optional[List[Batch]]=None, **kwargs) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (len(batches) > 1):
            logger.debug(f"{len(batches)} batches will be added to this Validator. The batch_identifiers for the active batch are {self.active_batch.batch_definition['batch_identifiers'].items()}")
    ...
    def __dir__(self):
        ...
        combined_dir = (validator_attrs | class_expectation_impls)
    ...
    def __getattr__(self, name):
        ...
        elif (self._expose_dataframe_methods and isinstance(self.active_batch.data, PandasBatchData) and hasattr(pd.DataFrame, name)):
    ...
    def validate_expectation(self, name: str) -> Callable:
        ...
        def inst_expectation(*args, **kwargs):
            ...
            basic_default_expectation_args: dict = {k: v for (k, v) in self.default_expectation_args.items() if (k in Validator.RUNTIME_KEYS)}
            basic_runtime_configuration: dict = copy.deepcopy(basic_default_expectation_args)
            basic_runtime_configuration.update({k: v for (k, v) in kwargs.items() if (k in Validator.RUNTIME_KEYS)})
            ...
            for (idx, arg) in enumerate(args):
                try:
                    ...
                    if (arg_name == 'meta'):
                        logger.warning('Setting meta via args could be ambiguous; please use a kwarg instead.')
                        meta = arg
                except IndexError:
                    raise InvalidExpectationConfigurationError(f'Invalid positional argument: {arg}')
            configuration: ExpectationConfiguration = self._build_expectation_configuration(expectation_type=name, expectation_kwargs=expectation_kwargs, meta=meta, expectation_impl=expectation_impl)
            exception_info: ExceptionInfo
            if self.interactive_evaluation:
                configuration.process_evaluation_parameters(self._expectation_suite.evaluation_parameters, True, self._data_context)
            try:
                ...
                if ((not self.interactive_evaluation) and (not self._active_validation)):
                    validation_result = ExpectationValidationResult(expectation_config=copy.deepcopy(expectation.configuration))
                else:
                    validation_result = expectation.validate(validator=self, evaluation_parameters=self._expectation_suite.evaluation_parameters, data_context=self._data_context, runtime_configuration=basic_runtime_configuration)
                if (self._active_validation is True):
                    stored_config = configuration.get_raw_configuration()
                else:
                    stored_config = self._expectation_suite._add_expectation(expectation_configuration=configuration.get_raw_configuration(), send_usage_event=False)
                if (validation_result.success is not None):
                    stored_config.success_on_last_run = validation_result.success
                if (self._data_context is not None):
                    validation_result = self._data_context.update_return_obj(self, validation_result)
            except Exception as err:
                if basic_runtime_configuration.get('catch_exceptions'):
                    exception_traceback = traceback.format_exc()
                    exception_message = f'{type(err).__name__}: {str(err)}'
                    exception_info = ExceptionInfo(exception_traceback=exception_traceback, exception_message=exception_message)
                    validation_result = ExpectationValidationResult(success=False, exception_info=exception_info, expectation_config=configuration)
                else:
                    raise err
    ...
    def _build_expectation_configuration(self, expectation_type: str, expectation_kwargs: dict, meta: dict, expectation_impl: 'Expectation') -> ExpectationConfiguration:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        profiler_config: Optional[RuleBasedProfilerConfig] = expectation_kwargs.get('profiler_config')
        default_profiler_config: Optional[RuleBasedProfilerConfig] = expectation_impl.default_kwarg_values.get('profiler_config')
        if (auto and (profiler_config is None) and (default_profiler_config is None)):
            raise ValueError('Automatic Expectation argument estimation requires a Rule-Based Profiler to be provided.')
        configuration: ExpectationConfiguration
        profiler: Optional[BaseRuleBasedProfiler] = self.build_rule_based_profiler_for_expectation(expectation_type=expectation_type)(*(), **expectation_kwargs)
        if (profiler is not None):
            profiler_result: RuleBasedProfilerResult = profiler.run(variables=None, rules=None, batch_list=list(self.batches.values()), batch_request=None, recompute_existing_parameter_values=False, reconciliation_directives=DEFAULT_RECONCILATION_DIRECTIVES)
            expectation_configurations: List[ExpectationConfiguration] = profiler_result.expectation_configurations
            configuration = expectation_configurations[0]
            success_keys: Tuple[str] = (expectation_impl.success_keys if hasattr(expectation_impl, 'success_keys') else tuple())
            arg_keys: Tuple[str] = (expectation_impl.arg_keys if hasattr(expectation_impl, 'arg_keys') else tuple())
            runtime_keys: Tuple[str] = ((expectation_impl.runtime_keys if hasattr(expectation_impl, 'runtime_keys') else None) or tuple())
            override_keys: Tuple[str] = ((success_keys + arg_keys) + runtime_keys)
            key: str
            value: Any
            expectation_kwargs_overrides: dict = {key: value for (key, value) in expectation_kwargs.items() if (key in override_keys)}
            expectation_kwargs_overrides = convert_to_json_serializable(data=expectation_kwargs_overrides)
            ...
            configuration = ExpectationConfiguration(expectation_type=expectation_type, kwargs=expectation_kwargs, meta=meta)
        else:
            configuration = ExpectationConfiguration(expectation_type=expectation_type, kwargs=expectation_kwargs, meta=meta)
    ...
    def build_rule_based_profiler_for_expectation(self, expectation_type: str) -> Callable:
        ...
        def inst_rule_based_profiler(*args, **kwargs) -> Optional[BaseRuleBasedProfiler]:
            ...
            if auto:
                ...
                profiler = self._build_rule_based_profiler_from_config_and_runtime_args(expectation_type=expectation_type, expectation_kwargs=expectation_kwargs, success_keys=success_keys, profiler_config=profiler_config, override_profiler_config=override_profiler_config)
            else:
                profiler = None
            return profiler
        return inst_rule_based_profiler

    def _build_rule_based_profiler_from_config_and_runtime_args(self, expectation_type: str, expectation_kwargs: dict, success_keys: Tuple[str], profiler_config: RuleBasedProfilerConfig, override_profiler_config: Optional[RuleBasedProfilerConfig]=None) -> BaseRuleBasedProfiler:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        assert (profiler_config.name == expectation_type), 'The name of profiler used to build an ExpectationConfiguration must equal to expectation_type of the expectation being invoked.'
        profiler: BaseRuleBasedProfiler = BaseRuleBasedProfiler(profiler_config=profiler_config, data_context=self.data_context)
        rules: List[Rule] = profiler.rules
        assert (len(rules) == 1), 'A Rule-Based Profiler for an Expectation can have exactly one rule.'
        ...
        effective_variables: Optional[ParameterContainer] = profiler.reconcile_profiler_variables(variables=override_variables, reconciliation_strategy=ReconciliationStrategy.UPDATE)
        profiler.variables = effective_variables
        override_rules: Dict[(str, Dict[(str, Any)])] = override_profiler_config.get('rules', {})
        assert (len(override_rules) <= 1), 'An override Rule-Based Profiler for an Expectation can have exactly one rule.'
        if override_rules:
            profiler.rules[0].name = list(override_rules.keys())[0]
            effective_rules: List[Rule] = profiler.reconcile_profiler_rules(rules=override_rules, reconciliation_directives=ReconciliationDirectives(domain_builder=ReconciliationStrategy.UPDATE, parameter_builder=ReconciliationStrategy.REPLACE, expectation_configuration_builder=ReconciliationStrategy.REPLACE))
            profiler.rules = effective_rules
        self._validate_profiler_and_update_rules_properties(profiler=profiler, expectation_type=expectation_type, expectation_kwargs=expectation_kwargs, success_keys=success_keys)
        return profiler

    def _validate_profiler_and_update_rules_properties(self, profiler: BaseRuleBasedProfiler, expectation_type: str, expectation_kwargs: dict, success_keys: Tuple[str]) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        assert (rule.expectation_configuration_builders[0].expectation_type == expectation_type), 'ExpectationConfigurationBuilder in profiler used to build an ExpectationConfiguration must have the same expectation_type as the expectation being invoked.'
        key: str
        value: Any
        metric_value_kwargs_override: dict = {key: value for (key, value) in expectation_kwargs.items() if ((key in success_keys) and (key not in BaseRuleBasedProfiler.EXPECTATION_SUCCESS_KEYS))}
        domain_type: MetricDomainTypes = rule.domain_builder.domain_type
        if (domain_type not in MetricDomainTypes):
            raise ValueError(f'Domain type declaration "{domain_type}" in "MetricDomainTypes" does not exist.')
        ...
        for parameter_builder in parameter_builders:
            self._update_metric_value_kwargs_for_success_keys(parameter_builder=parameter_builder, metric_value_kwargs=metric_value_kwargs_override)
        expectation_configuration_builders: List[ExpectationConfigurationBuilder] = (rule.expectation_configuration_builders or [])
        expectation_configuration_builder: ExpectationConfigurationBuilder
        for expectation_configuration_builder in expectation_configuration_builders:
            validation_parameter_builders: List[ParameterBuilder] = (expectation_configuration_builder.validation_parameter_builders or [])
            for parameter_builder in validation_parameter_builders:
                self._update_metric_value_kwargs_for_success_keys(parameter_builder=parameter_builder, metric_value_kwargs=metric_value_kwargs_override)

    def _update_metric_value_kwargs_for_success_keys(self, parameter_builder: ParameterBuilder, metric_value_kwargs: Optional[dict]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (metric_value_kwargs is None):
            metric_value_kwargs = {}
        if (hasattr(parameter_builder, 'metric_name') and hasattr(parameter_builder, 'metric_value_kwargs')):
            parameter_builder_metric_value_kwargs: dict = (parameter_builder.metric_value_kwargs or {})
            parameter_builder_metric_value_kwargs = {key: (metric_value_kwargs.get(key) or value) for (key, value) in parameter_builder_metric_value_kwargs.items()}
            parameter_builder.metric_value_kwargs = parameter_builder_metric_value_kwargs
        evaluation_parameter_builders: List[ParameterBuilder] = (parameter_builder.evaluation_parameter_builders or [])
        evaluation_parameter_builder: ParameterBuilder
        for evaluation_parameter_builder in evaluation_parameter_builders:
            self._update_metric_value_kwargs_for_success_keys(parameter_builder=evaluation_parameter_builder, metric_value_kwargs=metric_value_kwargs)
    ...
    def compute_metrics(self, metric_configurations: List[MetricConfiguration]) -> Dict[(Tuple[(str, str, str)], Any)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for metric_configuration in metric_configurations:
            (provider_cls, _) = get_metric_provider(metric_configuration.metric_name, self.execution_engine)
            self._get_default_domain_kwargs(metric_provider_cls=provider_cls, metric_configuration=metric_configuration)
            self._get_default_value_kwargs(metric_provider_cls=provider_cls, metric_configuration=metric_configuration)
            self.build_metric_dependency_graph(graph=graph, execution_engine=self._execution_engine, metric_configuration=metric_configuration)
        resolved_metrics: Dict[(Tuple[(str, str, str)], Any)] = {}
        aborted_metrics_info: Dict[(Tuple[(str, str, str)], Dict[(str, Union[(MetricConfiguration, Set[ExceptionInfo], int)])])] = self.resolve_validation_graph(graph=graph, metrics=resolved_metrics)
...
occurred while resolving metrics.''')
    ...
    def get_metrics(self, metrics: Dict[(str, MetricConfiguration)]) -> Dict[(str, Any)]:
        ...
        resolved_metrics: Dict[(Tuple[(str, str, str)], Any)] = self.compute_metrics(metric_configurations=list(metrics.values()))
        return {metric_configuration.metric_name: resolved_metrics[metric_configuration.id] for metric_configuration in metrics.values()}

    @staticmethod
    def _get_default_domain_kwargs(metric_provider_cls: 'MetricProvider', metric_configuration: MetricConfiguration) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for key in metric_provider_cls.domain_keys:
            if ((key not in metric_configuration.metric_domain_kwargs) and (key in metric_provider_cls.default_kwarg_values)):
                metric_configuration.metric_domain_kwargs[key] = metric_provider_cls.default_kwarg_values[key]

    @staticmethod
    def _get_default_value_kwargs(metric_provider_cls: 'MetricProvider', metric_configuration: MetricConfiguration) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for key in metric_provider_cls.value_keys:
            if ((key not in metric_configuration.metric_value_kwargs) and (key in metric_provider_cls.default_kwarg_values)):
                metric_configuration.metric_value_kwargs[key] = metric_provider_cls.default_kwarg_values[key]
    ...
    def get_metric(self, metric: MetricConfiguration) -> Any:
        ...
        return self.get_metrics(metrics={metric.metric_name: metric})[metric.metric_name]

    def graph_validate(self, configurations: List[ExpectationConfiguration], metrics: Optional[Dict[(Tuple[(str, str, str)], Any)]]=None, runtime_configuration: Optional[dict]=None) -> List[ExpectationValidationResult]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        (evrs, processed_configurations) = self._generate_metric_dependency_subgraphs_for_each_expectation_configuration(expectation_configurations=configurations, expectation_validation_graphs=expectation_validation_graphs, processed_configurations=processed_configurations, catch_exceptions=catch_exceptions, runtime_configuration=runtime_configuration)
        if (metrics is None):
            metrics = {}
        graph: ValidationGraph = self._generate_suite_level_graph_from_expectation_level_sub_graphs(expectation_validation_graphs=expectation_validation_graphs)
        try:
            (evrs, processed_configurations) = self._resolve_suite_level_graph_and_process_metric_evaluation_errors(validation_graph=graph, metrics=metrics, runtime_configuration=runtime_configuration, expectation_validation_graphs=expectation_validation_graphs, evrs=evrs, processed_configurations=processed_configurations)
        except Exception as err:
            if catch_exceptions:
                exception_traceback: str = traceback.format_exc()
                evrs = self._catch_exceptions_in_failing_expectation_validations(exception_traceback=exception_traceback, exception=err, failing_expectation_configurations=processed_configurations, evrs=evrs)
                return evrs
            else:
                raise err
        configuration: ExpectationConfiguration
        ...
        for configuration in processed_configurations:
            try:
                result = configuration.metrics_validate(metrics, execution_engine=self._execution_engine, runtime_configuration=runtime_configuration)
                evrs.append(result)
            except Exception as err:
                if catch_exceptions:
                    exception_traceback: str = traceback.format_exc()
                    evrs = self._catch_exceptions_in_failing_expectation_validations(exception_traceback=exception_traceback, exception=err, failing_expectation_configurations=[configuration], evrs=evrs)
                else:
                    raise err
        return evrs

    def _generate_metric_dependency_subgraphs_for_each_expectation_configuration(self, expectation_configurations: List[ExpectationConfiguration], expectation_validation_graphs: List[ExpectationValidationGraph], processed_configurations: List[ExpectationConfiguration], catch_exceptions: bool, runtime_configuration: Optional[dict]=None) -> Tuple[(List[ExpectationValidationResult], List[ExpectationConfiguration])]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for configuration in expectation_configurations:
            ...
            validation_dependencies: dict = expectation_impl().get_validation_dependencies(evaluated_config, self._execution_engine, runtime_configuration)['metrics']
            try:
                expectation_validation_graph: ExpectationValidationGraph = ExpectationValidationGraph(configuration=evaluated_config)
                for metric_configuration in validation_dependencies.values():
                    graph = ValidationGraph()
                    self.build_metric_dependency_graph(graph=graph, execution_engine=self._execution_engine, metric_configuration=metric_configuration, configuration=evaluated_config, runtime_configuration=runtime_configuration)
                ...
                if catch_exceptions:
                    ...
                    exception_info: ExceptionInfo = ExceptionInfo(exception_traceback=exception_traceback, exception_message=exception_message)
                    result: ExpectationValidationResult = ExpectationValidationResult(success=False, exception_info=exception_info, expectation_config=evaluated_config)
                    evrs.append(result)
                else:
                    raise err
    ...
    @staticmethod
    def _generate_suite_level_graph_from_expectation_level_sub_graphs(expectation_validation_graphs: List[ExpectationValidationGraph]) -> ValidationGraph:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        edges: List[MetricEdge] = list(itertools.chain.from_iterable([expectation_validation_graph.graph.edges for expectation_validation_graph in expectation_validation_graphs]))
        validation_graph: ValidationGraph = ValidationGraph(edges=edges)
        return validation_graph

    def _resolve_suite_level_graph_and_process_metric_evaluation_errors(self, validation_graph: ValidationGraph, metrics: Dict[(Tuple[(str, str, str)], Any)], runtime_configuration: dict, expectation_validation_graphs: List[ExpectationValidationGraph], evrs: List[ExpectationValidationResult], processed_configurations: List[ExpectationConfiguration]) -> Tuple[(List[ExpectationValidationResult], List[ExpectationConfiguration])]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        aborted_metrics_info: Dict[(Tuple[(str, str, str)], Dict[(str, Union[(MetricConfiguration, Set[ExceptionInfo], int)])])] = self.resolve_validation_graph(graph=validation_graph, metrics=metrics, runtime_configuration=runtime_configuration)
        rejected_configurations: List[ExpectationConfiguration] = []
        for expectation_validation_graph in expectation_validation_graphs:
            metric_exception_info: Set[ExceptionInfo] = expectation_validation_graph.get_exception_info(metric_info=aborted_metrics_info)
            if (len(metric_exception_info) > 0):
                ...
                for exception_info in metric_exception_info:
                    result = ExpectationValidationResult(success=False, exception_info=exception_info, expectation_config=configuration)
                    evrs.append(result)
    ...
    @staticmethod
    def _catch_exceptions_in_failing_expectation_validations(exception_traceback: str, exception: Exception, failing_expectation_configurations: List[ExpectationConfiguration], evrs: List[ExpectationValidationResult]) -> List[ExpectationValidationResult]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def build_metric_dependency_graph(self, graph: ValidationGraph, execution_engine: ExecutionEngine, metric_configuration: MetricConfiguration, configuration: Optional[ExpectationConfiguration]=None, runtime_configuration: Optional[dict]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        metric_impl = get_metric_provider(metric_configuration.metric_name, execution_engine=execution_engine)[0]
        metric_dependencies = metric_impl.get_evaluation_dependencies(metric=metric_configuration, configuration=configuration, execution_engine=execution_engine, runtime_configuration=runtime_configuration)
        if (len(metric_dependencies) == 0):
            graph.add(MetricEdge(left=metric_configuration))
        else:
            ...
            for metric_dependency in metric_dependencies.values():
                if (metric_dependency.id == metric_configuration.id):
                    logger.warning(f'Metric {str(metric_configuration.id)} has created a circular dependency')
                    continue
                graph.add(MetricEdge(left=metric_configuration, right=metric_dependency))
                self.build_metric_dependency_graph(graph=graph, execution_engine=execution_engine, metric_configuration=metric_dependency, configuration=configuration, runtime_configuration=runtime_configuration)

    def resolve_validation_graph(self, graph: ValidationGraph, metrics: Dict[(Tuple[(str, str, str)], Any)], runtime_configuration: Optional[dict]=None, min_graph_edges_pbar_enable: int=0) -> Dict[(Tuple[(str, str, str)], Dict[(str, Union[(MetricConfiguration, Set[ExceptionInfo], int)])])]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if runtime_configuration.get('catch_exceptions', True):
            ...
            catch_exceptions = False
        failed_metric_info: Dict[(Tuple[(str, str, str)], Dict[(str, Union[(MetricConfiguration, Set[ExceptionInfo], int)])])] = {}
        aborted_metrics_info: Dict[(Tuple[(str, str, str)], Dict[(str, Union[(MetricConfiguration, Set[ExceptionInfo], int)])])] = {}
        ...
        while (not done):
            (ready_metrics, needed_metrics) = self._parse_validation_graph(validation_graph=graph, metrics=metrics)
            disable = False
            ...
            if (progress_bar is None):
                progress_bar = tqdm(total=(len(ready_metrics) + len(needed_metrics)), desc='Calculating Metrics', disable=disable)
                progress_bar.update(0)
                progress_bar.refresh()
            computable_metrics = set()
            for metric in ready_metrics:
                if ((metric.id in failed_metric_info) and (failed_metric_info[metric.id]['num_failures'] >= MAX_METRIC_COMPUTATION_RETRIES)):
                    aborted_metrics_info[metric.id] = failed_metric_info[metric.id]
                else:
                    computable_metrics.add(metric)
            try:
                metrics.update(self._resolve_metrics(execution_engine=self._execution_engine, metrics_to_resolve=computable_metrics, metrics=metrics, runtime_configuration=runtime_configuration))
                ...
                if catch_exceptions:
                    ...
                    for failed_metric in err.failed_metrics:
                        if (failed_metric.id in failed_metric_info):
                            ...
                            failed_metric_info[failed_metric.id]['exception_info'].add(exception_info)
                        else:
                ...
                if catch_exceptions:
                    logger.error(f'''Caught exception {str(e)} while trying to resolve a set of {len(ready_metrics)} metrics; aborting graph resolution.
''')
                    done = True
                else:
                    raise e
    ...
    def append_expectation(self, expectation_config: ExpectationConfiguration) -> None:
        ...
        warnings.warn(('append_expectation is deprecated as of v0.13.0 and will be removed in v0.16. ' + 'Please use ExpectationSuite.add_expectation instead.'), DeprecationWarning)
    ...
    def find_expectation_indexes(self, expectation_configuration: ExpectationConfiguration, match_type: str='domain') -> List[int]:
        ...
        warnings.warn(('find_expectation_indexes is deprecated as of v0.13.0 and will be removed in v0.16. ' + 'Please use ExpectationSuite.find_expectation_indexes instead.'), DeprecationWarning)
    ...
    def find_expectations(self, expectation_configuration: ExpectationConfiguration, match_type: str='domain', ge_cloud_id: Optional[str]=None) -> List[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        warnings.warn(('find_expectations is deprecated as of v0.13.0 and will be removed in v0.16. ' + 'Please use ExpectationSuite.find_expectation_indexes instead.'), DeprecationWarning)
        return self._expectation_suite.find_expectations(expectation_configuration=expectation_configuration, match_type=match_type, ge_cloud_id=ge_cloud_id)

    def remove_expectation(self, expectation_configuration: ExpectationConfiguration, match_type: str='domain', remove_multiple_matches: bool=False, ge_cloud_id: Optional[str]=None) -> List[ExpectationConfiguration]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return self._expectation_suite.remove_expectation(expectation_configuration=expectation_configuration, match_type=match_type, remove_multiple_matches=remove_multiple_matches, ge_cloud_id=ge_cloud_id)
    ...
    def load_batch_list(self, batch_list: List[Batch]) -> None:
        ...
        for batch in batch_list:
            try:
                assert isinstance(batch, Batch), 'batches provided to Validator must be Great Expectations Batch objects'
            except AssertionError as e:
    ...
    def active_batch(self) -> Optional[Batch]:
        ...
        batch: Optional[Batch] = (self.batches.get(active_batch_id) if active_batch_id else None)
        return batch
    ...
    def active_batch_id(self) -> Optional[str]:
        ...
        if (active_engine_batch_id != self._active_batch_id):
            logger.debug('This validator has a different active batch id than its Execution Engine.')
    ...
    def active_batch_id(self, batch_id: str) -> None:
        ...
        available_batch_ids: Set[str] = set(self.batches.keys()).union(set(self.loaded_batch_ids))
        if (batch_id not in available_batch_ids):
            raise ValueError(f'''batch_id {batch_id} not found in loaded batches.  Batches must first be loaded before they can be set as active.
''')
        else:
    ...
    def get_expectations_config(self, discard_failed_expectations: bool=True, discard_result_format_kwargs: bool=True, discard_include_config_kwargs: bool=True, discard_catch_exceptions_kwargs: bool=True, suppress_warnings: bool=False) -> ExpectationSuite:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        warnings.warn(('get_expectations_config is deprecated as of v0.13.0 and will be removed in v0.16. ' + 'Please use get_expectation_suite instead.'), DeprecationWarning)
    ...
    def get_expectation_suite(self, discard_failed_expectations: bool=True, discard_result_format_kwargs: bool=True, discard_include_config_kwargs: bool=True, discard_catch_exceptions_kwargs: bool=True, suppress_warnings: bool=False, suppress_logging: bool=False) -> ExpectationSuite:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def save_expectation_suite(self, filepath: Optional[str]=None, discard_failed_expectations: bool=True, discard_result_format_kwargs: bool=True, discard_include_config_kwargs: bool=True, discard_catch_exceptions_kwargs: bool=True, suppress_warnings: bool=False) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
    ...
    def validate(self, expectation_suite=None, run_id=None, data_context: Optional[Any]=None, evaluation_parameters: Optional[dict]=None, catch_exceptions: bool=True, result_format: Optional[str]=None, only_return_failures: bool=False, run_name: Optional[str]=None, run_time: Optional[str]=None) -> Union[(ExpectationValidationResult, ExpectationSuiteValidationResult)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        try:
            ...
            if (isinstance(run_id, str) and (not run_name)):
                warnings.warn('String run_ids are deprecated as of v0.13.0 and support will be removed in v0.16. Please provide a run_id of type RunIdentifier(run_name=None, run_time=None), or a dictionary containing run_name and run_time (both optional). Instead of providing a run_id, you may also providerun_name and run_time separately.', DeprecationWarning)
                try:
            ...
            if (expectation_suite is None):
                ...
                if getattr(data_context, '_usage_statistics_handler', None):
                    ...
                    handler.send_usage_message(event='data_asset.validate', event_payload=handler.anonymizer.anonymize(obj=self), success=False)
            ...
            for expectation in expectation_suite.expectations:
                expectation.process_evaluation_parameters(evaluation_parameters=runtime_evaluation_parameters, interactive_evaluation=self.interactive_evaluation, data_context=self._data_context)
            ...
            runtime_configuration = self._get_runtime_configuration(catch_exceptions=catch_exceptions, result_format=result_format)
            results = self.graph_validate(configurations=expectations_to_evaluate, runtime_configuration=runtime_configuration)
        ...
        if getattr(data_context, '_usage_statistics_handler', None):
            handler = data_context._usage_statistics_handler
            handler.send_usage_message(event='data_asset.validate', event_payload=handler.anonymizer.anonymize(obj=self), success=True)
        return result
    ...
    def add_citation(self, comment: str, batch_spec: Optional[dict]=None, batch_markers: Optional[dict]=None, batch_definition: Optional[dict]=None, citation_date: Optional[str]=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        self._expectation_suite.add_citation(comment, batch_spec=batch_spec, batch_markers=batch_markers, batch_definition=batch_definition, citation_date=citation_date)
    ...
    def columns(self, domain_kwargs: Optional[Dict[(str, Any)]]=None) -> List[str]:
        ...
        columns: List[str] = self.get_metric(metric=MetricConfiguration(metric_name='table.columns', metric_domain_kwargs=domain_kwargs))
        return columns

    def head(self, n_rows: int=5, domain_kwargs: Optional[Dict[(str, Any)]]=None, fetch_all: bool=False) -> pd.DataFrame:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (domain_kwargs is None):
            domain_kwargs = {'batch_id': self.execution_engine.active_batch_data_id}
        data: Any = self.get_metric(metric=MetricConfiguration(metric_name='table.head', metric_domain_kwargs=domain_kwargs, metric_value_kwargs={'n_rows': n_rows, 'fetch_all': fetch_all}))
        df: pd.DataFrame
        if isinstance(self.execution_engine, (PandasExecutionEngine, SqlAlchemyExecutionEngine)):
            ...
            raise GreatExpectationsError('Unsupported or unknown ExecutionEngine type encountered in Validator class.')
        return df.reset_index(drop=True, inplace=False)

    @staticmethod
    def _parse_validation_graph(validation_graph: ValidationGraph, metrics: Dict[(Tuple[(str, str, str)], Any)]) -> Tuple[(Set[MetricConfiguration], Set[MetricConfiguration])]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for edge in validation_graph.edges:
            if (edge.left.id not in metrics):
                ...
                elif (edge.left.id not in unmet_dependency_ids):
                    unmet_dependency_ids.add(edge.left.id)
                    unmet_dependency.add(edge.left)
        return ((maybe_ready - unmet_dependency), unmet_dependency)

    @staticmethod
    def _resolve_metrics(execution_engine: ExecutionEngine, metrics_to_resolve: Iterable[MetricConfiguration], metrics: Dict[(Tuple[(str, str, str)], Any)]=None, runtime_configuration: dict=None) -> Dict[(Tuple[(str, str, str)], MetricConfiguration)]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        return execution_engine.resolve_metrics(metrics_to_resolve=metrics_to_resolve, metrics=metrics, runtime_configuration=runtime_configuration)

    def _initialize_expectations(self, expectation_suite: ExpectationSuite=None, expectation_suite_name: str=None) -> None:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (expectation_suite and (not isinstance(expectation_suite, ExpectationSuite))):
            raise TypeError('expectation_suite must be of type ExpectationSuite, not {}'.format(type(expectation_suite)))
        if (expectation_suite is not None):
            if isinstance(expectation_suite, dict):
                expectation_suite_dict: dict = expectationSuiteSchema.load(expectation_suite)
                expectation_suite = ExpectationSuite(**expectation_suite_dict, data_context=self._data_context)
            else:
    ...
    def _get_runtime_configuration(self, catch_exceptions: Optional[bool]=None, result_format: Optional[Union[(dict, str)]]=None) -> dict:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        if (self.default_expectation_args['result_format'] == Validator.DEFAULT_RUNTIME_CONFIGURATION['result_format']):
            if (result_format is None):
                runtime_configuration.pop('result_format')
            else:
                runtime_configuration.update({'result_format': result_format})
        elif (result_format is not None):
            runtime_configuration.update({'result_format': result_format})
...
class BridgeValidator():
    ...
    def __init__(self, batch, expectation_suite, expectation_engine=None, **kwargs) -> None:
        ...
        if isinstance(expectation_engine, ClassConfig):
            ...
            expectation_engine = load_class(class_name=expectation_engine.class_name, module_name=module_name)
    ...
    def get_dataset(self):
        ...
        if issubclass(self.expectation_engine, PandasDataset):
            ...
            return self.expectation_engine(batch_kwargs=self.batch.batch_kwargs, batch_parameters=self.batch.batch_parameters, batch_markers=self.batch.batch_markers, data_context=self.batch.data_context, expectation_suite=self._expectation_suite, **init_kwargs, **self.batch.batch_kwargs.get('dataset_options', {}))
            ...
            return self.expectation_engine(spark_df=self.batch.data, expectation_suite=self._expectation_suite, batch_kwargs=self.batch.batch_kwargs, batch_parameters=self.batch.batch_parameters, batch_markers=self.batch.batch_markers, data_context=self.batch.data_context, **self.init_kwargs, **self.batch.batch_kwargs.get('dataset_options', {}))
