...
class ExpectationValidationGraph():
    ...
    def get_exception_info(self, metric_info: Dict[(Tuple[(str, str, str)], Dict[(str, Union[(MetricConfiguration, Set[ExceptionInfo], int)])])]) -> Set[ExceptionInfo]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        for (metric_id, metric_info_item) in metric_info.items():
            metric_exception_info.update(cast(Set[ExceptionInfo], metric_info_item['exception_info']))
        return metric_exception_info

    def _filter_metric_info_in_graph(self, metric_info: Dict[(Tuple[(str, str, str)], Dict[(str, Union[(MetricConfiguration, Set[ExceptionInfo], int)])])]) -> Dict[(Tuple[(str, str, str)], Dict[(str, Union[(MetricConfiguration, Set[ExceptionInfo], int)])])]:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
        ...
        return {metric_id: metric_info_item for (metric_id, metric_info_item) in metric_info.items() if (metric_id in graph_metric_ids)}
