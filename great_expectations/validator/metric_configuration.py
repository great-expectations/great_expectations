...
class MetricConfiguration():
    ...
    def __init__(self, metric_name: str, metric_domain_kwargs: dict, metric_value_kwargs: dict=None, metric_dependencies: dict=None) -> None:
        import inspect
    ...
    def id(self) -> Tuple[(str, str, str)]:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return (self.metric_name, self.metric_domain_kwargs_id, self.metric_value_kwargs_id)
    ...
    def to_json_dict(self) -> dict:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        json_dict: dict = {'metric_name': self.metric_name, 'metric_domain_kwargs': self.metric_domain_kwargs, 'metric_domain_kwargs_id': self.metric_domain_kwargs_id, 'metric_value_kwargs': self.metric_value_kwargs, 'metric_value_kwargs_id': self.metric_value_kwargs_id, 'id': self.id}
        return json_dict
