...
def execute_shell_command(command: str) -> int:
    ...
    status_code: int = 0
    try:
        res: CompletedProcess = run(args=['bash', '-c', command], stdin=None, input=None, stdout=None, stderr=None, capture_output=True, shell=False, cwd=cwd, timeout=None, check=True, encoding=None, errors=None, text=None, env=env, universal_newlines=True)
...
def execute_shell_command_with_progress_polling(command: str) -> int:
    ...
    with click.progressbar(length=bar_length_100_percent, label=command) as bar:
        try:
            with Popen(args=['bash', '-c', command], bufsize=(- 1), executable=None, stdin=None, stdout=PIPE, stderr=PIPE, preexec_fn=None, close_fds=True, shell=False, cwd=cwd, env=env, universal_newlines=True, startupinfo=None, creationflags=0, restore_signals=True, start_new_session=False, pass_fds=(), encoding=None, errors=None) as proc:
