...
def init(ctx: click.Context, usage_stats: bool) -> None:
    ...
    if SerializableDataContext.does_config_exist_on_disk(ge_dir):
        message = f"""Warning. An existing `{SerializableDataContext.GE_YML}` was found here: {ge_dir}."""
        warnings.warn(message)
        try:
            project_file_structure_exists = (
                SerializableDataContext.does_config_exist_on_disk(ge_dir)
                and SerializableDataContext.all_uncommitted_directories_exist(ge_dir)
                and SerializableDataContext.config_variables_yml_exist(ge_dir)
            )
        ...
        try:
            context = SerializableDataContext.create(
                target_directory, usage_statistics_enabled=usage_stats
            )
            send_usage_message(
...
def _get_full_path_to_ge_dir(target_directory: str) -> str:
    return os.path.abspath(
        os.path.join(target_directory, SerializableDataContext.GE_DIR)
    )
