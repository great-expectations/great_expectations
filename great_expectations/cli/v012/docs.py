...
@click.option('--site-name', '-s', help='The site for which to generate documentation. See data_docs section in great_expectations.yml')
@click.option('--view/--no-view', help='By default open in browser unless you specify the --no-view flag', default=True)
@click.option('--assume-yes', '--yes', '-y', is_flag=True, help='By default request confirmation to build docs unless you specify -y/--yes/--assume-yes flag to skip dialog', default=False)
def docs_build(directory, site_name, view=True, assume_yes=False) -> None:
    ...
    send_usage_message(data_context=context, event='cli.docs.build', api_version='v2', success=True)
...
def docs_list(directory) -> None:
    ...
    send_usage_message(data_context=context, event='cli.docs.list', api_version='v2', success=True)
...
@click.option('--all', '-a', is_flag=True, help='With this, all sites will get their data docs cleaned out. See data_docs section in great_expectations.yml')
def clean_data_docs(directory, site_name=None, all=None) -> None:
    ...
    if ((site_name is None) and (all is None)):
        cli_message('<red>{}</red>'.format('Please specify --all to remove all sites or specify a specific site using --site_name'))
        sys.exit(1)
    context.clean_data_docs(site_name=site_name)
    failed = False
    if ((not failed) and (context is not None)):
        send_usage_message(data_context=context, event='cli.docs.clean', api_version='v2', success=True)
        cli_message('<green>Cleaned data docs</green>')
    if (failed and (context is not None)):
        send_usage_message(data_context=context, event='cli.docs.clean', api_version='v2', success=False)
