...
@click.option('--directory', '-d', default='./great_expectations', help="The project's great_expectations directory.")
def project_check_config(directory) -> None:
    ...
    if context:
        send_usage_message(data_context=context, event='cli.project.check_config', api_version='v2', success=True)
    if (not is_config_ok):
...
def project_upgrade(directory) -> None:
    ...
    if load_data_context_with_error_handling(directory=directory, from_cli_upgrade_command=True):
        up_to_date_message = 'Your project is up-to-date - no further upgrade is necessary.\n'
