...
def store_list(directory):
    ...
    try:
        ...
        if (len(stores) == 0):
            ...
            send_usage_message(data_context=context, event='cli.store.list', api_version='v2', success=True)
            return
        elif (len(stores) == 1):
        ...
        send_usage_message(data_context=context, event='cli.store.list', api_version='v2', success=False)
        raise e
