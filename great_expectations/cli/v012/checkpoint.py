...
def checkpoint_new(checkpoint, suite, directory, datasource) -> None:
    ...
    suite: ExpectationSuite = toolkit.load_expectation_suite(context, suite_name, usage_event)
    ...
    _ = context.add_checkpoint(name=checkpoint, **{'class_name': 'LegacyCheckpoint', 'batches': [{'batch_kwargs': dict(batch_kwargs), 'expectation_suite_names': [suite.expectation_suite_name]}]})
    cli_message(f'''<green>A Checkpoint named `{checkpoint}` was added to your project!</green>
  - To run this Checkpoint, run `great_expectations --v2-api checkpoint run {checkpoint}`''')
    send_usage_message(data_context=context, event=usage_event, api_version='v2', success=True)

def _verify_checkpoint_does_not_exist(context: DataContext, checkpoint: str, usage_event: str) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    try:
        if (checkpoint in context.list_checkpoints()):
            toolkit.exit_with_failure_message_and_stats(context, usage_event, f'A checkpoint named `{checkpoint}` already exists. Please choose a new name.')
    except InvalidTopLevelConfigKeyError as e:
        toolkit.exit_with_failure_message_and_stats(context, usage_event, f'<red>{e}</red>')

def _write_checkpoint_to_disk(context: DataContext, checkpoint: Dict, checkpoint_name: str) -> str:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    checkpoint_dir = os.path.join(context.root_directory, DataContextConfigDefaults.CHECKPOINTS_BASE_DIRECTORY.value)
...
def _load_checkpoint_yml_template() -> dict:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    template_file = file_relative_path(__file__, os.path.join('..', 'data_context', 'checkpoint_template.yml'))
...
def checkpoint_list(directory) -> None:
    ...
    if (not checkpoints):
        ...
        send_usage_message(data_context=context, event='cli.checkpoint.list', api_version='v2', success=True)
        sys.exit(0)
...
def checkpoint_run(checkpoint, directory) -> None:
    ...
    context = toolkit.load_data_context_with_error_handling(directory=directory, from_cli_upgrade_command=False)
...
{'Suite Name'.ljust(max_suite_display_width)}     Status     Expectations met''')
    for (id, result) in results.run_results.items():
        ...
        percentage_slug = f'{(round(((passed / evaluated) * 100), 2) if (evaluated > 0) else 100)} %'
...
def checkpoint_script(checkpoint, directory) -> None:
    ...
    script_path = os.path.join(context.root_directory, context.GE_UNCOMMITTED_DIR, script_name)
    if os.path.isfile(script_path):
        toolkit.exit_with_failure_message_and_stats(context, usage_event, f'''<red>Warning! A script named {script_name} already exists and this command will not overwrite it.</red>
  - Existing file path: {script_path}''')
    _write_checkpoint_script_to_disk(context.root_directory, checkpoint, script_path)
    cli_message(f'''<green>A python script was created that runs the checkpoint named: `{checkpoint}`</green>
...
def _write_checkpoint_script_to_disk(context_directory: str, checkpoint_name: str, script_path: str) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
