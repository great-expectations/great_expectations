...
from great_expectations.render.renderer.suite_scaffold_notebook_renderer import SuiteScaffoldNotebookRenderer
...
@click.option('--datasource', '-ds', default=None, help='The name of the datasource. The datasource must contain a single BatchKwargGenerator that can list data assets in the datasource ')
@click.option('--batch-kwargs', default=None, help='Batch_kwargs that specify the batch of data to be used a sample when editing the suite. Must be a valid JSON dictionary.\nMake sure to escape quotes. Example: "{"datasource": "my_db", "query": "select * from my_table"}"\n')
...
def _suite_edit(suite, datasource, directory, jupyter, batch_kwargs, usage_event, suppress_usage_message=False) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    try:
        ...
        if (not batch_kwargs):
            ...
            if (batch_kwargs is None):
                (datasource_name, batch_kwargs_generator, data_asset, batch_kwargs) = get_batch_kwargs(context, datasource_name=data_source.name, batch_kwargs_generator_name=None, data_asset_name=None, additional_batch_kwargs=additional_batch_kwargs)
        ...
        if (not jupyter):
            cli_message(f'To continue editing this suite, run <green>jupyter notebook {notebook_path}</green>')
        payload = edit_expectation_suite_usage_statistics(data_context=context, expectation_suite_name=suite.expectation_suite_name)
        if (not suppress_usage_message):
            send_usage_message(data_context=context, event=usage_event, event_payload=payload, api_version='v2', success=True)
        if jupyter:
...
def suite_demo(suite, directory, view) -> None:
    ...
    _suite_new(suite=suite, directory=directory, empty=False, jupyter=False, view=view, batch_kwargs=None, usage_event='cli.suite.demo')

@suite.command(name='new')
@click.option('--suite', '-es', default=None, help='Expectation suite name.')
@click.option('--directory', '-d', default=None, help="The project's great_expectations directory.")
@click.option('--jupyter/--no-jupyter', is_flag=True, help='By default launch jupyter notebooks unless you specify the --no-jupyter flag', default=True)
@click.option('--batch-kwargs', default=None, help='Additional keyword arguments to be provided to get_batch when loading the data asset. Must be a valid JSON dictionary')
def suite_new(suite, directory, jupyter, batch_kwargs) -> None:
    ...
    _suite_new(suite=suite, directory=directory, empty=True, jupyter=jupyter, view=False, batch_kwargs=batch_kwargs, usage_event='cli.suite.new')
...
def _suite_new(suite: str, directory: str, empty: bool, jupyter: bool, view: bool, batch_kwargs, usage_event: str) -> None:
    ...
    try:
        ...
        (success, suite_name, profiling_results) = toolkit.create_expectation_suite(context, datasource_name=datasource_name, batch_kwargs_generator_name=generator_name, data_asset_name=data_asset_name, batch_kwargs=batch_kwargs, expectation_suite_name=suite, additional_batch_kwargs={'limit': 1000}, empty_suite=empty, show_intro_message=False, open_docs=view)
        if success:
            if empty:
                if jupyter:
            ...
            send_usage_message(data_context=context, event=usage_event, api_version='v2', success=True)
            _suite_edit(suite_name, datasource_name, directory, jupyter=jupyter, batch_kwargs=batch_kwargs, usage_event='cli.suite.edit', suppress_usage_message=True)
        else:
            send_usage_message(data_context=context, event=usage_event, api_version='v2', success=False)
    except (ge_exceptions.DataContextError, ge_exceptions.ProfilerError, OSError, SQLAlchemyError) as e:
...
def suite_delete(suite, directory) -> None:
    ...
    if (not suite_names):
        toolkit.exit_with_failure_message_and_stats(context, usage_event, '</red>No expectation suites found in the project.</red>')
    if (suite not in suite_names):
        toolkit.exit_with_failure_message_and_stats(context, usage_event, f'No expectation suite named {suite} found.')
...
def suite_list(directory):
    ...
    context = toolkit.load_data_context_with_error_handling(directory)
    try:
        suite_names = [f' - <cyan>{suite_name}</cyan>' for suite_name in context.list_expectation_suite_names()]
        if (len(suite_names) == 0):
            cli_message('No Expectation Suites found')
            send_usage_message(data_context=context, event='cli.suite.list', api_version='v2', success=True)
            return
        ...
        send_usage_message(data_context=context, event='cli.suite.list', api_version='v2', success=False)
        raise e
