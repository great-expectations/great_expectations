...
def validation_operator_list(directory):
    ...
    try:
        ...
        send_usage_message(data_context=context, event='cli.validation_operator.list', api_version='v2', success=True)
    except Exception as e:
        send_usage_message(data_context=context, event='cli.validation_operator.list', api_version='v2', success=False)
        raise e

@validation_operator.command(name='run')
@click.option('--validation_config_file', '-f', default=None, help='The path of the validation config file (JSON). ')
@click.option('--name', '-n', default=None, help='The name of the validation operator. ')
@click.option('--suite', '-s', default=None, help='The name of the expectation suite. ')
@click.option('--run_name', '-r', default=None, help='Run name. If not specified, a timestamp-based run id will be automatically generated. ')
@click.option('--directory', '-d', default=None, help="The project's great_expectations directory.")
...
def validation_operator_run(name, run_name, validation_config_file, suite, directory) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    try:
        if (validation_config_file is not None):
            try:
                ...
                send_usage_message(data_context=context, event='cli.validation_operator.run', api_version='v2', success=False)
                sys.exit(1)
            ...
            if (validation_config_error_message is not None):
                cli_message('<red>The validation config in {:s} is misconfigured: {:s}</red>'.format(validation_config_file, validation_config_error_message))
                send_usage_message(data_context=context, event='cli.validation_operator.run', api_version='v2', success=False)
                sys.exit(1)
        else:
            if (suite is None):
...
''')
            ...
            if (batch_kwargs is None):
                (datasource_name, batch_kwargs_generator, data_asset, batch_kwargs) = get_batch_kwargs(context, datasource_name=data_source.name, batch_kwargs_generator_name=None, data_asset_name=None, additional_batch_kwargs=None)
            validation_config = {'validation_operator_name': name, 'batches': [{'batch_kwargs': batch_kwargs, 'expectation_suite_names': [suite.expectation_suite_name]}]}
        try:
            ...
            if (suite is None):
                results = context.run_validation_operator(validation_operator_name, assets_to_validate=batches_to_validate, run_id=run_id)
            elif (suite.evaluation_parameters is None):
                results = context.run_validation_operator(validation_operator_name, assets_to_validate=batches_to_validate, run_id=run_id)
            else:
                results = context.run_validation_operator(validation_operator_name, assets_to_validate=batches_to_validate, run_id=run_id, evaluation_parameters=suite.evaluation_parameters)
        ...
        if (not results['success']):
            ...
            send_usage_message(data_context=context, event='cli.validation_operator.run', api_version='v2', success=True)
            sys.exit(1)
        else:
