...
@click.option(
    ...
    "-g",
    default=None,
    help="The name of the batch kwargs generator configured in the datasource. It will list data assets in the datasource",
)
@click.option(
    "--data-assets",
    "-l",
    ...
    try:
        ...
        if datasource is None:
            ...
            if not datasources:
                ...
                cli_message(
                    (
                        "<red>Error: please specify the datasource to profile. Available datasources: "
                        + ", ".join(datasources)
                    )
                    + "</red>"
                )
...
def _add_pandas_datasource(
    ...
    toolkit.confirm_proceed_or_exit(
        continuation_message="Okay, exiting now. To learn more about adding datasources, run great_expectations datasource --help or visit https://docs.greatexpectations.io/"
    )
    ...
    if batch_kwargs_generator_name is not None:
        ...
        if option_selection == "1":
            available_data_asset_names = sorted(
                generator.get_available_data_asset_names()["names"],
                key=(lambda x: x[0]),
            )
            ...
            prompt = (
                (msg_prompt_enter_data_asset_name + choices) + "\n"
        ...
        if reader_method is None:
            while True:
                ...
                if reader_method is not None:
                    ...
                    if isinstance(datasource, SparkDFDatasource) and (
                        reader_method == "csv"
                    ):
