...
from great_expectations.cli.v012.util import CLI_ONLY_SQLALCHEMY_ORDERED_DEPENDENCY_MODULE_NAMES, cli_message, cli_message_dict, verify_library_dependent_modules
...
from great_expectations.datasource.batch_kwargs_generator.table_batch_kwargs_generator import TableBatchKwargsGenerator
from great_expectations.exceptions import BatchKwargsError, DatasourceInitializationError
...
@click.option('--directory', '-d', default=None, help="The project's great_expectations directory.")
def datasource_new(directory) -> None:
    ...
    if datasource_name:
        ...
        send_usage_message(data_context=context, event='cli.datasource.new', api_version='v2', success=True)
    else:
        send_usage_message(data_context=context, event='cli.datasource.new', api_version='v2', success=False)
        sys.exit(1)
...
def datasource_list(directory) -> None:
    ...
    send_usage_message(data_context=context, event='cli.datasource.list', api_version='v2', success=True)
...
@click.option('--batch-kwargs-generator-name', '-g', default=None, help='The name of the batch kwargs generator configured in the datasource. It will list data assets in the datasource')
@click.option('--data-assets', '-l', default=None, help='Comma-separated list of the names of data assets that should be profiled. Requires datasource specified.')
@click.option('--profile_all_data_assets', '-A', is_flag=True, default=False, help='Profile ALL data assets within the target data source. If True, this will override --max_data_assets.')
@click.option('--assume-yes', '--yes', '-y', is_flag=True, default=False, help='By default request confirmation unless you specify -y/--yes/--assume-yes flag to skip dialog')
@click.option('--directory', '-d', default=None, help="The project's great_expectations directory.")
@click.option('--view/--no-view', help='By default open in browser unless you specify the --no-view flag', default=True)
@click.option('--additional-batch-kwargs', default=None, help='Additional keyword arguments to be provided to get_batch when loading the data asset. Must be a valid JSON dictionary')
@mark.cli_as_experimental
def datasource_profile(datasource, batch_kwargs_generator_name, data_assets, profile_all_data_assets, directory, view, additional_batch_kwargs, assume_yes) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    try:
        ...
        if (datasource is None):
            ...
            if (not datasources):
                ...
                send_usage_message(data_context=context, event='cli.datasource.profile', api_version='v2', success=False)
                sys.exit(1)
            elif (len(datasources) > 1):
                cli_message((('<red>Error: please specify the datasource to profile. Available datasources: ' + ', '.join(datasources)) + '</red>'))
                send_usage_message(data_context=context, event='cli.datasource.profile', api_version='v2', success=False)
                sys.exit(1)
            else:
                profile_datasource(context, datasources[0], batch_kwargs_generator_name=batch_kwargs_generator_name, data_assets=data_assets, profile_all_data_assets=profile_all_data_assets, open_docs=view, additional_batch_kwargs=additional_batch_kwargs, skip_prompt_flag=assume_yes)
                send_usage_message(data_context=context, event='cli.datasource.profile', api_version='v2', success=True)
        else:
            profile_datasource(context, datasource, batch_kwargs_generator_name=batch_kwargs_generator_name, data_assets=data_assets, profile_all_data_assets=profile_all_data_assets, open_docs=view, additional_batch_kwargs=additional_batch_kwargs, skip_prompt_flag=assume_yes)
            send_usage_message(data_context=context, event='cli.datasource.profile', api_version='v2', success=True)
...
def add_datasource(context, choose_one_data_asset=False):
    ...
    data_source_location_selection = click.prompt(msg_prompt_where_is_your_data, type=click.Choice(['1', '2']), show_choices=False)
    datasource_name = None
    data_source_type = None
    if (data_source_location_selection == '1'):
        data_source_compute_selection = click.prompt(msg_prompt_files_compute_engine, type=click.Choice(['1', '2']), show_choices=False)
        if (data_source_compute_selection == '1'):
        ...
        elif (data_source_compute_selection == '2'):
...
def _add_pandas_datasource(context, passthrough_generator_only=True, prompt_for_datasource_name=True):
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    send_usage_message(data_context=context, event='cli.new_ds_choice', event_payload={'type': 'pandas'}, api_version='v2', success=True)
    if passthrough_generator_only:
        ...
        configuration = PandasDatasource.build_configuration(batch_kwargs_generators={'subdir_reader': {'class_name': 'SubdirReaderBatchKwargsGenerator', 'base_directory': os.path.join('..', path)}})
        ...
        if (len(errors) != 0):
            raise ge_exceptions.GreatExpectationsError(f'Invalid Datasource configuration: {errors:s}')
    ...
    toolkit.confirm_proceed_or_exit(continuation_message='Okay, exiting now. To learn more about adding datasources, run great_expectations datasource --help or visit https://docs.greatexpectations.io/')
...
def _add_sqlalchemy_datasource(context, prompt_for_datasource_name=True):
    ...
    selected_database = (int(click.prompt(msg_prompt_choose_database, type=click.Choice(db_choices), show_choices=False)) - 1)
    selected_database = list(SupportedDatabases)[selected_database]
    send_usage_message(data_context=context, event='cli.new_ds_choice', event_payload={'type': 'sqlalchemy', 'db': selected_database.name}, api_version='v2', success=True)
    ...
    while True:
        ...
        if (selected_database == SupportedDatabases.MYSQL):
            ...
            sqlalchemy_url = click.prompt('What is the url/connection string for the sqlalchemy connection?\n(reference: https://docs.sqlalchemy.org/en/latest/core/engines.html#database-urls)\n', show_default=False).strip()
        ...
        try:
            ...
            configuration = SqlAlchemyDatasource.build_configuration(credentials=f'${{{datasource_name}}}')
            ...
            if (not click.confirm('Enter the credentials again?', default=True)):
                context.add_datasource(datasource_name, initialize=False, module_name='great_expectations.datasource', class_name='SqlAlchemyDatasource', data_asset_type={'class_name': 'SqlAlchemyDataset'}, credentials=f'${{{datasource_name}}}')
...
def _collect_postgres_credentials(default_credentials=None):
    ...
    credentials['host'] = click.prompt('What is the host for the postgres connection?', default=default_credentials.get('host', 'localhost')).strip()
    credentials['port'] = click.prompt('What is the port for the postgres connection?', default=default_credentials.get('port', '5432')).strip()
    credentials['username'] = click.prompt('What is the username for the postgres connection?', default=default_credentials.get('username', 'postgres')).strip()
    credentials['password'] = click.prompt('What is the password for the postgres connection?', default='', show_default=False, hide_input=_should_hide_input())
    credentials['database'] = click.prompt('What is the database name for the postgres connection?', default=default_credentials.get('database', 'postgres'), show_default=True).strip()
    return credentials
...
def _collect_snowflake_credentials(default_credentials=None):
    ...
    auth_method = click.prompt('What authentication method would you like to use?\n    1. User and Password\n    2. Single sign-on (SSO)\n    3. Key pair authentication\n', type=click.Choice(['1', '2', '3']), show_choices=False)
    credentials['username'] = click.prompt('What is the user login name for the snowflake connection?', default=default_credentials.get('username', '')).strip()
    credentials['host'] = click.prompt("What is the account name for the snowflake connection (include region -- ex 'ABCD.us-east-1')?", default=default_credentials.get('host', '')).strip()
    database = click.prompt('What is database name for the snowflake connection?', default=default_credentials.get('database', '')).strip()
    if (len(database) > 0):
        credentials['database'] = database
    credentials['query'] = {}
    schema = click.prompt('What is schema name for the snowflake connection?', default=default_credentials.get('schema_name', '')).strip()
    if (len(schema) > 0):
        credentials['query']['schema'] = schema
    warehouse = click.prompt('What is warehouse name for the snowflake connection?', default=default_credentials.get('warehouse', '')).strip()
    if (len(warehouse) > 0):
        credentials['query']['warehouse'] = warehouse
    role = click.prompt('What is role name for the snowflake connection?', default=default_credentials.get('role', '')).strip()
    if (len(role) > 0):
...
def _collect_snowflake_credentials_user_password():
    ...
    for (k, v) in __frame.f_locals.items():
        ...
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    credentials = {}
    credentials['password'] = click.prompt('What is the password for the snowflake connection?', default='', show_default=False, hide_input=True)
    return credentials
...
def _collect_snowflake_credentials_sso():
    ...
    credentials['connect_args']['authenticator'] = click.prompt("Valid okta URL or 'externalbrowser' used to connect through SSO", default='externalbrowser', show_default=False)
    return credentials
...
def _collect_snowflake_credentials_key_pair():
    ...
    for (k, v) in __frame.f_locals.items():
        ...
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    credentials = {}
    credentials['private_key_path'] = click.prompt('Path to the private key used for authentication', show_default=False)
    credentials['private_key_passphrase'] = click.prompt('Passphrase for the private key used for authentication (optional -- leave blank for none)', default='', show_default=False)
    return credentials
...
def _collect_bigquery_credentials(default_credentials=None):
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    sqlalchemy_url = click.prompt('What is the SQLAlchemy url/connection string for the BigQuery connection?\n(reference: https://github.com/googleapis/python-bigquery-sqlalchemy#connection-string-parameters)\n', show_default=False).strip()
    credentials = {'url': sqlalchemy_url}
    return credentials
...
def _collect_mysql_credentials(default_credentials=None):
    ...
    credentials['host'] = click.prompt('What is the host for the MySQL connection?', default=default_credentials.get('host', 'localhost')).strip()
    credentials['port'] = click.prompt('What is the port for the MySQL connection?', default=default_credentials.get('port', '3306')).strip()
    credentials['username'] = click.prompt('What is the username for the MySQL connection?', default=default_credentials.get('username', '')).strip()
    credentials['password'] = click.prompt('What is the password for the MySQL connection?', default='', show_default=False, hide_input=True)
    credentials['database'] = click.prompt('What is the database name for the MySQL connection?', default=default_credentials.get('database', '')).strip()
    return credentials
...
def _collect_redshift_credentials(default_credentials=None):
    ...
    credentials['host'] = click.prompt('What is the host for the Redshift connection?', default=default_credentials.get('host', '')).strip()
    credentials['port'] = click.prompt('What is the port for the Redshift connection?', default=default_credentials.get('port', '5439')).strip()
    credentials['username'] = click.prompt('What is the username for the Redshift connection?', default=default_credentials.get('username', '')).strip()
    credentials['password'] = click.prompt('What is the password for the Redshift connection?', default='', show_default=False, hide_input=_should_hide_input())
    credentials['database'] = click.prompt('What is the database name for the Redshift connection?', default=default_credentials.get('database', '')).strip()
    credentials['query'] = {}
    credentials['query']['sslmode'] = click.prompt('What is sslmode name for the Redshift connection?', default=default_credentials.get('sslmode', 'prefer'))
    return credentials
...
def _add_spark_datasource(context, passthrough_generator_only=True, prompt_for_datasource_name=True):
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    send_usage_message(data_context=context, event='cli.new_ds_choice', event_payload={'type': 'spark'}, api_version='v2', success=True)
    ...
    if passthrough_generator_only:
        ...
        if prompt_for_datasource_name:
            datasource_name = click.prompt(msg_prompt_datasource_name, default=datasource_name)
        configuration = SparkDFDatasource.build_configuration(batch_kwargs_generators={'subdir_reader': {'class_name': 'SubdirReaderBatchKwargsGenerator', 'base_directory': os.path.join('..', path)}})
...
def select_batch_kwargs_generator(context, datasource_name, available_data_assets_dict=None):
    ...
    if (len(available_data_asset_names_by_generator.keys()) == 0):
        ...
        choices = '\n'.join([f'    {i}. {generator_name}' for (i, generator_name) in enumerate(generator_names, 1)])
        option_selection = click.prompt(f'''{msg_prompt_select_generator}
{choices}''', type=click.Choice([str(i) for (i, generator_name) in enumerate(generator_names, 1)]), show_choices=False)
        batch_kwargs_generator_name = generator_names[(int(option_selection) - 1)]
        return batch_kwargs_generator_name

def get_batch_kwargs(context, datasource_name=None, batch_kwargs_generator_name=None, data_asset_name=None, additional_batch_kwargs=None):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if (batch_kwargs_generator_name is None):
        batch_kwargs_generator_name = select_batch_kwargs_generator(context, datasource_name, available_data_assets_dict=available_data_assets_dict)
    ...
    if isinstance(context.get_datasource(datasource_name), (PandasDatasource, SparkDFDatasource)):
        (data_asset_name, batch_kwargs) = _get_batch_kwargs_from_generator_or_from_file_path(context, datasource_name, batch_kwargs_generator_name=batch_kwargs_generator_name)
    elif isinstance(context.get_datasource(datasource_name), SqlAlchemyDatasource):
        (data_asset_name, batch_kwargs) = _get_batch_kwargs_for_sqlalchemy_datasource(context, datasource_name, additional_batch_kwargs=additional_batch_kwargs)
    else:
        raise ge_exceptions.DataContextError('Datasource {:s} is expected to be a PandasDatasource or SparkDFDatasource, but is {:s}'.format(datasource_name, str(type(context.get_datasource(datasource_name)))))
    return (datasource_name, batch_kwargs_generator_name, data_asset_name, batch_kwargs)

def _get_batch_kwargs_from_generator_or_from_file_path(context, datasource_name, batch_kwargs_generator_name=None, additional_batch_kwargs=None):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    reader_method_file_extensions = {'1': 'csv', '2': 'parquet', '3': 'xlsx', '4': 'json', '5': 'sas'}
    ...
    if (batch_kwargs_generator_name is not None):
        ...
        option_selection = click.prompt(msg_prompt_generator_or_file_path, type=click.Choice(['1', '2']), show_choices=False)
        if (option_selection == '1'):
            available_data_asset_names = sorted(generator.get_available_data_asset_names()['names'], key=(lambda x: x[0]))
            available_data_asset_names_str = [f'{name[0]} ({name[1]})' for name in available_data_asset_names]
            data_asset_names_to_display = available_data_asset_names_str[:50]
            choices = '\n'.join([f'    {i}. {name}' for (i, name) in enumerate(data_asset_names_to_display, 1)])
            ...
            try:
                ...
                try:
                    data_asset_name = [name[0] for name in available_data_asset_names][data_asset_index]
                except IndexError:
                    pass
            except ValueError:
    ...
    path = None
    while True:
        path = click.prompt(msg_prompt_file_path, type=click.Path(dir_okay=dir_okay), default=path)
        ...
        if (reader_method is None):
            while True:
                option_selection = click.prompt(msg_prompt_file_type, type=click.Choice(['1', '2', '3', '4', '5']), show_choices=False)
                try:
                    reader_method = datasource.guess_reader_method_from_path(f'{path}.{reader_method_file_extensions[option_selection]}')['reader_method']
                except BatchKwargsError:
                    pass
            ...
            try:
                batch_kwargs['reader_method'] = reader_method
                reader_options = {**batch_kwargs.get('reader_options', {}), **reader_options}
                if reader_options:
...
def _get_batch_kwargs_for_sqlalchemy_datasource(context, datasource_name, additional_batch_kwargs=None):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    while (data_asset_name is None):
        single_or_multiple_data_asset_selection = click.prompt(msg_prompt_how_to_connect_to_data, type=click.Choice(['1', '2', '3']), show_choices=False)
        if (single_or_multiple_data_asset_selection == '1'):
            schema_name = click.prompt('Please provide the schema name of the table (this is optional)', default=default_schema)
        ...
        elif (single_or_multiple_data_asset_selection == '3'):
            msg_prompt_warning: str = 'Warning: I you have a large number of tables in your datasource, this may take a very long time.\nWould you like to continue?'
    ...
    if (sql_query is None):
        ...
        batch_kwargs.update(temp_table_kwargs)
        BridgeValidator(batch=datasource.get_batch(batch_kwargs), expectation_suite=ExpectationSuite('throwaway', data_context=context)).get_dataset()
    batch_kwargs['data_asset_name'] = data_asset_name
    return (data_asset_name, batch_kwargs)
...
def _verify_sqlalchemy_dependent_modules() -> bool:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    return verify_library_dependent_modules(python_import_name='sqlalchemy', pip_library_name='sqlalchemy')
...
def _verify_mysql_dependent_modules() -> bool:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    return verify_library_dependent_modules(python_import_name='pymysql', pip_library_name='pymysql', module_names_to_reload=CLI_ONLY_SQLALCHEMY_ORDERED_DEPENDENCY_MODULE_NAMES)
...
def _verify_postgresql_dependent_modules() -> bool:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    psycopg2_success: bool = verify_library_dependent_modules(python_import_name='psycopg2', pip_library_name='psycopg2-binary', module_names_to_reload=CLI_ONLY_SQLALCHEMY_ORDERED_DEPENDENCY_MODULE_NAMES)
    postgresql_psycopg2_success: bool = verify_library_dependent_modules(python_import_name='sqlalchemy.dialects.postgresql.psycopg2', pip_library_name='psycopg2-binary', module_names_to_reload=CLI_ONLY_SQLALCHEMY_ORDERED_DEPENDENCY_MODULE_NAMES)
...
def _verify_redshift_dependent_modules() -> bool:
    ...
    redshift_success: bool = verify_library_dependent_modules(python_import_name='sqlalchemy_redshift.dialect', pip_library_name='sqlalchemy-redshift', module_names_to_reload=CLI_ONLY_SQLALCHEMY_ORDERED_DEPENDENCY_MODULE_NAMES)
...
def _verify_snowflake_dependent_modules() -> bool:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    return verify_library_dependent_modules(python_import_name='snowflake.sqlalchemy.snowdialect', pip_library_name='snowflake-sqlalchemy', module_names_to_reload=CLI_ONLY_SQLALCHEMY_ORDERED_DEPENDENCY_MODULE_NAMES)
...
def _verify_bigquery_dependent_modules() -> bool:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    pybigquery_ok = verify_library_dependent_modules(python_import_name='pybigquery.sqlalchemy_bigquery', pip_library_name='pybigquery', module_names_to_reload=CLI_ONLY_SQLALCHEMY_ORDERED_DEPENDENCY_MODULE_NAMES)
    sqlalchemy_bigquery_ok = verify_library_dependent_modules(python_import_name='sqlalchemy_bigquery', pip_library_name='sqlalchemy_bigquery', module_names_to_reload=CLI_ONLY_SQLALCHEMY_ORDERED_DEPENDENCY_MODULE_NAMES)
...
def _verify_pyspark_dependent_modules() -> bool:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    return verify_library_dependent_modules(python_import_name='pyspark', pip_library_name='pyspark')
...
def profile_datasource(context, datasource_name, batch_kwargs_generator_name=None, data_assets=None, profile_all_data_assets=False, max_data_assets=20, additional_batch_kwargs=None, open_docs=False, skip_prompt_flag=False):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    logging.getLogger('great_expectations.profile.basic_dataset_profiler').setLevel(logging.INFO)
    ...
    msg_skipping = 'Skipping profiling for now. You can always do this later by running `<green>great_expectations datasource profile</green>`.'
    ...
    profiling_results = context.profile_datasource(datasource_name, batch_kwargs_generator_name=batch_kwargs_generator_name, data_assets=data_assets, profile_all_data_assets=profile_all_data_assets, max_data_assets=max_data_assets, dry_run=True, additional_batch_kwargs=additional_batch_kwargs)
    if (profiling_results['success'] is True):
        if (data_assets or profile_all_data_assets or skip_prompt_message(skip_prompt_flag, msg_confirm_ok_to_proceed.format(datasource_name))):
            profiling_results = context.profile_datasource(datasource_name, batch_kwargs_generator_name=batch_kwargs_generator_name, data_assets=data_assets, profile_all_data_assets=profile_all_data_assets, max_data_assets=max_data_assets, dry_run=False, additional_batch_kwargs=additional_batch_kwargs)
        else:
        ...
        while (not do_exit):
            ...
            option_selection = click.prompt(msg_options, type=click.Choice(['1', '2', '3']), show_choices=False)
            if (option_selection == '1'):
                data_assets = click.prompt(msg_prompt_enter_data_asset_list.format(', '.join([data_asset[0] for data_asset in profiling_results['error']['data_assets']][:3])), show_default=False)
                if data_assets:
                    data_assets = [item.strip() for item in data_assets.split(',')]
