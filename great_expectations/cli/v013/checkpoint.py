...
from great_expectations.cli.v013 import toolkit
from great_expectations.cli.v013.mark import Mark as mark
from great_expectations.cli.v013.util import cli_message, cli_message_list
...
def checkpoint_new(checkpoint, suite, directory, datasource, legacy):
    ...
    if legacy:
        suite_name = suite
        usage_event = "cli.v013.checkpoint.new"
...
def checkpoint_list(directory):
    ...
    if not checkpoints:
        ...
        send_usage_message(context, event="cli.v013.checkpoint.list", success=True)
        sys.exit(0)
...
def checkpoint_run(checkpoint, directory):
    ...
    usage_event = "cli.v013.checkpoint.run"
...
def checkpoint_script(checkpoint, directory):
    ...
    """
    context = toolkit.load_data_context_with_error_handling(directory)
    usage_event = "cli.v013.checkpoint.script"
    ge_config_version = context.get_config().config_version
    if ge_config_version >= 3:
        cli_message(
