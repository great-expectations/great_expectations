...
from great_expectations.data_context.types.base import DataContextConfig, DataContextConfigDefaults

class UpgradeHelperV13(BaseUpgradeHelper):

    def __init__(self, data_context: Optional[DataContext]=None, context_root_dir: Optional[str]=None, update_version: bool=False) -> None:
        import inspect
        ...
        self.upgrade_log = {'update_version': update_version, 'skipped_checkpoint_store_upgrade': False, 'added_checkpoint_store': {}, 'skipped_checkpoint_config_upgrade': False, 'skipped_datasources_upgrade': False, 'skipped_validation_operators_upgrade': False}
        self.upgrade_checklist = {'automatic': {'stores': {}, 'store_names': {}}, 'manual': {'checkpoints': {}, 'datasources': {}, 'validation_operators': {}}}
    ...
    def _process_checkpoint_store_for_checklist(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        if CheckpointStore.default_checkpoints_exist(directory_path=self.data_context.root_directory):
            config_commented_map: CommentedMap = self.data_context.get_config().commented_map
            checkpoint_store_name: Optional[str] = config_commented_map.get('checkpoint_store_name')
            ...
            if checkpoint_store_name:
                if stores.get(checkpoint_store_name):
                    ...
                    self.upgrade_checklist['automatic']['stores'] = {checkpoint_store_name: DataContextConfigDefaults.DEFAULT_STORES.value[DataContextConfigDefaults.DEFAULT_CHECKPOINT_STORE_NAME.value]}
            else:
                checkpoint_store_name = DataContextConfigDefaults.DEFAULT_CHECKPOINT_STORE_NAME.value
                self.upgrade_checklist['automatic']['store_names']['checkpoint_store_name'] = checkpoint_store_name
                if (not stores.get(checkpoint_store_name)):
                    self.upgrade_checklist['automatic']['stores'] = {checkpoint_store_name: DataContextConfigDefaults.DEFAULT_STORES.value[checkpoint_store_name]}
        else:
            self.upgrade_log['skipped_checkpoint_store_upgrade'] = True
    ...
    def _process_checkpoint_config_for_checklist(self) -> None:
        ...
        try:
            ...
            self.upgrade_checklist['manual']['checkpoints'] = {checkpoint.name: checkpoint.get_config() for checkpoint in legacy_checkpoints}
    ...
    def _process_datasources_for_checklist(self) -> None:
        ...
        self.upgrade_checklist['manual']['datasources'] = {datasource_name: datasource_config for (datasource_name, datasource_config) in datasources.items() if ((set(datasource_config.keys()) & {'execution_engine', 'data_connectors', 'introspection', 'tables'}) == set())}
    ...
    def _process_validation_operators_for_checklist(self) -> None:
        ...
        for (k, v) in __frame.f_locals.items():
            ...
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        config_commented_map: CommentedMap = self.data_context.get_config().commented_map
        validation_operators: dict = (config_commented_map.get('validation_operators') or {})
        validation_operator_name: str
        validation_operator_config: dict
        self.upgrade_checklist['manual']['validation_operators'] = {validation_operator_name: validation_operator_config for (validation_operator_name, validation_operator_config) in validation_operators.items() if validation_operator_config}
    ...
    def manual_steps_required(self) -> bool:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        return any([(len(manual_upgrade_item.keys()) > 0) for manual_upgrade_item in self.upgrade_checklist['manual'].values()])
    ...
    def _upgrade_overview_common_content(self, manual_steps_required: bool) -> str:
        ...
        for (k, v) in __frame.f_locals.items():
            if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                continue
            print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
        stores_upgrade_checklist = list(self.upgrade_checklist['automatic']['stores'].keys())
        store_names_upgrade_checklist = list(self.upgrade_checklist['automatic']['store_names'].keys())
        checkpoint_config_upgrade_checklist = self.upgrade_checklist['manual']['checkpoints']
        datasources_upgrade_checklist = self.upgrade_checklist['manual']['datasources']
        upgrade_overview = ''
        if (self.upgrade_log['skipped_checkpoint_store_upgrade'] and self.upgrade_log['skipped_checkpoint_config_upgrade'] and self.upgrade_log['skipped_datasources_upgrade'] and self.upgrade_log['skipped_validation_operators_upgrade']):
            ...
            if (not self.upgrade_log['skipped_checkpoint_store_upgrade']):
                if (stores_upgrade_checklist or store_names_upgrade_checklist):
                    ...
                    upgrade_overview += (f'''    - Stores: {', '.join(stores_upgrade_checklist)}
''' if stores_upgrade_checklist else '')
                    upgrade_overview += (f'''    - Store Names: {', '.join(store_names_upgrade_checklist)}
''' if store_names_upgrade_checklist else '')
            if manual_steps_required:
                ...
                if (not self.upgrade_log['skipped_checkpoint_config_upgrade']):
                    ...
                    upgrade_overview += (f'''    - Checkpoints: {', '.join(checkpoint_config_upgrade_checklist)}
''' if checkpoint_config_upgrade_checklist else '')
                if (not self.upgrade_log['skipped_datasources_upgrade']):
                    ...
                    upgrade_overview += (f'''    - Data Sources: {', '.join(datasources_upgrade_checklist)}
''' if datasources_upgrade_checklist else '')
    ...
    def _upgrade_configuration_automatically(self) -> None:
        ...
        if (not self.upgrade_log['skipped_checkpoint_store_upgrade']):
            ...
            for (key, value) in self.upgrade_checklist['automatic']['store_names'].items():
                config_commented_map[key] = value
            data_context_config: DataContextConfig = DataContextConfig.from_commented_map(commented_map=config_commented_map)
            self.data_context.set_config(project_config=data_context_config)
            self.data_context._save_project_config()
            checkpoint_log_entry = {'stores': {DataContextConfigDefaults.DEFAULT_CHECKPOINT_STORE_NAME.value: data_context_config.stores[DataContextConfigDefaults.DEFAULT_CHECKPOINT_STORE_NAME.value]}, 'checkpoint_store_name': data_context_config.checkpoint_store_name}
...
A log detailing the upgrade can be found here:
    ...
    def _save_upgrade_log(self):
        ...
        dest_path = os.path.join(self.data_context.root_directory, 'uncommitted', 'logs', 'project_upgrades', f'UpgradeHelperV13_{current_time}.json')
