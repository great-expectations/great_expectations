        ...
        def wrapper(*args, **kwargs) -> None:
            ...
            for (k, v) in __frame.f_locals.items():
                if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
                    continue
                print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
            cli_message('<yellow>Heads up! This feature is in Beta. Please give us your feedback!</yellow>')
            func(*args, **kwargs)
        return wrapper

    @staticmethod
    def cli_as_deprecation(message: str='<yellow>Heads up! This feature will be deprecated in the next major release</yellow>') -> Callable:
        import inspect
        __frame = inspect.currentframe()
        __file = __frame.f_code.co_filename
        __func = __frame.f_code.co_name
