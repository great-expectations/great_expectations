...
from great_expectations.datasource import BaseDatasource, Datasource, SimpleSqlalchemyDatasource
...
try:
    ...
    sqlalchemy = None
    Inspector = None
DEFAULT_DATA_CONNECTOR_NAMES: Final[List[str]] = ['default_runtime_data_connector_name', 'default_inferred_data_connector_name']

def get_batch_request(datasource: BaseDatasource, additional_batch_request_args: Optional[Dict[(str, Any)]]=None) -> Dict[(str, Union[(str, Dict[(str, Any)])])]:
    import inspect
    ...
    available_data_asset_names_by_data_connector_dict: Dict[(str, List[str])] = datasource.get_available_data_asset_names()
    data_connector_name: Optional[str] = select_data_connector_name(available_data_asset_names_by_data_connector_dict=available_data_asset_names_by_data_connector_dict)
    batch_request: Dict[(str, Union[(str, int, Dict[(str, Any)])])] = {'datasource_name': datasource.name, 'data_connector_name': data_connector_name}
...
'''
        data_asset_name = _get_data_asset_name_from_data_connector(datasource=datasource, data_connector_name=data_connector_name, msg_prompt_enter_data_asset_name=msg_prompt_enter_data_asset_name)
    elif isinstance(datasource, SimpleSqlalchemyDatasource):
        msg_prompt_enter_data_asset_name: str = '\nWhich table would you like to use? (Choose one)\n'
        data_asset_name = _get_data_asset_name_for_simple_sqlalchemy_datasource(datasource=datasource, data_connector_name=data_connector_name, msg_prompt_enter_data_asset_name=msg_prompt_enter_data_asset_name)
    else:
        raise ge_exceptions.DataContextError(f"Datasource '{datasource.name}' of unsupported type {type(datasource)} was encountered.")
    batch_request.update({'data_asset_name': data_asset_name})
    if (additional_batch_request_args and isinstance(additional_batch_request_args, dict)):
        batch_request.update(additional_batch_request_args)
    batch_spec_passthrough: Dict[(str, Union[(str, Dict[(str, Any)])])] = batch_request.get('batch_spec_passthrough')
...
def select_data_connector_name(available_data_asset_names_by_data_connector_dict: Optional[Dict[(str, List[str])]]=None) -> Optional[str]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    num_available_data_asset_names_by_data_connector = len(available_data_asset_names_by_data_connector_dict)
    ...
    data_connector_names: List[str] = list(available_data_asset_names_by_data_connector_dict.keys())
    choices: str = '\n'.join([f'    {i}. {data_connector_name}' for (i, data_connector_name) in enumerate(data_connector_names, 1)])
    choices += '\n'
    option_selection: str = click.prompt(f'''{msg_prompt_select_data_connector_name}
{choices}''', type=click.Choice([str(i) for (i, data_connector_name) in enumerate(data_connector_names, 1)]), show_choices=False)
    data_connector_name: str = data_connector_names[(int(option_selection) - 1)]
    return data_connector_name

def _get_data_asset_name_from_data_connector(datasource: BaseDatasource, data_connector_name: str, msg_prompt_enter_data_asset_name: str) -> Optional[str]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    available_data_asset_names_by_data_connector_dict: Dict[(str, List[str])] = datasource.get_available_data_asset_names(data_connector_names=data_connector_name)
    available_data_asset_names: List[str] = sorted(available_data_asset_names_by_data_connector_dict[data_connector_name], key=(lambda x: x))
...
'''
        ...
        while (user_selected_option is None):
            ...
            if user_selected_option.startswith('l'):
                data_asset_name = _list_available_data_asset_names(available_data_asset_names, msg_prompt_enter_data_asset_name)
            elif user_selected_option.startswith('s'):
                data_asset_name = _search_through_available_data_asset_names(available_data_asset_names, msg_prompt_enter_data_asset_name)
            else:
...
def _list_available_data_asset_names(available_data_asset_names: List[str], msg_prompt_enter_data_asset_name: str) -> Optional[str]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    available_data_asset_names_str: List[str] = [f'{name}' for name in available_data_asset_names]
    PAGE_SIZE = 50
    data_asset_pages: List[List[str]] = [available_data_asset_names_str[i:(i + PAGE_SIZE)] for i in range(0, len(available_data_asset_names_str), PAGE_SIZE)]
    ...
    while (data_asset_name is None):
        ...
        choices: str = '\n'.join([f'    {(i + (display_idx * PAGE_SIZE))}. {name}' for (i, name) in enumerate(current_page, 1)])
...
def _search_through_available_data_asset_names(available_data_asset_names: List[str], msg_prompt_enter_data_asset_name: str) -> Optional[str]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    while (data_asset_name is None):
        available_data_asset_names_str: List[str] = [f'{name}' for name in available_data_asset_names]
        choices: str = '\n'.join([f'    {i}. {name}' for (i, name) in enumerate(available_data_asset_names_str, 1)])
...
'''
        ...
        if user_response.isdigit():
            ...
            available_data_asset_names = list(filter(r.match, available_data_asset_names))
    return data_asset_name

def _get_data_asset_name_for_simple_sqlalchemy_datasource(datasource: SimpleSqlalchemyDatasource, data_connector_name: str, msg_prompt_enter_data_asset_name: str) -> str:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    table_name: str
    while (data_asset_name is None):
        single_or_multiple_data_asset_selection: str = click.prompt(msg_prompt_how_to_connect_to_data, type=click.Choice(['1', '2', '3']), show_choices=False)
        if (single_or_multiple_data_asset_selection == '1'):
            ...
            confirmation: str = click.prompt(msg_prompt_warning, type=click.Choice(['y', 'n']), show_choices=True)
            if (confirmation == 'y'):
                data_asset_name = _get_data_asset_name_from_data_connector(datasource=datasource, data_connector_name=data_connector_name, msg_prompt_enter_data_asset_name=msg_prompt_enter_data_asset_name)
    if ((datasource.execution_engine.engine.dialect.name.lower() == 'bigquery') and (parse_bigquery_url is not None)):
...
def _get_default_schema(datasource: SimpleSqlalchemyDatasource) -> str:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    execution_engine: SqlAlchemyExecutionEngine = cast(SqlAlchemyExecutionEngine, datasource.execution_engine)
...
def _get_batch_spec_passthrough(datasource: BaseDatasource) -> Dict[(str, Union[(str, Dict[(str, Any)])])]:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    if isinstance(datasource, Datasource):
        ...
        raise ge_exceptions.DataContextError('Datasource {:s} of unsupported type {:s} was encountered.'.format(datasource.name, str(type(datasource))))
