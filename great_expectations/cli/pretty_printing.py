...
def display_not_implemented_message_and_exit() -> None:
    ...
    for (k, v) in __frame.f_locals.items():
        if any(((var in k) for var in ('self', 'cls', '__frame', '__file', '__func'))):
            continue
        print(f'<INTROSPECT> {__file}:{__func}:{k} - {v.__class__.__name__}')
    cli_message('<red>This command is not yet implemented for the v3 (Batch Request) API</red>')
    sys.exit(1)

def cli_message_list(string_list: List[str], list_intro_string: Optional[str]=None) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def cli_message_dict(dict_: dict, indent: int=3, bullet_char: str='-', message_list: Optional[list]=None, recursion_flag: bool=False) -> None:
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
