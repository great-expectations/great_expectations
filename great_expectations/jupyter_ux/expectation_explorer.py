...
class ExpectationExplorer(object):
    def __init__(self):
        ...
        self.expectation_kwarg_field_names = {
            ...
            'expect_column_values_to_be_between': ['min_max_type', 'parse_strings_as_datetimes', 'output_strftime_format', 'min_value', 'max_value', 'mostly', 'allow_cross_type_comparisons'],
            'expect_column_max_to_be_between': ['min_max_type', 'parse_strings_as_datetimes', 'output_strftime_format', 'min_value', 'max_value'],
            'expect_column_min_to_be_between': ['min_max_type', 'parse_strings_as_datetimes', 'output_strftime_format', 'min_value', 'max_value'],
    ...
    def expectation_kwarg_dict_to_ge_kwargs(self, kwarg_dict):
        ...
        for kwarg_name, widget_dict in kwarg_dict.items():
            ...
            if kwarg_name in ancillary_kwargs:
                continue
            elif not hasattr(self, "generate_{kwarg_name}_widget_dict".format(kwarg_name=kwarg_name)):
    ...
    def update_expectation_state(self, existing_expectation_state, expectation_validation_result, validation_time):
        ...
        for ge_kwarg_name, ge_kwarg_value in new_ge_expectation_kwargs.items():
            ...
            if current_widget_dict:
                if not hasattr(self, "generate_{ge_kwarg_name}_widget_dict".format(ge_kwarg_name=ge_kwarg_name)):
                ...
                widget_dict_generator = getattr(
                    self, "generate_{ge_kwarg_name}_widget_dict".format(ge_kwarg_name=ge_kwarg_name), None)
    ...
    def generate_regex_list_widget_dict(self, expectation_state, regex_list=[], column=None, **expectation_kwargs):
        ...
        regex_list_label = widgets.HTML(
            value='<div style="text-align:right"><strong>regex list: </strong></div>', layout={'width': '150px'})
    ...
    def generate_column_list_widget_dict(self, expectation_state, column_list=[], column=None, **expectation_kwargs):
        ...
        column_list_widget_input = self.generate_text_widget(
            value=None, description='add column', placeholder='press enter to add...', continuous_update=True)
        column_list_label = widgets.HTML(
            value='<div style="text-align:right"><strong>column list: </strong></div>', layout={'width': '150px'})
        column_list_widget = widgets.VBox([widgets.HBox(
            [column_list_label, column_list_widget_display]), column_list_widget_input])
    ...
    def generate_strictly_widget_dict(self, expectation_state, strictly=None, column=None, **expectation_kwargs):
        ...
        strictly_widget = self.generate_boolean_checkbox_widget(
            value=strictly,
            description="strictly {inc_dec}".format(inc_dec=inc_dec)
        )
    ...
    def generate_expectation_kwarg_fallback_widget_dict(self, expectation_kwarg_name, **expectation_kwargs):
        ...
        warning_message = widgets.HTML(
            value='<div><strong>Warning: </strong>No input widget for kwarg "{expectation_kwarg_name}". To change value, call expectation again with the modified value.</div>'.format(expectation_kwarg_name=expectation_kwarg_name),
            layout={
                'width': '400px',
        ...
        static_widget = widgets.Textarea(
            ...
            description="<strong>{expectation_kwarg_name}: </strong>".format(expectation_kwarg_name=expectation_kwarg_name),
            style=self.styles.get('description_width'),
            layout={'width': '400px'},
            disabled=True
        )
    ...
    def generate_column_widget(self, column=None):
        return widgets.HTML(value="<div><strong>Column: </strong>{column}</div>".format(column=column)) if column else None
        ...
        if column:
            return widgets.VBox(
                [
                    ...
                    success_widget,
                    widgets.HTML(value="<div><strong>Date/Time Validated (UTC): </strong>{validation_time}</div>".format(validation_time=validation_time)),
                ],
            ...
            return widgets.VBox(
                [
                    ...
                    success_widget,
                    widgets.HTML(
                        value="<div><strong>Date/Time Validated (UTC): </strong>{validation_time}</div>".format(validation_time=validation_time)),
                ],
                layout=widgets.Layout(margin='10px', width='40%')
            )
        ...
        for result_title, result_value in result.items():
            result_detail_widgets.append(
                widgets.HTML(
                    ...
                    else "<span><strong>{result_title}: </strong>{result_value}</span>".format(result_title=result_title, result_value=result_value)
    ...
    def create_expectation_widget(
        ...
        exception_accordion = widgets.Accordion(
            children=[expectation_feedback_widget]
        )
        ...
        for expectation_kwarg_name in self.expectation_kwarg_field_names.get(expectation_type):
            widget_dict_generator = getattr(
                self, "generate_{expectation_kwarg_name}_widget_dict".format(expectation_kwarg_name=expectation_kwarg_name), None)
        ...
        kwarg_widgets = [dict['kwarg_widget']
        ...
        expectation_editor_title = "{column} | {expectation_type}".format(column=column, expectation_type=expectation_type) if column \
            else "non_column_expectation | {expectation_type}".format(expectation_type=expectation_type)
    ...
    def generate_expectation_suite_editor_widgets(self, data_asset, expectation_suite):
        ...
        for column_name in column_names:
            ...
            column_accordion_content = widgets.VBox(
                children=expectation_editor_widgets)
            column_accordion = widgets.Accordion(
                children=[column_accordion_content])
