...
from great_expectations.render.renderer import ExpectationSuiteColumnSectionRenderer, ProfilingResultsColumnSectionRenderer, ValidationResultsColumnSectionRenderer
...
def set_data_source(context, data_source_type=None):
    ...
    if (not data_source_type):
        ...
        if (len(configured_datasources) == 0):
            ...
            display(HTML("Will be using this data source from your project's great_expectations.yml: <b>{:s}</b>".format(data_source_name)))
    else:
        configured_datasources = [datasource['name'] for datasource in context.list_datasources() if (datasource['type'] == data_source_type)]
        if (len(configured_datasources) == 0):
            ...
            display(HTML("Will be using this {:s} data source from your project's great_expectations.yml: <b>{:s}</b>".format(data_source_type, data_source_name)))
...
def setup_notebook_logging(logger=None, log_level=logging.INFO):
    ...
    logger.info(f'Great Expectations logging enabled at {log_level} level by JupyterUX module.')
...
def show_available_data_asset_names(context, data_source_name=None) -> None:
    ...
    for datasource in datasources:
        if (data_source_name and (datasource['name'] != data_source_name)):
            continue
        html += "<h2 style='margin: 0'>Datasource: {:s} ({:s})</h2>".format(datasource['name'], datasource['class_name'])
        ds = context.get_datasource(datasource['name'])
        generators = ds.list_batch_kwargs_generators()
        for generator_info in generators:
            html += 'batch_kwargs_generator: {:s} ({:s})'.format(generator_info['name'], generator_info['class_name'])
            ...
            if (len(data_asset_names) > 0):
                ...
                for data_asset_name in data_asset_names:
                    ...
                    data_asset_expectation_suite_keys = [es_key for es_key in expectation_suite_keys if ((es_key.data_asset_name.datasource == datasource['name']) and (es_key.data_asset_name.generator == generator_info['name']) and (es_key.data_asset_name.generator_asset == data_asset_name))]
                    if (len(data_asset_expectation_suite_keys) > 0):
                        html += '<ul>'
                        for es_key in data_asset_expectation_suite_keys:
                            html += "<li><span class='expectation-suite'>Expectation Suite</span>: {:s}</li>".format(es_key.expectation_suite_name)
                        html += '</ul>'
                html += '</ul>'
            else:
...
def display_column_expectations_as_section(expectation_suite, column, include_styling=True, return_without_displaying=False):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
...
def display_profiled_column_evrs_as_section(evrs, column, include_styling=True, return_without_displaying=False):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
    ...
    view = DefaultJinjaSectionView().render({'section': document, 'section_loop': {'index': 1}})
    return _render_for_jupyter(view, include_styling, return_without_displaying)

def display_column_evrs_as_section(evrs, column, include_styling=True, return_without_displaying=False):
    import inspect
    __frame = inspect.currentframe()
    __file = __frame.f_code.co_filename
    __func = __frame.f_code.co_name
